...
- Admin bulk delegation may select multiple assigned owners or one owner Department. Department mode resolves current Employee Master membership intersected with existing `CCCF_Assignments` and materializes exact owner/delegate rows transactionally; it never grants an unassigned or future owner implicitly. Re-run Department mode after future Assignment changes. Node and PHP must remain behaviorally aligned.
