...
- Added support for a narrowly scoped, non-secret PHP release override so Production can enable only Account Recovery delivery flags and its public URL without downloading, replacing or exposing the existing secret-bearing `.env` or `config.local.php`.
...
- Pushed `507b615` and deployed the four PHP runtime paths. FTPS hashes passed 4/4, HTTPS hashes passed 3/3, and Production read-only API smoke passed every filter over 100 records with zero writes.
- Headless Chrome verified live search, reset, reversed-date validation and mobile layout with zero console errors or mutation requests. No schema, upload path, KY record or rollout setting changed; exact runtime rollback files and verification evidence are retained under `backups/production/ky-history-filter-predeploy-20260911-132847/`.
...
- Frontend PDF layout and cache keys only. Deployed from source commit `c5a9636`; no CCCF API, schema, Assignment, upload, submission or review behavior changed.

## Hiyari PDF overview reconciliation (2026-09-07, deployed)
...
- Only the Hiyari PDF overview and cache keys changed. Deployed from source commit `c5a9636`; no API, schema, report, Assignment or upload-path behavior changed.
...
- Source commit `f164dd7` is pushed on `main` and the four-file PHP/shared-hosting release is deployed. Fresh database/application backup, FTPS hashes `4/4`, HTTPS cache/feature markers and anonymous `401` checks passed; no temporary business data was created.
...
- Added independent Foundation/Checklist/Inspector recovery, truthful KPI-rule presentation, active Pilot preview/confirmation and setup guidance into the canonical BBS workspaces.
- Retired legacy hierarchy mutation controls from this UI. Existing assignments remain visible read-only, while create/update/team operations are directed to Inspector Enrollment/Team Management so KPI and team-event history remain authoritative.
- Added Checklist/UAT search, status/type filters and pagination; added Eligibility search/status pagination and a searchable Employee Master picker. No Checklist/UAT or historical record was deleted.
- Added labelled keyboard-scrollable data regions, 44 px primary controls and authenticated 390 px browser coverage. The Pilot form now defaults to a Master Department with an available Safety Unit when no Active Pilot exists.
- Deployed only `index.html`, `public/js/main.js`, `public/js/pages/admin.js`, `public/js/pages/bbs-smart-card.js` and `deploy-manifest.json`. FTP download-back passed `5/5`, HTTPS passed `4/4`, Admin API/browser smoke passed, and staged non-Admin/anonymous access remains `403/401`. No API, schema, business record, upload path or rollout flag changed; the completed BBS source is included in the 2026-09-02 GitHub synchronization.

## BBS Smart Card Phase 10E Staged Production Deployment (2026-09-01)

- Deployed Phase 10B-4, 10D-1 through 10D-5 and the Phase 10E Controlled Pilot access boundary to Production after fresh database/application/private-upload backups.
- Installed the additive `pilot_scope_only` setting but retained `staged_admin_only=1` and `pilot_scope_only=0`; Production has no effective Active Pilot scope, inspector enrollment or assignment, so ordinary-user rollout was not opened.
- FTP SHA-256 download-back passed `28/28`, HTTPS hashes passed `4/4`, authenticated Admin read smoke passed, ordinary-user/anonymous denials remained `403/401`, and Chrome passed all 8 tabs at three phone orientations with zero console errors.
- No business fixture or test row/file was created; residue is `0`. The temporary rollout helper was deleted and returns `404`. The completed Phase 10E implementation is included in the 2026-09-02 GitHub synchronization.

## BBS Smart Card Phase 10E Pilot Acceptance Gate (2026-09-01, implementation history)

- Added a separate Controlled Pilot access mode without weakening `staged_admin_only`: Admin-only is `1/*`, controlled multi-role Pilot is `0/1`, and company-wide remains `0/0`. Node and PHP authorize only Admin, effective appointed inspectors and effective assigned Pilot members; same-scope but unassigned accounts and anonymous QR resolution fail closed.
- Added the idempotent `pilot_scope_only=0` migration, Node/PHP parity/security contracts, a local-only migration runner and a read-only Employee workbook reference audit. The supplied workbook was read but not imported; it lists 14 Maintenance / Tube Cutting references, none of whom receive access without an existing enrollment/team assignment.
- Added a SELECT-only `audit:bbs-phase10e` gate covering staged safety, feature flags, approved Pilot roster/configuration, representative workflow evidence, data integrity, Unsafe-to-Action reconciliation and Action Email Outbox state.
- Added a rollout contract and Pilot acceptance runbook. The audit never creates business fixtures, changes `staged_admin_only`, sends email, deploys or opens access.
- Fixed two test-maintenance defects exposed by the complete phase matrix: Phase 9 now recognizes the authoritative schedule-based `KpiRequired` evaluation, and Phase 10B/10C/10D regression cache assertions accept the current Phase 10D-5 asset.
- Phase 7-10E contracts, full Backend suite (`131/131`) and authenticated browser UAT (8 tabs, 5 phone/landscape viewports) pass. No runtime/API/schema/private-upload/business-data change occurred.
- The initial local decision was `PLATFORM_BLOCKED` because the local setting row for `staged_admin_only` was absent. Business configuration was also incomplete for Maintenance / Tube Cutting, so no ordinary-user rollout or deployment was performed.
- Added and applied the idempotent local staged-gate migration. Local `staged_admin_only=1` now passes with 0 business rows created, advancing the decision to `CONFIGURATION_REQUIRED`.
- Added a SELECT-only Pilot Master audit. It found real Group Leader `002671`, test account `111111`, no Tube Cutting Operator, and Maintenance Operator candidate `012816` with a blank Unit. No roster, enrollment, assignment, schedule or other business record was changed.
- After explicit owner approval, added and ran an API-driven Local-only Pilot configuration. Employee `012816` now belongs to Tube Cutting, Group Leader `002671` has one Active KPI-required/self-managed enrollment, `012816` has one effective team assignment, and the enrollment has one Monday-Friday target-1 schedule; test account `111111` remains excluded. A second run was idempotent with no new changes.
- The Phase 10E gate explicitly blocks Pilot acceptance when fewer than two effective assignments exist because Batch Observation requires at least two distinct employees. Production business configuration and ordinary-user rollout remain unchanged.

## BBS Smart Card Phase 10D-5 Mobile Agenda & Risk Detail (2026-09-01, Local only)

- Added a responsive Inspector Schedule Agenda as the default view, with status, target/actual, source, reason and authorized override removal; Admin can switch to the existing Calendar presentation.
- Added Admin-only Node/PHP Community Risk detail projection and accessible mobile dialog with reporter/observed context, private evidence links, Action ownership, due date, status controls and ordered Action History.
- Private stored filenames/paths are excluded from JSON; evidence continues through the existing authenticated object-authorized endpoint.
- Phase 8/9B/10D-3/10D-4/10D-5 contracts, PHP/Node syntax checks, authenticated 390 px browser UAT and the full Backend suite pass (`131/131` read/permission preflight).
- No schema, business row, schedule/Action rule, private upload path, Production deployment or GitHub push changed. `staged_admin_only=1` remains enabled.

## BBS Smart Card Phase 10D-4 Operational Follow-up (2026-09-01, Local only)

- Added an Admin-only Action Email Outbox panel with global Queued/Sent/Failed totals, delivery-readiness explanation, search, status/event/year filters and bounded server-side pagination.
- Hardened Node/PHP Retry parity: only Failed rows can be retried, the selected row is locked to prevent concurrent duplicate sends, and success/failure attempts update retry metadata and Audit Log without replacing the original outbox event.
- Added responsive 44 px controls, explicit error/reload state and cache bust `20260901-bbs-phase10d4`. No real Retry or email delivery was performed during UAT.
- Phase 5/8/10C-1..3/10D-1..4 contracts, authenticated 390 px browser UAT and the full Backend suite pass (`131/131` read/permission preflight).
- No schema, business row, notification recipient/content, SMTP setting, authorization, private upload path, Production deployment or GitHub push changed.

## BBS Smart Card Phase 10D-3 Scalable Lists & Filtering (2026-09-01, Local only)

- Added opt-in server-side pagination with Node/PHP parity for Observation History, Corrective Actions, Community Good/Risky reports, Personal Card recipients and issued Card history. Legacy requests still receive their existing array response.
- Added server-side search plus year/status/priority/Department/Safety Unit filters as applicable, with counts calculated after the established permission and privacy scope.
- Added responsive filter controls and accessible 44 px previous/next navigation. Community Good and Admin-only Risky lists page independently; Cards now support server-backed recipient search and Active/Revoked/Replaced history.
- Added `test:bbs-phase10d3` and extended authenticated browser UAT to validate paged APIs and mobile/desktop rendering. Phase 4/5/8/10B-1..4/10C-1..3/10D-1..3 contracts, 390 px/1365 px browser UAT and the full Backend suite (`131/131` preflight) pass.
- No schema, business row, authorization, Master/Pilot configuration, QR/card lifecycle, private upload path, Production deployment or GitHub push changed.

## BBS Smart Card Phase 10D-2 KPI Status Clarity (2026-09-01, Local only)

- Added a Node/PHP parity KPI semantic contract: `N_A`, `NOT_CONFIGURED`, `NOT_INSPECTED`, `ZERO_PERCENT` and `PERCENT`.
- Workspace, Inspector Compliance and Analytics now expose and render the server status without turning unknown/not-applicable values into `0%`.
- Excel Summary and KPI People sheets export both readable status and stable status code; nullable percentages remain blank.
- Mobile/desktop Browser UAT passed at 390 px and 1365 px against the authenticated Admin workspace. No schema, stored data, target formula, authorization, upload path, Production or GitHub state changed.

## BBS Smart Card Phase 10D-1 Checklist Readiness & Observation Eligibility (2026-09-01, Local only)

- Added Node/PHP-parity Checklist Readiness classification to the existing eligible-employee response without changing route authorization or request payloads.
- Added accessible Ready/non-ready status and reasons to Team, Single and Mobile Batch selectors; only Ready employees can be selected, and Batch select-all skips disabled rows.
- Kept Single/Batch server resolution, immutable Observation snapshots, atomic per-employee Batch records and Draft recovery unchanged.
- Added contract/parity and authenticated mobile/desktop browser UAT coverage. No schema, data, upload, deployment, rollout or GitHub change occurred.

## BBS Smart Card Phase 10B-4 Template Preview & Print Readiness (2026-09-01, Local only)

- Replaced raw Personal/Department template viewing with a composite card preview showing existing background artwork, representative identity/Department data, QR placement, print cut line and safe area.
- Added client-side Ready/Warning/Blocked checks for image decoding/type, aspect ratio, estimated DPI, configured millimetre dimensions, QR availability/size, Personal scope and text area.
- Routed Personal Activate/Issue/Replace and Department Activate/Print through preview confirmation. Existing popup-before-one-time-QR mutation, private template reads, server validation and print logs remain authoritative.
- Added `test:bbs-phase10b4` and updated cache/regression contracts. Phase 10B-1/2/3/4 and 10C-1/2/3 tests plus authenticated mobile Department browser UAT pass.
- No API, schema, authorization, card/QR lifecycle, upload path, stored file, business row, Production deployment or GitHub push changed.
...
- Reorganized the existing Admin `บันทึกผล PPE Inspection` modal into three
  guided UI steps: inspection information, inspected employee/inspector, and
  PPE checklist/evidence. The wider modal adds step progress, back/next
  navigation, a sticky action footer, and larger status controls.
- The PPE template, fields, validation, payload, API endpoint, evidence upload,
  and automatic Violation behavior are unchanged. Every checklist item still
  defaults to `N/A`, and `N/A` remains selectable exactly as before.
- Frontend cache key: `20260824-safety-culture-ppe-form-r1`. No schema, API,
  permission, business-data, or upload-storage migration is required.
- Focused PPE form regression passed `12/12`; existing Safety Culture
  Assessment layout regression remained green at `13/13`; JavaScript syntax,
  diff whitespace, and UTF-8 replacement-character checks passed.
- Production deployment completed from rollback backup
  `backups/production/safety-culture-ppe-form-predeploy-20260824-170100`.
  FTP download-back and public HTTPS SHA-256 passed `4/4`. Authenticated
  read-only UAT passed the new PPE markers and the existing system surfaces.
  Headless Chrome UAT opened the live PPE form, navigated all three steps, and
  confirmed all `3/3` rendered checklist items still default to `N/A` without
  submitting. Assessment rows remained `32/32`, PPE inspection rows `2/2`,
  Yokoten responses `101/101`, and outbox `128/128`. No schema, business-data,
  permission, API, or upload-storage mutation occurred; expected side effects
  were limited to Admin login audit/housekeeping.

## Safety Culture Assessment history layout (2026-08-24, deployed)

- Reorganized the existing `ผลการประเมิน` tab into three client-side views:
  `ประวัติการประเมิน`, `ภาพรวมและแนวโน้ม`, and the Admin-only
  `ตั้งค่าจุดตรวจ`. Existing assessment data, APIs, permissions, CRUD forms,
  detail modal, and PDF exports are unchanged.
- History now defaults to a compact decision view (date, area, checkpoint,
  average, weakest topic, maturity, note, and actions). `คะแนนทั้งหมด` keeps
  T1-T7 available in a fixed-width table with explicit column widths and
  table-local horizontal scrolling. Page size supports 10/20/50 rows, and all
  assessment filters reset pagination to page 1.
- Monthly summary, heatmap, insight, and location follow-up moved to the
  Overview view. Follow-up Notes and the Culture Maturity guide are retained in
  a collapsed additional-information section, reducing the default page
  length without removing information.
- Focused regression passed `13/13`; JavaScript syntax passed; authenticated
  local Chrome UAT passed compact/detailed History, Overview, and Admin Setup
  with no page-level horizontal overflow and unchanged assessment/response
  counts. Evidence:
  `backups/local/yokoten-dashboard-browser-uat-20260824T091046`. No schema,
  API, permission, business-data, or upload-storage change. Cache key:
  `20260824-safety-culture-assessment-r1`.
- Production deployment completed after rollback backup
  `backups/production/safety-culture-assessment-predeploy-20260824-162134`.
  FTP download-back SHA-256 passed `4/4`; public HTTPS SHA-256 and cache checks
  passed `4/4`; authenticated read-only UAT preserved Yokoten responses
  `101/101` and outbox `128/128`; Chrome UAT passed compact History (10 rows),
  detailed T1-T7, Overview, and Admin Setup. Assessment count remained `24/24`.
  No schema, API, business-data, permission, or upload-storage mutation.

## Yokoten topic follow-up actions (2026-08-24, deployed)

- Extended the existing Admin Topic Coverage view with year/risk filters,
  topic-specific Excel export, due date and overdue days for incomplete rows,
  per-row/current-view Reminder actions, and a direct response-on-behalf link
  for Departments that have not responded.
- Added Admin-only `POST /api/yokoten/reminders/send` to Node development and
  PHP shared-hosting routes. Both paths recompute missing Department/Unit scope
  server-side, skip completed scope, resolve recipients through Email
  Requirement Rules with company-domain validation, and suppress duplicate
  reminders for the same topic/Department on the same day.
- A successful Reminder uses the existing `Yokoten_EmailOutbox` and Admin audit
  log. No MySQL schema, response-data migration, permission widening, or upload
  storage change is required. Cache key:
  `20260824-yokoten-topic-coverage-r3`.
- Focused Topic Coverage regression passed `20/20`; Node/PHP Reminder
  permission and completed-scope guards passed with unchanged response/outbox
  fingerprints; the full backend suite and read/permission UAT passed `97/97`;
  JavaScript syntax and PHP lint passed. Automated Chrome UAT was attempted
  twice but timed out at DevTools `Page.enable` before page navigation, so it
  made no UI or business-data write.
- After explicit authorization, one real SMTP delivery test was sent to the
  configured company Admin mailbox. Local Yokoten outbox row `109` is `Sent`
  with marker `YOKOTEN_REMINDER_TEST_20260824082655` and no ResponseID. The
  temporary sender script was removed after verification; response and
  completion data were not modified.
- Production deployment completed after rollback backup
  ...
  authenticated read-only UAT passed with responses unchanged `101/101` and
  outbox unchanged `128/128`; browser UAT passed 10 Dashboard rows, 10 Yokoten
  matrix rows, and 10 Topic Coverage options. Current Production data has zero
  incomplete/missing rows, so conditional Reminder and response-on-behalf
  actions are correctly hidden. No Production Reminder was sent during deploy;
  expected side effects were limited to Admin login audit/housekeeping.

## Yokoten cross-view year-scope parity (2026-08-24, deployed)

- Read-only Production comparison found the same 10-Department scope in
  Company Overview and Admin Department/Topic completion. Every active 2026
  topic matched at `10/10`; the earlier localhost `2 complete / 1 partial /
  7 missing` result comes from its older local database snapshot.
- Closed a future cross-year mismatch in Activity Targets. Node and PHP now
  filter active Yokoten topics by the requested year, including legacy topics
  with a null `DateIssued`, exactly like Company Overview and Dashboard.
  Department completion still requires the existing shared full-Unit coverage
  result.
- Focused year-scope parity passed `4/4`; authenticated Node/PHP Personal Target
  runtime parity passed without changing the database fingerprint; the full
  backend suite and read/permission UAT `97/97` passed. No schema,
  business-data, permission, upload-storage, or frontend cache change.
  Deployed together with the topic-centric coverage follow-up release.

## Yokoten topic-centric Department/Unit coverage (2026-08-24, deployed)

- Extended the existing Admin `ภาพรวม + อนุมัติ` surface with a two-mode switch:
  the current Department overview remains unchanged, while `ดูตามหัวข้อ` shows
  exactly which targeted Departments have completed, partially covered Units,
  or not responded to the selected topic.
- Topic Coverage reuses `dept-completion.topicBreakdown` and `all-responses`.
  Its KPI strip shows completed Departments, partial responses, missing
  responses, and covered/required Units. The table lists covered and missing
  Unit names, response time, and opens the existing response detail modal.
  Filters cover all/complete/partial/missing states. Department PDF/Excel
  buttons are hidden in Topic mode to avoid exporting a mismatched view.
- No API, schema, business-data, permission, upload-storage, or workflow change.
  Focused regression passes `12/12`; local browser UAT passed 10 topics, the
  Robot topic at 2 complete + 1 partial (`1/6`) + 7 missing, a one-row partial
  filter, and unchanged response count. Evidence:
  `backups/local/yokoten-dashboard-browser-uat-20260824T075348`. Cache key:
  `20260824-yokoten-topic-coverage-r2`. Deployed through the final `r3`
  follow-up release.

## Yokoten Unit-complete department workflow and Admin-on-behalf privacy (2026-08-24, deployed)

- Kept the existing one-response-per `(Department, YokotenID)` storage model,
  but changed completion semantics: when a topic targets Units in a Department,
  that Department-topic pair counts as 1 only after the response covers every
  required Unit. Topics without Unit scope still complete with one Department
  response. Node and PHP fail closed when legacy Unit scope cannot be resolved.
- New and edited responses validate full Unit coverage before any transaction.
  The existing response form now shows the required Unit checklist; Admin work
  queue, response detail, Department matrix, user progress, employee completion,
  company overview, Activity Targets, and Overview Dashboard use the same rule.
- Configured dashboard Units continue to select in-scope topics but never
  discard a stored response before its target Unit coverage is evaluated.
- Node and PHP now identify an Admin responder from Employee Master. User-facing
  topic/detail/history payloads replace the real Admin actor with
  `ผู้ดูแลระบบตอบแทนหน่วยงาน`; Admin views and stored response/audit identity
  remain unchanged.
- Production read-only diagnosis found the former dashboard filter excluded 13
  responses, while one Production 1 response covers only `PD1 Assy 3/1` of six
  required Units. Under the corrected rule that Department remains `9/10`
  until Admin edits the existing response and selects all six Units; the other
  five named Units do not require separate response rows.
- No schema, business-data, permission, or upload-storage change was required.
  Focused Node/PHP parity tests pass `14/14`, bulk-response smoke passes
  `20/20`, the full backend suite passes, and local read/permission UAT passes
  `97/97`.
- Production rollback backup is
  `backups/production/yokoten-unit-coverage-predeploy-20260824-135900`. FTP
  download-back SHA-256 passed `8/8` at
  `backups/production/yokoten-unit-coverage-upload-verify-20260824-140019`;
  public HTTPS SHA-256 passed `3/3` at
  `backups/production/yokoten-unit-coverage-https-verify-20260824-140847`.
  Authenticated read-only smoke passed `11/11` at
  `backups/production/yokoten-unit-coverage-readonly-uat-20260824-140237`:
  company completion is correctly `99/100`, Production 1 is `9/10`, and the
  one partial topic is `1/6` Units with the expected five missing Units.
  Response count stayed `100/100`. Browser UAT at
  `backups/production/yokoten-dashboard-browser-uat-20260824T070818` rendered
  the 10-row Overview and 10-row Yokoten Unit matrix, including `1/6`, without
  changing response data. No temporary helper/test row was created; normal UAT
  login audit/housekeeping was the only expected side effect. Final manifest
  download-back verification passed at
  `backups/production/yokoten-unit-coverage-final-manifest-verify-20260824-140938`;
  FTP temporary-helper count is `0` and the HTTP helper probe returns `404`.

## System Console Employee Master sorting and source filters (2026-08-24, deployed)

- Added an in-place `เรียงลำดับ` control to the existing Employee Master filter
  bar: default, newest, oldest, EmployeeID ascending/descending, and Thai name
  ascending. EmployeeID natural sorting preserves six-digit leading zeroes and
  orders prefix IDs such as `AP0002` before `AP0010`.
- Admin employee list responses now include nullable `CreatedAt` and
  `CreationSource` metadata from the latest successful `CREATE_EMPLOYEE` audit
  for that EmployeeID, with Node/PHP parity. Employees without historical audit
  time remain in the list and are placed after timestamped employees in both
  time modes.
- Added `ทั้งหมด / เพิ่มทีละคน / Import Excel` source chips to the existing
  recent-additions strip. The UI reads the latest 20 audit entries but displays
  at most five matching cards. No schema, business-data, permission, or upload
  storage change is required. Cache key: `20260824-employee-sort-r1`.
- Focused cross-path regression, the full backend suite, API smoke, and
  read/permission UAT `96/96` pass. Node/PHP write UAT expanded to `26/26` and
  cleanup restored baseline fingerprints with zero synthetic residue. Local
  Chrome UAT passed natural ID sorting, newest sorting, source filtering, 25
  table rows, and zero blocking runtime errors. The only ignored localhost
  errors were three expected 404s for a Production Branding image URL that is
  not stored in the local uploads directory.
- Production code rollback is
  `backups/production/employee-sort-predeploy-20260824-113422`; FTP SHA-256
  passed `5/5` at
  ...
  Authenticated read-only UAT passed `9/9`: 5 of 2,495 current employees have
  auditable creation metadata, unauthenticated access returned 401, and Viewer
  access returned 403. Production browser evidence is
  `backups/production/employee-master-browser-uat-20260824T043702`; natural ID
  sort, newest sort, source filtering, and 25 rendered rows passed with zero
  runtime errors. No Production business-data write or temporary helper was
  used; expected side effects were normal UAT login audit/housekeeping only.
  Final manifest download-back verification passed at
  `backups/production/employee-sort-final-manifest-verify-20260824-113900`.

## System Console Employee Master recent additions (2026-08-24, deployed)

- Extended the existing Employee Master card with a compact five-person recent
  additions strip. It shows employee name/ID, department, adding Admin, date and
  time, plus `เพิ่มทีละคน` or `Import Excel`; clicking a person reuses the
  existing EmployeeID table search, and the Audit Log button opens the existing
  audit tab.
- Added Admin-only `GET /api/admin/employee/recent-additions` with Node/PHP
  parity. It reads successful `CREATE_EMPLOYEE` audit entries and joins current
  Employee Master rows, so duplicates, failed rows, and employees later deleted
  from the master are excluded.
- Manual create, current multipart Excel import, and legacy JSON import now
  write per-created-employee audit entries while preserving the existing batch
  `IMPORT_EMPLOYEES` summary. No schema, new table, business-data migration,
  permission widening, or upload-storage change is required. Frontend cache
  key: `20260824-employee-recent-additions-r2`.
- Local verification passed the full backend suite, read/permission UAT
  `96/96`, and Node/PHP employee write UAT `24/24`. The write UAT verified both
  `manual` and `import` recent-addition sources, removed every synthetic row,
  and restored the baseline data fingerprints.
- Production deployment included the create-only import protection and recent
  additions UI/API. Code rollback backup:
  `backups/production/employee-master-predeploy-20260824-101440`; FTP upload
  verification: `backups/production/employee-master-upload-verify-20260824-101635`
  (`6/6`); HTTPS verification:
  `backups/production/employee-master-https-verify-20260824-101900` (`4/4`).
  Authenticated read-only UAT passed `8/8`, including Admin 200,
  unauthenticated 401, and Viewer 403. Browser UAT evidence is
  `backups/production/employee-master-browser-uat-20260824T032659`: three recent
  cards and 25 employee rows rendered with zero JavaScript/runtime errors.
  Production business-data writes were false; only normal UAT login
  audit/housekeeping side effects occurred, and no temporary helper was used.
  Final manifest download-back verification passed at
  `backups/production/employee-master-final-manifest-verify-20260824-102900`.

## System Console Employee Import create-only protection (2026-08-24, deployed above)

- Confirmed manual Admin employee creation already used the shared `CREATE`
  path and rejected duplicate `EmployeeID` with HTTP 409 without changing the
  existing row. The UI now explains that duplicate IDs must be edited through
  the explicit employee-edit action.
- Changed both the current multipart Excel import and the legacy JSON import
  from `UPSERT` to `CREATE` in Node development and PHP shared-hosting routes.
  Existing EmployeeIDs and repeated IDs inside the same file are skipped; they
  can no longer clear profile fields, clear CompanyEmail, or change an existing
  Role.
- Multipart import results now separate `addedCount`, `duplicateCount`,
  `errorCount`, and row-level duplicate reasons while retaining
  `successCount` compatibility. The System Console presents the same split and
  HTML-escapes all workbook/API values shown in the result table.
- Completed the existing advertised `Team` import path in Node/PHP and added
  Team to the downloadable template/reference data. No MySQL schema,
  business-data, permission, or upload-storage change is required. Frontend cache key:
  `20260824-employee-import-create-only-r1`.
- Verification passed: JavaScript syntax, PHP lint, focused Node/PHP import
  contract regression, full backend verification, API smoke, read/permission
  UAT `94/94`, and local write UAT `22/22`. The write UAT proved that manual
  duplicate creation and Import both preserve the existing employee's
  name/Role, a new ID is inserted once, duplicate workbook rows are skipped,
  and cleanup leaves zero synthetic rows with baseline database fingerprints
  restored.

## Hiyari dashboard, History filters, and Overview alignment deployed (2026-08-24)

- Corrected the Hiyari column in `Department Coverage Overview` to use the same annual people-progress definition as the Hiyari Dashboard: current assignees with at least one non-deleted report in the selected year divided by current assignments in each department.
- Reporter and assignment matching is EmployeeID-based, case/whitespace tolerant, and independent of report status. Unassigned reporters do not inflate the numerator, and departments without current Hiyari assignments display `N/A` instead of a misleading percentage.
- Node and shared-hosting PHP now return matching `coverageMeta` numerator, denominator, and source values. Focused parity regression and the live local 10-department coverage smoke passed; the full backend suite, API smoke, and read/permission UAT passed `94/94`. No MySQL schema/data, permission, or upload-storage change was required.
- Production deployment combined all Hiyari work from 2026-08-24: the Dashboard assignee progress card, complete PHP History filter parity, and Overview assignment-based Hiyari coverage. The six runtime files were `index.html`, `public/js/main.js`, `public/js/pages/hiyari.js`, `api/handlers/workflow_phase6.php`, `api/index.php`, and `deploy-manifest.json`; cache key is `20260824-hiyari-dept-progress-r1`.
- Code rollback backup is `backups/production/hiyari-today-predeploy-20260824-091147`; upload verification is `backups/production/hiyari-today-upload-verify-20260824-091147`. FTP SHA-256 matched `6/6`, HTTPS matched `4/4`, and no temporary Production helper/file remains. A full database export was not captured because this was a code-only release with no schema or business-data mutation.
- Authenticated read-only Production UAT passed at `backups/production/hiyari-today-readonly-uat-20260824T021630`: annual assignment progress was `33/66 = 50%`, Overview Hiyari numerator/denominator/percentage matched the Hiyari Dashboard for all 10 scoped departments, and combined Department + Stop Type + month + area History filtering returned `1/35` matching rows. Business-data writes were `false`; expected side effects were limited to normal successful-login audit/housekeeping.

## Hiyari Dashboard assignment progress by department (2026-08-24, deployed above)

- Reworked the existing Dashboard department-summary card into annual assignment progress by department. Each configured department now shows distinct assignees who submitted at least one Hiyari report, total current assignments, completion percentage, and remaining people; the displayed-scope summary shows the same totals.
- The existing `pinnedDepts` Dashboard configuration still selects which departments are shown. With no pinned selection, every department with a Hiyari assignment is shown; the Dashboard department filter narrows the card to that department.
- Extended the existing `/api/hiyari/stats` response with `assignmentCompletion.byDepartment` in both Node and shared-hosting PHP. The aggregate is calculated from the selected year independently of month/status/rank filters and viewer report visibility, so normal users receive accurate counts without receiving other employees' report details.
- No new endpoint, MySQL table/column, business-data migration, permission, or upload-storage change is required. Frontend cache keys advanced to `20260824-hiyari-dept-progress-r1`. Focused Hiyari regressions, JavaScript syntax, PHP lint, UTF-8 scan, and `git diff --check` passed locally. After local MySQL became available, the full backend suite, API smoke, and read/permission UAT also passed `94/94`.

## CCCF Form A Worker annual mode-aware UI (2026-08-22, deployed)

- Production deployment completed with final cache key `20260822-cccf-worker-pdf-r10`. The exact runtime scope was `index.html`, `public/js/main.js`, `public/js/pages/cccf.js`, `public/js/pages/admin.js`, `public/js/pages/dashboard.js`, `api/handlers/platform.php`, `api/handlers/admin_phase8.php`, `api/index.php`, and `deploy-manifest.json`. FTP and HTTPS SHA-256 verification passed. Authenticated Production UAT passed Manual, Actual, Form A Permanent, Dashboard, Safety Core Data, and both Worker PDF modes with zero browser runtime errors. The annual Worker source was restored to Manual / Override after testing.
- Fresh rollback backup: `backups/production/cccf-worker-predeploy-20260822-160532` (147 database tables; DB SHA-256 `d43ff5ca6414c2c0ba1f9728780c6614a16756e79da9f2773570772a8a7243a8`; 777 upload files / 1,153,857,700 bytes). No schema, business-record, or upload-storage mutation was required. Temporary backup helper cleanup passed by both absent FTP listing and HTTP 404; temporary test rows remaining: `0`.

- Follow-up r7 makes the existing Worker PDF export mode-aware without changing Form A Permanent. Manual / Override exports the document metadata, four Unit KPIs, target-allocation state, and a paginated register containing every Unit with `ต้องส่ง / ส่งแล้ว / ยังไม่ส่ง / %`. Actual records uses the same target denominator, adds secondary raw-record/Stop/Rank audit information, and appends readable 14-row detail pages with EmployeeID, name, Unit, Stop, Rank, area, hazard description, and attachment status. The Export PDF button is available in both modes; the Worker submit action remains Actual-only. Fixed-A4 header placement is deterministic and the exporter validates the top header band before accepting each captured page. Final authenticated UAT exported Manual 3 pages and Actual 4 pages, visually verified all 7 pages, passed rendered pixel checks `7/7`, recorded zero browser errors, restored the annual source to Manual, and passed the full backend/read-permission suite `94/94`. Cache key: `20260822-cccf-worker-pdf-r7`.
- Follow-up r6 fixes Admin controls missing immediately after a same-page login. The CCCF module previously captured `TSHSession.getUser()` at import time, before login completed, and retained `isAdmin=false` until a full refresh. Every `loadCccfPage()` now refreshes the current session role before selecting Admin/User requests and rendering controls; switching to a normal User in the same loaded app also removes Admin UI correctly. Backend authorization remains unchanged.
- Auth regression passed for null, Admin role variants, explicit Admin flags, normal User, and same-module Admin -> User transitions. Full backend permission/read UAT passed `94/94`. Authenticated browser navigation Hiyari -> CCCF without refresh showed 2 Admin mode buttons and 21 Manual edit buttons, showed no User fallback label, and logged zero runtime errors. Cache key: `20260822-cccf-auth-refresh-r6`.
- Follow-up r4 makes `CCCF_Unit_Targets.yearly_target` the shared Target for both Manual and Actual. Switching mode now changes only Achieved: Manual uses the existing override/fallback behavior, while Actual counts distinct Form A Worker submitters per selected year/Unit. Multiple forms from the same EmployeeID count as one person; stable legacy fallbacks preserve old rows without an EmployeeID.
- Follow-up r5 simplifies the primary Worker Unit summary to four decision values: `ต้องส่ง`, `ส่งแล้ว`, `ยังไม่ส่ง`, and capped `% ความคืบหน้า`, with a progress bar on every Unit row. The technical Personal Target, Raw records, Diff, and per-person status diagnostics remain Admin-only but are collapsed behind `รายละเอียดตรวจสอบสำหรับแอดมิน` by default.
- Authenticated r5 browser UAT passed: Actual displays totals `1538 / 1 / 1537 / 0%` and Maintenance `23 / 1 / 22 / 4%`; the Admin disclosure is closed initially and expands to the retained Personal Target, Raw records, Diff, and status data. Manual remains `1538 / 1315 / 223 / 86%`, Maintenance remains `23 / 23 / 0 / 100%`, edit controls remain available, Actual-only surfaces remain hidden, and browser errors are `0`. The annual mode was restored to Manual after testing.
- Overview and Safety Core Data now use the same selected Unit scope and shared Unit targets in both modes instead of replacing the Actual denominator with the per-person calculation engine. The per-person engine remains visible only as Admin audit/diagnostic information on the Worker page.
- Local authenticated UAT verified two Worker forms from one employee produce one Actual submitter, Maintenance Overview reports `1/37` across the selected Maintenance and Tube Cutting Unit targets, Safety Core Data reports `1/23` for Maintenance, and Form A Permanent remains `21/66` with its existing workflow. The local annual mode was restored to Manual after the test. Cache key: `20260822-cccf-unit-summary-r5`.
- Form A Worker now resolves `Manual / Override` or `Actual records` per calendar year. The annual map is stored in the existing Dashboard config JSON as `cccfWorkerSourceByYear`; the legacy `cccfWorkerSource` remains the fallback for years without an explicit selection.
- Overview and System Console > Safety Core Data now resolve the same year-specific Worker source in both Node and shared-hosting PHP. Safety Core Data exposes the resolved source and labels the column `CCCF Form A Worker (Manual)` or `(Actual)` while retaining the separate `CCCF Permanent` column.
- Manual mode keeps the existing Unit Summary, target, achieved override, progress chart, Admin edit controls, and a Unit-only PDF export. Rank criteria, My Records, Stop 1-6, related Worker forms, submit action, Actual-detail PDF pages, and the normal All Records surface are hidden because they are based on actual submissions.
- Existing Worker submissions are not deleted or changed. Admin can expand a year-scoped legacy Actual-record disclosure for audit; switching back to Actual restores the original submission UI and actions. Form A Permanent is unchanged.
- Node and shared-hosting PHP dashboard compliance both use the same year-aware resolver. No MySQL schema, business-record, upload-storage, permission, or Form A Permanent change is required.
- Verification passed for Node/PHP annual fallback parity across Worker, Overview, and Safety Core Data; frontend/backend syntax; PHP lint; the full backend suite with read/permission UAT `94/94`; and authenticated browser UAT across Manual, Actual, Overview, Safety Core Data, retained legacy records, and Form A Permanent. Browser runtime errors were `0` (the existing Tailwind CDN warning remains). This change is local and has not been deployed.

## Hiyari History filter parity fix (2026-08-22, local)

- Fixed the Production PHP Hiyari list route to honor the complete History-tab filter contract. The frontend sends `dept`, but PHP previously read only `department`; PHP also ignored `stopType`, `month`, `area`, and `q`, which caused unrelated rows to remain visible.
- PHP now supports `status`, `risk`, `dept`/`department`, `stopType`, `rank`, `month`, `area`, `year`, `q`, and `review`/`reviewStatus`, matching the Node development route. Node also accepts both department and review aliases for compatibility.
- All SQL remains parameterized and the existing Hiyari visibility rules remain applied after the selected filters. No schema, business-data, permission, frontend-cache, or upload-storage change is required.
- Verification passed: focused Node/PHP filter contract, JavaScript syntax, PHP lint, UTF-8 scan, `git diff --check`, the full backend suite with read/permission UAT `94/94`, and browser UAT on the 35-row local dataset. Department, Stop Type, month, area, combined filters, search, empty state, and Clear all behaved correctly with no new browser errors.

## Hiyari assignment progress year-scope fix (2026-08-22, deployed)

- Changed the Hiyari heading from `รายงานเหตุการณ์เฉียดอุบัติเหตุ` to `รายงานเหตุการณ์เกือบเกิดอุบัติเหตุ`.
- Fixed Manage > Assignment Progress and each assignment status to count distinct assignees who submitted during the selected year instead of only the current calendar month. The imported Production snapshot verifies the root cause: 33 of 66 assignees submitted in 2026, while August alone has zero submissions.
- Added right-click PNG export for the complete Manage > Assignment panel. Only the exact `hiyari-assignment-list` target uses the deterministic tall-card exporter; all existing Hiyari card targets retain their previous exporter.
- Extended the existing Hiyari PDF without removing its two summary pages. The generated report now appends a paginated Assignment Submission Register with assignee name, EmployeeID, department, due date, sent/not-sent status, report count, and follow-up status; footer page totals are dynamic.
- Added focused annual-scope/export regression coverage and advanced the Hiyari/main cache keys. No API, MySQL schema, business-data, permission, or upload-storage change.
- Production deployment completed together with the KY PNG fix. Fresh rollback backup `backups/production/ky-hiyari-predeploy-20260822-130637/` contains 147 database tables (SHA-256 `c0da174e9093d48748d99ec0fd63bab9e84963ccfd93e7454f2384443e611c66`), 777 uploads / 1,153,857,700 bytes, and the six previous runtime files. The HMAC backup helper was removed and verified FTP-absent plus HTTP 404 before deployment.
- FTP download-back and HTTPS cache verification both matched 6/6 files. Authenticated Production UAT confirmed the corrected heading, annual `33/66` progress with 66 assignment rows, complete right-click PNG, and a readable seven-page PDF containing all assignees. JavaScript runtime errors were 0; no production database, schema, business-data, or upload-storage mutation occurred.

## KY progress PNG export fix (2026-08-22, deployed)

- `ky-program-progress` and `ky-evidence-completion` now use the shared card-image export path with deterministic 1400 px layout, font readiness, full-height capture viewport, expanded truncated text, and clone-only row normalization.
- The change prevents off-screen Safety Unit rows from exporting as blank boxes and prevents department/unit labels from being cut off. Other KY card targets retain the existing legacy exporter as a fallback boundary.
- No API, database, permission, upload-storage, or business-data change.
- Authenticated Production UAT downloaded and visually verified both `ky-program-progress` and `ky-evidence-completion`: all department/Safety Unit rows and labels are present with no blank off-screen rows or clipped text. An unrelated 4M page also loaded successfully, and Production recorded zero JavaScript errors.
