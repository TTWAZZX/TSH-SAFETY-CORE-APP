...
- Local follow-up on 2026-09-07 expands the existing delegation manager: one delegate may receive multiple individually selected assigned owners or all currently assigned owners in one Department. The server materializes the Department selection into exact owner/delegate rows in one transaction, so authorization, KPI ownership and direct-PDF boundaries remain unchanged. No schema or upload-path change; not pushed or deployed.
