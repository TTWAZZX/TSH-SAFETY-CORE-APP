...
function handle_bbs_observation_routes(string $method, string $path): bool
{
    ...
    if ($method === 'GET' && $path === '/bbs/workspace') {
        ...
        json_response(['success' => true, 'data' => ['observer' => $observer, 'period' => compact('year', 'month', 'from', 'to'), 'kpi' => ['numerator' => $numerator, 'denominator' => $target, 'percentage' => $target ? $compliance['summary']['percentage'] : null, 'formula' => 'Capped submitted observations / effective inspector schedule target (Asia/Bangkok)', 'enrolled'=>(bool)$enrollment, 'uniqueObserved' => (int) ($metric['UniqueObserved'] ?? 0), 'safe' => (int) ($metric['SafeCount'] ?? 0), 'unsafe' => (int) ($metric['UnsafeCount'] ?? 0)], 'team' => $team, 'recent' => $recent]]);
    }
    ...
    if ($method === 'GET' && $path === '/bbs/observations') {
        ...
        $rows = db_rows("SELECT o.id,o.ObservationNo,o.ObserverEmployeeID,o.ObserverNameSnapshot,o.ObservedEmployeeID,o.ObservedNameSnapshot,o.ObservedDepartmentSnapshot,o.ObservedUnitSnapshot,o.Status,o.ObservationDate,o.SubmittedAt,(SELECT COUNT(*) FROM BBS_Observation_Answers a WHERE a.ObservationID=o.id AND a.Response='Safe') SafeCount,(SELECT COUNT(*) FROM BBS_Observation_Answers a WHERE a.ObservationID=o.id AND a.Response='Unsafe') UnsafeCount,(SELECT COUNT(*) FROM BBS_Observation_Answers a WHERE a.ObservationID=o.id AND a.Response='N/A') NACount FROM BBS_Observations o WHERE $where ORDER BY o.ObservationDate DESC,o.id DESC LIMIT 250", $params); json_response(['success' => true, 'data' => $rows]);
    }
