...
function handle_forklift_routes(string $method, string $path): bool
{
    ...
    if ($method === 'GET' && $path === '/forklift/requests') {
        ...
        if(isset($_GET['sourceLicenseId'])){ $sourceLicenseId=(int)$_GET['sourceLicenseId']; if($sourceLicenseId<=0)json_response(['success'=>false,'message'=>'Invalid source license ID.'],400); $where[]='r.SourceLicenseID=?'; $p[]=$sourceLicenseId; }
    ...
    if($renewalDraft!==null&&$method==='POST'){
        ...
        $pdo=db();try{
            ...
            $lock=$pdo->prepare('SELECT ID FROM forklift_licenses WHERE ID=? FOR UPDATE');$lock->execute([$license['ID']]);
            $open=$pdo->prepare("SELECT ID,RequestNo,RequestStatus FROM forklift_license_requests WHERE SourceLicenseID=? AND RequestKind='RENEWAL' AND RequestStatus IN ('DRAFT','RETURNED','SUBMITTED','UNDER_REVIEW','PENDING') ORDER BY FIELD(RequestStatus,'UNDER_REVIEW','SUBMITTED','PENDING','RETURNED','DRAFT'),ID DESC LIMIT 1 FOR UPDATE");$open->execute([$license['ID']]);$existing=$open->fetch();
            if($existing&&!in_array(strtoupper((string)$existing['RequestStatus']),['DRAFT','RETURNED'],true)){$pdo->rollBack();json_response(['success'=>false,'code'=>'RENEWAL_REQUEST_ALREADY_OPEN','message'=>'A renewal request is already open for this license.','id'=>(int)$existing['ID'],'RequestNo'=>$existing['RequestNo'],'RequestStatus'=>$existing['RequestStatus']],409);}
            if($existing){
                $stmt=$pdo->prepare("UPDATE forklift_license_requests SET LicenseTypeID=?,IssueDate=?,ExpireDate=?,CertificateNo=?,RequestNote=?,EmployeeNameSnapshot=?,DepartmentSnapshot=?,UnitSnapshot=?,PositionSnapshot=? WHERE ID=? AND RequestStatus IN ('DRAFT','RETURNED')");
                $stmt->execute([$license['LicenseTypeID'],$issue,$expire,fl_text($body['NewCertificateNo']??$body['CertificateNo']??'',120)?:$license['CertificateNo'],$body['RenewalNote']??$body['Note']??null,$emp['EmployeeName']??$license['EmployeeNameSnapshot'],$emp['Department']??$license['DepartmentSnapshot'],$emp['Unit']??$license['UnitSnapshot'],$emp['Position']??$license['PositionSnapshot'],$existing['ID']]);
                fl_sync_type_map($pdo,'forklift_request_type_map','RequestID',(int)$existing['ID'],$typeIds);
                fl_request_event((int)$existing['ID'],'RENEWAL_DRAFT_REUSED',$existing['RequestStatus'],$existing['RequestStatus'],'License '.($license['LicenseNo']??$license['ID']),$user);
                $pdo->commit();
                fl_audit($user,'REUSE_RENEWAL_REQUEST_DRAFT','forklift_license_request',$existing['ID'],['SourceLicenseID'=>$license['ID'],'EmployeeID'=>$license['EmployeeID'],'RequestStatus'=>$existing['RequestStatus']],200);
                json_response(['success'=>true,'id'=>(int)$existing['ID'],'RequestNo'=>$existing['RequestNo'],'RequestStatus'=>$existing['RequestStatus'],'RequestKind'=>'RENEWAL','reused'=>true]);
            }
            $requestNo=fl_next_no($pdo,'REQUEST','FLR');
            $stmt=$pdo->prepare("INSERT INTO forklift_license_requests(RequestNo,RequestKind,SourceLicenseID,EmployeeID,LicenseTypeID,IssueDate,ExpireDate,CertificateNo,RequestStatus,RequestNote,EmployeeNameSnapshot,DepartmentSnapshot,UnitSnapshot,PositionSnapshot,RequestedBy,RequestedByID) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->execute([$requestNo,'RENEWAL',$license['ID'],$license['EmployeeID'],$license['LicenseTypeID'],$issue,$expire,fl_text($body['NewCertificateNo']??$body['CertificateNo']??'',120)?:$license['CertificateNo'],'DRAFT',$body['RenewalNote']??$body['Note']??null,$emp['EmployeeName']??$license['EmployeeNameSnapshot'],$emp['Department']??$license['DepartmentSnapshot'],$emp['Unit']??$license['UnitSnapshot'],$emp['Position']??$license['PositionSnapshot'],fl_user_name($user),fl_user_id($user)]);
            $id=(int)$pdo->lastInsertId();fl_sync_type_map($pdo,'forklift_request_type_map','RequestID',$id,$typeIds);fl_request_event($id,'RENEWAL_DRAFT_CREATED',null,'DRAFT','License '.($license['LicenseNo']??$license['ID']),$user);$pdo->commit();fl_audit($user,'CREATE_RENEWAL_REQUEST_DRAFT','forklift_license_request',$id,['SourceLicenseID'=>$license['ID'],'EmployeeID'=>$license['EmployeeID']],201);json_response(['success'=>true,'id'=>$id,'RequestNo'=>$requestNo,'RequestStatus'=>'DRAFT','RequestKind'=>'RENEWAL','reused'=>false],201);
