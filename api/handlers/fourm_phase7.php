...
function fm_curriculum_active_scope_key(int $year,string $department,string $code,int $isActive=1): ?string {
    return $isActive===1?hash('sha256',$year.'|'.fm_key($department).'|'.fm_key($code)):null;
}
...
function fm_curriculum_duplicate(int $year,string $department,string $code,?string $excludeId=null): bool {
    foreach(db_rows('SELECT id,`Year`,Department,CurriculumCode FROM fourm_curriculums WHERE `Year`=? AND IsActive=1',[$year]) as $row){
...
function fm_bulk_code_preview(array $rows,array $options): array {
    ...
    foreach($rows as $row){
        if((int)($row['Year']??0)!==$options['year'])continue;
        if((int)($row['IsActive']??0)!==1)continue;
    ...
    foreach($preview as &$item){
        ...
        if((int)($item['IsActive']??0)!==1)continue;
...
function fm_log_pick(array $source,array $keys,$fallback=null) {
    foreach($keys as $key)if(array_key_exists($key,$source)&&$source[$key]!==null&&$source[$key]!=='')return $source[$key];
    return $fallback;
}
function fm_log_entity_type(string $action): string {
    if(strpos($action,'ASSIGNMENT')!==false)return strpos($action,'CURRICULUM_')===0?'curriculum_assignment':'course_assignment';
    if(strpos($action,'COURSE_MASTER_')===0)return 'course_master';
    if(strpos($action,'COURSE_')===0)return 'course';
    if(strpos($action,'CURRICULUM_')===0)return 'curriculum';
    return 'training_matrix';
}
function fm_log_status(string $action,string $side,array $source) {
    $status=fm_log_pick($source,['status','Status']); if($status!==null)return $status;
    $active=fm_log_pick($source,['isActive','IsActive']); if($active!==null)return (int)$active===1?'Active':'Inactive';
    if($side!=='after')return null;
    if(strpos($action,'REMOVE')!==false)return 'Removed';
    if(strpos($action,'DISABLE')!==false)return 'Inactive';
    if(strpos($action,'DELETE')!==false)return 'Deleted';
    if(strpos($action,'ASSIGNMENT')!==false&&(strpos($action,'CREATE')!==false||strpos($action,'REASSIGN')!==false||strpos($action,'TRANSFER')!==false))return 'Assigned';
    if(strpos($action,'CREATE')!==false||strpos($action,'LINK')!==false||strpos($action,'RESTORE')!==false)return 'Active';
    return null;
}
function fm_log_context(?string $cur,?string $course,?string $emp,array $value,string $side): array {
    $useTarget=$side==='after';
    $courseId=$useTarget?fm_log_pick($value,['targetCourseId','TargetCourseID'],$course):$course;
    $curriculumId=$useTarget?fm_log_pick($value,['targetCurriculumId','TargetCurriculumID'],$cur):$cur;
    $ctx=['employeeId'=>$emp?:null,'curriculumId'=>$curriculumId?:null,'courseId'=>$courseId?:null];
    if($courseId){
        $row=db_row('SELECT co.id courseId,co.CourseCode courseCode,co.CourseTitle courseTitle,cur.id curriculumId,cur.CurriculumCode curriculumCode,cur.CurriculumTitle curriculumTitle,cur.Department curriculumDepartment,cur.`Year` year FROM fourm_courses co LEFT JOIN fourm_curriculums cur ON cur.id=co.CurriculumID WHERE co.id=? LIMIT 1',[$courseId]);
        if($row){$ctx=array_merge($ctx,$row);$curriculumId=$row['curriculumId']??$curriculumId;}
    }
    if($curriculumId&&!isset($ctx['curriculumCode'])){
        $row=db_row('SELECT id curriculumId,CurriculumCode curriculumCode,CurriculumTitle curriculumTitle,Department curriculumDepartment,`Year` year FROM fourm_curriculums WHERE id=? LIMIT 1',[$curriculumId]);
        if($row)$ctx=array_merge($ctx,$row);
    }
    if($emp){
        $row=db_row('SELECT EmployeeID employeeId,EmployeeName employeeName,Department employeeDepartment,Unit employeeUnit,Position employeePosition FROM employees WHERE EmployeeID=? LIMIT 1',[$emp]);
        if($row)$ctx=array_merge($ctx,$row);
    }
    return $ctx;
}
function fm_log_snapshot(string $action,string $side,$value,array $ctx): array {
    $source=is_array($value)?$value:[]; $entity=fm_log_entity_type($action); $assignment=strpos($entity,'assignment')!==false;
    $sourceDept=fm_log_pick($source,['Department']);
    $source['auditSchemaVersion']=1;
    $source['snapshotSide']=$side==='before'?'before':'after';
    $source['entityType']=$entity;
    $source['employeeId']=fm_log_pick($source,['employeeId','EmployeeID'],$ctx['employeeId']??null);
    $source['employeeName']=fm_log_pick($source,['employeeName','EmployeeName'],$ctx['employeeName']??null);
    $source['employeeDepartment']=fm_log_pick($source,['employeeDepartment','EmployeeDepartment'],$assignment?$sourceDept:($ctx['employeeDepartment']??null));
    $source['employeeUnit']=fm_log_pick($source,['employeeUnit','EmployeeUnit','Unit'],$ctx['employeeUnit']??null);
    $source['employeePosition']=fm_log_pick($source,['employeePosition','EmployeePosition','Position'],$ctx['employeePosition']??null);
    $source['curriculumId']=fm_log_pick($source,['curriculumId','CurriculumID','TargetCurriculumID'],$ctx['curriculumId']??null);
    $source['curriculumCode']=fm_log_pick($source,['curriculumCode','CurriculumCode'],$ctx['curriculumCode']??null);
    $source['curriculumTitle']=fm_log_pick($source,['curriculumTitle','CurriculumTitle'],$ctx['curriculumTitle']??null);
    $curriculumDept=$assignment?($ctx['curriculumDepartment']??null):($sourceDept??($ctx['curriculumDepartment']??null));
    $source['curriculumDepartment']=fm_log_pick($source,['curriculumDepartment','CurriculumDepartment'],$curriculumDept);
    $source['year']=fm_log_pick($source,['year','Year'],$ctx['year']??null);
    $source['courseId']=fm_log_pick($source,['courseId','CourseID','TargetCourseID'],$ctx['courseId']??null);
    $source['courseCode']=fm_log_pick($source,['courseCode','CourseCode'],$ctx['courseCode']??null);
    $source['courseTitle']=fm_log_pick($source,['courseTitle','CourseTitle'],$ctx['courseTitle']??null);
    $source['assignmentId']=fm_log_pick($source,['assignmentId','AssignmentID','id'],$ctx['assignmentId']??null);
    $source['status']=fm_log_status($action,$side,$source);
    $source['notes']=fm_log_pick($source,['notes','Notes'],$ctx['notes']??null);
    $source['reactivated']=(bool)fm_log_pick($source,['reactivated','Reactivated'],false);
    return $source;
}
function fm_log(array $u,string $action,?string $cur=null,?string $course=null,?string $emp=null,$old=null,$new=null): void {
    $oldSnapshot=$old===null?null:fm_log_snapshot($action,'before',$old,fm_log_context($cur,$course,$emp,is_array($old)?$old:[],'before'));
    $newSnapshot=fm_log_snapshot($action,'after',$new,fm_log_context($cur,$course,$emp,is_array($new)?$new:[],'after'));
    db_execute('INSERT INTO fourm_curriculumlogs (Action,CurriculumID,CourseID,EmployeeID,OldValue,NewValue,PerformedByID,PerformedBy) VALUES (?,?,?,?,?,?,?,?)',[$action,$cur,$course,$emp,fm_json($oldSnapshot),fm_json($newSnapshot),fm_uid($u),fm_actor($u)]);
}
function fm_training_log_date_filter($value): ?string {
...
function fm_training_log_json_expr(string $column,string $key): string {
    return "NULLIF(JSON_UNQUOTE(JSON_EXTRACT(CASE WHEN JSON_VALID($column) THEN $column ELSE '{}' END, '$.$key')), 'null')";
}
function fm_training_logs_response(array $u): void {
    $paged=(string)($_GET['paged']??'')==='1';
    $rawPage=trim((string)($_GET['page']??'')); $page=preg_match('/^\d+$/',$rawPage)?(int)$rawPage:1; $page=max(1,min(1000000,$page));
    $defaultPageSize=$paged?20:100; $rawPageSize=trim((string)($_GET['pageSize']??($_GET['limit']??'')));
    $pageSize=preg_match('/^\d+$/',$rawPageSize)?(int)$rawPageSize:$defaultPageSize; $pageSize=max(1,min($paged?100:300,$pageSize));
    $rawYear=trim((string)($_GET['year']??'')); $year=$rawYear===''?null:(int)$rawYear;
    $dateFrom=fm_training_log_date_filter($_GET['dateFrom']??'');
    $dateTo=fm_training_log_date_filter($_GET['dateTo']??'');
    if(($rawYear!==''&&($year<2000||$year>2100||!preg_match('/^\d{4}$/',$rawYear)))||$dateFrom===null||$dateTo===null||($dateFrom&&$dateTo&&$dateFrom>$dateTo))json_response(['success'=>false,'message'=>'Invalid Training Matrix log filter.'],400);
    $admin=fm_admin($u); $requestedDepartment=fm_text($_GET['department']??($_GET['dept']??''),100);
    $department=$admin?($requestedDepartment!==''&&$requestedDepartment!=='all'?$requestedDepartment:''):fm_text($u['department']??'',100);
    $filters=[
        'paged'=>$paged,'page'=>$page,'pageSize'=>$pageSize,
        'curriculumId'=>fm_text($_GET['curriculumId']??'',36),'courseId'=>fm_text($_GET['courseId']??'',36),
        'employeeId'=>fm_text($_GET['employeeId']??'',50),
        'action'=>preg_replace('/[^A-Z0-9_]/','',strtoupper(fm_text($_GET['action']??'',50))),
        'actorId'=>fm_text($_GET['actorId']??($_GET['performedById']??''),50),
        'department'=>$department,'unit'=>fm_text($_GET['unit']??'',100),'year'=>$year,
        'dateFrom'=>$dateFrom,'dateTo'=>$dateTo,'q'=>fm_text($_GET['q']??'',120),
    ];
    $pagination=function(int $total)use(&$page,$pageSize): array {$totalPages=max(1,(int)ceil($total/$pageSize));$page=min($page,$totalPages);return ['page'=>$page,'pageSize'=>$pageSize,'total'=>$total,'totalPages'=>$totalPages,'hasPrevious'=>$page>1,'hasNext'=>$page<$totalPages];};
    if(!$admin&&$department===''){json_response(['success'=>true,'data'=>$paged?['rows'=>[],'pagination'=>$pagination(0),'filters'=>$filters]:[]]);}
    $newDepartment=fm_training_log_json_expr('l.NewValue','curriculumDepartment'); $oldDepartment=fm_training_log_json_expr('l.OldValue','curriculumDepartment');
    $newUnit=fm_training_log_json_expr('l.NewValue','employeeUnit'); $oldUnit=fm_training_log_json_expr('l.OldValue','employeeUnit');
    $newYear=fm_training_log_json_expr('l.NewValue','year'); $oldYear=fm_training_log_json_expr('l.OldValue','year');
    $newEmployeeName=fm_training_log_json_expr('l.NewValue','employeeName'); $oldEmployeeName=fm_training_log_json_expr('l.OldValue','employeeName');
    $newCurriculumCode=fm_training_log_json_expr('l.NewValue','curriculumCode'); $oldCurriculumCode=fm_training_log_json_expr('l.OldValue','curriculumCode');
    $newCurriculumTitle=fm_training_log_json_expr('l.NewValue','curriculumTitle'); $oldCurriculumTitle=fm_training_log_json_expr('l.OldValue','curriculumTitle');
    $newCourseCode=fm_training_log_json_expr('l.NewValue','courseCode'); $oldCourseCode=fm_training_log_json_expr('l.OldValue','courseCode');
    $newCourseTitle=fm_training_log_json_expr('l.NewValue','courseTitle'); $oldCourseTitle=fm_training_log_json_expr('l.OldValue','courseTitle');
    $newCurriculumId=fm_training_log_json_expr('l.NewValue','curriculumId'); $oldCurriculumId=fm_training_log_json_expr('l.OldValue','curriculumId');
    $newCourseId=fm_training_log_json_expr('l.NewValue','courseId'); $oldCourseId=fm_training_log_json_expr('l.OldValue','courseId');
    $scopeDepartment="COALESCE($newDepartment,$oldDepartment,c.Department)"; $logUnit="COALESCE($newUnit,$oldUnit,e.Unit)"; $logYear="COALESCE($newYear,$oldYear,c.`Year`)";
    $where=[];$pa=[];
    if($filters['curriculumId']!==''){$where[]="(l.CurriculumID=? OR $newCurriculumId=? OR $oldCurriculumId=?)";for($i=0;$i<3;$i++)$pa[]=$filters['curriculumId'];}
    if($filters['courseId']!==''){$where[]="(l.CourseID=? OR $newCourseId=? OR $oldCourseId=?)";for($i=0;$i<3;$i++)$pa[]=$filters['courseId'];}
    if($filters['employeeId']!==''){$where[]='l.EmployeeID=?';$pa[]=$filters['employeeId'];}
    if($filters['action']!==''&&$filters['action']!=='ALL'){$where[]='l.Action=?';$pa[]=$filters['action'];}
    if($filters['actorId']!==''){$where[]='l.PerformedByID=?';$pa[]=$filters['actorId'];}
    if($filters['year']!==null){$where[]="($newYear=? OR $oldYear=? OR ($newYear IS NULL AND $oldYear IS NULL AND c.`Year`=?))";for($i=0;$i<3;$i++)$pa[]=$filters['year'];}
    if($filters['department']!==''){$where[]="($newDepartment=? OR $oldDepartment=? OR ($newDepartment IS NULL AND $oldDepartment IS NULL AND c.Department=?))";for($i=0;$i<3;$i++)$pa[]=$filters['department'];}
    if($filters['unit']!==''){$where[]="($newUnit=? OR $oldUnit=? OR ($newUnit IS NULL AND $oldUnit IS NULL AND e.Unit=?))";for($i=0;$i<3;$i++)$pa[]=$filters['unit'];}
    if($filters['dateFrom']!==''){$where[]='l.PerformedAt>=?';$pa[]=$filters['dateFrom'].' 00:00:00';}
    if($filters['dateTo']!==''){$where[]='l.PerformedAt<DATE_ADD(?,INTERVAL 1 DAY)';$pa[]=$filters['dateTo'].' 00:00:00';}
    if($filters['q']!==''){$like='%'.$filters['q'].'%';$where[]="(l.Action LIKE ? OR l.EmployeeID LIKE ? OR l.PerformedByID LIKE ? OR l.PerformedBy LIKE ? OR c.CurriculumCode LIKE ? OR c.CurriculumTitle LIKE ? OR co.CourseCode LIKE ? OR co.CourseTitle LIKE ? OR e.EmployeeName LIKE ? OR $newEmployeeName LIKE ? OR $oldEmployeeName LIKE ? OR $newCurriculumCode LIKE ? OR $oldCurriculumCode LIKE ? OR $newCurriculumTitle LIKE ? OR $oldCurriculumTitle LIKE ? OR $newCourseCode LIKE ? OR $oldCourseCode LIKE ? OR $newCourseTitle LIKE ? OR $oldCourseTitle LIKE ? OR $scopeDepartment LIKE ? OR $logUnit LIKE ?)";for($i=0;$i<21;$i++)$pa[]=$like;}
    $joins='FROM fourm_curriculumlogs l LEFT JOIN fourm_curriculums c ON c.id=l.CurriculumID LEFT JOIN fourm_courses co ON co.id=l.CourseID LEFT JOIN employees e ON e.EmployeeID=l.EmployeeID';
    $sqlWhere=$where?' WHERE '.implode(' AND ',$where):'';
    $employeeDepartmentNew=fm_training_log_json_expr('l.NewValue','employeeDepartment'); $employeeDepartmentOld=fm_training_log_json_expr('l.OldValue','employeeDepartment');
    $select="SELECT l.*,$scopeDepartment Department,$logYear `Year`,COALESCE($newCurriculumCode,$oldCurriculumCode,c.CurriculumCode) CurriculumCode,COALESCE($newCurriculumTitle,$oldCurriculumTitle,c.CurriculumTitle) CurriculumTitle,COALESCE($newCourseCode,$oldCourseCode,co.CourseCode) CourseCode,COALESCE($newCourseTitle,$oldCourseTitle,co.CourseTitle) CourseTitle,COALESCE($newEmployeeName,$oldEmployeeName,e.EmployeeName) EmployeeName,COALESCE($employeeDepartmentNew,$employeeDepartmentOld,e.Department) EmployeeDepartment,$logUnit Unit,e.Position,$oldDepartment SourceDepartment,$newDepartment DestinationDepartment,$oldUnit SourceUnit,$newUnit DestinationUnit";
    if(!$paged)json_response(['success'=>true,'data'=>db_rows("$select $joins$sqlWhere ORDER BY l.PerformedAt DESC,l.id DESC LIMIT $pageSize",$pa)]);
    $count=db_row("SELECT COUNT(*) total $joins$sqlWhere",$pa);$meta=$pagination((int)($count['total']??0));$filters['page']=$meta['page'];$offset=($meta['page']-1)*$meta['pageSize'];
    $rows=db_rows("$select $joins$sqlWhere ORDER BY l.PerformedAt DESC,l.id DESC LIMIT ".(int)$meta['pageSize'].' OFFSET '.(int)$offset,$pa);
    json_response(['success'=>true,'data'=>['rows'=>$rows,'pagination'=>$meta,'filters'=>$filters]]);
}
...
function fm_drop_curriculum_legacy_index(string $indexName): void {
    if(!in_array($indexName,['uq_fourm_curriculum','uq_cur'],true))return;
    $exists=db_row("SELECT INDEX_NAME FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND LOWER(TABLE_NAME)=LOWER('fourm_curriculums') AND INDEX_NAME=? LIMIT 1",[$indexName]);
    if($exists)db()->exec("ALTER TABLE fourm_curriculums DROP INDEX `$indexName`");
}
function fm_ensure(): void {
    ...
    db()->exec("CREATE TABLE IF NOT EXISTS fourm_curriculums (id VARCHAR(36) PRIMARY KEY,`Year` INT NOT NULL,Department VARCHAR(100) NOT NULL,CurriculumCode VARCHAR(50) NOT NULL,CurriculumTitle VARCHAR(255) NOT NULL,Notes TEXT,IsActive TINYINT(1) NOT NULL DEFAULT 1,ActiveScopeKey CHAR(64) NULL,CreatedByID VARCHAR(50),CreatedBy VARCHAR(100),CreatedAt DATETIME DEFAULT CURRENT_TIMESTAMP,UpdatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,UNIQUE KEY uq_fourm_curriculum_active(ActiveScopeKey)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    fm_try("ALTER TABLE fourm_curriculums ADD COLUMN ActiveScopeKey CHAR(64) NULL AFTER IsActive");
    db_execute("UPDATE fourm_curriculums SET ActiveScopeKey=CASE WHEN IsActive=1 THEN SHA2(CONCAT(CAST(`Year` AS CHAR),'|',LOWER(TRIM(Department)),'|',LOWER(TRIM(CurriculumCode))),256) ELSE NULL END WHERE (IsActive=1 AND (ActiveScopeKey IS NULL OR ActiveScopeKey<>SHA2(CONCAT(CAST(`Year` AS CHAR),'|',LOWER(TRIM(Department)),'|',LOWER(TRIM(CurriculumCode))),256))) OR (IsActive<>1 AND ActiveScopeKey IS NOT NULL)");
    try{db()->exec("ALTER TABLE fourm_curriculums ADD UNIQUE KEY uq_fourm_curriculum_active(ActiveScopeKey)");}catch(Throwable $e){$driverCode=$e instanceof PDOException?(int)($e->errorInfo[1]??0):0;if($driverCode!==1061)throw $e;}
    fm_drop_curriculum_legacy_index('uq_fourm_curriculum');
    fm_drop_curriculum_legacy_index('uq_cur');
...
function fm_assign(array $u,string $kind,string $parent,array $ids,?string $notes,?array $scope=null,bool $writeLog=true): array {
    ...
    foreach(array_unique(array_filter(array_map(fn($v)=>fm_text($v,50),$ids))) as $eid){
        ...
        if($writeLog)fm_log($u,$prefix.($old?'_REASSIGN':'_CREATE'),$kind==='curriculum'?$parent:($scope['CurriculumID']??null),$kind==='course'?$parent:null,$eid,$old,array_merge($e,['Status'=>'Assigned','Notes'=>$notes]));
    } return $out;
}
...
function handle_fourm_routes(string $method,string $path): bool {
    ...
    if($method==='PUT'&&$path==='/fourm/training-curriculums/bulk-code'){
        ...
        $result=fm_write(fn()=>fm_transaction(function()use($u,$options,$expected){
            ...
            foreach($preview['rows'] as $item){$temporary='__BULK__'.fm_uuid();db_execute('UPDATE fourm_curriculums SET CurriculumCode=?,ActiveScopeKey=? WHERE id=?',[$temporary,fm_curriculum_active_scope_key((int)$item['Year'],(string)$item['Department'],$temporary,(int)$item['IsActive']),$item['id']]);}
            foreach($preview['rows'] as $item){db_execute('UPDATE fourm_curriculums SET CurriculumCode=?,ActiveScopeKey=? WHERE id=?',[$item['newCode'],fm_curriculum_active_scope_key((int)$item['Year'],(string)$item['Department'],(string)$item['newCode'],(int)$item['IsActive']),$item['id']]);fm_log($u,'CURRICULUM_CODE_BULK_UPDATE',$item['id'],null,null,['CurriculumCode'=>$item['oldCode']],['CurriculumCode'=>$item['newCode'],'BatchScope'=>$options]);}
    ...
    if($method==='GET'&&$path==='/fourm/training-curriculums'){ $y=(int)($_GET['year']??date('Y'));$sql="SELECT cur.*,COUNT(DISTINCT CASE WHEN co.IsActive=1 THEN co.id END) CourseCount,COUNT(DISTINCT co.id) TotalCourseCount,COUNT(DISTINCT CASE WHEN ce.Status='Assigned' THEN ce.EmployeeID END) AssignedCount FROM fourm_curriculums cur LEFT JOIN fourm_courses co ON co.CurriculumID=cur.id LEFT JOIN fourm_curriculumemployees ce ON ce.CurriculumID=cur.id WHERE cur.`Year`=?";$pa=[$y];$includeInactive=fm_admin($u)&&($_GET['includeInactive']??'')==='1';if(!$includeInactive)$sql.=' AND cur.IsActive=1';$d=fm_admin($u)?fm_text($_GET['dept']??'',100):fm_text($u['department']??'',100);if($d&&$d!=='all'){$sql.=' AND cur.Department=?';$pa[]=$d;}json_response(['success'=>true,'data'=>db_rows($sql.' GROUP BY cur.id ORDER BY cur.Department,cur.IsActive DESC,cur.CurriculumCode',$pa)]);}
    if($method==='POST'&&$path==='/fourm/training-curriculums'){require_admin();$d=fm_required($b['Department']??'',100,'Department required.');$code=fm_required($b['CurriculumCode']??'',50,'Curriculum code required.');$title=fm_required($b['CurriculumTitle']??'',255,'Curriculum title required.');$year=(int)($b['Year']??date('Y'));if($year<2000||$year>2100)json_response(['success'=>false,'message'=>'Invalid curriculum year.'],400);if(db_row('SELECT id FROM fourm_curriculums WHERE IsActive=1 AND `Year`=? AND LOWER(TRIM(Department))=LOWER(TRIM(?)) AND LOWER(TRIM(CurriculumCode))=LOWER(TRIM(?)) LIMIT 1',[$year,$d,$code]))fm_duplicate_response('An active curriculum already uses this code for the selected year and department.');$id=fm_uuid();$scopeKey=fm_curriculum_active_scope_key($year,$d,$code);fm_write(fn()=>fm_transaction(function()use($u,$b,$id,$year,$d,$code,$title,$scopeKey){db_execute('INSERT INTO fourm_curriculums (id,`Year`,Department,CurriculumCode,CurriculumTitle,Notes,IsActive,ActiveScopeKey,CreatedByID,CreatedBy) VALUES (?,?,?,?,?,?,1,?,?,?)',[$id,$year,$d,$code,$title,fm_text($b['Notes']??'',1000)?:null,$scopeKey,fm_uid($u),fm_actor($u)]);fm_log($u,'CURRICULUM_CREATE',$id,null,null,null,['Year'=>$year,'Department'=>$d,'CurriculumCode'=>$code,'CurriculumTitle'=>$title,'IsActive'=>1]);}),'An active curriculum already uses this code for the selected year and department.');json_response(['success'=>true,'data'=>['id'=>$id]],201);}
    $reactivate=route_params($path,'/fourm/training-curriculums/:id/reactivate');
    if($reactivate!==null&&$method==='POST'){
        require_admin();$cur=fm_cur($reactivate['id']);if(!$cur)json_response(['success'=>false,'message'=>'Curriculum not found.'],404);
        if((int)$cur['IsActive']===1)json_response(['success'=>true,'data'=>['unchanged'=>true],'message'=>'Curriculum is already active.']);
        if(db_row('SELECT id FROM fourm_curriculums WHERE id<>? AND IsActive=1 AND `Year`=? AND LOWER(TRIM(Department))=LOWER(TRIM(?)) AND LOWER(TRIM(CurriculumCode))=LOWER(TRIM(?)) LIMIT 1',[$reactivate['id'],$cur['Year'],$cur['Department'],$cur['CurriculumCode']]))fm_duplicate_response('Cannot reactivate because an active curriculum already uses this code for the same year and department.');
        $restored=fm_write(fn()=>fm_transaction(function()use($u,$reactivate,$cur){$scopeKey=fm_curriculum_active_scope_key((int)$cur['Year'],(string)$cur['Department'],(string)$cur['CurriculumCode']);db_execute('UPDATE fourm_curriculums SET IsActive=1,ActiveScopeKey=? WHERE id=?',[$scopeKey,$reactivate['id']]);$count=db_execute('UPDATE fourm_courses SET IsActive=1 WHERE CurriculumID=?',[$reactivate['id']]);fm_log($u,'CURRICULUM_REACTIVATE',$reactivate['id'],null,null,$cur,array_merge($cur,['IsActive'=>1,'RestoredCourseCount'=>$count]));return $count;}),'Cannot reactivate because an active curriculum already uses this code for the same year and department.');
        json_response(['success'=>true,'data'=>['restoredCourseCount'=>$restored],'message'=>'Curriculum reactivated.']);
    }
    $p=route_params($path,'/fourm/training-curriculums/:id');
    if($p!==null&&in_array($method,['PUT','DELETE'],true)){
        require_admin();
        $cur=fm_cur($p['id']);
        if(!$cur)json_response(['success'=>false,'message'=>'Not found.'],404);
        if($method==='DELETE'){
            fm_transaction(function()use($u,$p,$cur){
                db_execute('UPDATE fourm_curriculums SET IsActive=0,ActiveScopeKey=NULL WHERE id=?',[$p['id']]);
                db_execute('UPDATE fourm_courses SET IsActive=0 WHERE CurriculumID=?',[$p['id']]);
                fm_log($u,'CURRICULUM_DISABLE',$p['id'],null,null,$cur,['IsActive'=>0]);
            });
            json_response(['success'=>true]);
        }
        $next=[
            'Year'=>(int)($b['Year']??$b['year']??$cur['Year']),
            'Department'=>fm_required($b['Department']??$b['department']??$cur['Department'],100,'Department required.'),
            'CurriculumCode'=>fm_required($b['CurriculumCode']??$b['curriculumCode']??$cur['CurriculumCode'],50,'Curriculum code required.'),
            'CurriculumTitle'=>fm_required($b['CurriculumTitle']??$b['curriculumTitle']??$cur['CurriculumTitle'],255,'Curriculum title required.'),
            'Notes'=>array_key_exists('Notes',$b)||array_key_exists('notes',$b)?(fm_text($b['Notes']??$b['notes'],1000)?:null):$cur['Notes'],
            'IsActive'=>array_key_exists('IsActive',$b)||array_key_exists('isActive',$b)?fm_bool($b['IsActive']??$b['isActive']):(int)$cur['IsActive'],
        ];
        if($next['Year']<2000||$next['Year']>2100)json_response(['success'=>false,'message'=>'Invalid curriculum year.'],400);
        if((int)$cur['IsActive']!==$next['IsActive'])json_response(['success'=>false,'code'=>'FOURM_CURRICULUM_STATUS_ACTION_REQUIRED','message'=>'Use Disable or Reactivate to change curriculum status.'],400);
        $unchanged=(int)$cur['Year']===$next['Year']
            &&(string)$cur['Department']===$next['Department']
            &&(string)$cur['CurriculumCode']===$next['CurriculumCode']
            &&(string)$cur['CurriculumTitle']===$next['CurriculumTitle']
            &&(fm_text($cur['Notes']??'',1000)?:null)===$next['Notes']
            &&(int)$cur['IsActive']===$next['IsActive'];
        if($unchanged)json_response(['success'=>true,'data'=>['unchanged'=>true],'message'=>'Curriculum data is unchanged.']);
        if($next['IsActive']===1&&db_row('SELECT id FROM fourm_curriculums WHERE id<>? AND IsActive=1 AND `Year`=? AND LOWER(TRIM(Department))=LOWER(TRIM(?)) AND LOWER(TRIM(CurriculumCode))=LOWER(TRIM(?)) LIMIT 1',[$p['id'],$next['Year'],$next['Department'],$next['CurriculumCode']]))fm_duplicate_response('An active curriculum already uses this code for the selected year and department.');
        fm_write(fn()=>fm_transaction(function()use($u,$p,$cur,$next){
            db_execute('UPDATE fourm_curriculums SET `Year`=?,Department=?,CurriculumCode=?,CurriculumTitle=?,Notes=?,IsActive=?,ActiveScopeKey=? WHERE id=?',[$next['Year'],$next['Department'],$next['CurriculumCode'],$next['CurriculumTitle'],$next['Notes'],$next['IsActive'],fm_curriculum_active_scope_key($next['Year'],$next['Department'],$next['CurriculumCode'],$next['IsActive']),$p['id']]);
            fm_log($u,'CURRICULUM_UPDATE',$p['id'],null,null,$cur,$next);
        }),'Curriculum code already exists for this year and department.');
        json_response(['success'=>true]);
    }
    if($method==='GET'&&$path==='/fourm/training-permissions')json_response(['success'=>true,'data'=>['permissionKey'=>'FOURM_TRAINING_MANAGE','canManageTraining'=>fm_admin($u)||(fm_text($u['department']??$u['Department']??'',100)!==''&&fm_has_permission($u,'FOURM_TRAINING_MANAGE')),'canManageAll'=>fm_admin($u),'canDeleteHistory'=>false,'department'=>fm_text($u['department']??$u['Department']??'',100),'stabilizationVersion'=>'20260914-training-audit-immutable']]);
    ...
    $p=route_params($path,'/fourm/training-curriculums/:id/courses');if($p!==null&&$method==='GET'){ $cur=fm_cur($p['id']);if(!$cur)json_response(['success'=>false,'message'=>'Not found.'],404);if(!fm_dept_ok($u,$cur['Department']))fm_deny();$includeInactive=fm_admin($u)&&($_GET['includeInactive']??'')==='1';$activeWhere=$includeInactive?'':' AND c.IsActive=1';json_response(['success'=>true,'data'=>db_rows("SELECT c.*,COUNT(DISTINCT CASE WHEN e.Status='Assigned' THEN e.EmployeeID END) AssignedCount FROM fourm_courses c LEFT JOIN fourm_courseemployees e ON e.CourseID=c.id WHERE c.CurriculumID=?$activeWhere GROUP BY c.id ORDER BY c.SortOrder,c.CourseCode",[$p['id']])]);}
    ...
    $p=route_params($path,'/fourm/training-assignments/:id/transfer');if($p!==null&&$method==='POST'){ $a=fm_course_ass($p['id']);$targetId=fm_text($b['TargetCourseID']??$b['NewCourseID']??'',36);$target=fm_course($targetId);if(!$a||!$target||!(int)$target['IsActive'])json_response(['success'=>false,'message'=>'Active destination course not found.'],404);if(($a['Status']??'')!=='Assigned')json_response(['success'=>false,'message'=>'Only assigned employees can be transferred.'],400);if($targetId===(string)$a['CourseID'])json_response(['success'=>false,'message'=>'Select a different destination course.'],400);if(!fm_training_manage_ok($u,$a['CurriculumDepartment'])||!fm_training_manage_ok($u,$target['Department']))fm_deny();$out=fm_transaction(function()use($u,$b,$p,$a,$target){$notes=fm_text($b['Notes']??'',1000)?:null;db_execute("UPDATE fourm_courseemployees SET Status='Transferred',RemovedAt=NOW(),RemovedByID=?,RemovedBy=? WHERE id=?",[fm_uid($u),fm_actor($u),$p['id']]);$result=fm_assign($u,'course',$target['id'],[$a['EmployeeID']],$notes,['Year'=>$target['Year'],'Department'=>$target['Department'],'CurriculumID'=>$target['CurriculumID']],false);fm_log($u,'ASSIGNMENT_TRANSFER',$a['CurriculumID'],$a['CourseID'],$a['EmployeeID'],$a,['TargetCurriculumID'=>$target['CurriculumID'],'TargetCourseID'=>$target['id'],'Status'=>'Assigned','Notes'=>$notes,'Reactivated'=>in_array($a['EmployeeID'],$result['reassigned'],true)]);return $result;});json_response(['success'=>true,'data'=>$out]);}
    if($method==='GET'&&$path==='/fourm/training-logs')fm_training_logs_response($u);
    $p=route_params($path,'/fourm/training-logs/:id');if($p!==null&&$method==='DELETE')json_response(['success'=>false,'code'=>'AUDIT_LOG_IMMUTABLE','message'=>'Training Matrix Audit Log is immutable and cannot be deleted.'],405);
