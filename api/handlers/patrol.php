...
function patrol_bangkok_datetime(): string
{
...
function patrol_checkin_v2_enabled(): bool
{
    try {
        $row = db_row("SELECT value FROM app_settings WHERE key_name='patrol_checkin_v2_enabled' LIMIT 1");
...
function patrol_today_bangkok(): string
{
    return (new DateTimeImmutable('now', new DateTimeZone('Asia/Bangkok')))->format('Y-m-d');
}

function patrol_idempotency_key($value): ?string
{
...
function ensure_patrol_schema(): void
{
    ...
    try {
        db_execute("INSERT INTO app_settings(key_name,value) VALUES('patrol_checkin_v2_enabled','0') ON DUPLICATE KEY UPDATE value=value");
    } catch (Throwable $e) {}
...
function patrol_attach_leave_items(array $items, array $leaveRows): array
{
    ...
    foreach ($items as $item) {
        ...
        if ($sid !== '') $occurrenceBySession[$sid] = $item['ScheduleOccurrenceKey'] ?? patrol_supervisor_occurrence_key($item);
    }
    $byOccurrence = [];
    $byDate = [];
    foreach ($leaveRows as $row) {
        ...
        if ($sid !== '') {
            $bySession[$sid] = $row;
            $occurrenceKey = $occurrenceBySession[$sid] ?? '';
            if ($occurrenceKey !== '') $byOccurrence[$occurrenceKey] = $row;
        }
    ...
    foreach ($items as &$item) {
        ...
        $occurrenceKey = $item['ScheduleOccurrenceKey'] ?? patrol_supervisor_occurrence_key($item);
        $leave = $bySession[$sid] ?? ($byOccurrence[$occurrenceKey] ?? ($byDate[$date] ?? null));
        $item['leave'] = $leave;
...
function patrol_supervisor_occurrence_key(array $item): string
{
    $date = substr((string) ($item['ScheduledDate'] ?? ($item['PatrolDate'] ?? ($item['date'] ?? ''))), 0, 10);
    $round = (int) ($item['PatrolRound'] ?? ($item['patrolRound'] ?? 0));
    return $date !== '' && $round > 0 ? $date . ':' . $round : '';
}

function patrol_supervisor_occurrence_count(array $items): int
{
    $keys = [];
    foreach ($items as $item) {
        $key = patrol_supervisor_occurrence_key($item);
        if ($key !== '') $keys[$key] = true;
    }
    return count($keys);
}

function patrol_base_membership(string $employeeId): ?array
{
    $rows = db_rows('SELECT tm.TeamID,tm.PatrolType,t.Name AS TeamName,t.PatrolGroup,t.Color FROM patrol_team_members tm JOIN patrol_teams t ON t.id=tm.TeamID WHERE tm.EmployeeID=? ORDER BY tm.id', [$employeeId]);
    if (count($rows) > 1) {
        json_response([
            'success' => false,
            'code' => 'PATROL_TEAM_CONFLICT',
            'message' => 'Employee belongs to more than one base Patrol team. Resolve the conflict in System Control.',
            'data' => ['teamIds' => array_values(array_map(static fn($row) => (int) $row['TeamID'], $rows))],
        ], 409);
    }
    return $rows[0] ?? null;
}

function patrol_effective_team(string $employeeId, int $year, int $month): ?array
{
    $base = patrol_base_membership($employeeId);
    if (!$base) return null;
    $rotation = db_row('SELECT mr.TeamID,t.Name AS TeamName,t.PatrolGroup,t.Color FROM patrol_member_rotation mr JOIN patrol_teams t ON t.id=mr.TeamID WHERE mr.EmployeeID=? AND mr.Year=? AND mr.Month=? LIMIT 1', [$employeeId, $year, $month]);
...
{
    $yearSessions = patrol_top_sessions($employeeId, $year);
    $sessions = array_values(array_filter($yearSessions, static function ($s) use ($month) {
        return (int) substr((string) $s['PatrolDate'], 5, 2) === $month;
    }));
    $attendance = db_rows("SELECT id,PatrolDate,CheckinAt,PatrolType,Area,Notes,RecordedBy,ScheduledSessionID,(IdempotencyKey IS NOT NULL) AS IsV2Request FROM patrol_attendance WHERE UserID=? AND YEAR(PatrolDate)=? AND MONTH(PatrolDate)=? ORDER BY PatrolDate,id", [$employeeId, $year, $month]);
    $linkedAttendance = [];
    $sessionIds = array_values(array_map(static fn($session) => (string) $session['id'], $sessions));
    if ($sessionIds) {
        $marks = implode(',', array_fill(0, count($sessionIds), '?'));
        $linkedAttendance = db_rows("SELECT id,PatrolDate,CheckinAt,PatrolType,Area,Notes,RecordedBy,ScheduledSessionID,(IdempotencyKey IS NOT NULL) AS IsV2Request FROM patrol_attendance WHERE UserID=? AND ScheduledSessionID IN ($marks) ORDER BY PatrolDate,id", array_merge([$employeeId], $sessionIds));
    }
    $scheduleAttendance = [];
    foreach (array_merge($attendance, $linkedAttendance) as $record) $scheduleAttendance[(int) $record['id']] = $record;
    $attByDate = [];
    $attBySession = [];
    foreach (array_values($scheduleAttendance) as $a) {
    ...
    $items = [];
    $completed = 0;
    $legacyAttendanceByDate = [];
    foreach ($attByDate as $legacyDate => $dateRecords) {
        $legacyAttendanceByDate[$legacyDate] = array_values(array_filter($dateRecords, static function ($r) {
            return empty($r['ScheduledSessionID']) && empty($r['IsV2Request']);
        }));
    }
    foreach ($sessions as $s) {
        ...
        $dateRecords = [];
        if (!$sessionRecords && !empty($legacyAttendanceByDate[$date])) $dateRecords[] = array_shift($legacyAttendanceByDate[$date]);
...
function patrol_supervisor_schedule_slots(int $year): array
{
    ...
    foreach ($rows as $row) {
        ...
        $row['ScheduleOccurrenceKey'] = $date . ':' . $round;
        $slots[] = $row;
    }
    return $slots;
}
...
{
    $slotBySession = [];
    foreach ($slots as $slot) $slotBySession[(string) $slot['id']] = $slot;
    $occurrenceRecords = [];
    $unlinkedByDate = [];
    foreach ($records as $record) {
        if (!empty($record['ScheduledSessionID'])) {
            $sid = (string) $record['ScheduledSessionID'];
            $key = isset($slotBySession[$sid]) ? patrol_supervisor_occurrence_key($slotBySession[$sid]) : '';
            if ($key !== '') $occurrenceRecords[$key][] = $record;
        } else {
            $date = substr((string) $record['CheckinDate'], 0, 10);
            $unlinkedByDate[$date][] = $record;
        }
    }

    foreach ($unlinkedByDate as $date => $unlinked) {
        $occurrenceKeys = [];
        foreach ($slots as $slot) {
            $slotDate = substr((string) ($slot['date'] ?? $slot['PatrolDate']), 0, 10);
            $key = patrol_supervisor_occurrence_key($slot);
            if ($slotDate === $date && $key !== '' && !in_array($key, $occurrenceKeys, true)) $occurrenceKeys[] = $key;
        }
        foreach ($unlinked as $index => $record) {
            if (!$occurrenceKeys) continue;
            $key = $occurrenceKeys[min($index, count($occurrenceKeys) - 1)];
            $occurrenceRecords[$key][] = $record;
        }
    }
    ...
    foreach ($slots as $slot) {
        ...
        $occurrenceKey = patrol_supervisor_occurrence_key($slot);
        $itemRecords = [];
        foreach ($occurrenceRecords[$occurrenceKey] ?? [] as $record) {
        ...
        $slot['status'] = $status;
        $slot['ScheduleOccurrenceKey'] = $occurrenceKey;
...
function patrol_resolve_supervisor_scheduled_session(string $employeeId, string $date, ?string $requestedSessionId, array $options = []): array
{
    ...
    if ($session) {
        ...
        $today = patrol_today_bangkok();
        $patrolType = patrol_self_checkin_type($options['patrolType'] ?? 'normal');
        if ($scheduledDate > $today) {
            json_response(['success' => false, 'code' => 'PATROL_FUTURE_SUPERVISOR_CHECKIN_NOT_ALLOWED', 'message' => 'Future scheduled rounds cannot be checked in early.'], 409);
        }
        if ($patrolType === 'normal' && $scheduledDate < $today) {
            json_response(['success' => false, 'code' => 'PATROL_SUPERVISOR_MAKEUP_REQUIRED', 'message' => 'A past scheduled round must be recorded as a makeup patrol.'], 409);
        }
        if ($patrolType === 'compensation' && $scheduledDate >= $today) {
            json_response(['success' => false, 'code' => 'PATROL_SUPERVISOR_MAKEUP_NOT_DUE', 'message' => 'Makeup patrol is available only for a missed earlier round.'], 409);
        }
        $occurrenceKey = $session['ScheduleOccurrenceKey'] ?? patrol_supervisor_occurrence_key($session);
        $occurrenceSessionIds = [];
        foreach ($sessions as $candidate) {
            $candidateKey = $candidate['ScheduleOccurrenceKey'] ?? patrol_supervisor_occurrence_key($candidate);
            if ($candidateKey === $occurrenceKey) $occurrenceSessionIds[] = (string) $candidate['id'];
        }
        $placeholders = implode(',', array_fill(0, count($occurrenceSessionIds), '?'));
        $linked = db_row(
            'SELECT id FROM patrol_self_checkin WHERE EmployeeID=? AND ScheduledSessionID IN (' . $placeholders . ') LIMIT 1',
            array_merge([$employeeId], $occurrenceSessionIds)
        );
...
function patrol_resolve_scheduled_session(string $employeeId, string $date, ?string $requestedSessionId, array $options = []): array
{
    $year = (int) substr($date, 0, 4);
    $sessionId = trim((string) ($requestedSessionId ?? ''));
    if ($sessionId !== '') {
        $candidate = db_row('SELECT YEAR(PatrolDate) AS ScheduledYear FROM patrol_sessions WHERE SessionID=? LIMIT 1', [$sessionId]);
        if (!empty($candidate['ScheduledYear'])) $year = (int) $candidate['ScheduledYear'];
    }
    ...
    if ($sessionId !== '') {
        ...
        $scheduledDate = substr((string) $session['PatrolDate'], 0, 10);
        if (empty($options['checkinV2Enabled']) && substr($date, 0, 7) !== substr($scheduledDate, 0, 7)) {
            json_response(['success' => false, 'message' => 'Makeup patrol must be linked to a scheduled round in the same month.'], 400);
        }
        if (!empty($options['checkinV2Enabled']) && ($options['mode'] ?? '') === 'scheduled' && $scheduledDate !== $date) {
            json_response(['success' => false, 'code' => 'PATROL_SCHEDULE_DATE_MISMATCH', 'message' => 'Scheduled patrol must use a round assigned for today. Choose Makeup for an earlier round.'], 400);
        }
        if (!empty($options['checkinV2Enabled']) && ($options['mode'] ?? '') === 'makeup' && $scheduledDate > $date) {
            json_response(['success' => false, 'code' => 'PATROL_FUTURE_MAKEUP_NOT_ALLOWED', 'message' => 'Makeup patrol cannot complete a future scheduled round.'], 400);
        }
    } else {
        $matches = [];
        foreach ($sessions as $s) {
        ...
        elseif (!empty($options['checkinV2Enabled']) && ($options['mode'] ?? '') === 'scheduled' && count($matches) > 1) {
            json_response(['success' => false, 'code' => 'PATROL_SESSION_SELECTION_REQUIRED', 'message' => 'More than one Patrol round is scheduled today. Select the round to check in.'], 409);
        }
    }
    if ($session) {
        $exists = db_row('SELECT id FROM patrol_attendance WHERE UserID=? AND ScheduledSessionID=? LIMIT 1', [$employeeId, $session['id']]);
        $scheduledDate = substr((string) $session['PatrolDate'], 0, 10);
        $legacyCount = (int) (safe_scalar('SELECT COUNT(*) FROM patrol_attendance WHERE UserID=? AND DATE(PatrolDate)=? AND (ScheduledSessionID IS NULL OR ScheduledSessionID="") AND IdempotencyKey IS NULL', [$employeeId, $scheduledDate]) ?? 0);
        $sameDateSessions = array_values(array_filter($sessions, static fn($item) => substr((string) $item['PatrolDate'], 0, 10) === $scheduledDate));
        $legacySlot = -1;
        foreach ($sameDateSessions as $index => $item) if ((string) $item['id'] === (string) $session['id']) { $legacySlot = $index; break; }
        $completedByLegacy = $legacySlot >= 0 && $legacySlot < $legacyCount;
        if ($exists || $completedByLegacy) {
            json_response(['success' => false, 'message' => 'Selected schedule is already completed.'], 409);
        }
...
function patrol_attendance_detail_top(string $employeeId, int $year): array
{
    ...
    $attendance = db_rows("SELECT pa.id,pa.PatrolDate,pa.CheckinAt,pa.PatrolType,pa.Area,pa.Notes,pa.RecordedBy,pa.ScheduledSessionID,(pa.IdempotencyKey IS NOT NULL) AS IsV2Request,e.EmployeeName AS RecordedByName FROM patrol_attendance pa LEFT JOIN employees e ON e.EmployeeID=pa.RecordedBy WHERE pa.UserID=? AND YEAR(pa.PatrolDate)=? ORDER BY pa.PatrolDate,pa.id", [$employeeId, $year]);
    $linkedAttendance = [];
    $sessionIds = array_values(array_map(static fn($session) => (string) $session['id'], $sessions));
    if ($sessionIds) {
        $marks = implode(',', array_fill(0, count($sessionIds), '?'));
        $linkedAttendance = db_rows("SELECT pa.id,pa.PatrolDate,pa.CheckinAt,pa.PatrolType,pa.Area,pa.Notes,pa.RecordedBy,pa.ScheduledSessionID,(pa.IdempotencyKey IS NOT NULL) AS IsV2Request,e.EmployeeName AS RecordedByName FROM patrol_attendance pa LEFT JOIN employees e ON e.EmployeeID=pa.RecordedBy WHERE pa.UserID=? AND pa.ScheduledSessionID IN ($marks) ORDER BY pa.PatrolDate,pa.id", array_merge([$employeeId], $sessionIds));
    }
    $scheduleAttendance = [];
    foreach (array_merge($attendance, $linkedAttendance) as $record) $scheduleAttendance[(int) $record['id']] = $record;
    $attByDate = [];
    ...
    foreach ($attendance as &$a) {
        $d = substr((string) $a['PatrolDate'], 0, 10);
        $a['mode'] = empty($a['RecordedBy']) || (string) $a['RecordedBy'] === $employeeId ? 'self' : 'admin_recorded';
        if (empty($a['ScheduledSessionID']) && (!empty($a['IsV2Request']) || empty($scheduledDates[$d]))) $extraRecords[] = $a;
    }
    unset($a);
    $actualActivity = patrol_actual_activity($attendance, patrol_session_map($sessions));
...
function patrol_attendance_detail_supervisor(string $employeeId, int $year, bool $allowPositionSupervisor = false): array
{
    ...
    for ($m = 1; $m <= 12; $m++) {
        $monthRequirement = patrol_supervisor_requirement_from_schedule_count(
            patrol_supervisor_occurrence_count($scheduleByMonth[$m] ?? [])
        );
    ...
    for ($m = 1; $m <= 12; $m++) {
        ...
        foreach ($monthItems as $item) {
            foreach (($item['records'] ?? []) as $record) {
                $monthRecordsById[(string) ($record['id'] ?? count($monthRecordsById))] = $record;
            }
        }
        $monthRecords = array_values($monthRecordsById);
        $completedOccurrences = [];
        foreach ($monthItems as $item) {
            if (!empty($item['isCompleted'])) {
                $key = patrol_supervisor_occurrence_key($item);
                if ($key !== '') $completedOccurrences[$key] = true;
            }
        }
        $completed = count($completedOccurrences);
        $isDue = $m <= $dueMonth;
    ...
    return [
        ...
        'summary' => [
            ...
            'leave' => $leaveStats,
            'targetSource' => $targetSource,
            'scheduledTotal' => patrol_supervisor_occurrence_count($schedule),
            'missingToDate' => max(0, $requiredToDate - $completedToDate),
            'upcomingMonths' => max(0, 12 - $dueMonth),
            'progressToDatePct' => patrol_pct($completedToDate, $requiredToDate),
...
function handle_patrol_routes(string $method, string $path): bool
{
    ...
    if ($method === 'GET' && $path === '/patrol/my-monthly-plan') {
        ...
        $base = patrol_base_membership($uid);
    ...
    if ($method === 'GET' && $path === '/patrol/my-yearly-stats') {
        ...
        $recent = db_rows('SELECT PatrolDate,CheckinAt,PatrolType,Area,Notes FROM patrol_attendance WHERE UserID=? ORDER BY PatrolDate DESC,id DESC LIMIT 6', [$uid]);
    ...
    if ($method === 'GET' && $path === '/patrol/attendance-stats') json_response(db_rows("SELECT UserName AS Name,COUNT(*) AS Total,MAX(PatrolDate) AS LastWalk,MAX(CheckinAt) AS LastCheckinAt,ROUND(COUNT(*)*100.0/NULLIF((SELECT COUNT(DISTINCT YEARWEEK(PatrolDate)) FROM patrol_attendance),0)) AS Percent FROM patrol_attendance GROUP BY UserID,UserName ORDER BY Total DESC LIMIT 20"));
    ...
    if ($method === 'POST' && $path === '/patrol/checkin') {
        $b = json_body();
        $v2 = patrol_checkin_v2_enabled();
        $date = $v2 ? patrol_today_bangkok() : (patrol_valid_date($b['PatrolDate'] ?? null) ?? date('Y-m-d'));
        ...
        $requestedMode = strtolower(trim((string) ($b['CheckinMode'] ?? '')));
        $mode = $v2
            ? ($requestedMode ?: ($type === 'compensation' ? 'makeup' : (!empty($b['ScheduledSessionID']) ? 'scheduled' : 'extra')))
            : ($type === 'compensation' ? 'makeup' : (!empty($b['ScheduledSessionID']) ? 'scheduled' : 'extra'));
        if (!in_array($mode, ['scheduled', 'makeup', 'extra'], true)) json_response(['success' => false, 'message' => 'CheckinMode must be scheduled, makeup or extra.'], 400);
        $type = $mode === 'makeup' ? 'compensation' : 'normal';
        if ($mode === 'makeup' && trim((string) ($b['ScheduledSessionID'] ?? '')) === '') json_response(['success' => false, 'message' => 'ScheduledSessionID is required for makeup patrol.'], 400);
        if ($v2 && $mode === 'extra' && trim((string) ($b['ScheduledSessionID'] ?? '')) !== '') json_response(['success' => false, 'message' => 'Extra patrol must not be linked to a scheduled round.'], 400);
        $idempotencyRaw = trim((string) ($b['IdempotencyKey'] ?? ''));
        $idempotencyKey = $idempotencyRaw === '' ? null : patrol_idempotency_key($idempotencyRaw);
        if ($idempotencyRaw !== '' && !$idempotencyKey) json_response(['success' => false, 'message' => 'IdempotencyKey is invalid.'], 400);
        if ($v2 && $idempotencyKey) {
            $existingRequest = db_row('SELECT id,UserName,TeamName,PatrolDate,CheckinAt,PatrolType,Area,ScheduledSessionID FROM patrol_attendance WHERE UserID=? AND IdempotencyKey=? LIMIT 1', [$uid, $idempotencyKey]);
            if ($existingRequest) json_response(['success' => true, 'message' => 'Check-in was already saved.', 'data' => ['idempotentReplay' => true, 'checkin' => ['id' => (int) $existingRequest['id'], 'employeeId' => $uid, 'employeeName' => $existingRequest['UserName'], 'type' => $existingRequest['PatrolType'], 'mode' => $mode, 'actualDate' => substr((string) $existingRequest['PatrolDate'], 0, 10), 'checkinAt' => $existingRequest['CheckinAt'] ?: null, 'scheduledSessionId' => $existingRequest['ScheduledSessionID'] ?: null, 'area' => $existingRequest['Area'] ?: null, 'teamName' => $existingRequest['TeamName'] ?? '']]]);
        }
        if (!$v2 && db_row('SELECT id FROM patrol_attendance WHERE UserID=? AND DATE(PatrolDate)=? AND PatrolType=? LIMIT 1', [$uid, $date, $type])) json_response(['success' => false, 'message' => 'Duplicate patrol check-in.'], 409);
        $emp = db_row('SELECT EmployeeID,EmployeeName,Department,Position,CompanyEmail FROM employees WHERE EmployeeID=? LIMIT 1', [$uid]);
        $session = null;
        if ($mode !== 'extra') {
            $resolved = patrol_resolve_scheduled_session($uid, $date, $b['ScheduledSessionID'] ?? null, ['checkinV2Enabled' => $v2, 'mode' => $mode]);
            $session = $resolved['session'] ?? null;
        }
        if ($v2 && $mode === 'scheduled' && !$session) json_response(['success' => false, 'code' => 'PATROL_SCHEDULE_NOT_FOUND', 'message' => 'No scheduled Patrol round is assigned to this user today.'], 409);
        $effectiveTeam = patrol_effective_team($uid, (int) substr($date, 0, 4), (int) substr($date, 5, 2));
        $teamName = (string) ($effectiveTeam['TeamName'] ?? ($user['team'] ?? ''));
        $area = trim((string) ($b['Area'] ?? '')) ?: null;
        if (!$area && $session) $area = $session['AreaName'] ?: ($session['AreaCode'] ?? null);
        $notes = trim((string) ($b['Notes'] ?? '')) ?: null;
        $checkinAt = patrol_bangkok_datetime();
        try {
            db_execute('INSERT INTO patrol_attendance(UserID,UserName,TeamName,WeekNumber,PatrolDate,Year,PatrolType,Area,Notes,CheckinAt,RecordedBy,ScheduledSessionID,IdempotencyKey) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)', [$uid, $emp['EmployeeName'] ?? patrol_user_name($user), $teamName, patrol_week($date), $date, (int) substr($date, 0, 4), $type, $area, $notes, $checkinAt, $uid, $session['id'] ?? null, $idempotencyKey]);
        } catch (Throwable $e) {
            $replayed = ($v2 && $idempotencyKey)
                ? db_row('SELECT id,UserName,TeamName,PatrolDate,CheckinAt,PatrolType,Area,ScheduledSessionID FROM patrol_attendance WHERE UserID=? AND IdempotencyKey=? LIMIT 1', [$uid, $idempotencyKey])
                : null;
            if ($replayed) json_response(['success' => true, 'message' => 'Check-in was already saved.', 'data' => ['idempotentReplay' => true, 'checkin' => ['id' => (int) $replayed['id'], 'employeeId' => $uid, 'employeeName' => $replayed['UserName'], 'type' => $replayed['PatrolType'], 'mode' => $mode, 'actualDate' => substr((string) $replayed['PatrolDate'], 0, 10), 'checkinAt' => $replayed['CheckinAt'] ?: null, 'scheduledSessionId' => $replayed['ScheduledSessionID'] ?: null, 'area' => $replayed['Area'] ?: null, 'teamName' => $replayed['TeamName'] ?? '']]]);
            $completedSession = !empty($session['id']) ? db_row('SELECT id FROM patrol_attendance WHERE UserID=? AND ScheduledSessionID=? LIMIT 1', [$uid, $session['id']]) : null;
            if ($completedSession) json_response(['success' => false, 'code' => 'PATROL_SESSION_ALREADY_COMPLETED', 'message' => 'Selected schedule is already completed.', 'data' => ['attendanceId' => (int) $completedSession['id'], 'scheduledSessionId' => (string) $session['id']]], 409);
            throw $e;
        }
        ...
        json_response(['success' => true, 'message' => 'Check-in saved.', 'data' => ['checkin' => ['id' => $attendanceId, 'employeeId' => $uid, 'employeeName' => $attendance['UserName'], 'position' => $emp['Position'] ?? null, 'department' => $emp['Department'] ?? null, 'type' => $type, 'mode' => $mode, 'actualDate' => $date, 'checkinAt' => $checkinAt, 'scheduledDate' => $session ? substr((string) $session['PatrolDate'], 0, 10) : $date, 'isMakeup' => $session && substr((string) $session['PatrolDate'], 0, 10) !== $date, 'scheduledSessionId' => $session['id'] ?? null, 'round' => $session['PatrolRound'] ?? null, 'area' => $area, 'teamName' => $teamName], 'email' => $email, 'totalWalks' => (int) (safe_scalar('SELECT COUNT(*) FROM patrol_attendance WHERE UserID=?', [$uid]) ?? 0), 'teamWalks' => (int) (safe_scalar('SELECT COUNT(*) FROM patrol_attendance WHERE TeamName=?', [$teamName]) ?? 0), 'todayWalkers' => db_rows('SELECT UserName,PatrolDate FROM patrol_attendance WHERE DATE(PatrolDate)=? ORDER BY PatrolDate DESC LIMIT 5', [$date])]]);
    }
    ...
    if ($p !== null && $method === 'POST') { require_admin(); $b = json_body(); $existingMembership = db_row('SELECT tm.TeamID,t.Name AS TeamName FROM patrol_team_members tm LEFT JOIN patrol_teams t ON t.id=tm.TeamID WHERE tm.EmployeeID=? LIMIT 1', [$b['EmployeeID'] ?? '']); if ($existingMembership) json_response(['success' => false, 'code' => 'PATROL_TEAM_CONFLICT', 'message' => 'Employee already belongs to a base Patrol team. Use Member Rotation for a monthly reassignment.', 'data' => ['teamId' => (int) $existingMembership['TeamID'], 'teamName' => $existingMembership['TeamName'] ?? '']], 409); db_execute('INSERT INTO patrol_team_members(TeamID,EmployeeID,PatrolType) VALUES(?,?,?)', [$p['id'], $b['EmployeeID'], $b['PatrolType'] ?? 'management']); json_response(['success' => true, 'id' => (int) db()->lastInsertId()]); }
    ...
    if ($method === 'POST' && $path === '/patrol/self-checkin') {
        ...
        $resolved = strpos($sid, 'FLEX:') === 0
            ...
            : patrol_resolve_supervisor_scheduled_session($uid, $date, $sid, ['requireSession' => true, 'preserveActualDate' => $type === 'compensation', 'patrolType' => $type]);
    ...
    if ($method === 'GET' && $path === '/patrol/my-missed-sessions') {
        $year = patrol_query_year();
        $v2 = patrol_checkin_v2_enabled();
        $today = $v2 ? patrol_today_bangkok() : date('Y-m-d');
        $years = [$year];
        if ($v2 && strtolower(trim((string) ($_GET['scope'] ?? ''))) === 'all') {
            $years = array_values(array_map(static fn($row) => (int) $row['Year'], db_rows('SELECT DISTINCT YEAR(PatrolDate) AS Year FROM patrol_sessions WHERE DATE(PatrolDate)<=? ORDER BY Year DESC', [$today])));
        }
        $sessions = [];
        foreach ($years as $scheduledYear) $sessions = array_merge($sessions, patrol_top_sessions($uid, (int) $scheduledYear));
        $completed = [];
        foreach (db_rows('SELECT DISTINCT ScheduledSessionID FROM patrol_attendance WHERE UserID=? AND ScheduledSessionID IS NOT NULL AND ScheduledSessionID<>""', [$uid]) as $row) $completed[(string) $row['ScheduledSessionID']] = true;
        $legacyRemaining = [];
        foreach (db_rows('SELECT DATE(PatrolDate) AS PatrolDate,COUNT(*) AS count FROM patrol_attendance WHERE UserID=? AND (ScheduledSessionID IS NULL OR ScheduledSessionID="") AND IdempotencyKey IS NULL GROUP BY DATE(PatrolDate)', [$uid]) as $row) {
            $legacyRemaining[substr((string) $row['PatrolDate'], 0, 10)] = (int) $row['count'];
        }
        $out = [];
        foreach ($sessions as $s) {
            $date = substr((string) $s['PatrolDate'], 0, 10);
            ...
            if (($legacyRemaining[$date] ?? 0) > 0) { $legacyRemaining[$date]--; continue; }
        ...
        usort($out, static function ($a, $b) {
            $dateOrder = strcmp((string) $b['PatrolDate'], (string) $a['PatrolDate']);
            return $dateOrder !== 0 ? $dateOrder : ((int) $a['PatrolRound'] <=> (int) $b['PatrolRound']);
        });
