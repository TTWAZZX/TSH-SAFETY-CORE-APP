...
function activity_definitions(): array
{
    return [
        ...
        ['key' => 'cccf_worker', 'label' => 'CCCF Form A Worker', 'desc' => 'Worker CCCF Form A coverage', 'metricType' => 'people_coverage', 'scopeType' => 'unit', 'unitLabel' => 'คน', 'targetMode' => 'manual'],
...
function people_coverage(string $key, string $department, string $unit, int $year, $yearlyTarget): array
{
    ...
    if ($key === 'cccf_worker') {
        ...
        $rows = safe_rows("SELECT COUNT(DISTINCT NULLIF(EmployeeID,'')) AS numerator FROM cccf_forma_worker WHERE TRIM(COALESCE(Department,''))=? AND YEAR(SubmitDate)=?$unitFilter", $params);
...
function fixed_count_alignment(string $key, string $employeeId, string $department, string $unit, int $year, $fallbackTarget): array
{
    ...
    if ($key === 'patrol') {
        ...
        $rosterTarget = (int) (safe_scalar('SELECT TargetPerYear FROM patrol_roster WHERE EmployeeID=? ORDER BY id LIMIT 1', [$empId]) ?? 0);
        return fixed_count_result($actual, $rosterTarget > 0 ? $rosterTarget : $fallbackTarget, ['type' => 'employee', 'employeeId' => $empId], 'patrol_attendance_plus_self_checkin', $rosterTarget > 0 ? 'patrol_roster' : 'activity_target');
    }
    if ($key === 'ky') {
        $config = safe_rows('SELECT SafetyUnits,YearlyTarget FROM ky_program_config WHERE Year=? AND Department=? AND IsActive=1 LIMIT 1', [$year, $dept]);
        ...
        $moduleTarget = (int) ($config[0]['YearlyTarget'] ?? 0);
        return fixed_count_result($actual, $moduleTarget > 0 ? $moduleTarget : $fallbackTarget, ['type' => $useUnit ? 'department_unit' : 'department', 'department' => $dept, 'unit' => $useUnit ? $scopedUnit : ''], 'ky_scope_activity_count', $moduleTarget > 0 ? 'ky_program_config' : 'activity_target');
    }
...
function merged_activity_targets(string $employeeId): array
{
    ...
    foreach ($scopeRows as $row) {
        if (!isset($scopeMap[$row['ActivityKey']])) {
            $scopeMap[$row['ActivityKey']] = $row;
        }
    }
    $overrideMap = [];
    ...
    return compact('position', 'department', 'unit', 'templateMap', 'scopeMap', 'overrideMap');
}
...
function activity_target_coverage_matrix_data(): array
{
...
function handle_target_routes(string $method, string $path): bool
{
    ...
    if ($method === 'GET' && $path === '/activity-targets/position-templates') {
        ...
        json_response(['success' => true, 'data' => $position !== ''
    ...
    if ($method === 'GET' && $path === '/activity-targets/coverage-matrix') {
        require_admin();
        json_response(['success' => true, 'data' => activity_target_coverage_matrix_data()]);
    }
    if ($method === 'PUT' && $path === '/activity-targets/scope-overrides') {
        ...
        db_execute(
            ...
            [$department, $unit, $key, activity_target_storage_value($key, $body['YearlyTarget'] ?? 0, $isNA), (int) ($body['PassPct'] ?? 80), $isNA, (string) ($admin['name'] ?? '')]
        );
    ...
    if ($method === 'PUT' && $path === '/activity-targets/position-templates') {
        ...
        db_execute(
            'INSERT INTO activity_position_templates (PositionName,ActivityKey,YearlyTarget,PassPct,IsNA,UpdatedBy) VALUES (?,?,?,?,?,?)
             ON DUPLICATE KEY UPDATE YearlyTarget=VALUES(YearlyTarget),PassPct=VALUES(PassPct),IsNA=VALUES(IsNA),UpdatedBy=VALUES(UpdatedBy)',
            [$position, $key, activity_target_storage_value($key, $body['YearlyTarget'] ?? 0, $isNA), (int) ($body['PassPct'] ?? 80), $isNA, (string) ($admin['name'] ?? '')]
        );
    ...
    if ($method === 'POST' && $path === '/activity-targets/position-templates/bulk-apply') {
        $admin = require_admin();
        $position = trim((string) (json_body()['PositionName'] ?? ''));
        $templates = db_rows('SELECT ActivityKey,YearlyTarget,PassPct,IsNA FROM activity_position_templates WHERE PositionName=?', [$position]);
    ...
    if ($params !== null && $method === 'GET') {
        $merged = merged_activity_targets($params['empId']);
        $targets = [];
        foreach (activity_definitions() as $activity) {
            $targets[] = target_row($activity, $merged);
        }
        json_response(['success' => true, 'data' => ['empId' => $params['empId'], 'position' => $merged['position'], 'department' => $merged['department'], 'unit' => $merged['unit'], 'targets' => $targets]]);
    }
    if ($params !== null && $method === 'PUT') {
        ...
        db_execute(
            'INSERT INTO employee_activity_targets (EmployeeID,ActivityKey,YearlyTarget,PassPct,IsNA,UpdatedBy) VALUES (?,?,?,?,?,?)
             ON DUPLICATE KEY UPDATE YearlyTarget=VALUES(YearlyTarget),PassPct=VALUES(PassPct),IsNA=VALUES(IsNA),UpdatedBy=VALUES(UpdatedBy)',
            [$params['empId'], $key, activity_target_storage_value($key, $body['YearlyTarget'], $isNA), (int) ($body['PassPct'] ?? 80), $isNA, (string) ($admin['name'] ?? '')]
        );
    ...
    if ($method === 'GET' && $path === '/activity-targets/me') {
        ...
        $actuals = [
            ...
            'cccf_worker' => safe_scalar('SELECT COUNT(*) FROM cccf_forma_worker WHERE EmployeeID=? AND YEAR(SubmitDate)=?', [$user['id'], $year]),
