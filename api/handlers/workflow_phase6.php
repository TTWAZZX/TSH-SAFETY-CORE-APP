...
function wf_ensure_ky_tables(): void
{
    ...
    db()->exec("CREATE TABLE IF NOT EXISTS ky_video_file_inventory (
        id VARCHAR(36) NOT NULL PRIMARY KEY,EvidenceYear SMALLINT NOT NULL,ActivityID VARCHAR(36) NOT NULL,Department VARCHAR(100) NOT NULL,SafetyUnit VARCHAR(100),
        ExternalBackupConfirmed TINYINT(1) NOT NULL DEFAULT 0,ExternalReference TEXT,OriginalFileName VARCHAR(255) NOT NULL,MimeType VARCHAR(120),
        FileSize BIGINT UNSIGNED NOT NULL DEFAULT 0,SHA256 CHAR(64) NOT NULL,ProductionVideoUrl TEXT,ProductionStoredName VARCHAR(255),Status VARCHAR(30) NOT NULL DEFAULT 'Pending',
        DeclaredByID VARCHAR(50),DeclaredByName VARCHAR(100),DeclaredAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,VerifiedByID VARCHAR(50),VerifiedByName VARCHAR(100),
        VerifiedAt DATETIME,VerificationNote TEXT,ProductionDeletedByID VARCHAR(50),ProductionDeletedByName VARCHAR(100),ProductionDeletedAt DATETIME,
        ProductionDeletionReason TEXT,RowVersion INT UNSIGNED NOT NULL DEFAULT 1,CreatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UpdatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,UNIQUE KEY uq_ky_video_inventory_activity(ActivityID),
        KEY idx_ky_video_inventory_status(EvidenceYear,Status),KEY idx_ky_video_inventory_department(EvidenceYear,Department,SafetyUnit)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    db()->exec("CREATE TABLE IF NOT EXISTS ky_video_file_inventory_audit (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,InventoryID VARCHAR(36) NOT NULL,ActivityID VARCHAR(36) NOT NULL,Action VARCHAR(80) NOT NULL,
        ActorID VARCHAR(50),ActorName VARCHAR(100),BeforeJson LONGTEXT,AfterJson LONGTEXT,Detail TEXT,CreatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        KEY idx_ky_video_inventory_audit(InventoryID,CreatedAt),KEY idx_ky_video_inventory_audit_action(Action,CreatedAt)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    db()->exec("CREATE TABLE IF NOT EXISTS ky_activity_external_video_evidence (
        id VARCHAR(36) NOT NULL PRIMARY KEY,EvidenceYear SMALLINT NOT NULL,ActivityID VARCHAR(36) NOT NULL,Department VARCHAR(100) NOT NULL,SafetyUnit VARCHAR(100),
        ExternalReference TEXT NOT NULL,OriginalFileName VARCHAR(255) NOT NULL,MimeType VARCHAR(120),FileSize BIGINT UNSIGNED NOT NULL DEFAULT 0,SHA256 CHAR(64) NOT NULL,
        Status VARCHAR(30) NOT NULL DEFAULT 'Pending',DeclaredByID VARCHAR(50),DeclaredByName VARCHAR(100),DeclaredAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        VerifiedByID VARCHAR(50),VerifiedByName VARCHAR(100),VerifiedAt DATETIME,VerificationNote TEXT,RowVersion INT UNSIGNED NOT NULL DEFAULT 1,
        CreatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,UpdatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_ky_activity_external_video(ActivityID),KEY idx_ky_activity_external_status(EvidenceYear,Status),
        KEY idx_ky_activity_external_scope(EvidenceYear,Department,SafetyUnit)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    db()->exec("CREATE TABLE IF NOT EXISTS ky_activity_external_video_evidence_audit (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,EvidenceID VARCHAR(36) NOT NULL,ActivityID VARCHAR(36) NOT NULL,Action VARCHAR(80) NOT NULL,
        ActorID VARCHAR(50),ActorName VARCHAR(100),BeforeJson LONGTEXT,AfterJson LONGTEXT,Detail TEXT,CreatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        KEY idx_ky_activity_external_audit(EvidenceID,CreatedAt),KEY idx_ky_activity_external_action(Action,CreatedAt)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    db()->exec("CREATE TABLE IF NOT EXISTS ky_annual_unit_contest_entries (
        id VARCHAR(36) NOT NULL PRIMARY KEY,EntryYear SMALLINT NOT NULL,ScopeKey VARCHAR(220) NOT NULL,Department VARCHAR(100) NOT NULL,SafetyUnit VARCHAR(100),
        ActivityID VARCHAR(36) NOT NULL,Status VARCHAR(30) NOT NULL DEFAULT 'Submitted',SubmittedByID VARCHAR(50),SubmittedByName VARCHAR(100),
        SubmittedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,UpdatedByID VARCHAR(50),UpdatedByName VARCHAR(100),RowVersion INT UNSIGNED NOT NULL DEFAULT 1,
        CreatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,UpdatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_ky_contest_year_scope(EntryYear,ScopeKey),KEY idx_ky_contest_activity(ActivityID),KEY idx_ky_contest_status(EntryYear,Status)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    db()->exec("CREATE TABLE IF NOT EXISTS ky_annual_unit_contest_entry_audit (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,EntryID VARCHAR(36) NOT NULL,ActivityID VARCHAR(36),Action VARCHAR(80) NOT NULL,
        ActorID VARCHAR(50),ActorName VARCHAR(100),BeforeJson LONGTEXT,AfterJson LONGTEXT,Detail TEXT,CreatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        KEY idx_ky_contest_audit_entry(EntryID,CreatedAt),KEY idx_ky_contest_audit_activity(ActivityID,CreatedAt)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    foreach ([
...
function wf_ky_can_manage_contest_scope(array $row,array $user): bool
{
    if(wf_ky_can_upload_followup_video($row,$user))return true;
    $department=$user['department']??$user['Department']??'';$unit=$user['unit']??$user['Unit']??$user['safetyUnit']??$user['SafetyUnit']??'';
    if(wf_ky_norm_key($department)===''||wf_ky_norm_key($department)!==wf_ky_norm_key($row['Department']??''))return false;
    return wf_ky_norm_key($row['SafetyUnit']??'')===''||wf_ky_norm_key($unit)===wf_ky_norm_key($row['SafetyUnit']??'');
}
...
function wf_ky_annual_compliance_state($target,$production,$externalVerified,$externalPending=0): array
{
    $target=max(0,(int)$target);$production=max(0,(int)$production);$externalVerified=max(0,(int)$externalVerified);$externalPending=max(0,(int)$externalPending);
    $productionRequired=$target>0?1:0;$externalRequired=max(0,$target-$productionRequired);$total=$production+$externalVerified;
    $missingProduction=max(0,$productionRequired-$production);$missingExternal=max(0,$externalRequired-$externalVerified);$missingTotal=$missingProduction+$missingExternal;
    return ['yearlyTarget'=>$target,'productionRequired'=>$productionRequired,'externalRequired'=>$externalRequired,'evidenceTotal'=>$total,
        'pendingExternalVideo'=>$externalPending,'missingProduction'=>$missingProduction,'missingExternal'=>$missingExternal,'missingEvidenceTotal'=>$missingTotal,
        'annualCompliant'=>$missingProduction===0&&$missingExternal===0&&$missingTotal===0,'evidenceProgressPct'=>$target>0?min(100,(int)round($total/$target*100)):100];
}

function wf_ky_contest_audit(array $user,string $action,?array $before,?array $after,string $detail=''): void
{
    $row=$after?:($before?:[]);db_execute('INSERT INTO ky_annual_unit_contest_entry_audit (EntryID,ActivityID,Action,ActorID,ActorName,BeforeJson,AfterJson,Detail) VALUES (?,?,?,?,?,?,?,?)',[
        $row['id']??'', $row['ActivityID']??null, $action, wf_user_id($user), wf_user_name($user),
        $before?json_encode($before,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES):null,$after?json_encode($after,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES):null,$detail?:null
    ]);
}

function wf_ky_active_contest_entry($activityId): ?array
{
    return db_row("SELECT * FROM ky_annual_unit_contest_entries WHERE ActivityID=? AND Status='Submitted' LIMIT 1",[$activityId]);
}
...
function wf_ky_annual_public(array $row): array
{
    ...
    $row['canDeleteProductionFile']=($row['Status']??'')==='Verified'&&($row['StorageMode']??'')==='CentralMachine'&&!empty($row['ExternalBackupConfirmed'])
        ...
        &&trim((string)($row['ProductionVideoUrl']??''))!==''&&empty($row['ProductionDeletedAt'])&&!wf_ky_active_contest_entry($row['ActivityID']??'');
    return $row;
}

function wf_ky_inventory_public(array $row): array
{
    ...
    $row['contestRetentionHold']=wf_ky_active_contest_entry($row['ActivityID']??'')!==null;
    $row['canDeleteProductionFile']=$registered&&$status==='Verified'&&!empty($row['ExternalBackupConfirmed'])&&trim((string)($row['ExternalReference']??''))!==''
        &&preg_match('/^[a-f0-9]{64}$/i',(string)($row['SHA256']??''))&&$production!==''&&!$deleted&&!wf_ky_active_contest_entry($row['ActivityID']??'');
    return $row;
}

// KY_CONTEST_ENTRY_RETENTION_HOLD: an active annual Unit contest entry keeps its Production file fail-closed.

function wf_ky_inventory_audit(array $user,string $action,?array $before,?array $after,string $detail=''): void
{
    $row=$after?:($before?:[]);db_execute('INSERT INTO ky_video_file_inventory_audit (InventoryID,ActivityID,Action,ActorID,ActorName,BeforeJson,AfterJson,Detail) VALUES (?,?,?,?,?,?,?,?)',[
        $row['id']??'', $row['ActivityID']??'', $action, wf_user_id($user), wf_user_name($user),
        $before?json_encode($before,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES):null,$after?json_encode($after,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES):null,$detail?:null
    ]);
}

function wf_ky_activity_external_public(array $row): array
{
    $row['id']=$row['EvidenceID']??$row['id']??null;$row['Status']=(string)($row['Status']??'Pending');
    $row['hasValidMetadata']=trim((string)($row['ExternalReference']??''))!==''&&trim((string)($row['OriginalFileName']??''))!==''
        &&(int)($row['FileSize']??0)>0&&preg_match('/^[a-f0-9]{64}$/i',(string)($row['SHA256']??''));
    return $row;
}

function wf_ky_activity_external_audit(array $user,string $action,?array $before,?array $after,string $detail=''): void
{
    $row=$after?:($before?:[]);db_execute('INSERT INTO ky_activity_external_video_evidence_audit (EvidenceID,ActivityID,Action,ActorID,ActorName,BeforeJson,AfterJson,Detail) VALUES (?,?,?,?,?,?,?,?)',[
        $row['id']??'', $row['ActivityID']??'', $action, wf_user_id($user), wf_user_name($user),
        $before?json_encode($before,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES):null,$after?json_encode($after,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES):null,$detail?:null
    ]);
}
...
{
    return "(EXISTS (SELECT 1 FROM ky_activity_external_video_evidence aev WHERE aev.ActivityID={$alias}.id AND aev.Status='Verified' AND COALESCE(TRIM(aev.ExternalReference),'')<>'' AND aev.SHA256 REGEXP '^[0-9a-fA-F]{64}$') OR EXISTS (SELECT 1 FROM ky_annual_video_evidence ave WHERE ave.ActivityID={$alias}.id AND ave.StorageMode='CentralMachine' AND ave.Status='Verified' AND ave.ExternalBackupConfirmed=1 AND COALESCE(TRIM(ave.ExternalReference),'')<>'' AND ave.SHA256 REGEXP '^[0-9a-fA-F]{64}$') OR EXISTS (SELECT 1 FROM ky_video_file_inventory inv WHERE inv.ActivityID={$alias}.id AND inv.Status='Verified' AND inv.ExternalBackupConfirmed=1 AND COALESCE(TRIM(inv.ExternalReference),'')<>'' AND inv.SHA256 REGEXP '^[0-9a-fA-F]{64}$'))";
}

function wf_ky_pending_external_video_sql(string $alias = 'a'): string
{
    return "(EXISTS (SELECT 1 FROM ky_activity_external_video_evidence aev WHERE aev.ActivityID={$alias}.id AND aev.Status IN ('Pending','NeedsCorrection') AND COALESCE(TRIM(aev.ExternalReference),'')<>'' AND aev.SHA256 REGEXP '^[0-9a-fA-F]{64}$') OR EXISTS (SELECT 1 FROM ky_annual_video_evidence ave WHERE ave.ActivityID={$alias}.id AND ave.StorageMode='CentralMachine' AND ave.Status IN ('Pending','NeedsCorrection') AND ave.ExternalBackupConfirmed=1 AND COALESCE(TRIM(ave.ExternalReference),'')<>'' AND ave.SHA256 REGEXP '^[0-9a-fA-F]{64}$') OR EXISTS (SELECT 1 FROM ky_video_file_inventory inv WHERE inv.ActivityID={$alias}.id AND inv.Status IN ('Pending','NeedsCorrection') AND inv.ExternalBackupConfirmed=1 AND COALESCE(TRIM(inv.ExternalReference),'')<>'' AND inv.SHA256 REGEXP '^[0-9a-fA-F]{64}$'))";
}
...
function wf_ky_video_evidence_select_sql(string $alias = 'a'): string
{
    ...
    return "({$production}) AS HasProductionVideo,({$verified}) AS HasVerifiedExternalVideo,({$pending}) AS HasPendingExternalVideo,({$production} OR {$verified}) AS HasVideoEvidence,CASE WHEN {$production} THEN 'Production' WHEN {$verified} THEN 'ExternalVerified' WHEN {$pending} THEN 'ExternalPending' ELSE 'Missing' END AS VideoEvidenceStorage,(SELECT aev.id FROM ky_activity_external_video_evidence aev WHERE aev.ActivityID={$alias}.id LIMIT 1) AS ActivityExternalEvidenceID,(SELECT aev.Status FROM ky_activity_external_video_evidence aev WHERE aev.ActivityID={$alias}.id LIMIT 1) AS ActivityExternalStatus,(SELECT aev.RowVersion FROM ky_activity_external_video_evidence aev WHERE aev.ActivityID={$alias}.id LIMIT 1) AS ActivityExternalRowVersion";
}
...
function wf_ky_video_upload_config(): array
{
    ...
    foreach (['upload_max_filesize', 'post_max_size'] as $setting) {
        $bytes = wf_ini_bytes((string)ini_get($setting));
        if ($bytes > 0) $limits[] = max(0, $bytes - $reserve);
    }
    $chunkSize = $limits ? min($maximum, min($limits)) : $maximum;
    if ($chunkSize < 64 * 1024) {
        return ['available' => false, 'chunkSize' => $chunkSize];
    }
    return [
        'available' => true,
        'maxFileSize' => 200 * 1024 * 1024,
        'chunkSize' => $chunkSize,
        'acceptedExtensions' => ['mp4','mov','webm','avi','mkv','mpeg','mpg'],
        'acceptedMimeTypes' => ['video/mp4','video/quicktime','video/webm','video/avi','video/x-msvideo','video/x-matroska','video/mpeg'],
        'maxAttempts' => 3,
...
function wf_ky_video_store_uploaded_chunk(string $source, string $target, int $expectedSize, ?string &$failureCode = null): bool
{
    ...
    // temporary file across their storage boundary. Stream it into the private
    // .part under an exclusive lock. Incomplete writes are removed and cannot
    // pass the completion route's exact-size checks.
    ...
    if (!flock($output, LOCK_EX) || !ftruncate($output, 0) || fseek($output, 0) !== 0) {
        $failureCode = 'KY_VIDEO_CHUNK_DESTINATION_UNWRITABLE';
        fclose($input);
        fclose($output);
        @unlink($target);
        return false;
...
function wf_ky_stats(int $year): array
{
    ...
    $videoEvidenceRow=db_row("SELECT SUM({$productionVideo}) AS productionVideo,SUM(NOT ({$productionVideo}) AND {$verifiedExternal}) AS verifiedExternalVideo,SUM({$productionVideo} OR {$verifiedExternal}) AS videoEvidenceTotal,SUM(NOT ({$productionVideo}) AND NOT ({$verifiedExternal}) AND {$pendingExternal}) AS pendingExternalVideo FROM ky_activities a WHERE YEAR(a.ActivityDate)=?{$evidenceDeptFilter}",array_merge([$year],$deptParams))?:[];
    $videoEvidenceTotal=(int)($videoEvidenceRow['videoEvidenceTotal']??0);$activityTotal=(int)($kpi['total']??0);
    $videoEvidence=['productionVideo'=>(int)($videoEvidenceRow['productionVideo']??0),'verifiedExternalVideo'=>(int)($videoEvidenceRow['verifiedExternalVideo']??0),'pendingExternalVideo'=>(int)($videoEvidenceRow['pendingExternalVideo']??0),'videoEvidenceTotal'=>$videoEvidenceTotal,'missingVideoEvidence'=>max(0,$activityTotal-$videoEvidenceTotal-(int)($videoEvidenceRow['pendingExternalVideo']??0)),'videoEvidenceRate'=>$activityTotal>0?(int)round($videoEvidenceTotal/$activityTotal*100):0];
    ...
    $rawByDept = safe_rows('SELECT Department,COUNT(*) AS count FROM ky_activities WHERE YEAR(ActivityDate)=? GROUP BY Department ORDER BY count DESC', [$year]);
    $rawDeptMonthly = safe_rows('SELECT Department,MONTH(ActivityDate) AS month,COUNT(*) AS count FROM ky_activities WHERE YEAR(ActivityDate)=? GROUP BY Department,MONTH(ActivityDate) ORDER BY Department,month', [$year]);
    $canonicalByKey=[];$departmentCounts=[];$departmentMonthCounts=[];$unmappedMap=[];
    foreach($targetDepts as $dept){$canonicalByKey[wf_ky_norm_key($dept)]=$dept;$departmentCounts[$dept]=0;}
    foreach($rawByDept as $row){$rawDept=trim((string)($row['Department']??''));$canonical=$canonicalByKey[wf_ky_norm_key($rawDept)]??null;$count=(int)($row['count']??0);if($canonical!==null)$departmentCounts[$canonical]=($departmentCounts[$canonical]??0)+$count;elseif($rawDept!=='')$unmappedMap[$rawDept]=($unmappedMap[$rawDept]??0)+$count;}
    foreach($rawDeptMonthly as $row){$canonical=$canonicalByKey[wf_ky_norm_key($row['Department']??'')]??null;if($canonical===null)continue;$key=$canonical.'||'.(int)($row['month']??0);$departmentMonthCounts[$key]=($departmentMonthCounts[$key]??0)+(int)($row['count']??0);}
    $byDept=[];if($targetDepts){foreach($targetDepts as $dept){$count=(int)($departmentCounts[$dept]??0);$byDept[]=['Department'=>$dept,'count'=>$count,'configured'=>$usingConfig,'hasActivity'=>$count>0];}}else{foreach($rawByDept as $row){$count=(int)($row['count']??0);$byDept[]=['Department'=>$row['Department']??'','count'=>$count,'configured'=>false,'hasActivity'=>$count>0];}}
    $deptMonthly=[];foreach($targetDepts as $dept){for($month=1;$month<=12;$month++)$deptMonthly[]=['Department'=>$dept,'month'=>$month,'count'=>(int)($departmentMonthCounts[$dept.'||'.$month]??0),'configured'=>$usingConfig];}
    $unmappedDepartments=[];foreach($unmappedMap as $dept=>$count)$unmappedDepartments[]=['Department'=>$dept,'count'=>(int)$count];usort($unmappedDepartments,static fn($a,$b)=>$b['count']<=>$a['count']?:strcmp($a['Department'],$b['Department']));
...
function handle_ky_routes(string $method, string $path): bool
{
    ...
    if($method==='GET'&&$path==='/ky/unit-contest-entries'){
        $year=(int)($_GET['year']??date('Y'));json_response(['success'=>true,'data'=>db_rows("SELECT ce.*,a.ActivityDate,a.TeamName,a.KYTKeyword,a.ReporterID,a.ReporterName,a.SubmittedByID,a.SubmittedByName,a.Participants,a.VideoUrl,a.Status AS ActivityStatus FROM ky_annual_unit_contest_entries ce INNER JOIN ky_activities a ON a.id=ce.ActivityID WHERE ce.EntryYear=? AND ce.Status='Submitted' ORDER BY ce.Department,ce.SafetyUnit",[$year])]);
    }
    if($method==='POST'&&$path==='/ky/unit-contest-entries'){
        $b=json_body();$activityId=trim((string)($b['activityId']??''));if($activityId==='')json_response(['success'=>false,'message'=>'Activity is required.'],400);
        $activity=db_row('SELECT * FROM ky_activities WHERE id=?',[$activityId]);if(!$activity)json_response(['success'=>false,'message'=>'KY Activity not found.'],404);
        if(trim((string)($activity['VideoUrl']??''))==='')json_response(['success'=>false,'code'=>'KY_CONTEST_PRODUCTION_VIDEO_REQUIRED','message'=>'Contest entry requires an existing Production video.'],409);
        if(!wf_ky_can_manage_contest_scope($activity,$user))json_response(['success'=>false,'message'=>'Only a user in this Unit or Admin can submit its contest entry.'],403);
        $year=(int)substr((string)$activity['ActivityDate'],0,4);$configs=db_rows('SELECT Department,SafetyUnits FROM ky_program_config WHERE Year=? AND IsActive=1',[$year]);$valid=false;
        foreach($configs as $cfg){if(wf_ky_norm_key($cfg['Department'])!==wf_ky_norm_key($activity['Department']))continue;$units=wf_ky_units($cfg['SafetyUnits']);$valid=$units?in_array(wf_ky_norm_key($activity['SafetyUnit']),array_map('wf_ky_norm_key',$units),true):wf_ky_norm_key($activity['SafetyUnit'])==='';if($valid)break;}
        if(!$valid)json_response(['success'=>false,'code'=>'KY_CONTEST_SCOPE_NOT_CONFIGURED','message'=>'Activity Department/Safety Unit is not active in Program Config.'],409);
        $scopeKey=wf_ky_annual_scope_key($activity['Department'],$activity['SafetyUnit']);$pdo=db();$pdo->beginTransaction();try{
            $before=db_row('SELECT * FROM ky_annual_unit_contest_entries WHERE EntryYear=? AND ScopeKey=? FOR UPDATE',[$year,$scopeKey]);
            if($before&&($before['Status']??'')==='Submitted'&&($before['ActivityID']??'')===$activityId){$pdo->commit();json_response(['success'=>true,'data'=>$before,'idempotent'=>true]);}
            if($before&&($before['Status']??'')==='Submitted'&&!$admin){$pdo->rollBack();json_response(['success'=>false,'code'=>'KY_CONTEST_ENTRY_ADMIN_REPLACE_REQUIRED','message'=>'This Unit already has a contest entry. Ask Admin to replace it.'],409);}
            $id=$before['id']??wf_uuid();if($before)db_execute("UPDATE ky_annual_unit_contest_entries SET ActivityID=?,Status='Submitted',SubmittedByID=?,SubmittedByName=?,SubmittedAt=NOW(),UpdatedByID=?,UpdatedByName=?,RowVersion=RowVersion+1 WHERE id=?",[$activityId,wf_user_id($user),$actor,wf_user_id($user),$actor,$id]);
            else db_execute("INSERT INTO ky_annual_unit_contest_entries (id,EntryYear,ScopeKey,Department,SafetyUnit,ActivityID,Status,SubmittedByID,SubmittedByName,UpdatedByID,UpdatedByName) VALUES (?,?,?,?,?,?,'Submitted',?,?,?,?)",[$id,$year,$scopeKey,$activity['Department'],$activity['SafetyUnit']?:null,$activityId,wf_user_id($user),$actor,wf_user_id($user),$actor]);
            $after=db_row('SELECT * FROM ky_annual_unit_contest_entries WHERE id=?',[$id]);wf_ky_contest_audit($user,$before?'ENTRY_REPLACED':'ENTRY_SUBMITTED',$before?:null,$after,$before?'Contest entry replaced.':'Annual Unit contest entry submitted.');$pdo->commit();json_response(['success'=>true,'data'=>$after]);
        }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
    }
    $p=route_params($path,'/ky/unit-contest-entries/:entryId/audit');if($p!==null&&$method==='GET'){require_admin();json_response(['success'=>true,'data'=>db_rows('SELECT * FROM ky_annual_unit_contest_entry_audit WHERE EntryID=? ORDER BY id DESC LIMIT 200',[$p['entryId']])]);}
    $p=route_params($path,'/ky/unit-contest-entries/:entryId');if($p!==null&&$method==='DELETE'){require_admin();$before=db_row('SELECT * FROM ky_annual_unit_contest_entries WHERE id=?',[$p['entryId']]);if(!$before)json_response(['success'=>false,'message'=>'Contest entry not found.'],404);db_execute("UPDATE ky_annual_unit_contest_entries SET Status='Withdrawn',UpdatedByID=?,UpdatedByName=?,RowVersion=RowVersion+1 WHERE id=?",[wf_user_id($user),$actor,$before['id']]);$after=db_row('SELECT * FROM ky_annual_unit_contest_entries WHERE id=?',[$before['id']]);wf_ky_contest_audit($user,'ENTRY_WITHDRAWN',$before,$after,trim((string)(json_body()['reason']??'')));json_response(['success'=>true,'data'=>$after]);}
    if($method==='GET'&&$path==='/ky/video-showcase'){ $year=(int)($_GET['year']??date('Y'));$limit=min(max((int)($_GET['limit']??6),1),50);$userId=wf_user_id(require_user())?:'__anonymous__'; json_response(['success'=>true,'data'=>db_rows("SELECT a.*,ce.id AS ContestEntryID,ce.EntryYear AS ContestEntryYear,ce.SubmittedAt AS ContestSubmittedAt,COALESCE(rc.UsefulCount,0) AS UsefulCount,COALESCE(rc.PracticeCount,0) AS PracticeCount,COALESCE(rc.AwarenessCount,0) AS AwarenessCount,COALESCE(rc.AttentionCount,0) AS AttentionCount,COALESCE(rc.ReactionTotal,0) AS ReactionTotal,COALESCE(rc.ReactionTotal,0) AS ReactionCount,ur.Reaction AS MyReaction FROM ky_annual_unit_contest_entries ce INNER JOIN ky_activities a ON a.id=ce.ActivityID LEFT JOIN (SELECT ActivityID,SUM(Reaction='useful') AS UsefulCount,SUM(Reaction='practice') AS PracticeCount,SUM(Reaction='awareness') AS AwarenessCount,SUM(Reaction='attention') AS AttentionCount,COUNT(*) AS ReactionTotal FROM ky_video_reactions GROUP BY ActivityID) rc ON rc.ActivityID=a.id LEFT JOIN ky_video_reactions ur ON ur.ActivityID=a.id AND ur.EmployeeID=? WHERE ce.EntryYear=? AND ce.Status='Submitted' AND a.VideoUrl IS NOT NULL AND a.VideoUrl<>'' AND COALESCE(a.ShowVideoOnDashboard,1)=1 ORDER BY COALESCE(a.IsVideoPinned,0) DESC,COALESCE(rc.ReactionTotal,0) DESC,a.CreatedAt DESC LIMIT ?",[$userId,$year,$limit])]);}
    ...
    if($method==='GET'&&$path==='/ky/activity-video-evidence'){
        require_admin();$year=(int)($_GET['year']??date('Y'));$rows=array_map('wf_ky_activity_external_public',db_rows("SELECT e.*,a.ActivityDate,a.TeamName,a.KYTKeyword,a.ReporterName,a.Status AS ActivityStatus FROM ky_activity_external_video_evidence e INNER JOIN ky_activities a ON a.id=e.ActivityID WHERE e.EvidenceYear=? ORDER BY a.ActivityDate DESC,e.UpdatedAt DESC",[$year]));json_response(['success'=>true,'data'=>$rows]);
    }
    $p=route_params($path,'/ky/activity-video-evidence/:evidenceId/audit');if($p!==null&&$method==='GET'){require_admin();json_response(['success'=>true,'data'=>db_rows('SELECT id,EvidenceID,ActivityID,Action,ActorID,ActorName,BeforeJson,AfterJson,Detail,CreatedAt FROM ky_activity_external_video_evidence_audit WHERE EvidenceID=? ORDER BY id DESC LIMIT 200',[$p['evidenceId']])]);}
    if($method==='POST'&&$path==='/ky/activity-video-evidence/declare'){
        $b=json_body();$activityId=trim((string)($b['activityId']??''));$external=trim((string)($b['externalReference']??''));$name=substr(str_replace(['\\','/'],'_',trim((string)($b['originalFileName']??''))),0,255);$mime=substr(trim((string)($b['mimeType']??'')),0,120)?:null;$size=(int)($b['fileSize']??0);$sha=strtolower(trim((string)($b['sha256']??'')));$version=(int)($b['rowVersion']??0);
        if($activityId===''||$external===''||$name===''||$size<=0||!preg_match('/^[a-f0-9]{64}$/',$sha))json_response(['success'=>false,'code'=>'KY_ACTIVITY_EXTERNAL_METADATA_REQUIRED','message'=>'Activity, central-machine reference, filename, size and SHA-256 are required.'],400);
        ...
        $year=(int)substr((string)$activity['ActivityDate'],0,4);$pdo=db();$pdo->beginTransaction();try{$before=db_row('SELECT * FROM ky_activity_external_video_evidence WHERE ActivityID=? FOR UPDATE',[$activityId]);if(($before['Status']??'')==='Verified')json_response(['success'=>false,'code'=>'KY_ACTIVITY_EXTERNAL_ALREADY_VERIFIED','message'=>'This activity video is already verified. Mark it for correction before replacing it.'],409);$same=$before&&($before['Status']??'')==='Pending'&&(string)$before['ExternalReference']===$external&&(string)$before['OriginalFileName']===$name&&(int)$before['FileSize']===$size&&strtolower((string)$before['SHA256'])===$sha;if($same){$pdo->commit();json_response(['success'=>true,'data'=>wf_ky_activity_external_public($before),'idempotent'=>true]);}if($before&&$version>0&&$version!==(int)$before['RowVersion'])json_response(['success'=>false,'code'=>'KY_ACTIVITY_EXTERNAL_STALE','message'=>'Activity video metadata changed. Refresh and retry.'],409);$id=$before['id']??wf_uuid();if($before){db_execute("UPDATE ky_activity_external_video_evidence SET EvidenceYear=?,Department=?,SafetyUnit=?,ExternalReference=?,OriginalFileName=?,MimeType=?,FileSize=?,SHA256=?,Status='Pending',DeclaredByID=?,DeclaredByName=?,DeclaredAt=NOW(),VerifiedByID=NULL,VerifiedByName=NULL,VerifiedAt=NULL,VerificationNote=NULL,RowVersion=RowVersion+1 WHERE id=?",[$year,$activity['Department'],$activity['SafetyUnit']?:null,$external,$name,$mime,$size,$sha,wf_user_id($user),wf_user_name($user),$id]);}else{db_execute("INSERT INTO ky_activity_external_video_evidence (id,EvidenceYear,ActivityID,Department,SafetyUnit,ExternalReference,OriginalFileName,MimeType,FileSize,SHA256,Status,DeclaredByID,DeclaredByName) VALUES (?,?,?,?,?,?,?,?,?,?,'Pending',?,?)",[$id,$year,$activityId,$activity['Department'],$activity['SafetyUnit']?:null,$external,$name,$mime,$size,$sha,wf_user_id($user),wf_user_name($user)]);}$after=db_row('SELECT * FROM ky_activity_external_video_evidence WHERE id=?',[$id]);wf_ky_activity_external_audit($user,$before?'METADATA_UPDATED':'DECLARED',$before?:null,$after,'External activity video metadata recorded without uploading the video to Production.');$pdo->commit();json_response(['success'=>true,'data'=>wf_ky_activity_external_public($after)],$before?200:201);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
    }
    $p=route_params($path,'/ky/activity-video-evidence/:evidenceId/verify');if($p!==null&&$method==='POST'){
        require_admin();$b=json_body();$status=(string)($b['status']??'Verified');$version=(int)($b['rowVersion']??0);$note=trim((string)($b['note']??''));if(!in_array($status,['Verified','NeedsCorrection'],true)||$version<1)json_response(['success'=>false,'message'=>'Status and row version are required.'],400);$pdo=db();$pdo->beginTransaction();try{$before=db_row('SELECT * FROM ky_activity_external_video_evidence WHERE id=? FOR UPDATE',[$p['evidenceId']]);if(!$before)json_response(['success'=>false,'message'=>'Activity external video evidence not found.'],404);if((int)$before['RowVersion']!==$version)json_response(['success'=>false,'code'=>'KY_ACTIVITY_EXTERNAL_STALE','message'=>'Activity video metadata changed. Refresh and retry.'],409);if($status==='Verified'&&(trim((string)$before['ExternalReference'])===''||!preg_match('/^[a-f0-9]{64}$/i',(string)$before['SHA256'])))json_response(['success'=>false,'code'=>'KY_ACTIVITY_EXTERNAL_METADATA_REQUIRED','message'=>'Complete external video metadata is required before verification.'],400);db_execute('UPDATE ky_activity_external_video_evidence SET Status=?,VerificationNote=?,VerifiedByID=?,VerifiedByName=?,VerifiedAt=?,RowVersion=RowVersion+1 WHERE id=? AND RowVersion=?',[$status,$note?:null,wf_user_id($user),wf_user_name($user),$status==='Verified'?date('Y-m-d H:i:s'):null,$before['id'],$version]);$after=db_row('SELECT * FROM ky_activity_external_video_evidence WHERE id=?',[$before['id']]);wf_ky_activity_external_audit($user,$status==='Verified'?'ADMIN_VERIFIED':'NEEDS_CORRECTION',$before,$after,$note);$pdo->commit();json_response(['success'=>true,'data'=>wf_ky_activity_external_public($after)]);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
    }
    if($method==='GET'&&$path==='/ky/annual-video-evidence'){
        ...
        $activityExternal=array_map('wf_ky_activity_external_public',db_rows("SELECT e.*,a.ActivityDate,a.TeamName,a.KYTKeyword,a.ReporterName,a.Status AS ActivityStatus FROM ky_activity_external_video_evidence e INNER JOIN ky_activities a ON a.id=e.ActivityID WHERE e.EvidenceYear=? ORDER BY a.ActivityDate DESC,e.UpdatedAt DESC",[$year]));
        $scopeVerifiedExternal=wf_ky_verified_external_video_sql('a');$scopePendingExternal=wf_ky_pending_external_video_sql('a');$activityScope=[];foreach(db_rows("SELECT a.Department,a.SafetyUnit,SUM(CASE WHEN COALESCE(TRIM(a.VideoUrl),'')<>'' THEN 1 ELSE 0 END) AS ProductionVideo,SUM(CASE WHEN COALESCE(TRIM(a.VideoUrl),'')='' AND {$scopeVerifiedExternal} THEN 1 ELSE 0 END) AS VerifiedExternalVideo,SUM(CASE WHEN COALESCE(TRIM(a.VideoUrl),'')='' AND NOT ({$scopeVerifiedExternal}) AND {$scopePendingExternal} THEN 1 ELSE 0 END) AS PendingExternalVideo FROM ky_activities a WHERE YEAR(a.ActivityDate)=? GROUP BY a.Department,a.SafetyUnit",[$year]) as $state){$activityScope[wf_ky_annual_scope_key($state['Department'],$state['SafetyUnit']??'')]=['productionVideo'=>(int)$state['ProductionVideo'],'verifiedExternalVideo'=>(int)$state['VerifiedExternalVideo'],'pendingExternalVideo'=>(int)$state['PendingExternalVideo']];}
        $scopes=[];foreach(db_rows('SELECT Department,SafetyUnits,YearlyTarget FROM ky_program_config WHERE Year=? AND IsActive=1 ORDER BY Department',[$year]) as $cfg){$units=wf_ky_units($cfg['SafetyUnits']??null);if(!$units)$units=[''];foreach($units as $unit){$key=wf_ky_annual_scope_key($cfg['Department'],$unit);$item=$byScope[$key]??null;$state=$activityScope[$key]??['productionVideo'=>0,'verifiedExternalVideo'=>0,'pendingExternalVideo'=>0];$compliance=wf_ky_annual_compliance_state($cfg['YearlyTarget']??12,$state['productionVideo'],$state['verifiedExternalVideo'],$state['pendingExternalVideo']);$scopes[]=array_merge(['scopeKey'=>$key,'department'=>$cfg['Department'],'safetyUnit'=>$unit?:null,'compliant'=>$compliance['annualCompliant'],'productionVideo'=>$state['productionVideo'],'verifiedExternalVideo'=>$state['verifiedExternalVideo'],'complianceSource'=>$compliance['annualCompliant']?'DistinctActivities':null,'pendingActivityExternal'=>!$compliance['annualCompliant']&&$state['pendingExternalVideo']>0,'evidence'=>$item],$compliance);}}
        $verified=count(array_filter($scopes,fn($row)=>!empty($row['compliant'])));$pending=count(array_filter($scopes,fn($row)=>empty($row['compliant'])&&(!empty($row['evidence'])||!empty($row['pendingActivityExternal']))));$missing=count(array_filter($scopes,fn($row)=>empty($row['compliant'])&&empty($row['evidence'])&&empty($row['pendingActivityExternal'])));$reclaimable=array_filter($evidence,fn($row)=>!empty($row['canDeleteProductionFile']));
        ...
        $inventory=array_map('wf_ky_inventory_public',db_rows("SELECT a.id AS ActivityID,a.ActivityDate,a.Department,a.SafetyUnit,a.TeamName,a.KYTKeyword,a.ReporterName,a.VideoUrl AS CurrentVideoUrl,inv.id AS InventoryID,inv.EvidenceYear,inv.ExternalBackupConfirmed,inv.ExternalReference,inv.OriginalFileName,inv.MimeType,inv.FileSize,inv.SHA256,inv.ProductionVideoUrl,inv.ProductionStoredName,inv.Status,inv.DeclaredByID,inv.DeclaredByName,inv.DeclaredAt,inv.VerifiedByID,inv.VerifiedByName,inv.VerifiedAt,inv.VerificationNote,inv.ProductionDeletedByID,inv.ProductionDeletedByName,inv.ProductionDeletedAt,inv.ProductionDeletionReason,inv.RowVersion,inv.CreatedAt,inv.UpdatedAt,ave.id AS AnnualEvidenceID,ave.Status AS AnnualEvidenceStatus FROM ky_activities a LEFT JOIN ky_video_file_inventory inv ON inv.ActivityID=a.id LEFT JOIN ky_annual_video_evidence ave ON ave.ActivityID=a.id WHERE YEAR(a.ActivityDate)=? AND (COALESCE(TRIM(a.VideoUrl),'')<>'' OR inv.id IS NOT NULL) ORDER BY a.ActivityDate DESC,a.CreatedAt DESC",[$year]));
        $inventorySummary=['total'=>count($inventory),'productionFiles'=>count(array_filter($inventory,fn($row)=>trim((string)($row['CurrentVideoUrl']??''))!=='')),'unregistered'=>count(array_filter($inventory,fn($row)=>empty($row['registered'])&&trim((string)($row['CurrentVideoUrl']??''))!=='')),'pending'=>count(array_filter($inventory,fn($row)=>in_array($row['Status']??'',['Pending','NeedsCorrection'],true))),'verifiedExternal'=>count(array_filter($inventory,fn($row)=>($row['Status']??'')==='Verified')),'reclaimableFiles'=>count(array_filter($inventory,fn($row)=>!empty($row['canDeleteProductionFile']))),'reclaimableBytes'=>array_sum(array_map(fn($row)=>!empty($row['canDeleteProductionFile'])?(int)($row['FileSize']??0):0,$inventory)),'deletedFiles'=>count(array_filter($inventory,fn($row)=>!empty($row['fileDeleted'])))];
        json_response(['success'=>true,'data'=>['year'=>$year,'summary'=>['requiredScopes'=>count($scopes),'verifiedScopes'=>$verified,'pendingScopes'=>$pending,'missingScopes'=>$missing,'compliancePct'=>count($scopes)?(int)round($verified/count($scopes)*100):0,'productionFiles'=>count(array_filter($evidence,fn($row)=>!empty($row['ProductionVideoUrl'])&&empty($row['ProductionDeletedAt']))),'reclaimableFiles'=>count($reclaimable),'reclaimableBytes'=>array_sum(array_map(fn($row)=>(int)($row['FileSize']??0),$reclaimable))],'scopes'=>$scopes,'evidence'=>$evidence,'activityExternalEvidence'=>$activityExternal,'candidates'=>$candidates,'inventory'=>$inventory,'inventorySummary'=>$inventorySummary]]);
    }
    ...
    $p=route_params($path,'/ky/video-inventory/:inventoryId/audit');if($p!==null&&$method==='GET'){require_admin();json_response(['success'=>true,'data'=>db_rows('SELECT id,InventoryID,ActivityID,Action,ActorID,ActorName,BeforeJson,AfterJson,Detail,CreatedAt FROM ky_video_file_inventory_audit WHERE InventoryID=? ORDER BY id DESC LIMIT 200',[$p['inventoryId']])]);}
    $p=route_params($path,'/ky/video-inventory/:inventoryId/download');if($p!==null&&$method==='GET'){
        require_admin();$row=db_row('SELECT * FROM ky_video_file_inventory WHERE id=?',[$p['inventoryId']]);if(!$row)json_response(['success'=>false,'message'=>'Production video inventory record not found.'],404);if(!empty($row['ProductionDeletedAt']))json_response(['success'=>false,'message'=>'Production copy has already been removed.'],410);$local=wf_ky_annual_local_video($row['ProductionVideoUrl']??null);if(!$local)json_response(['success'=>false,'message'=>'Production video file is unavailable.'],404);$hash=hash_file('sha256',$local);if(!is_string($hash)||!hash_equals(strtolower((string)$row['SHA256']),strtolower($hash)))json_response(['success'=>false,'code'=>'KY_VIDEO_INVENTORY_HASH_MISMATCH','message'=>'Production file SHA-256 no longer matches its inventory record.'],409);
    ...
    if($method==='POST'&&$path==='/ky/video-inventory/declare'){
        require_admin();$b=json_body();$activityId=trim((string)($b['activityId']??''));$external=trim((string)($b['externalReference']??''));$name=substr(str_replace(['\\','/'],'_',trim((string)($b['originalFileName']??''))),0,255);$mime=substr(trim((string)($b['mimeType']??'')),0,120)?:null;$size=(int)($b['fileSize']??0);$sha=strtolower(trim((string)($b['sha256']??'')));$attested=(string)($b['registrationMode']??'')==='AdminAttested';$requestedVersion=(int)($b['rowVersion']??0);
        if($activityId===''||$external===''||$name===''||(!$attested&&($size<=0||!preg_match('/^[a-f0-9]{64}$/',$sha))))json_response(['success'=>false,'code'=>'KY_VIDEO_INVENTORY_METADATA_REQUIRED','message'=>'Activity, external reference, filename, size and SHA-256 are required.'],400);
        $activity=db_row('SELECT * FROM ky_activities WHERE id=?',[$activityId]);if(!$activity)json_response(['success'=>false,'message'=>'KY activity not found.'],404);$productionUrl=trim((string)($activity['VideoUrl']??''));$local=wf_ky_annual_local_video($productionUrl);if(!$local)json_response(['success'=>false,'code'=>'KY_VIDEO_INVENTORY_PRODUCTION_REQUIRED','message'=>'This activity has no readable Production video.'],400);$productionHash=hash_file('sha256',$local);$productionSize=(int)filesize($local);if(!$attested&&($productionSize!==$size||!is_string($productionHash)||!hash_equals(strtolower($productionHash),$sha)))json_response(['success'=>false,'code'=>'KY_VIDEO_INVENTORY_BACKUP_MISMATCH','message'=>'The selected external backup does not match the current Production video.'],409);$recordedSize=$attested?$productionSize:$size;$recordedHash=$attested?(string)$productionHash:$sha;
        $year=(int)substr((string)$activity['ActivityDate'],0,4);$pdo=db();$pdo->beginTransaction();try{$before=db_row('SELECT * FROM ky_video_file_inventory WHERE ActivityID=? FOR UPDATE',[$activityId]);if(($before['Status']??'')==='Verified')json_response(['success'=>false,'code'=>'KY_VIDEO_INVENTORY_ALREADY_VERIFIED','message'=>'This file is already verified. Mark it for correction before replacing its backup metadata.'],409);if($before&&$requestedVersion!==(int)$before['RowVersion'])json_response(['success'=>false,'code'=>'KY_VIDEO_INVENTORY_STALE','message'=>'Inventory metadata changed. Refresh and retry.'],409);$id=$before['id']??wf_uuid();if($before){db_execute("UPDATE ky_video_file_inventory SET EvidenceYear=?,Department=?,SafetyUnit=?,ExternalBackupConfirmed=1,ExternalReference=?,OriginalFileName=?,MimeType=?,FileSize=?,SHA256=?,ProductionVideoUrl=?,ProductionStoredName=?,Status='Pending',DeclaredByID=?,DeclaredByName=?,DeclaredAt=NOW(),VerifiedByID=NULL,VerifiedByName=NULL,VerifiedAt=NULL,VerificationNote=NULL,ProductionDeletedByID=NULL,ProductionDeletedByName=NULL,ProductionDeletedAt=NULL,ProductionDeletionReason=NULL,RowVersion=RowVersion+1 WHERE id=?",[$year,$activity['Department'],$activity['SafetyUnit']?:null,$external,$name,$mime,$recordedSize,$recordedHash,$productionUrl,basename($local),wf_user_id($user),$actor,$id]);}else{db_execute("INSERT INTO ky_video_file_inventory (id,EvidenceYear,ActivityID,Department,SafetyUnit,ExternalBackupConfirmed,ExternalReference,OriginalFileName,MimeType,FileSize,SHA256,ProductionVideoUrl,ProductionStoredName,Status,DeclaredByID,DeclaredByName) VALUES (?,?,?,?,?,1,?,?,?,?,?,?,?,'Pending',?,?)",[$id,$year,$activityId,$activity['Department'],$activity['SafetyUnit']?:null,$external,$name,$mime,$recordedSize,$recordedHash,$productionUrl,basename($local),wf_user_id($user),$actor]);}$after=db_row('SELECT * FROM ky_video_file_inventory WHERE id=?',[$id]);wf_ky_inventory_audit($user,$before?'BACKUP_METADATA_UPDATED':'BACKUP_DECLARED',$before?:null,$after,$attested?'Admin registered the external filename; Production size and SHA-256 were captured server-side. External copy awaits Admin verification.':'External backup metadata matched the Production video.');$pdo->commit();json_response(['success'=>true,'data'=>wf_ky_inventory_public($after)],$before?200:201);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
    }
    $p=route_params($path,'/ky/video-inventory/:inventoryId/verify');if($p!==null&&$method==='POST'){
        require_admin();$b=json_body();$status=(string)($b['status']??'Verified');$version=(int)($b['rowVersion']??0);$note=trim((string)($b['note']??''));if(!in_array($status,['Verified','NeedsCorrection'],true)||$version<1)json_response(['success'=>false,'message'=>'Status and row version are required.'],400);$pdo=db();$pdo->beginTransaction();try{$before=db_row('SELECT * FROM ky_video_file_inventory WHERE id=? FOR UPDATE',[$p['inventoryId']]);if(!$before)json_response(['success'=>false,'message'=>'Production video inventory record not found.'],404);if((int)$before['RowVersion']!==$version)json_response(['success'=>false,'code'=>'KY_VIDEO_INVENTORY_STALE','message'=>'Inventory metadata changed. Refresh and retry.'],409);if($status==='Verified'&&(empty($before['ExternalBackupConfirmed'])||trim((string)$before['ExternalReference'])===''||!preg_match('/^[a-f0-9]{64}$/i',(string)$before['SHA256'])))json_response(['success'=>false,'code'=>'KY_EXTERNAL_BACKUP_REQUIRED','message'=>'Complete external backup metadata is required before verification.'],400);db_execute('UPDATE ky_video_file_inventory SET Status=?,VerificationNote=?,VerifiedByID=?,VerifiedByName=?,VerifiedAt=?,RowVersion=RowVersion+1 WHERE id=? AND RowVersion=?',[$status,$note?:null,wf_user_id($user),$actor,$status==='Verified'?date('Y-m-d H:i:s'):null,$before['id'],$version]);$after=db_row('SELECT * FROM ky_video_file_inventory WHERE id=?',[$before['id']]);wf_ky_inventory_audit($user,$status==='Verified'?'ADMIN_VERIFIED':'NEEDS_CORRECTION',$before,$after,$note);$pdo->commit();json_response(['success'=>true,'data'=>wf_ky_inventory_public($after)]);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
    }
    if($method==='POST'&&$path==='/ky/video-inventory/delete-production'){
        require_admin();
        $probe=json_body();foreach(array_slice(is_array($probe['items']??null)?$probe['items']:[],0,100) as $probeItem){$hold=db_row("SELECT ce.id FROM ky_video_file_inventory vi INNER JOIN ky_annual_unit_contest_entries ce ON ce.ActivityID=vi.ActivityID AND ce.Status='Submitted' WHERE vi.id=? LIMIT 1",[(string)($probeItem['id']??'')]);if($hold)json_response(['success'=>false,'code'=>'KY_CONTEST_ENTRY_RETENTION_HOLD','message'=>'This Production video is the active annual Unit contest entry and cannot be deleted.'],409);}
        require_admin();$b=json_body();$items=array_slice(is_array($b['items']??null)?$b['items']:[],0,100);$reason=trim((string)($b['reason']??''));if(!$items||$reason==='')json_response(['success'=>false,'message'=>'Select inventory records and provide a deletion reason.'],400);$pdo=db();$quarantined=[];$pdo->beginTransaction();try{$rows=[];foreach($items as $item){$row=db_row('SELECT * FROM ky_video_file_inventory WHERE id=? FOR UPDATE',[(string)($item['id']??'')]);if(!$row||(int)$row['RowVersion']!==(int)($item['rowVersion']??0))throw new RuntimeException('Inventory record is missing or stale.',409);if(empty(wf_ky_inventory_public($row)['canDeleteProductionFile']))throw new RuntimeException('Every selected file must have a verified matching external backup.',409);$activity=db_row('SELECT id,VideoUrl FROM ky_activities WHERE id=? FOR UPDATE',[$row['ActivityID']]);if(!$activity||(string)($activity['VideoUrl']??'')!==(string)($row['ProductionVideoUrl']??''))throw new RuntimeException('The activity video reference changed. Refresh and register the current file again.',409);$local=wf_ky_annual_local_video($row['ProductionVideoUrl']);if(!$local)throw new RuntimeException('A selected Production file is unavailable.',404);$hash=hash_file('sha256',$local);if(!is_string($hash)||!hash_equals(strtolower((string)$row['SHA256']),strtolower($hash)))throw new RuntimeException('A selected Production file failed SHA-256 verification.',409);$rows[]=['row'=>$row,'local'=>$local];}foreach($rows as $entry){$quarantine=$entry['local'].'.ky-inventory-delete-'.bin2hex(random_bytes(8)).'.tmp';if(!rename($entry['local'],$quarantine))throw new RuntimeException('Unable to quarantine a selected Production file.');$entry['quarantine']=$quarantine;$quarantined[]=$entry;}foreach($quarantined as $entry){$row=$entry['row'];db_execute('UPDATE ky_activities SET VideoUrl=NULL WHERE id=? AND VideoUrl=?',[$row['ActivityID'],$row['ProductionVideoUrl']]);db_execute('UPDATE ky_video_file_inventory SET ProductionDeletedByID=?,ProductionDeletedByName=?,ProductionDeletedAt=NOW(),ProductionDeletionReason=?,RowVersion=RowVersion+1 WHERE id=?',[wf_user_id($user),$actor,$reason,$row['id']]);$after=db_row('SELECT * FROM ky_video_file_inventory WHERE id=?',[$row['id']]);wf_ky_inventory_audit($user,'PRODUCTION_FILE_DELETED',$row,$after,$reason);}$pdo->commit();foreach($quarantined as $entry)@unlink($entry['quarantine']);json_response(['success'=>true,'data'=>['deleted'=>count($quarantined)]]);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();foreach(array_reverse($quarantined) as $entry)if(is_file($entry['quarantine'])&&!is_file($entry['local']))@rename($entry['quarantine'],$entry['local']);$code=(int)$e->getCode();json_response(['success'=>false,'message'=>$e->getMessage()],$code>=400&&$code<600?$code:500);}
    }
    ...
    if($method==='POST'&&$path==='/ky/annual-video-evidence/delete-production'){
        require_admin();
        $probe=json_body();foreach(array_slice(is_array($probe['items']??null)?$probe['items']:[],0,100) as $probeItem){$hold=db_row("SELECT ce.id FROM ky_annual_video_evidence ae INNER JOIN ky_annual_unit_contest_entries ce ON ce.ActivityID=ae.ActivityID AND ce.Status='Submitted' WHERE ae.id=? LIMIT 1",[(string)($probeItem['id']??'')]);if($hold)json_response(['success'=>false,'code'=>'KY_CONTEST_ENTRY_RETENTION_HOLD','message'=>'This Production video is the active annual Unit contest entry and cannot be deleted.'],409);}
    ...
    if($method==='GET'&&$path==='/ky/evidence-overview'){
        ...
        $contestByScope=[];foreach(db_rows("SELECT * FROM ky_annual_unit_contest_entries WHERE EntryYear=? AND Status='Submitted'",[$year]) as $contestEntry)$contestByScope[$contestEntry['ScopeKey']]=$contestEntry;
        ...
        $unmatched=[];$activityExternalProjection=[];foreach($activities as $activityProjection){$activityExternalProjection[(string)($activityProjection['id']??'')]=['activityExternalEvidenceId'=>$activityProjection['ActivityExternalEvidenceID']??null,'activityExternalStatus'=>$activityProjection['ActivityExternalStatus']??null,'activityExternalRowVersion'=>(int)($activityProjection['ActivityExternalRowVersion']??0)];}
        foreach($activities as $a){$deptKey=wf_ky_norm_key($a['Department']??'');if(empty($deptKeys[$deptKey]))continue;$key=$deptKey.'||'.wf_ky_norm_key($a['SafetyUnit']??'');if(!isset($rowMap[$key])){$unmatched[]=['id'=>$a['id']??null,'department'=>$a['Department']??null,'safetyUnit'=>$a['SafetyUnit']??null,'reason'=>'Safety Unit not active in Program Config'];continue;}$idx=$rowMap[$key];$scopeKey=wf_ky_annual_scope_key($a['Department']??'',$a['SafetyUnit']??'');$contest=$contestByScope[$scopeKey]??null;$hasFile=trim((string)($a['AttachmentUrl']??''))!=='';$hasProduction=!empty($a['HasProductionVideo']);$hasVerifiedExternal=!empty($a['HasVerifiedExternalVideo']);$hasPendingExternal=!empty($a['HasPendingExternalVideo']);$hasVideo=!empty($a['HasVideoEvidence']);$status=$hasFile&&$hasVideo?'complete':($hasFile&&$hasPendingExternal?'external_pending':($hasFile?'waiting_video':'missing_file'));$rows[$idx]['submitted']++;if($status==='complete')$rows[$idx]['complete']++;elseif($status==='waiting_video')$rows[$idx]['waitingVideo']++;elseif($status==='missing_file')$rows[$idx]['missingFile']++;if($hasProduction)$rows[$idx]['productionVideo']++;elseif($hasVerifiedExternal)$rows[$idx]['verifiedExternalVideo']++;elseif($hasPendingExternal)$rows[$idx]['pendingExternalVideo']++;$rows[$idx]['records'][]=['id'=>$a['id']??null,'activityDate'=>$a['ActivityDate']??null,'reporterId'=>$a['ReporterID']??null,'reporterName'=>$a['ReporterName']??null,'submittedById'=>$a['SubmittedByID']??null,'submittedByName'=>$a['SubmittedByName']??null,'department'=>$rows[$idx]['department'],'safetyUnit'=>$rows[$idx]['safetyUnit'],'teamName'=>$a['TeamName']??null,'kytKeyword'=>$a['KYTKeyword']??null,'riskCategory'=>$a['RiskCategory']??null,'hazard'=>$a['HazardDescription']??null,'status'=>$a['Status']??null,'evidenceStatus'=>$status,'hasFile'=>$hasFile,'hasVideo'=>$hasVideo,'hasProductionVideo'=>$hasProduction,'hasVerifiedExternalVideo'=>$hasVerifiedExternal,'hasPendingExternalVideo'=>$hasPendingExternal,'videoEvidenceStorage'=>$a['VideoEvidenceStorage']??'Missing','canUploadVideo'=>wf_ky_can_upload_followup_video($a,$user)&&(!$hasVideo||wf_is_admin($user)),'canRegisterExternalVideo'=>wf_is_admin($user),'isContestEntry'=>$contest&&($contest['ActivityID']??'')===($a['id']??''),'contestEntryId'=>$contest['id']??null,'canSubmitContestEntry'=>$hasProduction&&wf_ky_can_upload_followup_video($a,$user)];}
        foreach($rows as &$scopeRow){foreach($scopeRow['records'] as &$record){$record=array_merge($record,$activityExternalProjection[(string)($record['id']??'')]??['activityExternalEvidenceId'=>null,'activityExternalStatus'=>null,'activityExternalRowVersion'=>0]);$sameDepartment=wf_ky_norm_key($user['department']??$user['Department']??'')===wf_ky_norm_key($record['department']??'');$sameUnit=wf_ky_norm_key($record['safetyUnit']??'')===''||wf_ky_norm_key($user['unit']??$user['Unit']??'')===wf_ky_norm_key($record['safetyUnit']??'');$record['canSubmitContestEntry']=!empty($record['hasProductionVideo'])&&(!empty($record['canSubmitContestEntry'])||($sameDepartment&&$sameUnit));}unset($record);}unset($scopeRow);
        $summary=['departments'=>count(array_unique(array_map(fn($r)=>$r['department'],$rows))),'safetyUnits'=>count($rows),'submitted'=>0,'complete'=>0,'waitingVideo'=>0,'missingFile'=>0,'productionVideo'=>0,'verifiedExternalVideo'=>0,'pendingExternalVideo'=>0,'evidenceTotal'=>0,'annualCompliantScopes'=>0,'contestEntries'=>0];
        $deptProgress=[];foreach($rows as &$r){$r['progressPct']=$r['yearlyTarget']>0?min(100,(int)round($r['submitted']/$r['yearlyTarget']*100)):0;$compliance=wf_ky_annual_compliance_state($r['yearlyTarget'],$r['productionVideo'],$r['verifiedExternalVideo'],$r['pendingExternalVideo']);$r=array_merge($r,$compliance);$r['contestEntry']=$contestByScope[wf_ky_annual_scope_key($r['department'],$r['safetyUnit'])]??null;$r['hasContestEntry']=!empty($r['contestEntry']);$dk=wf_ky_norm_key($r['department']);if(!isset($deptProgress[$dk]))$deptProgress[$dk]=['submitted'=>0,'target'=>0,'pct'=>0];$deptProgress[$dk]['submitted']+=$r['submitted'];$deptProgress[$dk]['target']+=$r['yearlyTarget'];$deptProgress[$dk]['pct']=$deptProgress[$dk]['target']>0?min(100,(int)round($deptProgress[$dk]['submitted']/$deptProgress[$dk]['target']*100)):0;$summary['submitted']+=$r['submitted'];$summary['complete']+=$r['complete'];$summary['waitingVideo']+=$r['waitingVideo'];$summary['missingFile']+=$r['missingFile'];$summary['productionVideo']+=$r['productionVideo'];$summary['verifiedExternalVideo']+=$r['verifiedExternalVideo'];$summary['pendingExternalVideo']+=$r['pendingExternalVideo'];$summary['evidenceTotal']+=$r['evidenceTotal'];if($r['annualCompliant'])$summary['annualCompliantScopes']++;if($r['hasContestEntry'])$summary['contestEntries']++;}unset($r);usort($rows,function($a,$b)use($deptProgress){$ap=$deptProgress[wf_ky_norm_key($a['department'])]['pct']??0;$bp=$deptProgress[wf_ky_norm_key($b['department'])]['pct']??0;return ($ap<=>$bp)?:strcmp((string)$a['department'],(string)$b['department'])?:((int)($a['order']??0)<=>(int)($b['order']??0));});foreach($rows as &$r){unset($r['order']);}unset($r);
    ...
    if($method==='GET'&&$path==='/ky'){
        ...
        elseif($evidence==='waiting_video')$sql.=" AND COALESCE(TRIM(a.AttachmentUrl),'')<>'' AND NOT ({$videoEvidence}) AND NOT ({$pendingExternal})";
        elseif($evidence==='no_video')$sql.=" AND NOT ({$videoEvidence}) AND NOT ({$pendingExternal})";
    ...
    if($path==='/ky/video-upload/config'&&$method==='GET'){
        $config=wf_ky_video_upload_config();
        if(empty($config['available']))json_response(['success'=>false,'code'=>'KY_VIDEO_SERVER_LIMIT_INVALID','message'=>'ข้อจำกัดอัปโหลดของเซิร์ฟเวอร์ต่ำเกินกว่าจะอัปโหลดวิดีโอแบบแบ่งส่วนได้'],503);
        unset($config['available']);
    ...
    $p=route_params($path,'/ky/:id/video-upload/init'); if($p!==null&&$method==='POST'){
        ...
        $mimeTypes=$config['acceptedMimeTypes'];
        if($fileSize<=0||$fileSize>(int)$config['maxFileSize'])json_response(['success'=>false,'code'=>'KY_VIDEO_SIZE_INVALID','message'=>'วิดีโอต้องมีขนาดไม่เกิน 200 MB'],400);
        ...
        unset($config['available']);
        json_response(['success'=>true,'data'=>array_merge($config,['uploadId'=>$uploadId,'totalChunks'=>$manifest['totalChunks'],'expiresAt'=>$manifest['expiresAt']])],201);
    }
    $p=route_params($path,'/ky/:id/video-upload/:uploadId/chunk/:index'); if($p!==null&&$method==='POST'){
        ...
        if(!is_array($file))json_response(['success'=>false,'code'=>'KY_VIDEO_CHUNK_MISSING','message'=>'ไม่พบข้อมูลส่วนวิดีโอ'],400);
        $uploadError=(int)($file['error']??UPLOAD_ERR_NO_FILE);
        if(in_array($uploadError,[UPLOAD_ERR_INI_SIZE,UPLOAD_ERR_FORM_SIZE],true))json_response(['success'=>false,'code'=>'KY_VIDEO_CHUNK_TOO_LARGE','message'=>'ส่วนวิดีโอมีขนาดเกินขีดจำกัด PHP กรุณาเริ่มอัปโหลดใหม่เพื่อรับขนาดส่วนล่าสุด','maxChunkSize'=>(int)$manifest['chunkSize']],413);
        if($uploadError===UPLOAD_ERR_PARTIAL)json_response(['success'=>false,'code'=>'KY_VIDEO_CHUNK_PARTIAL','message'=>'รับข้อมูลส่วนวิดีโอไม่ครบ กรุณาลองส่วนนี้ใหม่'],400);
        if($uploadError===UPLOAD_ERR_NO_FILE)json_response(['success'=>false,'code'=>'KY_VIDEO_CHUNK_MISSING','message'=>'ไม่พบข้อมูลส่วนวิดีโอ'],400);
        if($uploadError===UPLOAD_ERR_NO_TMP_DIR)json_response(['success'=>false,'code'=>'KY_VIDEO_SERVER_STORAGE_UNAVAILABLE','message'=>'เซิร์ฟเวอร์ไม่มีพื้นที่ชั่วคราวสำหรับรับวิดีโอ'],500);
        if($uploadError===UPLOAD_ERR_CANT_WRITE)json_response(['success'=>false,'code'=>'KY_VIDEO_SERVER_WRITE_FAILED','message'=>'เซิร์ฟเวอร์ไม่สามารถเขียนส่วนวิดีโอได้'],500);
        if($uploadError===UPLOAD_ERR_EXTENSION)json_response(['success'=>false,'code'=>'KY_VIDEO_UPLOAD_BLOCKED','message'=>'ส่วนขยาย PHP ปฏิเสธการอัปโหลดวิดีโอ'],500);
        if($uploadError!==UPLOAD_ERR_OK)json_response(['success'=>false,'code'=>'KY_VIDEO_CHUNK_INVALID','message'=>'ข้อมูลส่วนวิดีโอไม่ถูกต้อง'],400);
        $expected=wf_ky_video_expected_chunk_size($manifest,(int)$index);
        if((int)($file['size']??0)!==$expected||$expected<=0||$expected>(int)$manifest['chunkSize'])json_response(['success'=>false,'code'=>'KY_VIDEO_CHUNK_SIZE_MISMATCH','message'=>'ขนาดส่วนวิดีโอไม่ตรงกับที่ระบบกำหนด'],400);
        $suppliedHash=strtolower(trim((string)($_SERVER['HTTP_X_KY_CHUNK_SHA256']??'')));
        if(!preg_match('/^[a-f0-9]{64}$/',$suppliedHash))json_response(['success'=>false,'code'=>'KY_VIDEO_CHUNK_HASH_REQUIRED','message'=>'ไม่พบ SHA-256 ของส่วนวิดีโอหรือรูปแบบไม่ถูกต้อง'],400);
        $actualHash=hash_file('sha256',(string)$file['tmp_name']);
        ...
        $storageFailure=null;
        if(!wf_ky_video_store_uploaded_chunk((string)$file['tmp_name'],$target,$expected,$storageFailure)){@unlink($target);json_response(['success'=>false,'code'=>$storageFailure?:'KY_VIDEO_SERVER_WRITE_FAILED','message'=>'ไม่สามารถบันทึกส่วนวิดีโอ KY ได้'],500);}
        @chmod($target,0640);
        json_response(['success'=>true,'data'=>['uploadId'=>$manifest['uploadId'],'index'=>(int)$index,'receivedBytes'=>$expected,'sha256'=>$actualHash]]);
    }
    $p=route_params($path,'/ky/:id/video-upload/:uploadId/complete'); if($p!==null&&$method==='POST'){
        ...
        try{
            ...
            $sha256=hash_file('sha256',$final);if(!is_string($sha256))throw new RuntimeException('ไม่สามารถตรวจสอบ SHA-256 ของวิดีโอได้');
            ...
            try{wf_ky_audit($user,'KY_VIDEO_CHUNK_UPLOAD_COMPLETE',(string)$p['id'],!empty($lockedAccess['admin'])&&!empty($previous)?'Admin replaced KY video with chunk upload':'Uploaded KY video in chunks',['chunks'=>$manifest['totalChunks'],'bytes'=>$manifest['fileSize'],'sha256'=>$sha256,'replacedExisting'=>!empty($previous)]);}catch(Throwable $auditError){error_log('KY video chunk audit error: '.$auditError->getMessage());}
            fclose($lock);@unlink($lockPath);wf_ky_video_remove_tree($manifest['uploadId']);
            json_response(['success'=>true,'data'=>['id'=>$p['id'],'videoUrl'=>$videoUrl,'sha256'=>$sha256]]);
        }catch(Throwable $e){
    ...
    $p=route_params($path,'/ky/:id'); if($p!==null&&$method==='DELETE'){
        require_admin();
        $row=db_row('SELECT a.AttachmentUrl,a.VideoUrl,EXISTS(SELECT 1 FROM ky_annual_video_evidence ave WHERE ave.ActivityID=a.id) AS HasAnnualVideoEvidence,EXISTS(SELECT 1 FROM ky_video_file_inventory inv WHERE inv.ActivityID=a.id) AS HasVideoInventory FROM ky_activities a WHERE a.id=?',[$p['id']]);
        if($row&&(!empty($row['VideoUrl'])||(int)($row['HasAnnualVideoEvidence']??0)>0||(int)($row['HasVideoInventory']??0)>0))json_response(['success'=>false,'code'=>'KY_VIDEO_PROTECTED_BY_RETENTION','message'=>'This activity is protected by its video evidence retention record and cannot be deleted.'],409);
        $pdo=db();
        try{
            $pdo->beginTransaction();
