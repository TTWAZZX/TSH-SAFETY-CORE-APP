...
function handle_ky_routes(string $method, string $path): bool
{
    ...
    if($method==='GET'&&$path==='/ky/annual-video-evidence'){
        ...
        json_response(['success'=>true,'data'=>['year'=>$year,'summary'=>['requiredScopes'=>count($scopes),'verifiedScopes'=>$verified,'pendingScopes'=>$pending,'missingScopes'=>$missing,'compliancePct'=>count($scopes)?(int)round($verified/count($scopes)*100):0,'productionFiles'=>count(array_filter($evidence,fn($row)=>!empty($row['ProductionVideoUrl'])&&empty($row['ProductionDeletedAt']))),'reclaimableFiles'=>count($reclaimable),'reclaimableBytes'=>array_sum(array_map(fn($row)=>(int)($row['FileSize']??0),$reclaimable))],'scopes'=>$scopes,'evidence'=>$evidence,'candidates'=>$candidates,'inventory'=>$inventory,'inventorySummary'=>$inventorySummary]]);
    }
