...
function handle_ky_routes(string $method, string $path): bool
{
    ...
    if($method==='GET'&&$path==='/ky/annual-video-evidence'){
        ...
        $scopeVerifiedExternal=wf_ky_verified_external_video_sql('a');$scopePendingExternal=wf_ky_pending_external_video_sql('a');$activityScope=[];foreach(db_rows("SELECT a.Department,a.SafetyUnit,MAX(CASE WHEN COALESCE(TRIM(a.VideoUrl),'')<>'' THEN 1 ELSE 0 END) AS HasProduction,MAX(CASE WHEN {$scopeVerifiedExternal} THEN 1 ELSE 0 END) AS HasVerifiedExternal,MAX(CASE WHEN {$scopePendingExternal} THEN 1 ELSE 0 END) AS HasPendingExternal FROM ky_activities a WHERE YEAR(a.ActivityDate)=? GROUP BY a.Department,a.SafetyUnit",[$year]) as $state){$activityScope[wf_ky_annual_scope_key($state['Department'],$state['SafetyUnit']??'')]=['production'=>!empty($state['HasProduction']),'verifiedExternal'=>!empty($state['HasVerifiedExternal']),'pendingExternal'=>!empty($state['HasPendingExternal'])];}
        $scopes=[];foreach(db_rows('SELECT Department,SafetyUnits FROM ky_program_config WHERE Year=? AND IsActive=1 ORDER BY Department',[$year]) as $cfg){$units=wf_ky_units($cfg['SafetyUnits']??null);if(!$units)$units=[''];foreach($units as $unit){$key=wf_ky_annual_scope_key($cfg['Department'],$unit);$item=$byScope[$key]??null;$state=$activityScope[$key]??['production'=>false,'verifiedExternal'=>false,'pendingExternal'=>false];$compliant=($item['Status']??'')==='Verified'||$state['production']||$state['verifiedExternal'];$scopes[]=['scopeKey'=>$key,'department'=>$cfg['Department'],'safetyUnit'=>$unit?:null,'compliant'=>$compliant,'complianceSource'=>($item['Status']??'')==='Verified'?'AnnualEvidence':($state['production']?'ProductionActivity':($state['verifiedExternal']?'ExternalActivity':null)),'pendingActivityExternal'=>!$compliant&&$state['pendingExternal'],'evidence'=>$item];}}
    ...
    if($method==='GET'&&$path==='/ky/evidence-overview'){
        ...
        foreach($activities as $a){$deptKey=wf_ky_norm_key($a['Department']??'');if(empty($deptKeys[$deptKey]))continue;$key=$deptKey.'||'.wf_ky_norm_key($a['SafetyUnit']??'');if(!isset($rowMap[$key])){$unmatched[]=['id'=>$a['id']??null,'department'=>$a['Department']??null,'safetyUnit'=>$a['SafetyUnit']??null,'reason'=>'Safety Unit not active in Program Config'];continue;}$idx=$rowMap[$key];$hasFile=trim((string)($a['AttachmentUrl']??''))!=='';$hasProduction=!empty($a['HasProductionVideo']);$hasVerifiedExternal=!empty($a['HasVerifiedExternalVideo']);$hasPendingExternal=!empty($a['HasPendingExternalVideo']);$hasVideo=!empty($a['HasVideoEvidence']);$status=$hasFile&&$hasVideo?'complete':($hasFile&&$hasPendingExternal?'external_pending':($hasFile?'waiting_video':'missing_file'));$rows[$idx]['submitted']++;if($status==='complete')$rows[$idx]['complete']++;elseif($status==='waiting_video')$rows[$idx]['waitingVideo']++;elseif($status==='missing_file')$rows[$idx]['missingFile']++;if($hasProduction)$rows[$idx]['productionVideo']++;elseif($hasVerifiedExternal)$rows[$idx]['verifiedExternalVideo']++;elseif($hasPendingExternal)$rows[$idx]['pendingExternalVideo']++;$rows[$idx]['records'][]=['id'=>$a['id']??null,'activityDate'=>$a['ActivityDate']??null,'reporterId'=>$a['ReporterID']??null,'reporterName'=>$a['ReporterName']??null,'submittedById'=>$a['SubmittedByID']??null,'submittedByName'=>$a['SubmittedByName']??null,'department'=>$rows[$idx]['department'],'safetyUnit'=>$rows[$idx]['safetyUnit'],'teamName'=>$a['TeamName']??null,'kytKeyword'=>$a['KYTKeyword']??null,'riskCategory'=>$a['RiskCategory']??null,'hazard'=>$a['HazardDescription']??null,'status'=>$a['Status']??null,'evidenceStatus'=>$status,'hasFile'=>$hasFile,'hasVideo'=>$hasVideo,'hasProductionVideo'=>$hasProduction,'hasVerifiedExternalVideo'=>$hasVerifiedExternal,'hasPendingExternalVideo'=>$hasPendingExternal,'videoEvidenceStorage'=>$a['VideoEvidenceStorage']??'Missing','canUploadVideo'=>wf_ky_can_upload_followup_video($a,$user)&&(!$hasVideo||wf_is_admin($user)),'canRegisterExternalVideo'=>wf_is_admin($user)];}
        foreach($rows as &$scopeRow){foreach($scopeRow['records'] as &$record){$record=array_merge($record,$activityExternalProjection[(string)($record['id']??'')]??['activityExternalEvidenceId'=>null,'activityExternalStatus'=>null,'activityExternalRowVersion'=>0]);}unset($record);}unset($scopeRow);
