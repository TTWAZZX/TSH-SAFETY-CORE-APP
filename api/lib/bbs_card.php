...
function bbs_card_employees(array $ids, ?PDO $pdo=null): array {
    if(!$ids)return[];$pdo=$pdo?:db();$marks=implode(',',array_fill(0,count($ids),'?'));$stmt=$pdo->prepare("SELECT e.EmployeeID,e.EmployeeName,e.Department,e.Unit,e.Position,m.BBSLevel,md.id DepartmentID,mu.id SafetyUnitID FROM employees e LEFT JOIN master_departments md ON LOWER(TRIM(md.Name))=LOWER(TRIM(e.Department)) LEFT JOIN master_safetyunits mu ON mu.department_id=md.id AND LOWER(TRIM(mu.name))=LOWER(TRIM(e.Unit)) LEFT JOIN master_positions p ON LOWER(TRIM(p.Name))=LOWER(TRIM(e.Position)) LEFT JOIN BBS_Position_Level_Mappings m ON m.PositionID=p.id AND m.IsActive=1 WHERE e.EmployeeID IN ($marks)");$stmt->execute($ids);return$stmt->fetchAll()?:[];
}
function bbs_card_template_matches(array $template,array $employee): bool { return (empty($template['DepartmentID'])||(int)$template['DepartmentID']===(int)($employee['DepartmentID']??0))&&(empty($template['SafetyUnitID'])||(int)$template['SafetyUnitID']===(int)($employee['SafetyUnitID']??0))&&(empty($template['BBSLevel'])||(string)$template['BBSLevel']===(string)($employee['BBSLevel']??'')); }
function bbs_card_template_mismatch_fields(array $template,array $employee): array { $fields=[];if(!empty($template['DepartmentID'])&&(int)$template['DepartmentID']!==(int)($employee['DepartmentID']??0))$fields[]='Department';if(!empty($template['SafetyUnitID'])&&(int)$template['SafetyUnitID']!==(int)($employee['SafetyUnitID']??0))$fields[]='Unit';if(!empty($template['BBSLevel'])&&(string)$template['BBSLevel']!==(string)($employee['BBSLevel']??''))$fields[]='BBSLevel';return$fields; }
