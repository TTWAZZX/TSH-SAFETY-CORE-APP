...
{
    global $config;
    return ($config['email_enabled'] ?? true)
...
function mailer_ehlo_domain($value): string
{
...
function mailer_send_mail($to, string $subject, ?string $text, ?string $html = null): array
{
    ...
    if (!mailer_smtp_configured()) {
        return [
            'skipped' => true,
            'reason' => ($config['email_enabled'] ?? true)
    ...
    try {
        ...
        $ehloDomain = mailer_ehlo_domain($config['smtp_ehlo_domain'] ?? 'localhost');
        mailer_send_line($socket, 'EHLO ' . $ehloDomain, 250);
