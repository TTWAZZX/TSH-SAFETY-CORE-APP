...
async function employeeCardData(employeeIds, queryable = db) {
    ...
    const [rows] = await queryable.query(`SELECT e.EmployeeID,e.EmployeeName,e.Department,e.Unit,e.Position,m.BBSLevel,md.id DepartmentID,mu.id SafetyUnitID FROM Employees e LEFT JOIN Master_Departments md ON LOWER(TRIM(md.Name))=LOWER(TRIM(e.Department)) LEFT JOIN Master_SafetyUnits mu ON mu.department_id=md.id AND LOWER(TRIM(mu.name))=LOWER(TRIM(e.Unit)) LEFT JOIN Master_Positions p ON LOWER(TRIM(p.Name))=LOWER(TRIM(e.Position)) LEFT JOIN BBS_Position_Level_Mappings m ON m.PositionID=p.id AND m.IsActive=1 WHERE e.EmployeeID IN (${placeholders})`, employeeIds);
...
function templateMatches(template, employee) {
    return (!template.DepartmentID || Number(template.DepartmentID) === Number(employee.DepartmentID))
        && (!template.SafetyUnitID || Number(template.SafetyUnitID) === Number(employee.SafetyUnitID))
        && (!template.BBSLevel || String(template.BBSLevel) === String(employee.BBSLevel));
}
function templateMismatchFields(template,employee){
    const fields=[];
    if(template.DepartmentID&&Number(template.DepartmentID)!==Number(employee.DepartmentID))fields.push('Department');
    if(template.SafetyUnitID&&Number(template.SafetyUnitID)!==Number(employee.SafetyUnitID))fields.push('Unit');
    if(template.BBSLevel&&String(template.BBSLevel)!==String(employee.BBSLevel))fields.push('BBSLevel');
    return fields;
}
function validPersonalTemplateLevel(level) {
    return !level || levelRank(level) >= levelRank('Group Leader');
}
...
function safeDesignerLayout(version,sides,elements,assets){
    const assetByStored=new Map(assets.map(asset=>[String(asset.StoredName),asset]));
    return { layoutVersionId:Number(version.id),versionNo:Number(version.VersionNo),widthMM:Number(version.WidthMM),heightMM:Number(version.HeightMM),dpi:Number(version.DPI),duplexFlip:String(version.DuplexFlip),backRotation:Number(version.BackRotation),sides:sides.map(side=>{const asset=assetByStored.get(String(side.BackgroundStoredName));return{side:String(side.Side),storageClass:String(side.StorageClass),backgroundAssetId:side.StorageClass==='DesignerAsset'&&asset?Number(asset.id):null,artworkVersionId:asset?.ArtworkVersionID==null?null:Number(asset.ArtworkVersionID),artworkRole:side.ArtworkRole||null,backgroundUrl:`/bbs/admin/card-designer/versions/${Number(version.id)}/sides/${String(side.Side).toLowerCase()}/background`,backgroundFit:String(side.BackgroundFit),backgroundPositionXBP:Number(side.BackgroundPositionXBP),backgroundPositionYBP:Number(side.BackgroundPositionYBP),bleedMM:Number(side.BleedMM),safeMarginMM:Number(side.SafeMarginMM)};}),elements:elements.map(element=>({elementKey:String(element.ElementKey),side:String(element.Side),elementType:String(element.ElementType),dataSourceKey:element.DataSourceKey||null,staticText:element.StaticText||null,assetId:element.AssetID==null?null:Number(element.AssetID),assetUrl:element.AssetID==null?null:`/bbs/admin/card-designer/assets/${Number(element.AssetID)}/file`,xBP:Number(element.XBP),yBP:Number(element.YBP),widthBP:Number(element.WidthBP),heightBP:Number(element.HeightBP),rotationDeg:Number(element.RotationDeg),zIndex:Number(element.ZIndex),visible:Boolean(element.Visible),style:JSON.parse(element.StyleJSON||'{}')})),assets:assets.map(asset=>({id:Number(asset.id),assetKey:String(asset.AssetKey),artworkVersionId:asset.ArtworkVersionID==null?null:Number(asset.ArtworkVersionID),originalName:String(asset.OriginalName),mimeType:String(asset.MimeType)}))};
}
...
async function personalDesignerRender(queryable,employee,template,card,rawToken,departmentQr,actor){
    ...
    const resolved=await resolveActiveArtwork(queryable,{kind:'Personal',departmentId:employee.DepartmentID,safetyUnitId:employee.SafetyUnitID});
    if(resolved.missing.length)throw new CardReadinessError(`${resolved.missing.join(' and ')} Card Artwork is not configured for this employee's current Department and Unit.`,'PERSONAL_CARD_ARTWORK_REQUIRED');
    const artworkBySide={Front:resolved.front,Back:resolved.back};
    layout.sides=layout.sides.map(side=>{const expectedRole=side.side==='Front'?ARTWORK_ROLES.SCOPED_FRONT:ARTWORK_ROLES.GLOBAL_BACK,artwork=artworkBySide[side.side];if(side.storageClass!=='CardArtwork'||side.artworkRole!==expectedRole||!artwork)throw new CardReadinessError(`The Active Personal layout does not contain a valid logical ${expectedRole} binding.`,'PERSONAL_CARD_ARTWORK_BINDING_REQUIRED');return{...side,artworkRole:expectedRole,artworkVersionId:Number(artwork.ArtworkVersionID),artworkSlotKey:String(artwork.SlotKey),backgroundUrl:`/bbs/admin/card-artwork/versions/${Number(artwork.ArtworkVersionID)}/file`,backgroundOriginalName:String(artwork.OriginalName),backgroundMimeType:String(artwork.MimeType),pixelWidth:artwork.PixelWidth==null?null:Number(artwork.PixelWidth),pixelHeight:artwork.PixelHeight==null?null:Number(artwork.PixelHeight)};});
...
router.get('/admin/card-templates', isAdmin, async (req, res) => {
    try { const deleted=String(req.query.trash||'0')==='1'?1:0;const [rows] = await db.query(`SELECT t.*,d.Name DepartmentName,u.name SafetyUnitName FROM BBS_Card_Templates t LEFT JOIN Master_Departments d ON d.id=t.DepartmentID LEFT JOIN Master_SafetyUnits u ON u.id=t.SafetyUnitID WHERE t.IsDeleted=? ORDER BY FIELD(t.Status,'Active','Draft','Archived'),t.UpdatedAt DESC,t.id DESC`,[deleted]); return res.json({ success:true, data:rows }); }
...
router.post('/admin/card-templates', isAdmin, templateUpload.single('template'), async (req, res) => {
    ...
    try {
        ...
        if (!validPersonalTemplateLevel(level)) return reject('Personal Card templates may use Group Leader level or higher, or All Levels.');
...
router.put('/admin/card-templates/:id', isAdmin, async (req,res) => {
    ...
    try { const id=positiveInt(req.params.id),version=positiveInt(req.body?.rowVersion); if(!id||!version)return res.status(400).json({success:false,message:'Valid template ID and RowVersion are required.'}); await connection.beginTransaction(); const [[row]]=await connection.query('SELECT * FROM BBS_Card_Templates WHERE id=? FOR UPDATE',[id]); if(!row){await connection.rollback();return res.status(404).json({success:false,message:'Template was not found.'});} if(Number(row.RowVersion)!==version){await connection.rollback();return res.status(409).json({success:false,code:'VERSION_CONFLICT',message:'Template was changed by another user.'});}
        ...
        if(action==='activate'){if(row.Status==='Archived'){await connection.rollback();return res.status(409).json({success:false,message:'Archived templates cannot be activated.'});}if(!validPersonalTemplateLevel(row.BBSLevel)){await connection.rollback();return res.status(409).json({success:false,code:'PERSONAL_TEMPLATE_LEVEL_INELIGIBLE',message:'Personal Card templates must target Group Leader level or higher, or All Levels.'});} await connection.query(`UPDATE BBS_Card_Templates SET Status='Archived',ArchivedAt=NOW(),ArchivedBy=?,UpdatedBy=?,RowVersion=RowVersion+1 WHERE Status='Active' AND id<>? AND DepartmentID <=> ? AND SafetyUnitID <=> ? AND BBSLevel <=> ?`,[actorId(req),actorId(req),id,row.DepartmentID,row.SafetyUnitID,row.BBSLevel]);await connection.query(`UPDATE BBS_Card_Templates SET Status='Active',ActivatedAt=NOW(),ActivatedBy=?,UpdatedBy=?,RowVersion=RowVersion+1 WHERE id=?`,[actorId(req),actorId(req),id]);}
...
router.post('/admin/card-templates/:id/repair-personal-level',isAdmin,async(req,res)=>{
    const connection=await db.getConnection();
    try{
        const id=positiveInt(req.params.id),rowVersion=positiveInt(req.body?.rowVersion),level=clean(req.body?.bbsLevel,40)||null;
        if(!id||!rowVersion||!level||!BBS_LEVELS.includes(level)||!validPersonalTemplateLevel(level))return res.status(400).json({success:false,message:'Choose a valid Personal Card level from Group Leader upward.'});
        await connection.beginTransaction();
        const[[row]]=await connection.query('SELECT * FROM BBS_Card_Templates WHERE id=? AND IsDeleted=0 FOR UPDATE',[id]);
        if(!row){await connection.rollback();return res.status(404).json({success:false,message:'Template was not found.'});}
        if(Number(row.RowVersion)!==rowVersion){await connection.rollback();return res.status(409).json({success:false,code:'VERSION_CONFLICT',message:'Template was changed. Reload and try again.'});}
        if(validPersonalTemplateLevel(row.BBSLevel)){await connection.rollback();return res.status(409).json({success:false,code:'PERSONAL_TEMPLATE_LEVEL_ALREADY_VALID',message:'This Template already has an eligible Personal Card level.'});}
        const[[used]]=await connection.query('SELECT COUNT(*) count FROM BBS_Cards WHERE TemplateID=?',[id]);
        if(Number(used?.count||0)>0){await connection.rollback();return res.status(409).json({success:false,code:'PERSONAL_TEMPLATE_LEVEL_REPAIR_BLOCKED',message:'This Template already has card history. Create a new compatible Template instead.'});}
        if(row.Status==='Active'){
            const[[conflict]]=await connection.query("SELECT id FROM BBS_Card_Templates WHERE id<>? AND IsDeleted=0 AND Status='Active' AND DepartmentID <=> ? AND SafetyUnitID <=> ? AND BBSLevel <=> ? LIMIT 1",[id,row.DepartmentID,row.SafetyUnitID,level]);
            if(conflict){await connection.rollback();return res.status(409).json({success:false,code:'PERSONAL_TEMPLATE_LEVEL_CONFLICT',message:'An Active Template already exists for this Department, Unit and BBS level.'});}
        }
        await connection.query('UPDATE BBS_Card_Templates SET BBSLevel=?,UpdatedBy=?,RowVersion=RowVersion+1 WHERE id=?',[level,actorId(req),id]);
        await connection.commit();
        await logAudit(req,{action:'BBS_CARD_TEMPLATE_LEVEL_REPAIR',module:'bbs',targetType:'BBS_Card_Template',targetId:id,detail:`Repaired unusable Personal Card scope from ${row.BBSLevel||'All Levels'} to ${level}; template, layout, artwork, files and lifecycle status were preserved.`,metadata:{beforeLevel:row.BBSLevel||null,afterLevel:level}});
        const[[updated]]=await db.query('SELECT * FROM BBS_Card_Templates WHERE id=?',[id]);
        return res.json({success:true,data:updated});
    }catch(error){await connection.rollback().catch(()=>{});return phase4Error(res,error,'template level repair');}finally{connection.release();}
});
...
router.post('/admin/cards/:id/replace', isAdmin, async (req,res) => {
    ...
    try{await connection.beginTransaction();const[[old]]=await connection.query("SELECT c.id CardID,c.EmployeeID,c.TemplateID,t.TemplateName,t.DepartmentID,t.SafetyUnitID,t.BBSLevel,t.WidthMM,t.HeightMM,t.IncludeEmployeeID,t.Status TemplateStatus FROM BBS_Cards c JOIN BBS_Card_Templates t ON t.id=c.TemplateID WHERE c.id=? AND c.Status='Active' FOR UPDATE",[id]);if(!old){await connection.rollback();return res.status(409).json({success:false,message:'Only an Active card can be replaced.'});}if(old.TemplateStatus!=='Active'){await connection.rollback();return res.status(409).json({success:false,message:'The card template is not Active.'});}const[employee]=await employeeCardData([old.EmployeeID],connection);if(!employee){await connection.rollback();return res.status(404).json({success:false,message:'Employee was not found.'});}if(levelRank(employee.BBSLevel)<levelRank('Group Leader')){await connection.rollback();return res.status(409).json({success:false,message:'The employee is no longer eligible for a personal BBS card.'});}const template={...old,id:old.TemplateID,Status:old.TemplateStatus};if(!templateMatches(template,employee)){await connection.rollback();return res.status(409).json({success:false,code:'CARD_TEMPLATE_SCOPE_CHANGED',message:'The employee no longer belongs to this template Department/Unit scope. Issue with a matching Active template instead.'});}await connection.query("UPDATE BBS_Cards SET Status='Replaced',RevokedAt=NOW(),RevokedBy=?,RevokeReason=? WHERE id=?",[actorId(req),reason,id]);const replacement=await issueWithin(connection,req,employee,template,reason);await connection.query('UPDATE BBS_Cards SET ReplacedByCardID=? WHERE id=?',[replacement.cardId,id]);await connection.commit();await logAudit(req,{action:'BBS_CARD_REPLACE',module:'bbs',targetType:'BBS_Card',targetId:id,detail:'Replaced BBS card and rotated QR token.',metadata:{replacementCardId:replacement.cardId,reason}});return res.status(201).json({success:true,data:replacement});}catch(error){await connection.rollback().catch(()=>{});return phase4Error(res,error,'card replace');}finally{connection.release();}
});
