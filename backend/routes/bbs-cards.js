...
router.post('/admin/card-templates', isAdmin, templateUpload.single('template'), async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.query(`INSERT INTO BBS_Card_Templates(TemplateName,DepartmentID,BBSLevel,BackgroundStoredName,OriginalName,MimeType,FileSize,WidthMM,HeightMM,IncludeEmployeeID,CreatedBy,UpdatedBy) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`, [name,departmentId,level,req.file.filename,clean(req.file.originalname),actualMime,req.file.size,width,height,String(req.body?.includeEmployeeId) === '0' ? 0 : 1,actorId(req),actorId(req)]);
