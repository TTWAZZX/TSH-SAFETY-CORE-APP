...
function canManageTrainingDept(req, department) {
...
function fourmEmailLine(label, value) {
    return `${label}: ${value || '-'}`;
}

function fourmEmailFooter() {
    return [
        '',
        '------------------------------------------------------------',
        'TSH Safety Core Activity System',
        '4M Change Management Module',
        'This is an automatic notification from the system. Please do not reply to this email.',
    ].join('\n');
}
...
async function queueFourMEmail({ to, noticeId, taskId, eventType, subject, body }) {
    ...
    const [insert] = await db.query(
        `INSERT INTO FourM_EmailOutbox
            (NoticeID, TaskID, EventType, Recipients, Subject, Body, Status)
         VALUES (?, ?, ?, ?, ?, ?, 'Queued')`,
        [noticeId || null, taskId || null, eventType || 'General', recipients.join(','), subject, body]
    ).catch(err => {
    ...
    setImmediate(async () => {
        try {
            await sendMail({ to: recipients.join(','), subject, text: body });
            await db.query(
...
function buildNoticeCreatedEmail(notice) {
    return {
        ...
        body: [
            'A new 4M Change Notice has been submitted.',
            '',
            fourmEmailLine('Notice No', notice.NoticeNo),
            fourmEmailLine('Title', notice.Title),
            fourmEmailLine('Change Type', notice.ChangeType),
            fourmEmailLine('Department', notice.Department),
            fourmEmailLine('Request Date', notice.RequestDate),
            fourmEmailLine('Created By', notice.CreatedBy),
            fourmEmailLine('Status', notice.Status || 'Open'),
            fourmEmailFooter(),
        ].join('\n'),
    };
}
...
function buildNoticeStatusEmail(notice, status) {
    return {
        ...
        body: [
            'Your 4M Change Notice status has been updated.',
            '',
            fourmEmailLine('Notice No', notice.NoticeNo),
            fourmEmailLine('Title', notice.Title),
            fourmEmailLine('Department', notice.Department),
            fourmEmailLine('Status', status),
            fourmEmailFooter(),
        ].join('\n'),
    };
}
...
function buildTaskEmail(notice, task, eventLabel) {
    return {
        ...
        body: [
            `4M Action Plan has been ${eventLabel.toLowerCase()}.`,
            '',
            fourmEmailLine('Notice No', notice.NoticeNo),
            fourmEmailLine('Notice Title', notice.Title),
            fourmEmailLine('Task', task.TaskTitle),
            fourmEmailLine('Owner', task.OwnerName),
            fourmEmailLine('Due Date', task.DueDate),
            fourmEmailLine('Status', task.Status),
            fourmEmailFooter(),
        ].join('\n'),
    };
}
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const [[trainingSummary]] = await db.query(`
            ...
            WHERE cur.IsActive = 1 AND cur.Year = ?
        `, [year]);
...
router.post('/training-curriculums', async (req, res) => {
    try {
        ...
        if (!canManageTrainingDept(req, Department)) return denyDept(res);
        const id = randomUUID();
        const actorName = getActorName(req);
        await db.query(
...
router.put('/training-curriculums/:id', async (req, res) => {
    try {
        ...
        if (!canManageTrainingDept(req, current.Department) || !canManageTrainingDept(req, nextDept)) return denyDept(res);
        const fields = []; const vals = [];
        const updates = [
...
router.delete('/training-curriculums/:id', async (req, res) => {
    try {
        ...
        const current = await getTrainingCurriculum(req.params.id);
        if (!current) return res.status(404).json({ success: false, message: 'ไม่พบหลักสูตร' });
        if (!canManageTrainingDept(req, current.Department)) return denyDept(res);
...
router.get('/training-curriculums/:id/courses', async (req, res) => {
    try {
        ...
        if (!canManageTrainingDept(req, curriculum.Department)) return denyDept(res);
        const includeInactive = req.query.includeInactive === '1';
        const [rows] = await db.query(
...
router.delete('/training-curriculum-assignments/:id', async (req, res) => {
    try {
        ...
        if (!canManageTrainingDept(req, current.CurriculumDepartment)) return denyDept(res);
        await db.query(
            `UPDATE FourM_CurriculumEmployees
...
router.post('/training-curriculum-assignments/:id/transfer', async (req, res) => {
    ...
    try {
        ...
        if (!canManageTrainingDept(req, target.Department)) return denyDept(res);
...
router.get('/training-courses/:id/assignments', async (req, res) => {
    try {
        ...
        if (!canManageTrainingDept(req, course.Department)) return denyDept(res);
        const status = cleanText(req.query.status, 20);
        const params = [req.params.id];
        let statusWhere = '';
        if (status && status !== 'all') {
...
router.post('/training-assignments/:id/transfer', async (req, res) => {
    ...
    try {
        ...
        if (!canManageTrainingDept(req, targetCourse.Department)) return denyDept(res);
...
router.post('/email-outbox/:id/retry', isAdmin, async (req, res) => {
    try {
        ...
        const item = rows[0];
        try {
            await sendMail({ to: item.Recipients, subject: item.Subject, text: item.Body });
            await db.query(
                `UPDATE FourM_EmailOutbox SET Status = 'Sent', SentAt = NOW(), Error = NULL WHERE id = ?`,
                [item.id]
            );
