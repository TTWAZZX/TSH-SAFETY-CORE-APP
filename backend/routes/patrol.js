        ...
        res.json({
            ...
            data: {
                ...
                checkin: {
                    ...
                    checkinAt: stats[0].LastCheckinAt || checkinAt,
...
async function buildPersonalMonthlySchedule(employeeId, year, month) {
    ...
    const [attendance] = await db.query(
        `SELECT id,PatrolDate,CheckinAt,PatrolType,Area,Notes,RecordedBy,ScheduledSessionID,(IdempotencyKey IS NOT NULL) AS IsV2Request
           FROM Patrol_Attendance
...
router.get('/member-attendance', async (req, res) => {
    ...
    try {
        const [rows] = await db.query(`
            SELECT id, PatrolDate, CheckinAt, PatrolType, Area, Notes
            FROM Patrol_Attendance
            WHERE UserID = ? AND YEAR(PatrolDate) = ?
...
router.post('/admin-record', isAdmin, async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.query(
            `INSERT INTO Patrol_Attendance (UserID, UserName, TeamName, WeekNumber, PatrolDate, CheckinAt, PatrolType, Area, Notes, RecordedBy, ScheduledSessionID)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [EmployeeID, emp.EmployeeName, emp.TeamName || '', week,
             dateStr, patrolBangkokDateTime(), PatrolType || 'normal', area, Notes || null, req.user.id, session?.id || null]
        );
        await logAudit(req, {
            module: 'patrol',
