...
(async()=>{
    ...
    const html=designerPrintDocument(cards,resources,()=>image,{paperSize:'A4',preset:'compact',backOffsetXMM:0,backOffsetYMM:0,autoPrint:false,title:'BBS Batch Duplex Calibration'});
    ...
    await waitFor(`document.querySelectorAll('.bbs-print-sheet').length===2 && [...document.images].every(image=>image.complete)`);
    ...
    const nativeRaster=await evaluate(`(async()=>{const cards=[...document.querySelectorAll('.designer-card[data-card-side]')],source=cards[0];cards.slice(1).forEach(card=>card.remove());const text=source.querySelector('.designer-text'),span=text.querySelector('span');span.textContent='ลักษิกา กะการดี';const cardRect=source.getBoundingClientRect(),textRect=text.getBoundingClientRect(),spanRect=span.getBoundingClientRect(),before={card:{width:cardRect.width,height:cardRect.height},text:{x:(textRect.left-cardRect.left)/cardRect.width,y:(textRect.top-cardRect.top)/cardRect.height,width:textRect.width/cardRect.width,height:textRect.height/cardRect.height,font:parseFloat(getComputedStyle(text).fontSize),baseline:(spanRect.top-textRect.top)/textRect.height}};let during=null;const originalClick=HTMLAnchorElement.prototype.click;HTMLAnchorElement.prototype.click=function(){};window.html2canvas=async(target,options)=>{const nativeCard=target.getBoundingClientRect(),nativeText=target.querySelector('.designer-text'),nativeTextRect=nativeText.getBoundingClientRect(),nativeSpanRect=nativeText.querySelector('span').getBoundingClientRect();during={contract:target.dataset.nativeExportContract,dpi:target.dataset.nativeExportDpi,options:{scale:options.scale,width:options.width,height:options.height},card:{width:nativeCard.width,height:nativeCard.height},text:{x:(nativeTextRect.left-nativeCard.left)/nativeCard.width,y:(nativeTextRect.top-nativeCard.top)/nativeCard.height,width:nativeTextRect.width/nativeCard.width,height:nativeTextRect.height/nativeCard.height,font:parseFloat(getComputedStyle(nativeText).fontSize),baseline:(nativeSpanRect.top-nativeTextRect.top)/nativeTextRect.height}};const canvas=document.createElement('canvas');canvas.width=options.width;canvas.height=options.height;return canvas;};try{const module=await import('/tsh-safety-core/public/js/utils/bbs-card-print.js?v=native-raster-browser-uat');await module.saveDesignerPrintImages(document,{filename:'BBS_Native_Raster_UAT',format:'png',dpi:600});}finally{HTMLAnchorElement.prototype.click=originalClick;delete window.html2canvas;}return{before,during,hostResidue:document.querySelectorAll('[data-native-designer-export]').length};})()`);
    assert.equal(nativeRaster.during.contract,'bbs-designer-native-raster-v1');assert.equal(nativeRaster.during.dpi,'600');assert.deepEqual(nativeRaster.during.options,{scale:1,width:1417,height:2008});assert.ok(Math.abs(nativeRaster.during.card.width-1417)<.01);assert.ok(Math.abs(nativeRaster.during.card.height-2008)<.01);assert.ok(Math.abs(nativeRaster.during.text.x-nativeRaster.before.text.x)<.001);assert.ok(Math.abs(nativeRaster.during.text.y-nativeRaster.before.text.y)<.001);assert.ok(Math.abs(nativeRaster.during.text.width-nativeRaster.before.text.width)<.001);assert.ok(Math.abs(nativeRaster.during.text.height-nativeRaster.before.text.height)<.001);assert.ok(Math.abs(nativeRaster.during.text.font/nativeRaster.before.text.font-6.25)<.02);assert.ok(Math.abs(nativeRaster.during.text.baseline-nativeRaster.before.text.baseline)<.02);assert.equal(nativeRaster.hostResidue,0);
    assert.deepEqual(browserErrors,[],`Browser errors: ${browserErrors.join(' | ')}`);
    const report={result:'PASS',paper:'A4',preset:'compact',cardsPerSide:9,pages:pageCount,duplexFlip:'LongEdge',backOffsetXMM:0,backOffsetYMM:0,pageCssPixels:measurements.page,cardCssPixels:measurements.card,nativeRaster,pdf:path.relative(root,pdfPath),screenshot:path.relative(root,screenshotPath),printerInstruction:'Actual size / 100%; two-sided; flip on long edge'};
