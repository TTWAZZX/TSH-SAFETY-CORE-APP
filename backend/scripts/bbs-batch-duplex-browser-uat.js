...
(async()=>{
    ...
    const domCapture=await evaluate(`(async()=>{const cards=[...document.querySelectorAll('.designer-card[data-card-side]')],source=cards[0];cards.slice(1).forEach(card=>card.remove());const text=source.querySelector('.designer-text'),span=text.querySelector('span');span.textContent='ลักษิกา กะการดี';const measure=()=>{const cardRect=source.getBoundingClientRect(),textRect=text.getBoundingClientRect(),spanRect=span.getBoundingClientRect();return{card:{width:cardRect.width,height:cardRect.height},text:{x:(textRect.left-cardRect.left)/cardRect.width,y:(textRect.top-cardRect.top)/cardRect.height,width:textRect.width/cardRect.width,height:textRect.height/cardRect.height,font:parseFloat(getComputedStyle(text).fontSize),baseline:(spanRect.top-textRect.top)/textRect.height},background:source.querySelector('.designer-background').style.backgroundImage};};const before=measure();let during=null;const originalClick=HTMLAnchorElement.prototype.click,originalRenderer=window.html2canvas;HTMLAnchorElement.prototype.click=function(){};window.html2canvas=async(target,options)=>{during={sameNode:target===source,options:{scale:options.scale,width:options.width,height:options.height},measurement:measure()};const canvas=document.createElement('canvas');canvas.width=Math.round(options.width*options.scale);canvas.height=Math.round(options.height*options.scale);return canvas;};let contract='';try{const module=await import('/tsh-safety-core/public/js/utils/bbs-card-print.js?v=dom-capture-browser-uat');contract=module.DESIGNER_RASTER_EXPORT_CONTRACT;await module.saveDesignerPrintImages(document,{filename:'BBS_DOM_Capture_UAT',format:'png',dpi:600});}finally{HTMLAnchorElement.prototype.click=originalClick;window.html2canvas=originalRenderer;}return{contract,before,during,hostResidue:document.querySelectorAll('[data-native-designer-export]').length};})()`);
    assert.equal(domCapture.contract,'bbs-designer-dom-capture-v3');assert.equal(domCapture.during.sameNode,true);assert.ok(Math.abs(domCapture.during.options.scale-6.25)<.02);assert.ok(Math.abs(domCapture.during.options.width-domCapture.before.card.width)<.01);assert.ok(Math.abs(domCapture.during.options.height-domCapture.before.card.height)<.01);assert.deepEqual(domCapture.during.measurement,domCapture.before);assert.equal(domCapture.hostResidue,0);
    const fidelity=await evaluate(`(async()=>{let blob=null;const click=HTMLAnchorElement.prototype.click,create=URL.createObjectURL;HTMLAnchorElement.prototype.click=function(){};URL.createObjectURL=value=>{blob=value;return create.call(URL,value);};try{const module=await import('/tsh-safety-core/public/js/utils/bbs-card-print.js?v=artwork-fidelity-browser-uat');await module.saveDesignerPrintImages(document,{filename:'BBS_Artwork_Fidelity_UAT',format:'png',dpi:600});return await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve({data:reader.result,size:blob.size});reader.onerror=reject;reader.readAsDataURL(blob);});}finally{HTMLAnchorElement.prototype.click=click;URL.createObjectURL=create;}})()`,120000),fidelityPath=path.join(artifactDir,'BBS_Artwork_Fidelity_600dpi.png');fs.writeFileSync(fidelityPath,Buffer.from(fidelity.data.split(',')[1],'base64'));
    assert.deepEqual(browserErrors,[],`Browser errors: ${browserErrors.join(' | ')}`);
    const report={result:'PASS',paper:'A4',preset:'compact',cardsPerSide:9,pages:pageCount,duplexFlip:'LongEdge',backOffsetXMM:0,backOffsetYMM:0,pageCssPixels:measurements.page,cardCssPixels:measurements.card,domCapture,artworkFidelity:{bytes:fidelity.size,path:path.relative(root,fidelityPath)},realExport:{width:realExport.width,height:realExport.height,path:path.relative(root,realExportPath)},pdf:path.relative(root,pdfPath),screenshot:path.relative(root,screenshotPath),printerInstruction:'Actual size / 100%; two-sided; flip on long edge'};
