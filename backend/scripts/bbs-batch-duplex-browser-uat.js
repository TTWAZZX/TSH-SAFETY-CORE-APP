...
(async()=>{
    ...
    assert.equal(domCapture.contract,'bbs-designer-dom-capture-v2');assert.equal(domCapture.during.sameNode,true);assert.ok(Math.abs(domCapture.during.options.scale-6.25)<.02);assert.ok(Math.abs(domCapture.during.options.width-domCapture.before.card.width)<.01);assert.ok(Math.abs(domCapture.during.options.height-domCapture.before.card.height)<.01);assert.deepEqual(domCapture.during.measurement,domCapture.before);assert.equal(domCapture.hostResidue,0);
