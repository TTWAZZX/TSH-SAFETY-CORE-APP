...
(async()=>{
    ...
    const consume=main.slice(main.indexOf('async function consumeBbsQrIntent'),main.indexOf('function consumePendingGuideRoute'));
    assert.match(consume,/BBS_QR_PENDING_KEY/);
    assert.match(consume,/isBbsRolloutBlockedError/);
    assert.match(consume,/normalizeBbsQrRoute\(result\.data\?\.route\)/);
