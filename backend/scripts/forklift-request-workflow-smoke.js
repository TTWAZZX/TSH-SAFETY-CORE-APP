...
const checks = [
    ...
    ['Renewal draft retry reuses existing request', node.includes('RENEWAL_DRAFT_REUSED') && php.includes('RENEWAL_DRAFT_REUSED') && node.includes('REUSE_RENEWAL_REQUEST_DRAFT') && php.includes('REUSE_RENEWAL_REQUEST_DRAFT')],
    ['Renewal in-review conflict returns existing request identity', node.includes('RENEWAL_REQUEST_ALREADY_OPEN') && php.includes('RENEWAL_REQUEST_ALREADY_OPEN') && node.includes('RequestNo: existing.RequestNo') && php.includes("'RequestNo'=>$existing['RequestNo']")],
    ['Renewal source filter parity', node.includes('req.query.sourceLicenseId') && php.includes("$_GET['sourceLicenseId']") && node.includes('r.SourceLicenseID=?') && php.includes("'r.SourceLicenseID=?'" )],
    ['Renewal retry serialized by source license lock', node.includes('SELECT ID FROM forklift_licenses WHERE ID=? FOR UPDATE') && php.includes('SELECT ID FROM forklift_licenses WHERE ID=? FOR UPDATE')],
    ...
    ['Renewal UI resumes and focuses existing requests', ui.includes('findOpenRenewalRequest') && ui.includes('focusExistingRenewalRequest') && ui.includes('ดำเนินการต่อจากคำขอ')],
    ['Renewal UI preserves existing required documents', ui.includes('existingDocuments') && ui.includes('มีไฟล์เดิมแล้ว เลือกใหม่เมื่อต้องการแทนที่')],
