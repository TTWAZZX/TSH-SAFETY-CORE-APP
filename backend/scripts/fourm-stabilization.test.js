...
assert.match(activeFrontend, /aria-pressed="\$\{selected \? 'true' : 'false'\}"/);
assert.match(activeFrontend, /คลิกเพื่อเปิดรายละเอียด/);
assert.match(frontend, /const selectedCurriculum = _tmCurriculums\.find\(c => String\(c\.id\) === String\(tmCurriculum\.dataset\.id\)\)/);
assert.match(activeFrontend, /const requestId = \+\+_tmCurriculumRequestId/);
assert.match(activeFrontend, /requestId !== _tmCurriculumRequestId/);
assert.match(activeFrontend, /fetchTrainingCourseMaster\(\)/);
