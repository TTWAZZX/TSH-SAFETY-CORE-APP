...
assert.match(phpRoute, /\$maximum\s*=\s*1024\s*\*\s*1024/, 'PHP adaptive chunk ceiling must be 1 MiB');
assert.match(nodeRoute, /KY_VIDEO_CHUNK_SIZE\s*=\s*1\s*\*\s*1024\s*\*\s*1024/, 'Node chunk ceiling must be 1 MiB');
...
assert.match(main, /ky\.js\?v=20260918-ky-video-adaptive-r1/, 'KY page cache key must serve adaptive chunk upload');
assert.match(html, /main\.js\?v=20260918-ky-video-adaptive-r1/, 'SPA cache key must serve adaptive chunk upload');
