...
function smtpConfigured() {
    return emailEnabled() && Boolean(process.env.SMTP_HOST);
}

function emailEnabled() {
    const configured = String(process.env.EMAIL_ENABLED || '').trim().toLowerCase();
...
function smtpFromName() {
    return process.env.SMTP_FROM_NAME || 'TSH Safety Core';
}
...
function stripAddress(value) {
    ...
    return (angle ? angle[1] : text).replace(/[<>]/g, '').trim();
}

function addressHeader(email, name = '') {
    const address = stripAddress(email);
    const cleanName = String(name || '').replace(/[\r\n<>]/g, ' ').trim();
    if (!cleanName) return `<${address}>`;
    if (/^[\x20-\x7E]*$/.test(cleanName)) {
        return `"${cleanName.replace(/["\\]/g, '\\$&')}" <${address}>`;
    }
    return `${encodeHeader(cleanName)} <${address}>`;
}

function base64Lines(value) {
    return Buffer.from(normalizeCrlf(value), 'utf8')
        .toString('base64')
        .replace(/.{1,76}/g, '$&\r\n')
        .trim();
}
...
function smtpEhloDomain() {
    const value = String(process.env.SMTP_EHLO_DOMAIN || 'localhost').trim();
...
function buildMessage({ from, to, subject, text, html }) {
    ...
    const safeFrom = addressHeader(from, smtpFromName());
    ...
    return [
        ...
        hasHtml
            ...
            : 'Content-Transfer-Encoding: base64',
        '',
        hasHtml ? multipartBody : base64Lines(text),
...
async function sendMail({ to, subject, text, html }) {
    if (!smtpConfigured()) {
        return {
            skipped: true,
            reason: emailEnabled() ? 'SMTP_HOST is not configured' : 'Email delivery is disabled',
        };
    }
    ...
    try {
        ...
        await client.send(`EHLO ${smtpEhloDomain()}`, 250);
        ...
        socket.write(`${dotStuff(buildMessage({ from, to: recipients.join(','), subject, text, html }))}\r\n.\r\n`);
