...
function buildPlainText({ title, greeting, intro, details, actions, note, footerNote, appUrl }) {
    ...
    lines.push(
        ...
        'Hiyari-Hatto / Near-Miss Reporting Module',
...
function buildHiyariEmail({ title, kicker, tone = 'neutral', greeting, intro, details, actions, note, footerNote, appUrl }) {
    const style = TONE[tone] || TONE.neutral;
    const rows = detailRows(details);
    const actionLines = normalizeLines(actions);
    const noteLines = normalizeLines(note);
    const loginUrl = String(appUrl || DEFAULT_APP_URL).trim();
    const text = buildPlainText({ title, greeting, intro, details: rows, actions: actionLines, note: noteLines, footerNote, appUrl: loginUrl });
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:720px;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e2e8f0;box-shadow:0 18px 45px rgba(15,23,42,.08);">
          ...
          <tr>
            <td style="padding:18px 28px;background:#0f172a;">
              ...
              <div style="color:#94a3b8;font-size:12px;margin-top:4px;">Hiyari-Hatto / Near-Miss Reporting Module</div>
