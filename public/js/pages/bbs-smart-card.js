...
import { designerPrintDocument, renderDesignerElement, saveDesignerPrintPdf, saveDesignerPrintImages } from '../utils/bbs-card-print.js?v=20260916-bbs-exact-image-export-r1';
...
function installPersonalIssueCompatibility(){
    ...
    const sync=()=>{
        const employees=[...document.querySelectorAll('[data-card-employee]:checked')].map(input=>state.cardEmployees.find(row=>String(row.EmployeeID)===String(input.dataset.cardEmployee))).filter(Boolean);
        let compatible=0;
        ...
        if(selectedTemplate&&select.selectedOptions[0]?.disabled)select.value='';
    ...
    document.querySelectorAll('[data-card-employee]').forEach(input=>input.onchange=sync);
    sync();
}
...
function designerCompositeCardPreview(kind,template,designer,assessment,{employee=null,departmentContext=null}={}){
    ...
    const sideHtml=side=>{
        const background=designer.resources.get(side.backgroundUrl)||'',fit=side.backgroundFit==='Stretch'?'100% 100%':String(side.backgroundFit||'Cover').toLowerCase(),rotation=side.side==='Back'?n(layout.backRotation):0;
        const safeX=Math.max(0,n(side.safeMarginMM,3))/assessment.width*100,safeY=Math.max(0,n(side.safeMarginMM,3))/assessment.height*100;
        const elements=(layout.elements||[]).filter(element=>element.side===side.side).map(element=>renderDesignerElement(element,values,designer.resources,qrDataUrl,{pixelsPerMM})).join('');
        return`<div><div class="mb-2 text-center text-[10px] font-black uppercase tracking-widest text-slate-500">${side.side==='Front'?(kind==='personal'?'ด้านหน้า · Personal QR':'ด้านหน้า'):'ด้านหลัง · Department QR'}</div><div class="relative mx-auto max-w-full" style="width:${displayWidth}px;aspect-ratio:${assessment.width}/${assessment.height}"><article class="bbs-card-cut-line absolute inset-0 overflow-hidden bg-white shadow-2xl" data-designer-preview-side="${side.side}" style="transform:rotate(${rotation}deg)"><div class="absolute" style="inset:0;background-image:url('${background}');background-size:${fit};background-position:${n(side.backgroundPositionXBP,5000)/100}% ${n(side.backgroundPositionYBP,5000)/100}%"></div><div class="absolute inset-0 overflow-hidden">${elements}</div><div class="bbs-card-safe-area absolute pointer-events-none" style="left:${safeX}%;right:${safeX}%;top:${safeY}%;bottom:${safeY}%"></div></article></div></div>`;
    };
