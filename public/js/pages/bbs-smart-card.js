...
import { openBbsCardDesigner } from './bbs-card-designer.js?v=20260911-bbs-master-rebase-r1';
...
function masterArtworkView(kindFilter='') {
    const data=state.masterArtwork||{},slots=data.slots||{},kinds=kindFilter?[kindFilter]:['Personal','Department'];
    ...
    const cards=kinds.flatMap(kind=>['Front','Back'].map(side=>{const row=slots?.[kind]?.[side],tone=kind==='Personal'?'sky':'emerald',label=side==='Front'?'ด้านหน้า':'ด้านหลัง';return `<form data-master-artwork-upload data-kind="${kind}" data-side="${side}" class="rounded-2xl border border-${tone}-200 bg-white p-4"><div class="flex items-start justify-between gap-3"><div><span class="text-[10px] font-black tracking-widest text-${tone}-700">${kind.toUpperCase()} · ${side.toUpperCase()}</span><h4 class="mt-1 font-black text-slate-800">ภาพมาสเตอร์${label}</h4></div><span class="rounded-full px-2 py-1 text-[10px] font-black ${row?'bg-emerald-100 text-emerald-700':'bg-amber-100 text-amber-700'}">${row?`ACTIVE V${n(row.versionNo)}`:'ยังไม่ตั้งค่า'}</span></div>${row?`<div class="mt-3 rounded-xl bg-slate-50 p-3"><div class="truncate text-xs font-bold text-slate-700">${escHtml(row.originalName)}</div><div class="mt-1 text-[10px] text-slate-500">${Math.max(1,Math.round(n(row.fileSize)/1024))} KB${row.pixelWidth&&row.pixelHeight?` · ${n(row.pixelWidth)}×${n(row.pixelHeight)} px`:''}</div><button type="button" data-master-artwork-preview="${n(row.id)}" class="mt-2 min-h-11 rounded-lg border border-slate-300 px-3 text-xs font-black text-slate-700">เปิดดูภาพปัจจุบัน</button></div>`:'<p class="mt-3 text-xs text-amber-700">ต้องอัปโหลดช่องนี้ก่อนสร้าง Designer Draft ใหม่</p>'}<label class="mt-3 block text-[11px] font-bold text-slate-600">${row?'อัปโหลดเวอร์ชันใหม่':'เลือกไฟล์มาสเตอร์'}<input name="artwork" required type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 block w-full text-xs text-slate-500"></label><button class="mt-3 min-h-11 w-full rounded-xl bg-${tone}-600 px-3 text-sm font-black text-white">${row?'แทนที่สำหรับ Draft ใหม่':'ตั้งเป็น Master Artwork'}</button></form>`;})).join('');
    const history=(data.records||[]).filter(row=>!kindFilter||row.templateKind===kindFilter).filter(row=>row.status==='Archived').slice(0,8);
    return `<section data-master-artwork-manager class="rounded-2xl border border-violet-200 bg-violet-50/30 p-5"><div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div class="text-[10px] font-black tracking-widest text-violet-700">MASTER ARTWORK · STRICT ISOLATION</div><h3 class="mt-1 font-black text-slate-800">พื้นหลังกลางแยก Personal / Department และ Front / Back</h3><p class="mt-1 max-w-3xl text-xs text-slate-600">Draft ใหม่จะ snapshot ภาพ Active ทั้งสองด้านของประเภทนั้นเท่านั้น การเปลี่ยนภาพมาสเตอร์ไม่แก้ Draft, Active หรือ Archived เดิมย้อนหลัง</p></div><span class="rounded-full bg-white px-3 py-2 text-xs font-black text-violet-700">${kinds.reduce((sum,kind)=>sum+['Front','Back'].filter(side=>slots?.[kind]?.[side]).length,0)}/${kinds.length*2} ช่องพร้อม</span></div><div class="mt-4 grid grid-cols-1 md:grid-cols-2 ${kindFilter?'':'2xl:grid-cols-4'} gap-3">${cards}</div>${history.length?`<details class="mt-4 rounded-xl border border-violet-100 bg-white p-3"><summary class="cursor-pointer text-xs font-black text-slate-700">ประวัติเวอร์ชันที่ถูกแทนที่ (${history.length})</summary><div class="mt-3 grid gap-2 sm:grid-cols-2">${history.map(row=>`<div class="rounded-lg bg-slate-50 p-2 text-[11px] text-slate-600"><strong>${escHtml(row.templateKind)} ${escHtml(row.side)} V${n(row.versionNo)}</strong> · ${escHtml(row.originalName)}</div>`).join('')}</div></details>`:''}</section>`;
}
      ...
      <div class="grid grid-cols-1 xl:grid-cols-[360px_minmax(0,1fr)] min-w-0">
        ...
        <div data-department-config-detail="${selected.id}" class="min-w-0">
          ...
          <div class="grid grid-cols-1 2xl:grid-cols-2 gap-5 p-5">
            <form id="bbs-dept-template-form" class="rounded-2xl border border-slate-200 p-4 space-y-3"><div><h4 class="font-black text-slate-800">1. เพิ่ม Template</h4><p class="mt-1 text-[11px] text-slate-500">Template นี้จะผูกกับ ${escHtml(selected.name)} ส่วน Designer Draft ใช้ Department Master Artwork เท่านั้น</p></div><input type="hidden" name="departmentId" value="${selected.id}"><label class="block text-[11px] font-bold text-slate-600">ชื่อ Template<input name="templateName" required maxlength="160" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm" placeholder="เช่น BBS Department Card"></label>${cardSizeEditor({includeDisplayOrder:true})}<label class="block text-[11px] font-bold text-slate-600">ภาพสำรองสำหรับ Legacy Renderer<input name="template" required type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 w-full text-xs"></label><button class="w-full rounded-xl bg-emerald-600 px-3 py-2.5 text-sm font-black text-white">อัปโหลดเป็น Draft</button></form>
...
function personalCardsView() {
    ...
    return `<div class="space-y-5">
      ...
      <section class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <form id="bbs-template-form" class="rounded-2xl border border-slate-200 bg-white p-5 space-y-3"><div><h3 class="font-black text-slate-800">Personal Card Template</h3><p class="text-xs text-slate-500 mt-1">ตั้งค่าขอบเขตและขนาด · Designer Draft ใช้ Personal Master Artwork แยกจาก Department โดยเด็ดขาด</p></div><label class="block text-[11px] font-bold text-slate-600">ชื่อ Template<input name="templateName" required maxlength="160" placeholder="เช่น BBS Personal Card" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"></label><div class="grid grid-cols-1 sm:grid-cols-2 gap-2"><select name="departmentId" data-master-source="departments" aria-label="ขอบเขตแผนกของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">ทุกแผนก</option>${masterDepartmentOptions()}</select><select name="bbsLevel" data-master-source="bbs-levels" aria-label="ระดับ BBS ของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">ทุกระดับ</option>${masterBbsLevelOptions()}</select></div>${cardSizeEditor()}<label class="flex items-center gap-2 text-xs font-bold text-slate-600"><input name="includeEmployeeId" type="checkbox" checked> แสดง EmployeeID บนบัตร</label><label class="block text-[11px] font-bold text-slate-600">ภาพสำรองสำหรับ Legacy Renderer<input name="template" aria-label="ไฟล์สำรอง Card Template" required type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 block w-full text-xs text-slate-500"></label><button class="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white">อัปโหลดเป็น Draft</button></form>
...
function bind() {
    ...
    document.querySelectorAll('[data-master-artwork-upload]').forEach(form=>bindBusy(form,'onsubmit',uploadMasterArtwork,'กำลังอัปโหลด Master Artwork...'));
