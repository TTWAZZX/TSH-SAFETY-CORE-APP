...
function personalCardsView() {
    ...
    return `<div class="space-y-5">
      ...
      <section class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <form id="bbs-template-form" data-template-scope-form data-template-kind="Personal" class="rounded-2xl border border-slate-200 bg-white p-5 space-y-3"><div><h3 class="font-black text-slate-800">Personal Card Template</h3><p class="text-xs text-slate-500 mt-1">ระบบเลือก Scoped Front ตามแผนก/Unit และใช้ Global Back ร่วมกัน ไม่ต้องอัปโหลดภาพซ้ำ</p></div><label class="block text-[11px] font-bold text-slate-600">ชื่อ Template<input name="templateName" required maxlength="160" placeholder="เช่น BBS Personal Card" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"></label><div class="grid grid-cols-1 sm:grid-cols-3 gap-2"><select name="departmentId" required data-master-source="departments" aria-label="ขอบเขตแผนกของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">เลือกแผนก</option>${masterDepartmentOptions()}</select><select name="safetyUnitId" aria-label="ขอบเขต Unit ของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm" disabled><option value="">ค่าเริ่มต้นทั้งแผนก</option></select><select name="bbsLevel" data-master-source="bbs-levels" aria-label="ระดับ BBS ของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">ทุกระดับ</option>${masterBbsLevelOptions()}</select></div><div data-template-artwork-match role="status" aria-live="polite"></div>${cardSizeEditor()}<label class="flex items-center gap-2 text-xs font-bold text-slate-600"><input name="includeEmployeeId" type="checkbox" checked> แสดง EmployeeID บนบัตร</label><label data-legacy-fallback-field class="hidden block text-[11px] font-bold text-slate-600">Legacy fallback ชั่วคราว (ใช้เฉพาะเมื่อยังไม่มี Personal Scoped Front)<input name="template" aria-label="ไฟล์สำรอง Card Template" type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 block w-full text-xs text-slate-500"></label><button class="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white">สร้าง Personal Template Draft</button></form>
...
function templateRow(row) { return `<div class="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div class="min-w-0"><div class="font-bold text-slate-800 truncate">${escHtml(row.TemplateName)}</div><div class="text-xs text-slate-500 mt-1">${escHtml(row.DepartmentName || 'ยังไม่กำหนดแผนก')} · ${escHtml(row.SafetyUnitName||'ค่าเริ่มต้นทั้งแผนก')} · ${escHtml(row.BBSLevel || 'ทุกระดับ')} · ${n(row.WidthMM).toFixed(2)}×${n(row.HeightMM).toFixed(2)} mm · ${escHtml(row.Status)}</div></div><div class="flex flex-wrap gap-2"><button data-card-designer-personal="${row.id}" class="rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-xs font-bold text-violet-700">Visual Designer</button><button data-bbs-template-preview="${row.id}" class="rounded-lg border px-3 py-2 text-xs font-bold text-sky-700">Preview + Readiness</button>${row.Status === 'Draft' ? `<button data-bbs-template-action="activate" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Activate</button>` : ''}${row.Status !== 'Archived' ? `<button data-bbs-template-action="archive" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600">Archive</button>` : ''}</div></div>`; }
