    ...
    return `<section data-department-configuration class="rounded-2xl border border-emerald-200 bg-white overflow-hidden">
      ...
      <div class="grid grid-cols-1 xl:grid-cols-[360px_minmax(0,1fr)] min-w-0">
        ...
        <div data-department-config-detail="${selected.id}" class="min-w-0">
          ...
          <div class="border-t border-slate-100"><div class="p-5 flex items-center justify-between gap-3"><div><h4 class="font-black text-slate-800">3. Template ของแผนกนี้</h4><p class="mt-1 text-[11px] text-slate-500">Composite Preview จะตรวจรูป ขนาด QR และพื้นที่พิมพ์ก่อน Activate</p></div><span class="text-xs font-bold text-slate-500">${selected.templates.length} รายการ</span></div><div class="divide-y">${selected.templates.map(row=>`<div class="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><div class="font-bold text-slate-800">${escHtml(row.TemplateName)}</div><div class="mt-1 text-xs text-slate-500">${escHtml(row.Status)} · ${n(row.WidthMM)}×${n(row.HeightMM)} mm · ลำดับ ${n(row.DisplayOrder)}</div></div><div class="flex flex-wrap gap-2"><button data-dept-template-preview="${row.id}" class="rounded-lg border px-3 py-2 text-xs font-bold text-sky-700">Preview + Readiness</button>${row.Status==='Draft'?`<button data-dept-template-action="activate" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Activate</button>`:''}${row.Status!=='Archived'?`<button data-dept-template-action="archive" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600">Archive</button>`:''}</div></div>`).join('')||'<div class="p-8 text-center text-sm text-slate-400">ยังไม่มี Template สำหรับแผนกนี้</div>'}</div></div>
        </div>
      </div>
    </section>`;
}
...
function templateRow(row) { return `<div class="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div class="min-w-0"><div class="font-bold text-slate-800 truncate">${escHtml(row.TemplateName)}</div><div class="text-xs text-slate-500 mt-1">${escHtml(row.DepartmentName || 'ทุกแผนก')} · ${escHtml(row.BBSLevel || 'ทุกระดับ')} · ${n(row.WidthMM).toFixed(2)}×${n(row.HeightMM).toFixed(2)} mm · ${escHtml(row.Status)}</div></div><div class="flex gap-2"><button data-bbs-template-preview="${row.id}" class="rounded-lg border px-3 py-2 text-xs font-bold text-sky-700">Preview + Readiness</button>${row.Status === 'Draft' ? `<button data-bbs-template-action="activate" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Activate</button>` : ''}${row.Status !== 'Archived' ? `<button data-bbs-template-action="archive" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600">Archive</button>` : ''}</div></div>`; }
