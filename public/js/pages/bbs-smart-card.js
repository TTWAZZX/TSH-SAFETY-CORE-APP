...
import { designerPrintDocument, designerCardFaceHtml, designerSheetMetrics, saveDesignerPrintPdf, saveDesignerPrintImages } from '../utils/bbs-card-print.js?v=20260921-bbs-artwork-fidelity-r3';
import { openBbsCardDesigner } from './bbs-card-designer.js?v=20260921-bbs-artwork-fidelity-r3';
...
state.personalReprint={selected:new Map(),paperSize:'A4',preset:'compact',backOffsetXMM:0,backOffsetYMM:0};
state.departmentPrintFilters={q:'',departmentId:'',page:1,pageSize:20};
...
let persistenceScrollTimer = null;
const BBS_UI_STATE_VERSION = 1;
const BBS_TABS = new Set(['workspace','start','community','team-management','actions','analytics','history','cards']);
const CARD_WORKSPACES = new Set(['overview','personal','department']);

function bbsUiStorageKey() {
    const employeeId = String(state.context?.employee?.EmployeeID || state.context?.employee?.id || 'unknown').replace(/[^A-Za-z0-9_-]/g, '').slice(0, 40);
    return `tsh_bbs_ui_v${BBS_UI_STATE_VERSION}_${employeeId || 'unknown'}`;
}

function resetBbsUiState() {
    state.tab='workspace';state.view='observer';state.cardWorkspace='overview';state.departmentConfigQuery='';state.departmentConfigStatus='all';state.departmentConfigSelectedId=null;
    state.historyYear=yearNow;state.actionScope='all';state.actionStatus='';state.actionPriority='';state.inspectorScheduleMode='agenda';state.inspectorScheduleFilters={year:yearNow,month:new Date().getMonth()+1};
    state.analyticsFilters={scope:'',year:yearNow,month:0,departmentId:'',safetyUnitId:'',risk:''};
    state.historyFilters={...state.historyFilters,status:'',departmentId:'',safetyUnitId:'',q:'',page:1};
    state.actionFilters={...state.actionFilters,year:'',departmentId:'',safetyUnitId:'',q:'',page:1};
    state.actionOutboxFilters={...state.actionOutboxFilters,status:'',eventType:'',year:'',q:'',page:1};
    state.communityFilters={...state.communityFilters,year:yearNow,month:0,departmentId:'',safetyUnitId:'',q:'',actionStatus:'',priority:'',goodPage:1,riskyPage:1};
    state.cardFilters={...state.cardFilters,q:'',status:'Active',departmentId:'',page:1};
    state.cardEmployeeFilters={...state.cardEmployeeFilters,q:'',departmentId:'',page:1};
    state.departmentPrintFilters={...state.departmentPrintFilters,q:'',departmentId:'',page:1};
    state.restoredScrollTop=0;
}

function safeStoredObject(value) { return value && typeof value === 'object' && !Array.isArray(value) ? value : {}; }
function restoreBbsUiState() {
    resetBbsUiState();
    try {
        const saved=JSON.parse(sessionStorage.getItem(bbsUiStorageKey())||'null');
        if(!saved||n(saved.version)!==BBS_UI_STATE_VERSION)return;
        if(BBS_TABS.has(saved.tab))state.tab=saved.tab;
        if(CARD_WORKSPACES.has(saved.cardWorkspace))state.cardWorkspace=saved.cardWorkspace;
        if(['observer','observed','team'].includes(saved.view))state.view=saved.view;
        if(['all','mine','overdue','verify'].includes(saved.actionScope))state.actionScope=saved.actionScope;
        if(['','Open','In Progress','Pending Verification','Closed','Reopened'].includes(saved.actionStatus))state.actionStatus=saved.actionStatus;
        if(['','Critical','High','Medium','Low'].includes(saved.actionPriority))state.actionPriority=saved.actionPriority;
        state.historyYear=Math.max(yearNow-10,Math.min(yearNow+1,n(saved.historyYear)||yearNow));
        if(['agenda','calendar'].includes(saved.inspectorScheduleMode))state.inspectorScheduleMode=saved.inspectorScheduleMode;
        const copyAllowed=(target,source,keys)=>keys.forEach(key=>{if(source[key]!==undefined)target[key]=source[key];});
        copyAllowed(state.analyticsFilters,safeStoredObject(saved.analyticsFilters),['scope','year','month','departmentId','safetyUnitId','risk']);
        copyAllowed(state.historyFilters,safeStoredObject(saved.historyFilters),['status','departmentId','safetyUnitId','page','pageSize']);
        copyAllowed(state.actionFilters,safeStoredObject(saved.actionFilters),['year','departmentId','safetyUnitId','page','pageSize']);
        copyAllowed(state.actionOutboxFilters,safeStoredObject(saved.actionOutboxFilters),['status','eventType','year','page','pageSize']);
        copyAllowed(state.communityFilters,safeStoredObject(saved.communityFilters),['year','month','departmentId','safetyUnitId','actionStatus','priority','goodPage','riskyPage','pageSize']);
        copyAllowed(state.cardFilters,safeStoredObject(saved.cardFilters),['status','departmentId','page','pageSize']);
        copyAllowed(state.cardEmployeeFilters,safeStoredObject(saved.cardEmployeeFilters),['departmentId','page','pageSize']);
        copyAllowed(state.departmentPrintFilters,safeStoredObject(saved.departmentPrintFilters),['departmentId','page','pageSize']);
        copyAllowed(state.inspectorScheduleFilters,safeStoredObject(saved.inspectorScheduleFilters),['year','month']);
        state.departmentConfigStatus=['all','ready','incomplete'].includes(saved.departmentConfigStatus)?saved.departmentConfigStatus:'all';
        state.departmentConfigSelectedId=n(saved.departmentConfigSelectedId)||null;
        state.restoredScrollTop=Math.max(0,n(saved.scrollTop));
    } catch (_) {}
}

function persistBbsUiState(scrollTopOverride = null) {
    if(!state.context)return;
    const container=document.getElementById('main-content');
    const withoutSearch=(value)=>{const copy={...value};delete copy.q;return copy;};
    const saved={version:BBS_UI_STATE_VERSION,tab:state.tab,cardWorkspace:state.cardWorkspace,view:state.view,historyYear:state.historyYear,actionScope:state.actionScope,actionStatus:state.actionStatus,actionPriority:state.actionPriority,departmentConfigStatus:state.departmentConfigStatus,departmentConfigSelectedId:state.departmentConfigSelectedId,inspectorScheduleMode:state.inspectorScheduleMode,inspectorScheduleFilters:state.inspectorScheduleFilters,analyticsFilters:state.analyticsFilters,historyFilters:withoutSearch(state.historyFilters),actionFilters:withoutSearch(state.actionFilters),actionOutboxFilters:withoutSearch(state.actionOutboxFilters),communityFilters:withoutSearch(state.communityFilters),cardFilters:withoutSearch(state.cardFilters),cardEmployeeFilters:withoutSearch(state.cardEmployeeFilters),departmentPrintFilters:withoutSearch(state.departmentPrintFilters),scrollTop:scrollTopOverride===null?n(container?.scrollTop):n(scrollTopOverride),savedAt:new Date().toISOString()};
    try{sessionStorage.setItem(bbsUiStorageKey(),JSON.stringify(saved));}catch(_){}
}

function ensureBbsPersistenceListeners() {
    if(persistenceListenersBound)return;
    persistenceListenersBound=true;
    window.addEventListener('pagehide',()=>persistBbsUiState());
    document.getElementById('main-content')?.addEventListener('scroll',()=>{
        if(!document.getElementById('bbs-smart-card-page')||document.getElementById('bbs-smart-card-page').classList.contains('hidden'))return;
        clearTimeout(persistenceScrollTimer);
        persistenceScrollTimer=setTimeout(persistBbsUiState,150);
    },{passive:true});
}

function sanitizeBbsRestoredTab() {
    const allowed=new Set(bbsNavigationGroups().flatMap(group=>group.tabs.map(([key])=>key)));
    if(!allowed.has(state.tab))state.tab='workspace';
...
function showPersonalQrVerification(verification){
    if(!verification||verification.kind!=='Personal'||verification.active!==true)return;
    const returnFocus=document.activeElement,modal=document.createElement('div');
    const issued=verification.issuedAt?new Date(verification.issuedAt):null,issuedLabel=issued&&!Number.isNaN(issued.getTime())?issued.toLocaleString('th-TH',{dateStyle:'medium',timeStyle:'short'}):'-';
    const detail=(label,value)=>`<div class="rounded-xl border border-emerald-100 bg-white p-3"><div class="text-[10px] font-black uppercase tracking-wide text-slate-400">${escHtml(label)}</div><div class="mt-1 text-sm font-black text-slate-800">${escHtml(value||'-')}</div></div>`;
    modal.className='fixed inset-0 z-[10020] flex items-center justify-center bg-slate-950/70 p-4';
    modal.innerHTML=`<section class="w-full max-w-xl overflow-hidden rounded-3xl bg-slate-50 shadow-2xl"><header class="border-b border-emerald-100 bg-emerald-700 p-5 text-white"><div class="flex items-start justify-between gap-4"><div><p class="text-[10px] font-black tracking-[.18em] text-emerald-100">PERSONAL CARD VERIFICATION</p><h2 class="mt-1 text-xl font-black">ตรวจสอบบัตรสำเร็จ</h2><p class="mt-1 text-xs text-emerald-100">QR นี้เชื่อมกับบัตร Personal Card ที่ Active ในระบบ</p></div><div class="rounded-full bg-white px-3 py-1 text-xs font-black text-emerald-700">ACTIVE</div></div></header><div class="space-y-4 p-5"><div class="grid grid-cols-1 gap-3 sm:grid-cols-2">${detail('ชื่อพนักงาน',verification.employeeName)}${detail('รหัสพนักงาน',verification.employeeId)}${detail('แผนก / Unit',[verification.department,verification.unit].filter(Boolean).join(' / '))}${detail('ตำแหน่ง / ระดับ',[verification.position,verification.bbsLevel].filter(Boolean).join(' / '))}${detail('Template',verification.templateName)}${detail('วันที่ออกบัตร',issuedLabel)}</div><div class="rounded-xl border border-sky-200 bg-sky-50 p-3 text-xs font-bold text-sky-800">ข้อมูลนี้ตรวจจากสถานะบัตรและเจ้าของบัตรบนเซิร์ฟเวอร์ ไม่ได้อ่านจากข้อความที่พิมพ์อยู่บนภาพบัตร</div></div><footer class="flex justify-end border-t bg-white p-4"><button type="button" data-close class="rounded-xl bg-emerald-700 px-5 py-3 text-sm font-black text-white">ดำเนินการต่อ</button></footer></section>`;
    document.body.appendChild(modal);mountBbsDialog(modal,'ผลการตรวจสอบ Personal Card',returnFocus);
}

function reopenStoredDesignerIfNeeded() {
    if(state.tab!=='cards'||!state.context?.permissions?.configure)return;
    try{
        const saved=JSON.parse(sessionStorage.getItem('tsh_bbs_designer_session_v1')||'null');
        if(!saved)return;
        const kind=String(saved.kind)==='Department'?'Department':'Personal';
...
const statusBadge = value => { const submitted=value==='Submitted';return `<span data-bbs-status-tone="${submitted?'success':'warning'}" class="inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${submitted?'bg-emerald-100 text-emerald-700':'bg-amber-100 text-amber-700'}"><span class="sr-only">สถานะ </span>${submitted?'ส่งแล้ว':'ฉบับร่าง'}</span>`; };
...
function accessibilityStyles() {
    return `<style>
      ...
      #bbs-smart-card-page [data-bbs-skip-link]{position:fixed;left:1rem;top:1rem;z-index:1000;width:1px;height:1px;margin:-1px;padding:0!important;overflow:hidden;clip:rect(0 0 0 0);clip-path:inset(50%);white-space:nowrap;border:0}
      #bbs-smart-card-page [data-bbs-skip-link]:focus-visible{width:auto;height:auto;margin:0;padding:.75rem 1rem!important;overflow:visible;clip:auto;clip-path:none;white-space:normal}
      #bbs-smart-card-page [data-bbs-empty-state]{border-style:dashed}
      #bbs-smart-card-page [data-bbs-status-tone="success"]{forced-color-adjust:auto}
      ...
      @media(max-width:767px),(max-height:500px){
        ...
        #bbs-smart-card-page [data-bbs-compact-header] [data-bbs-tagline]{display:none}
        #bbs-smart-card-body{scroll-margin-top:10rem}
      }
      @media(prefers-reduced-motion:reduce){
        #bbs-smart-card-page,#bbs-smart-card-page *,#bbs-smart-card-page *::before,#bbs-smart-card-page *::after{scroll-behavior:auto!important;animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important}
      }
      @media(forced-colors:active){
        #bbs-smart-card-page [data-bbs-status-tone],#bbs-smart-card-page [data-bbs-empty-state]{border:1px solid CanvasText}
        #bbs-smart-card-page :where(button,a[href],summary,input,select,textarea):focus-visible{outline:3px solid Highlight}
      }
    </style>`;
}
...
function enhanceAccessibility() {
    ...
    page.querySelectorAll('input,select,textarea').forEach((control,index) => {
        if(control.hasAttribute('aria-label')||control.hasAttribute('aria-labelledby')||control.closest('label')||control.id&&page.querySelector(`label[for="${CSS.escape(control.id)}"]`))return;
        const label=control.getAttribute('placeholder')||control.getAttribute('title')||(control.tagName==='SELECT'?control.options?.[0]?.textContent?.trim():'')||`ช่องข้อมูล BBS ${index+1}`;
...
function bbsNavigationGroups() {
    ...
    if(state.context?.permissions?.configure)groups.push({key:'admin',label:'เครื่องมือผู้ดูแล',description:'ตั้งค่าระบบ',admin:true,tabs:[['cards','จัดการบัตร / QR']]});
    return groups.filter(group=>group.tabs.length);
}

function bbsNavigation() {
    const groups=bbsNavigationGroups(),active=groups.find(group=>group.tabs.some(([key])=>key===state.tab))||groups[0],activeTabs=active?.tabs||[];
    return `<nav aria-label="กลุ่มงาน BBS" class="border-t border-white/15 bg-emerald-950/15">
      <div data-bbs-primary-nav class="flex gap-2 overflow-x-auto px-3 py-2 sm:px-4">${groups.map(group=>{const selected=group.key===active?.key,target=selected?state.tab:group.tabs[0][0];return`<button type="button" data-bbs-group="${group.key}" data-bbs-group-target="${target}" aria-current="${selected?'page':'false'}" class="group min-w-max rounded-xl border px-3 py-2 text-left transition ${selected?'border-white bg-white text-emerald-900 shadow-sm':group.admin?'border-amber-200/50 bg-amber-300/15 text-amber-50 hover:bg-amber-300/25':'border-white/10 bg-emerald-950/20 text-white hover:bg-emerald-950/35'}"><span class="block text-xs font-black">${group.label}</span><span class="mt-0.5 block text-[10px] ${selected?'text-emerald-700':'text-emerald-100/75'}">${group.description}</span></button>`;}).join('')}</div>
      <div data-bbs-secondary-nav class="flex items-center gap-2 overflow-x-auto border-t border-white/10 bg-white/95 px-3 py-2 text-slate-700 sm:px-4" role="tablist" aria-label="เมนูย่อย ${active?.label||'BBS'}"><span class="shrink-0 text-[10px] font-black uppercase tracking-wider text-emerald-700">${active?.label||''}</span><span aria-hidden="true" class="h-5 w-px shrink-0 bg-slate-200"></span>${activeTabs.map(([key,label])=>`<button type="button" data-bbs-tab="${key}" class="min-h-9 whitespace-nowrap rounded-lg px-3 py-1.5 text-xs font-bold ${state.tab===key?'bg-emerald-100 text-emerald-800':'hover:bg-slate-100'}">${label}</button>`).join('')}</div>
    </nav>`;
}
...
function empty(title, detail, options = {}) {
    ...
    return `<section data-bbs-empty-state role="status" aria-live="polite" class="rounded-2xl border border-slate-300 bg-slate-50 text-center ${compact?'p-6':'p-8 md:p-10'}"><div aria-hidden="true" class="mx-auto flex ${compact?'h-9 w-9 text-base':'h-12 w-12 text-xl'} items-center justify-center rounded-2xl border border-slate-200 bg-white font-black text-slate-500">i</div><h3 class="mt-3 font-black text-slate-800">${escHtml(title)}</h3><p class="mx-auto mt-2 max-w-2xl text-sm leading-relaxed text-slate-600">${escHtml(detail)}</p></section>`;
}
...
function kpiSemantic(value = {}) {
    ...
    const labels = { N_A:'ไม่ใช้เกณฑ์ KPI', NOT_CONFIGURED:'ยังไม่ตั้งค่า', NOT_INSPECTED:'ยังไม่ถึงกำหนดตรวจ', ZERO_PERCENT:'0%', PERCENT:`${n(value.percentage ?? status.percentage)}%` };
    ...
    return { code, label:String(labels[code] || status.label || 'ยังไม่ตั้งค่า'), description:String(descriptions[code] || status.description || ''), tone:tones[code] || 'slate' };
}
...
function kpiStatusBadge(value = {}) {
    ...
    return `<span data-bbs-status-tone="${view.tone==='emerald'?'success':view.tone==='rose'?'danger':view.tone==='amber'?'warning':'info'}" class="inline-flex rounded-full px-2.5 py-1 text-xs font-black ${colors[view.tone]||colors.slate}" title="${escHtml(view.description)}" aria-label="สถานะ KPI: ${escHtml(view.label)}. ${escHtml(view.description)}">${escHtml(view.label)}</span>`;
}
...
async function trackSectionLoad(section, loader) {
    ...
    try {
        ...
        operation.finish();
        if(!state.loadingSections.size)document.querySelector('#bbs-smart-card-body')?.removeAttribute('aria-busy');
    }
}
...
async function withBusy(control, task, label = 'กำลังดำเนินการ...') {
    ...
    const snapshots = targets.map(target => ({ target, disabled:target.disabled, ariaDisabled:target.getAttribute('aria-disabled'), html:target.tagName === 'BUTTON' ? target.innerHTML : null, value:target.value }));
    control.setAttribute('aria-busy', 'true');
    targets.forEach(target => { target.disabled = true; target.setAttribute('aria-disabled','true'); if (target.tagName === 'BUTTON') target.textContent = label; else if(target.type!=='file')target.value = label; });
    try {
        ...
        operation.finish();
        snapshots.forEach(({target,disabled,ariaDisabled,html,value}) => {
            if (!target.isConnected) return;
            target.disabled = disabled;
            if(ariaDisabled===null)target.removeAttribute('aria-disabled');else target.setAttribute('aria-disabled',ariaDisabled);
            if (target.tagName === 'BUTTON') target.innerHTML = html;
            else if(target.type==='file')target.value='';
...
function bindBusy(control, eventName, handler, label) {
    if (!control) return;
    control[eventName] = event => withBusy(control, operation => handler(event, operation), label);
}
...
function workspaceView() {
    ...
    const drafts = resumableDrafts();
    const firstDraft = drafts[0];
    const canObserve = Boolean(state.context?.permissions?.observe);
    const readyEmployees = state.eligible.filter(row => checklistReadinessMeta(row).ready && String(row.EmployeeID) !== String(state.context?.employee?.EmployeeID));
    const observer = state.workspace?.observer || state.context?.employee || {};
    const period = state.workspace?.period || {};
    const periodLabel = period.year && period.month
        ? new Date(Date.UTC(n(period.year), n(period.month) - 1, 1)).toLocaleDateString('th-TH', { month:'long', year:'numeric', timeZone:'UTC' })
        : 'เดือนนี้';
    const overdue = n(action.OverdueCount);
    const pendingVerification = n(action.PendingVerificationCount);
    const followUpCount = overdue + pendingVerification;
    return `<div class="space-y-5">
      <section data-bbs-overview-priorities class="overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-950 via-emerald-800 to-teal-700 text-white shadow-sm">
        <div class="grid gap-5 p-5 md:p-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(20rem,.65fr)] xl:items-center">
          <div>
            <p class="text-[11px] font-black tracking-[.18em] text-emerald-200">ภาพรวม BBS ของฉัน</p>
            <h2 class="mt-2 text-2xl font-black">สวัสดี ${escHtml(observer.EmployeeName || 'ผู้ใช้งาน')}</h2>
            <p class="mt-2 max-w-2xl text-sm leading-relaxed text-emerald-50">เริ่มจากงานที่ต้องทำต่อ แล้วค่อยตรวจผล KPI และกิจกรรมของทีมใน ${escHtml(periodLabel)}</p>
            <div class="mt-4 flex flex-wrap gap-2 text-xs font-bold text-emerald-50"><span class="rounded-full bg-white/10 px-3 py-1.5">${escHtml(observer.BBSLevel || 'BBS Member')}</span><span class="rounded-full bg-white/10 px-3 py-1.5">${escHtml(observer.Department || '-')} / ${escHtml(observer.Unit || '-')}</span></div>
          </div>
          <div data-bbs-workspace-kpi-status="${escHtml(kpiView.code)}" class="rounded-2xl border border-white/20 bg-white/10 p-5 backdrop-blur">
            <div class="flex items-start justify-between gap-4"><div><p class="text-xs font-bold text-emerald-100">KPI ${escHtml(periodLabel)}</p><div class="mt-2 text-4xl font-black">${escHtml(kpiView.label)}</div></div><span class="rounded-full bg-white/15 px-3 py-1 text-[10px] font-black">คำนวณโดย Server</span></div>
            <p class="mt-3 text-xs leading-relaxed text-emerald-50">${escHtml(kpiView.description)}</p>
            ${kpiView.code==='PERCENT'?`<div class="mt-4 h-2 overflow-hidden rounded-full bg-black/20" role="progressbar" aria-label="ความคืบหน้า KPI" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.max(0,Math.min(100,n(kpi.percentage)))}"><div class="h-full rounded-full bg-emerald-300" style="width:${Math.max(0,Math.min(100,n(kpi.percentage)))}%"></div></div>`:''}
          </div>
        </div>
      </section>

      <section aria-labelledby="bbs-overview-next-title" class="rounded-3xl border border-slate-200 bg-white p-4 md:p-5">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-[10px] font-black tracking-widest text-emerald-700">งานแนะนำลำดับถัดไป</p><h3 id="bbs-overview-next-title" class="mt-1 text-lg font-black text-slate-800">สิ่งที่ควรทำต่อ</h3><p class="mt-1 text-xs text-slate-500">เรียงงานสำคัญไว้ในจุดเดียว เพื่อไม่ต้องค้นหาจากหลายแท็บ</p></div>${followUpCount===0&&!drafts.length?'<span data-bbs-status-tone="success" class="rounded-full bg-emerald-100 px-3 py-1.5 text-xs font-black text-emerald-700">ไม่มีงานค้างที่ระบบพบ</span>':''}</div>
        <div class="mt-4 grid gap-3 lg:grid-cols-3">
          <article class="flex min-h-48 flex-col rounded-2xl border border-emerald-200 bg-emerald-50 p-4 md:p-5"><div class="flex items-start justify-between gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-600 text-sm font-black text-white">1</span><span class="rounded-full bg-white px-3 py-1 text-[10px] font-black text-emerald-700">${readyEmployees.length} คนพร้อมตรวจ</span></div><h4 class="mt-4 font-black text-slate-800">เริ่ม Observation</h4><p class="mt-1 flex-1 text-xs leading-relaxed text-slate-600">เลือกรายบุคคลหรือหลายคนจากขอบเขตทีมที่ Server อนุญาต และตรวจเฉพาะ Checklist ที่พร้อมใช้งาน</p>${canObserve?`<button type="button" data-overview-open="start" class="mt-4 min-h-11 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white hover:bg-emerald-700">ไปเลือกพนักงาน</button>`:`<div class="mt-4 rounded-xl bg-white px-3 py-2.5 text-xs font-bold text-slate-500">บัญชีนี้ยังไม่มีสิทธิ์เริ่ม Observation</div>`}</article>
          <article class="flex min-h-48 flex-col rounded-2xl border ${drafts.length?'border-amber-200 bg-amber-50':'border-slate-200 bg-slate-50'} p-4 md:p-5"><div class="flex items-start justify-between gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-xl ${drafts.length?'bg-amber-500 text-white':'bg-slate-200 text-slate-600'} text-sm font-black">2</span><span class="rounded-full bg-white px-3 py-1 text-[10px] font-black ${drafts.length?'text-amber-700':'text-slate-500'}">${drafts.length} Draft</span></div><h4 class="mt-4 font-black text-slate-800">ทำแบบร่างให้เสร็จ</h4><p class="mt-1 flex-1 text-xs leading-relaxed text-slate-600">${firstDraft?`${escHtml(firstDraft.ObservedNameSnapshot)} · ${escHtml(firstDraft.ObservationNo)} · ${fmtDate(firstDraft.ObservationDate)}`:'ไม่มี Observation Draft ที่ค้างอยู่ในบัญชีของคุณ'}</p>${firstDraft?`<button type="button" data-bbs-resume="${firstDraft.id}" class="mt-4 min-h-11 rounded-xl bg-amber-500 px-4 text-sm font-black text-white hover:bg-amber-600">ทำ Draft ล่าสุดต่อ</button>`:`<button type="button" data-overview-open="history" class="mt-4 min-h-11 rounded-xl border border-slate-300 bg-white px-4 text-sm font-black text-slate-700">ดูประวัติ</button>`}</article>
          <article class="flex min-h-48 flex-col rounded-2xl border ${overdue?'border-rose-200 bg-rose-50':pendingVerification?'border-amber-200 bg-amber-50':'border-slate-200 bg-slate-50'} p-4 md:p-5"><div class="flex items-start justify-between gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-xl ${overdue?'bg-rose-600':pendingVerification?'bg-amber-500':'bg-slate-200'} ${followUpCount?'text-white':'text-slate-600'} text-sm font-black">3</span><span class="rounded-full bg-white px-3 py-1 text-[10px] font-black ${overdue?'text-rose-700':pendingVerification?'text-amber-700':'text-slate-500'}">${followUpCount} รายการติดตาม</span></div><h4 class="mt-4 font-black text-slate-800">Corrective Action</h4><p class="mt-1 flex-1 text-xs leading-relaxed text-slate-600">${followUpCount?`เกินกำหนด ${overdue} รายการ · รอตรวจยืนยัน ${pendingVerification} รายการ`:'ไม่มีงานเกินกำหนดหรืองานรอตรวจยืนยันที่ระบบพบ'}</p><button type="button" data-open-actions data-action-scope="${overdue?'overdue':pendingVerification?'verify':'all'}" class="mt-4 min-h-11 rounded-xl ${overdue?'bg-rose-600 text-white':pendingVerification?'bg-amber-500 text-white':'border border-slate-300 bg-white text-slate-700'} px-4 text-sm font-black">${followUpCount?'เปิดงานที่ต้องติดตาม':'ดู Corrective Action'}</button></article>
        </div>
      </section>

      <section aria-label="สรุปผลการสังเกตเดือนนี้" class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ${metric('คนที่สังเกต', n(kpi.uniqueObserved), 'พนักงานไม่ซ้ำในเดือนนี้', 'sky')}
        ${metric('Safe', n(kpi.safe), 'คำตอบที่พบพฤติกรรมปลอดภัย', 'emerald')}
        ${metric('Unsafe', n(kpi.unsafe), 'คำตอบที่ต้องเรียนรู้หรือติดตาม', 'rose')}
      </section>
      <section class="grid lg:grid-cols-5 gap-5">
        <div class="lg:col-span-3 rounded-2xl border border-slate-200 bg-white overflow-hidden"><div class="p-5 border-b"><h3 class="font-black text-slate-800">ทีมของฉัน</h3><p class="text-xs text-slate-500 mt-1">รายชื่อจาก Hierarchy Assignment ที่ Active พร้อมสถานะ Checklist จาก Server</p></div>${teamRows()}</div>
        <div class="lg:col-span-2 rounded-2xl border border-slate-200 bg-white overflow-hidden"><div class="p-5 border-b"><h3 class="font-black text-slate-800">Observation ล่าสุด</h3><p class="text-xs text-slate-500 mt-1">รายการที่คุณสร้างล่าสุด กดเพื่อเปิดรายละเอียด</p></div>${recentRows()}</div>
      </section>
    </div>`;
}
...
function inspectorManagementView() {
    ...
    return `<div class="space-y-5">
      ...
      <section class="rounded-2xl border border-slate-200 bg-white overflow-hidden"><div class="p-5 border-b"><h3 class="font-black text-slate-800">ผู้ตรวจที่ Admin กำหนด</h3><p class="text-xs text-slate-500 mt-1">กด “ทีม / ตารางตรวจ” เพื่อจัดการในหน้าต่างเดียว โดยไม่ต้องเลื่อนลงด้านล่าง</p></div><div class="divide-y">${(adminData.enrollments||[]).map(inspectorEnrollmentRow).join('')||'<div class="p-10 text-center text-sm text-slate-400">ยังไม่มีผู้ตรวจที่ได้รับการแต่งตั้ง</div>'}</div></section>`:''}
      ${!isAdmin&&(teamData?.enrollment?inspectorTeamPanel(teamData,false):empty('Admin ยังไม่ได้แต่งตั้งคุณเป็นผู้ตรวจ','เมื่อได้รับการแต่งตั้งและเปิดสิทธิ์จัดทีม เมนูนี้จะแสดงสมาชิกที่เลือกได้ใน Unit ของคุณ'))}
      ${isAdmin&&state.inspectorScheduleDetail&&teamData?.enrollment?inspectorTeamModal(state.inspectorScheduleDetail,teamData):''}
    </div>`;
}

function inspectorTeamModal(schedule,team){return `<div data-inspector-team-modal class="fixed inset-0 z-[10020] flex items-end sm:items-center justify-center bg-slate-950/60 p-0 sm:p-6" role="presentation"><section role="dialog" aria-modal="true" aria-label="ทีมและตารางตรวจ" tabindex="-1" class="flex h-[100dvh] w-full max-w-7xl flex-col overflow-hidden bg-slate-50 shadow-2xl sm:h-[calc(100dvh-3rem)] sm:rounded-3xl"><header class="flex shrink-0 items-start justify-between gap-4 border-b bg-white px-5 py-4"><div><p class="text-[10px] font-black tracking-widest text-violet-700">BBS INSPECTOR MANAGEMENT</p><h3 class="mt-1 text-lg font-black text-slate-800">ทีมและตารางตรวจ</h3><p class="mt-1 text-xs text-slate-500">จัดการข้อมูลของผู้ตรวจในหน้าต่างนี้ กด Esc หรือปิดเพื่อกลับสู่รายการ</p></div><button type="button" data-inspector-modal-close class="min-h-11 rounded-xl border border-slate-300 px-4 py-2 text-sm font-black text-slate-700">ปิด</button></header><div class="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">${inspectorSchedulePanel(schedule,true)}${inspectorTeamPanel(team,true)}</div></section></div>`;}
...
function departmentTemplateGallery(cards){return `<section class="rounded-2xl border border-slate-200 bg-white overflow-hidden"><div class="p-5 border-b flex justify-between gap-3"><div><h3 class="font-black text-slate-800">บัตรรายแผนก ${escHtml(cards.department?.Name||'')}</h3><p class="text-xs text-slate-500 mt-1">ทุก Template ใช้ QR กลางเดียวกัน เลือก Preview แล้วพิมพ์หรือบันทึก PDF จาก Print Snapshot เดียวกัน</p></div><span class="text-xs font-bold ${cards.qr?'text-emerald-700':'text-amber-700'}">${cards.qr?`QR รุ่น ${n(cards.qr.Generation)}`:'ยังไม่มี QR'}</span></div><div class="grid md:grid-cols-2 xl:grid-cols-3 gap-4 p-5">${(cards.templates||[]).map(row=>`<article class="rounded-2xl border border-slate-200 p-4"><div class="font-black text-slate-800 truncate">${escHtml(row.TemplateName)}</div><div class="text-xs text-slate-500 mt-1">${n(row.WidthMM)}×${n(row.HeightMM)} mm</div><div class="grid grid-cols-[1fr_80px] gap-2 mt-4"><select data-dept-paper="${row.id}" class="rounded-xl border border-slate-200 px-2 py-2 text-xs"><option>A4</option><option>A5</option><option>A6</option></select><input data-dept-copies="${row.id}" type="number" min="1" max="100" value="1" class="rounded-xl border border-slate-200 px-2 py-2 text-xs"></div><div class="grid grid-cols-2 gap-2 mt-2"><button data-dept-template-preview="${row.id}" class="rounded-xl border px-3 py-2 text-xs font-bold text-sky-700">Preview</button><button data-dept-template-print="${row.id}" ${cards.qr?'':'disabled'} class="rounded-xl bg-emerald-600 disabled:bg-slate-300 px-3 py-2 text-xs font-black text-white">พิมพ์ / PDF</button></div></article>`).join('')||'<div class="md:col-span-2 xl:col-span-3 p-8 text-center text-sm text-slate-400">ยังไม่มี Template ที่ Active สำหรับแผนกนี้</div>'}</div></section>`;}
...
function analyticsView() {
    ...
    const periodLabel=n(f.month)?`${new Date(2000,n(f.month)-1,1).toLocaleString('th-TH',{month:'long'})} ${n(f.year)}`:`ปี ${n(f.year)}`;
    const topRisk=(d.pareto||[])[0],topArea=(d.comparison||[])[0],activeFilters=[f.departmentId,f.safetyUnitId,f.risk].filter(value=>value!==''&&value!==null).length;
    return `<div class="space-y-5">
      <section data-analytics-workspace class="overflow-hidden rounded-3xl border border-sky-200 bg-white shadow-sm"><div class="grid gap-5 bg-gradient-to-br from-slate-950 via-sky-900 to-emerald-800 p-5 text-white md:p-6 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center"><div><p class="text-[10px] font-black tracking-[.18em] text-sky-200">BBS ANALYTICS WORKSPACE</p><h2 class="mt-2 text-2xl font-black">${escHtml(scopeLabels[f.scope]||f.scope||'รายงาน')} · ${escHtml(periodLabel)}</h2><p class="mt-2 text-sm text-white/80">ข้อมูลถึง ${fmtDate(d.meta?.periodThrough)} · KPI, Dashboard และ Export ใช้ผลคำนวณและขอบเขตเดียวกันจาก Server</p></div>${d.permissions?.canExport?'<div class="flex flex-wrap gap-2"><button data-analytics-export="excel" class="min-h-11 rounded-xl bg-white px-4 text-xs font-black text-emerald-800 shadow-sm">Excel</button><button data-analytics-export="pdf" class="min-h-11 rounded-xl bg-white/15 px-4 text-xs font-black text-white">PDF</button><button data-analytics-export="print" class="min-h-11 rounded-xl bg-white/15 px-4 text-xs font-black text-white">Print</button></div>':''}</div>
        <div class="p-4 md:p-5" data-analytics-controls tabindex="-1"><div class="flex items-center justify-between gap-3"><div><h3 class="font-black text-slate-800">ขอบเขตการวิเคราะห์</h3><p class="mt-1 text-xs text-slate-500">ตัวเลือกทั้งหมดถูกตรวจสิทธิ์ซ้ำที่ Server</p></div>${activeFilters?`<button type="button" data-analytics-reset class="min-h-11 rounded-xl border border-slate-300 px-3 text-xs font-black text-slate-600">ล้างตัวกรอง ${activeFilters}</button>`:''}</div><div class="mt-4 grid grid-cols-2 lg:grid-cols-6 gap-2"><label class="text-[11px] font-bold text-slate-500">มุมมอง<select data-analytics-filter="scope" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm">${(o.scopes||[]).map(v=>`<option value="${v}" ${f.scope===v?'selected':''}>${scopeLabels[v]||v}</option>`).join('')}</select></label><label class="text-[11px] font-bold text-slate-500">ปี<select data-analytics-filter="year" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm">${Array.from({length:Math.max(1,n(o.years?.max)-n(o.years?.min)+1)},(_,i)=>n(o.years?.max)-i).map(v=>`<option ${n(f.year)===v?'selected':''}>${v}</option>`).join('')}</select></label><label class="text-[11px] font-bold text-slate-500">เดือน<select data-analytics-filter="month" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="0">ทั้งปี</option>${Array.from({length:12},(_,i)=>`<option value="${i+1}" ${n(f.month)===i+1?'selected':''}>${new Date(2000,i,1).toLocaleString('th-TH',{month:'long'})}</option>`).join('')}</select></label><label class="text-[11px] font-bold text-slate-500">แผนก<select data-analytics-filter="departmentId" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกแผนกที่มีสิทธิ์</option>${(o.departments||[]).map(row=>`<option value="${row.id}" ${n(f.departmentId)===n(row.id)?'selected':''}>${escHtml(row.Name)}</option>`).join('')}</select></label><label class="text-[11px] font-bold text-slate-500">Safety Unit<select data-analytics-filter="safetyUnitId" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุก Unit ที่มีสิทธิ์</option>${units.map(row=>`<option value="${row.id}" ${n(f.safetyUnitId)===n(row.id)?'selected':''}>${escHtml(row.name)}</option>`).join('')}</select></label><label class="text-[11px] font-bold text-slate-500">ความเสี่ยง<select data-analytics-filter="risk" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกระดับความเสี่ยง</option>${(o.risks||[]).map(v=>`<option ${f.risk===v?'selected':''}>${v}</option>`).join('')}</select></label></div></div>
      </section>
      <div id="bbs-analytics-report" class="space-y-5 bg-white print:p-4">
        <section data-bbs-analytics-kpi-status="${escHtml(kpiView.code)}" class="grid grid-cols-2 xl:grid-cols-6 gap-3">${analyticsMetric('ความสำเร็จ KPI',kpiView.label,kpiView.code==='PERCENT'?`${n(d.kpi?.numerator)}/${n(d.kpi?.denominator)} · ผ่าน ${n(d.kpi?.peopleMeeting)}/${n(d.kpi?.peopleTotal)} คน`:kpiView.description,kpiView.tone)}${analyticsMetric('Observation',n(d.totals?.observations),'นับเฉพาะรายการที่ส่งแล้ว','sky','observations')}${analyticsMetric('Safe',n(d.totals?.safe),'คำตอบ Safe','emerald','safe')}${analyticsMetric('Unsafe',n(d.totals?.unsafe),`${n(d.totals?.unsafeRate)}% ของ Safe+Unsafe`,'rose','unsafe')}${analyticsMetric('Action ค้าง',n(d.actions?.total)-n(d.actions?.closed),`ปิดแล้ว ${n(d.actions?.closed)}/${n(d.actions?.total)}`,'amber','actions')}${analyticsMetric('เกิน SLA',n(d.actions?.overdue),`เฉลี่ยปิด ${n(d.actions?.avgClosureDays)} วัน`,'rose','overdue')}</section>
        <section data-analytics-insights aria-label="ประเด็นที่ควรสนใจ" class="grid gap-3 md:grid-cols-3"><div data-bbs-status-tone="${n(d.actions?.overdue)?'danger':'success'}" class="rounded-2xl border ${n(d.actions?.overdue)?'border-rose-200 bg-rose-50':'border-emerald-200 bg-emerald-50'} p-4"><p class="text-[10px] font-black tracking-widest ${n(d.actions?.overdue)?'text-rose-700':'text-emerald-700'}">จุดเน้นด้าน Action</p><h4 class="mt-2 font-black text-slate-800">${n(d.actions?.overdue)?`เกิน SLA ${n(d.actions.overdue)} รายการ`:'ไม่พบ Action เกิน SLA'}</h4><p class="mt-1 text-xs text-slate-500">เปิดตัวเลขด้านบนเพื่อดูรายละเอียดตามขอบเขตปัจจุบัน</p></div><div data-bbs-status-tone="warning" class="rounded-2xl border border-amber-200 bg-amber-50 p-4"><p class="text-[10px] font-black tracking-widest text-amber-700">หมวด Unsafe สูงสุด</p><h4 class="mt-2 font-black text-slate-800">${escHtml(topRisk?.category||'ยังไม่มีข้อมูล Unsafe')}</h4><p class="mt-1 text-xs text-slate-500">${topRisk?`${n(topRisk.unsafe)} Unsafe · ${n(topRisk.unsafeRate)}%`:'ระบบยังไม่มีหมวดที่ต้องจัดลำดับในช่วงนี้'}</p></div><div data-bbs-status-tone="info" class="rounded-2xl border border-sky-200 bg-sky-50 p-4"><p class="text-[10px] font-black tracking-widest text-sky-700">สัญญาณตามพื้นที่</p><h4 class="mt-2 font-black text-slate-800">${escHtml(topArea?`${topArea.department||'-'} / ${topArea.unit||'-'}`:'ยังไม่มีข้อมูลเปรียบเทียบ')}</h4><p class="mt-1 text-xs text-slate-500">${topArea?`Unsafe ${n(topArea.unsafeRate)}% · Action ค้าง ${n(topArea.actionOpen)}`:'ระบบยังไม่มีพื้นที่ให้เปรียบเทียบในช่วงนี้'}</p></div></section>
        <section class="grid xl:grid-cols-2 gap-5"><div class="rounded-2xl border bg-white p-5"><h4 class="font-black text-slate-800">แนวโน้ม Safe / Unsafe</h4><p class="text-xs text-slate-500 mt-1">${f.month?'รายวัน':'รายเดือน'}</p>${analyticsTrend(d.trend||[])}</div><div class="rounded-2xl border bg-white p-5"><h4 class="font-black text-slate-800">หมวด Unsafe แบบ Pareto</h4><p class="text-xs text-slate-500 mt-1">เรียงจาก Unsafe สูงสุด</p>${analyticsPareto(d.pareto||[])}</div></section>
        <section class="grid xl:grid-cols-3 gap-5"><div class="xl:col-span-2 rounded-2xl border bg-white overflow-hidden"><div class="p-5 border-b"><h4 class="font-black text-slate-800">Heatmap ตามแผนก / Unit</h4><p class="text-xs text-slate-500 mt-1">สีเข้มขึ้นเมื่ออัตรา Unsafe สูง และมีตัวเลขกำกับทุกแถว</p></div>${analyticsComparison(d.comparison||[])}</div><div class="rounded-2xl border bg-white p-5"><h4 class="font-black text-slate-800">อายุงาน Corrective Action</h4><div class="mt-4 space-y-3">${(d.actions?.aging||[]).map(row=>analyticsBar(row.bucket,n(row.count),Math.max(1,...(d.actions?.aging||[]).map(x=>n(x.count))),'bg-amber-500')).join('')}</div><div class="mt-5 grid grid-cols-2 gap-2 text-xs">${[['เปิด',d.actions?.open],['กำลังดำเนินการ',d.actions?.inProgress],['รอตรวจยืนยัน',d.actions?.pendingVerification],['ปิดแล้ว',d.actions?.closed]].map(([label,value])=>`<div class="rounded-xl bg-slate-50 p-3"><div class="text-slate-500">${label}</div><div class="mt-1 text-lg font-black text-slate-800">${n(value)}</div></div>`).join('')}</div></div></section>
...
function actionBadge(value) {
    ...
    const label={Open:'เปิด','In Progress':'กำลังดำเนินการ','Pending Verification':'รอตรวจยืนยัน',Closed:'ปิดแล้ว',Reopened:'เปิดใหม่'}[value]||value||'-';
    const semantic={Open:'neutral','In Progress':'info','Pending Verification':'warning',Closed:'success',Reopened:'danger'}[value]||'neutral';
    return `<span data-bbs-status-tone="${semantic}" class="inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${tone[value] || 'bg-slate-100 text-slate-600'}"><span class="sr-only">สถานะ </span>${escHtml(label)}</span>`;
}

function actionsView() {
    const s = state.actionSummary || {};
    const admin = Boolean(state.context?.permissions?.configure);
    const overdue=n(s.OverdueCount),pending=n(s.PendingVerificationCount),nearDue=n(s.NearDueCount);
    const queueLabel={all:'รายการทั้งหมด',mine:'งานที่ฉันรับผิดชอบ',verify:'งานที่ฉันต้องยืนยัน',overdue:'งานเกินกำหนด'}[state.actionScope]||'รายการทั้งหมด';
    const activeFilterCount=[state.actionStatus,state.actionPriority,state.actionFilters.year,state.actionFilters.departmentId,state.actionFilters.safetyUnitId,state.actionFilters.q].filter(value=>value!==''&&value!==null).length;
    return `<div class="space-y-5">
      <section data-action-workspace class="overflow-hidden rounded-3xl border ${overdue?'border-rose-300':'border-sky-200'} bg-white shadow-sm">
        <div class="grid gap-5 ${overdue?'bg-gradient-to-br from-rose-950 via-rose-800 to-orange-700':'bg-gradient-to-br from-slate-900 via-sky-900 to-emerald-800'} p-5 text-white md:p-6 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center"><div><p class="text-[10px] font-black tracking-[.18em] ${overdue?'text-rose-200':'text-sky-200'}">พื้นที่ทำงาน Corrective Action</p><h2 class="mt-2 text-2xl font-black">${overdue?`มีงานเกินกำหนด ${overdue} รายการ`:'ติดตามการแก้ไขและการยืนยันผล'}</h2><p class="mt-2 max-w-2xl text-sm leading-relaxed text-white/80">เริ่มจากงานเร่งด่วน เปิดรายละเอียดเพื่อเพิ่มหลักฐาน ส่งตรวจยืนยัน หรือปิดงานตามสิทธิ์ที่ Server กำหนด</p><div class="mt-4 flex flex-wrap gap-2 text-xs font-bold"><span class="rounded-full bg-white/10 px-3 py-1.5">ใกล้ครบกำหนด ${nearDue}</span><span class="rounded-full bg-white/10 px-3 py-1.5">รอยืนยัน ${pending}</span></div></div>${admin?'<button type="button" data-action-reminder class="min-h-11 rounded-xl bg-white px-4 py-2.5 text-sm font-black text-slate-800 shadow-sm">จัดคิวแจ้งเตือน / ยกระดับ</button>':''}</div>
        <div class="grid grid-cols-2 gap-3 p-4 md:grid-cols-4 md:p-5" role="group" aria-label="เลือกคิว Corrective Action">${actionQueueButton('overdue','เร่งด่วน','เกินกำหนด',overdue,'rose')}${actionQueueButton('mine','งานของฉัน','ฉันเป็น Owner',null,'sky')}${actionQueueButton('verify','รอยืนยัน','ฉันเป็น Verifier',null,'amber')}${actionQueueButton('all','ทั้งหมด','ตามสิทธิ์ที่มองเห็น',n(s.Total),'slate')}</div>
      </section>
      <section class="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        <div class="p-4 md:p-5 border-b space-y-4"><div class="flex flex-col sm:flex-row sm:items-end justify-between gap-3"><div><p class="text-[10px] font-black tracking-widest text-sky-700">คิวงานที่กำลังติดตาม</p><h3 class="mt-1 font-black text-slate-800">${escHtml(queueLabel)}</h3><p class="text-xs text-slate-500 mt-1">Unsafe ที่ต้องติดตามจะสร้าง Action ตาม Workflow เดิมโดยอัตโนมัติ</p></div><div class="flex items-center gap-2"><span class="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600">${n(state.listMeta.actions?.total)||state.actions.length} รายการ</span>${activeFilterCount?`<button type="button" data-action-reset class="min-h-11 rounded-xl border border-slate-300 px-3 text-xs font-black text-slate-600">ล้างตัวกรอง ${activeFilterCount}</button>`:''}</div></div>
          <div class="grid sm:grid-cols-3 xl:grid-cols-4 gap-2"><select aria-label="ขอบเขต Corrective Action" data-action-filter="scope" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="all">ทั้งหมดที่มองเห็น</option><option value="mine" ${state.actionScope==='mine'?'selected':''}>ฉันรับผิดชอบ</option><option value="verify" ${state.actionScope==='verify'?'selected':''}>ฉันต้องยืนยัน</option><option value="overdue" ${state.actionScope==='overdue'?'selected':''}>เกินกำหนด</option></select><select aria-label="สถานะ Corrective Action" data-action-filter="status" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">ทุกสถานะ</option>${['Open','In Progress','Pending Verification','Closed','Reopened'].map(v=>`<option ${state.actionStatus===v?'selected':''}>${{Open:'เปิด','In Progress':'กำลังดำเนินการ','Pending Verification':'รอตรวจยืนยัน',Closed:'ปิดแล้ว',Reopened:'เปิดใหม่'}[v]}</option>`).join('')}</select><select aria-label="ระดับความสำคัญ" data-action-filter="priority" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">ทุกระดับความสำคัญ</option>${['Critical','High','Medium','Low'].map(v=>`<option ${state.actionPriority===v?'selected':''}>${v}</option>`).join('')}</select><select aria-label="ปีของ Corrective Action" data-action-list-filter="year" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกปี</option>${[yearNow,yearNow-1,yearNow-2].map(v=>`<option value="${v}" ${n(state.actionFilters.year)===v?'selected':''}>${v}</option>`).join('')}</select><input aria-label="ค้นหา Corrective Action" data-list-search="actions" value="${escHtml(state.actionFilters.q)}" type="search" placeholder="ค้นหา Action, พนักงาน หรือรายการตรวจ" class="rounded-xl border px-3 py-2.5 text-sm"><select aria-label="แผนกของ Corrective Action" data-action-list-filter="departmentId" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกแผนก</option>${masterFilterOptions(state.actionFilters.departmentId)}</select><select aria-label="Unit ของ Corrective Action" data-action-list-filter="safetyUnitId" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุก Unit</option>${masterFilterOptions(state.actionFilters.safetyUnitId,true,state.actionFilters.departmentId)}</select></div>
        </div>${actionRows()}${listPager('actions',state.listMeta.actions,'Action')}
      </section>
      ${admin ? `<details data-action-admin-tools class="rounded-2xl border border-slate-200 bg-white"><summary class="cursor-pointer list-none p-5"><div class="flex items-center justify-between gap-3"><div><p class="text-[10px] font-black tracking-widest text-violet-700">เครื่องมือสำหรับ Admin</p><h3 class="mt-1 font-black text-slate-800">คิวอีเมลและกฎ SLA</h3><p class="mt-1 text-xs text-slate-500">เครื่องมือดูแลระบบถูกแยกจากคิวงานประจำวัน</p></div><span class="text-sm font-black text-violet-700">เปิดเครื่องมือ ↓</span></div></summary><div class="space-y-5 border-t bg-slate-50 p-4 md:p-5">${actionOutboxView()}${slaView()}</div></details>` : ''}
    </div>`;
}

function actionQueueButton(scope,title,detail,count,tone) {
    const selected=state.actionScope===scope,colors={rose:selected?'border-rose-500 bg-rose-50 text-rose-800':'border-rose-200 text-rose-700 hover:bg-rose-50',sky:selected?'border-sky-500 bg-sky-50 text-sky-800':'border-sky-200 text-sky-700 hover:bg-sky-50',amber:selected?'border-amber-500 bg-amber-50 text-amber-800':'border-amber-200 text-amber-700 hover:bg-amber-50',slate:selected?'border-slate-600 bg-slate-100 text-slate-800':'border-slate-200 text-slate-700 hover:bg-slate-50'};
    return `<button type="button" data-action-queue="${scope}" aria-pressed="${selected?'true':'false'}" class="min-h-20 rounded-2xl border p-3 text-left transition ${colors[tone]}"><span class="flex items-start justify-between gap-2"><span><span class="block text-sm font-black">${escHtml(title)}</span><span class="mt-1 block text-[11px] opacity-75">${escHtml(detail)}</span></span>${count===null?'':`<strong class="text-2xl">${n(count)}</strong>`}</span></button>`;
}

function actionRows() {
    if (!state.actions.length) return `<div class="p-10 text-center"><div class="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-100 font-black text-emerald-700">✓</div><h4 class="mt-4 font-black text-slate-800">ไม่พบ Corrective Action ในคิวนี้</h4><p class="mt-1 text-sm text-slate-500">ลองเลือกคิวอื่นหรือล้างตัวกรองเพื่อดูรายการที่คุณมีสิทธิ์เข้าถึง</p><button type="button" data-action-reset class="mt-4 min-h-11 rounded-xl border border-slate-300 px-4 text-sm font-black text-slate-700">ล้างตัวกรองและดูทั้งหมด</button></div>`;
    return `<div data-action-queue-list class="divide-y">${state.actions.map(row => { const days=n(row.DaysRemaining),closed=row.Status==='Closed',overdue=!closed&&days<0,dueToday=!closed&&days===0,urgent=overdue||row.Priority==='Critical',dueLabel=closed?'ปิดงานแล้ว':overdue?`เกินกำหนด ${Math.abs(days)} วัน`:dueToday?'ครบกำหนดวันนี้':`เหลือ ${days} วัน`,nextLabel=row.permissions?.canVerify&&row.Status==='Pending Verification'?'ตรวจยืนยัน':row.permissions?.canWork&&row.Status==='Open'?'เริ่มดำเนินการ':row.permissions?.canWork&&row.Status==='In Progress'?'เพิ่มหลักฐาน / ส่งตรวจ':'เปิดรายละเอียด';return `<button type="button" data-action-detail="${row.id}" class="group relative grid w-full gap-4 overflow-hidden p-4 text-left hover:bg-slate-50 md:p-5 lg:grid-cols-[minmax(0,1fr)_auto]"><span aria-hidden="true" class="absolute inset-y-0 left-0 w-1 ${overdue?'bg-rose-500':dueToday?'bg-amber-500':closed?'bg-emerald-500':'bg-sky-500'}"></span><div class="min-w-0 pl-1"><div class="flex flex-wrap items-center gap-2"><span class="font-black text-slate-800">${escHtml(row.ActionNo)}</span>${actionBadge(row.Status)}<span class="rounded-full px-2 py-1 text-xs font-black ${row.Priority==='Critical'||row.Priority==='High'?'bg-rose-100 text-rose-700':row.Priority==='Medium'?'bg-amber-100 text-amber-700':'bg-slate-100 text-slate-600'}">${escHtml(row.Priority)}</span>${n(row.AfterEvidenceCount)?`<span class="rounded-full bg-sky-50 px-2 py-1 text-[10px] font-bold text-sky-700">After ${n(row.AfterEvidenceCount)} รูป</span>`:''}</div><div class="mt-2 text-sm font-bold text-slate-700">${escHtml(row.ItemCodeSnapshot)} · ${escHtml(row.ItemPromptSnapshot)}</div><div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500"><span>ผู้ถูกสังเกต: <b class="text-slate-700">${escHtml(row.ObservedNameSnapshot||row.ObservedEmployeeID)}</b></span><span>${escHtml(row.ObservationNo)}</span><span>${escHtml(row.ObservedDepartmentSnapshot||'-')} / ${escHtml(row.ObservedUnitSnapshot||'-')}</span></div><div class="mt-2 grid gap-1 text-xs text-slate-500 sm:grid-cols-2"><span>Owner: <b class="text-slate-700">${escHtml(row.OwnerName||row.OwnerEmployeeID)}</b></span><span>Verifier: <b class="text-slate-700">${escHtml(row.VerifierName||row.VerifierEmployeeID)}</b></span></div></div><div class="flex items-center justify-between gap-4 pl-1 lg:min-w-48 lg:flex-col lg:items-end lg:justify-center lg:pl-0 lg:text-right"><div><div class="text-sm font-black ${urgent?'text-rose-600':dueToday?'text-amber-700':closed?'text-emerald-700':'text-slate-700'}">${dueLabel}</div><div class="mt-1 text-xs text-slate-400">กำหนด ${fmtDate(row.DueDate)}</div></div><span class="rounded-xl ${urgent?'bg-rose-600 text-white':'bg-slate-100 text-slate-700 group-hover:bg-sky-100 group-hover:text-sky-800'} px-3 py-2 text-xs font-black">${nextLabel} →</span></div></button>`; }).join('')}</div>`;
}
...
function teamRows() {
    ...
    if (!rows.length) return `<div class="p-4">${empty('ยังไม่มีสมาชิกในทีม', 'ไม่พบสมาชิกใน Hierarchy Assignment ที่กำลังมีผล หากควรมีรายชื่อ กรุณาติดต่อ Admin เพื่อตรวจสอบการจัดทีม', {compact:true})}</div>`;
...
function recentRows() {
    ...
    if (!rows.length) return `<div class="p-4">${empty('ยังไม่มี Observation', 'เมื่อคุณส่ง Observation หรือบันทึกฉบับร่าง รายการล่าสุดจะแสดงที่นี่', {compact:true})}</div>`;
...
function draftRecoveryPanel() {
    ...
    return `<section data-bbs-status-tone="warning" class="rounded-2xl border border-amber-200 bg-amber-50 p-4 md:p-5"><div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><p class="text-xs font-black text-amber-700">ฉบับร่างที่บันทึกไว้</p><h3 class="mt-1 font-black text-slate-800">ทำ Observation ที่ค้างไว้ต่อ</h3><p class="mt-1 text-xs text-slate-500">Server เก็บฉบับร่างไว้เมื่อคุณเปลี่ยนแท็บหรือออกจากแบบฟอร์ม</p></div><span class="rounded-full bg-white px-3 py-1 text-xs font-black text-amber-700">${rows.length} ฉบับร่าง</span></div><div class="mt-3 grid gap-2">${rows.slice(0, 5).map(row => `<div class="rounded-xl border border-amber-100 bg-white p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div class="min-w-0"><div class="truncate text-sm font-bold text-slate-800">${escHtml(row.ObservedNameSnapshot)}</div><div class="mt-1 text-xs text-slate-500">${escHtml(row.ObservationNo)} · ${fmtDate(row.ObservationDate)}</div></div><button type="button" data-bbs-resume="${row.id}" class="min-h-11 rounded-xl bg-amber-500 px-4 text-xs font-black text-white">ทำฉบับร่างต่อ</button></div>`).join('')}</div></section>`;
}
...
function batchObservationForm() {
    ...
    const allAnswers=(batch.members||[]).flatMap(member=>member.observation?.answers||[]),answered=allAnswers.filter(answer=>answer.Response).length;
    return `<section data-observation-wizard="batch" class="space-y-4 pb-28"><div class="rounded-2xl border border-emerald-200 bg-white p-4 md:p-5 shadow-sm"><div class="flex justify-between gap-3"><div><p class="text-xs font-black text-emerald-700">${escHtml(batch.BatchNo)} · ${n(batch.EmployeeCount)} คน</p><h3 class="font-black text-slate-800 mt-1">${review?'ตรวจทานก่อนส่ง':`Checklist ${step}/${groups.length}`}</h3><p class="text-xs text-slate-500 mt-1">${review?'ตรวจคำตอบ Unsafe และหลักฐานของแต่ละคน':`${escHtml(group?.templateName||'')} v${n(group?.versionNo)} · ${group?.members?.length||0} คน`}</p><p class="mt-1 text-[11px] font-black text-emerald-700">ตอบแล้ว ${answered}/${allAnswers.length} ข้อ</p></div><span class="text-sm font-black text-emerald-700">${percent}%</span></div><div role="progressbar" aria-label="ความคืบหน้าการตรวจแบบกลุ่ม" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${percent}" class="h-2 rounded-full bg-slate-100 mt-3"><div class="h-full rounded-full bg-emerald-500" style="width:${percent}%"></div></div><ol class="mt-4 flex gap-2 overflow-x-auto pb-1" aria-label="ขั้นตอนการตรวจแบบกลุ่ม">${groups.map((item,index)=>wizardStepPill(index+1,step,`ชุด ${index+1}`)).join('')}${wizardStepPill(maxStep,step,'ตรวจทาน')}</ol></div>
      ${review?batchReview():batchGroupForm(group)}
      <div data-bbs-sticky-actions aria-label="คำสั่งแบบตรวจกลุ่ม" class="fixed md:sticky bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 backdrop-blur p-3 pb-[max(.75rem,env(safe-area-inset-bottom))]"><div class="mx-auto grid max-w-5xl grid-cols-[auto_auto_minmax(0,1fr)] items-center gap-2"><button type="button" data-batch-back class="min-h-12 rounded-xl border px-3 sm:px-4 font-bold text-slate-600">${step===1?'เลือกรายชื่อ':'ย้อนกลับ'}</button><button type="button" data-batch-save class="min-h-12 rounded-xl border border-emerald-300 px-3 sm:px-4 font-bold text-emerald-700"><span class="sm:hidden">บันทึก</span><span class="hidden sm:inline">บันทึกร่าง</span></button><button type="button" data-batch-next class="min-h-12 truncate rounded-xl bg-emerald-600 px-4 font-black text-white">${review?`ยืนยันส่ง ${n(batch.EmployeeCount)} คน`:step===groups.length?'ไปตรวจทาน':'บันทึกและถัดไป'}</button></div></div>
    </section>`;
}

function wizardStepPill(index,current,label) {
    const active=index===current,done=index<current;
    return `<li aria-current="${active?'step':'false'}" class="flex min-w-max items-center gap-2 rounded-full border px-3 py-1.5 text-[11px] font-black ${active?'border-emerald-500 bg-emerald-100 text-emerald-800':done?'border-emerald-200 bg-emerald-50 text-emerald-700':'border-slate-200 bg-slate-50 text-slate-400'}"><span class="flex h-5 w-5 items-center justify-center rounded-full ${active||done?'bg-emerald-600 text-white':'bg-slate-200 text-slate-500'}">${done?'✓':index}</span>${escHtml(label)}</li>`;
}
...
function observationForm() {
    const d = state.draft;
    const groups=singleObservationGroups(d),maxStep=groups.length+1,step=Math.max(1,Math.min(maxStep,state.singleStep||1)),review=step===maxStep,current=review?null:groups[step-1];
    state.singleStep=step;
    const answered=d.answers.filter(answer=>answer.Response).length,percent=Math.round((step/maxStep)*100);
    return `<form id="bbs-observation-form" data-observation-wizard="single" class="space-y-4 pb-28">
      <section class="rounded-2xl border border-emerald-200 bg-white p-4 md:p-5 shadow-sm"><div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4"><div><p class="text-xs font-black text-emerald-700">${escHtml(d.ObservationNo)} · ${escHtml(d.Status)}</p><h3 class="text-xl font-black text-slate-800 mt-1">${escHtml(d.ObservedNameSnapshot)}</h3><p class="text-sm text-slate-500 mt-1">${escHtml(d.ObservedDepartmentSnapshot || '-')} / ${escHtml(d.ObservedUnitSnapshot || '-')} · ${escHtml(d.ObservedPositionSnapshot || '-')}</p></div><div class="md:text-right"><div class="text-sm font-bold text-slate-700">${escHtml(d.TemplateName)} v${n(d.VersionNo)}</div><div class="text-xs text-slate-400 mt-1">Checklist version ถูกตรึงกับ Observation นี้</div><div class="mt-2 text-xs font-black text-emerald-700">ตอบแล้ว ${answered}/${d.answers.length} ข้อ</div></div></div><div class="mt-4 flex items-center justify-between gap-3"><div><p class="text-[10px] font-black tracking-widest text-emerald-700">OBSERVATION WIZARD</p><h4 class="mt-1 font-black text-slate-800">${review?'ตรวจทานก่อนส่ง':escHtml(current?.name||'Checklist')}</h4></div><span class="text-sm font-black text-emerald-700">${percent}%</span></div><div role="progressbar" aria-label="ความคืบหน้า Observation" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${percent}" class="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full bg-emerald-500" style="width:${percent}%"></div></div><ol class="mt-4 flex gap-2 overflow-x-auto pb-1" aria-label="ขั้นตอน Observation">${groups.map((group,index)=>wizardStepPill(index+1,step,group.name)).join('')}${wizardStepPill(maxStep,step,'ตรวจทาน')}</ol></section>
      ${review?singleObservationReview(d,groups):`<section aria-labelledby="bbs-single-group-title" class="space-y-3"><div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"><h4 id="bbs-single-group-title" class="font-black text-slate-800">${escHtml(current?.name||'Checklist')}</h4><p class="mt-1 text-xs text-slate-500">ตอบ Safe, Unsafe หรือ N/A ให้ครบ ${current?.answers.length||0} ข้อ</p></div>${(current?.answers||[]).map(answer => answerCard(answer, d.files.filter(file => n(file.AnswerID) === n(answer.id)))).join('')}</section>`}
      <div data-bbs-sticky-actions aria-label="คำสั่ง Observation" class="fixed md:sticky bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 backdrop-blur p-3 pb-[max(.75rem,env(safe-area-inset-bottom))]"><div class="mx-auto grid max-w-5xl grid-cols-[auto_auto_minmax(0,1fr)] items-center gap-2"><button type="button" data-single-back class="min-h-12 rounded-xl border px-3 sm:px-4 text-sm font-bold text-slate-600">${step===1?'เลือกรายชื่อ':'ย้อนกลับ'}</button><button type="button" data-bbs-save class="min-h-12 rounded-xl border border-emerald-300 px-3 sm:px-4 text-sm font-bold text-emerald-700"><span class="sm:hidden">บันทึก</span><span class="hidden sm:inline">บันทึกร่าง</span></button>${review?`<button type="submit" class="min-h-12 truncate rounded-xl bg-emerald-600 px-4 text-sm font-black text-white hover:bg-emerald-700">ยืนยันส่ง Observation</button>`:`<button type="button" data-single-next class="min-h-12 truncate rounded-xl bg-emerald-600 px-4 text-sm font-black text-white hover:bg-emerald-700">${step===groups.length?'ไปตรวจทาน':'บันทึกและถัดไป'}</button>`}</div></div>
    </form>`;
}

function singleObservationGroups(d) {
    const groups=[];
    for(const answer of d.answers||[]){const name=String(answer.CategoryNameSnapshot||'Checklist');let group=groups.find(item=>item.name===name);if(!group){group={name,answers:[]};groups.push(group);}group.answers.push(answer);}
    return groups;
}

function singleObservationReview(d,groups) {
    const safe=d.answers.filter(answer=>answer.Response==='Safe').length,unsafe=d.answers.filter(answer=>answer.Response==='Unsafe').length,na=d.answers.filter(answer=>answer.Response==='N/A').length,missing=d.answers.filter(answer=>n(answer.IsRequiredSnapshot)&&!answer.Response).length;
    return `<div class="space-y-4"><section class="rounded-2xl border ${missing?'border-amber-300 bg-amber-50':'border-emerald-200 bg-emerald-50'} p-4 md:p-5"><div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h4 class="font-black text-slate-800">${missing?`ยังขาดคำตอบ ${missing} ข้อ`:'คำตอบครบ พร้อมตรวจทาน'}</h4><p class="mt-1 text-xs text-slate-600">ตรวจ Unsafe และหลักฐานให้ถูกต้องก่อนยืนยันส่ง</p></div><div class="grid grid-cols-3 gap-2 text-center text-xs font-black"><span class="rounded-xl bg-white px-3 py-2 text-emerald-700">Safe ${safe}</span><span class="rounded-xl bg-white px-3 py-2 text-rose-700">Unsafe ${unsafe}</span><span class="rounded-xl bg-white px-3 py-2 text-slate-600">N/A ${na}</span></div></div></section><section class="grid gap-3 md:grid-cols-2">${groups.map((group,index)=>{const answered=group.answers.filter(answer=>answer.Response).length,unsafeCount=group.answers.filter(answer=>answer.Response==='Unsafe').length;return`<button type="button" data-single-jump="${index+1}" class="rounded-2xl border ${answered===group.answers.length?'border-emerald-200':'border-amber-300'} bg-white p-4 text-left hover:bg-slate-50"><div class="flex items-start justify-between gap-3"><div><p class="font-black text-slate-800">${escHtml(group.name)}</p><p class="mt-1 text-xs text-slate-500">ตอบแล้ว ${answered}/${group.answers.length} · Unsafe ${unsafeCount}</p></div><span class="text-xs font-black ${answered===group.answers.length?'text-emerald-700':'text-amber-700'}">${answered===group.answers.length?'ครบ':'แก้ไข'} →</span></div></button>`;}).join('')}</section><section class="rounded-2xl border border-slate-200 bg-white p-5"><label for="bbs-general-remark" class="text-sm font-bold text-slate-700">หมายเหตุภาพรวม</label><textarea id="bbs-general-remark" name="generalRemark" rows="3" class="mt-2 w-full rounded-xl border border-slate-200 p-3 text-base" placeholder="ข้อมูลเพิ่มเติม (ถ้ามี)">${escHtml(d.GeneralRemark || '')}</textarea></section></div>`;
}
...
function historyView() {
    ...
    const views=[['observer','ที่ฉันตรวจ','Observation ที่คุณเป็นผู้บันทึก'],['observed','ฉันถูกตรวจ','พฤติกรรมที่ผู้อื่นสังเกตคุณ'],['team','ทีมของฉัน','รายการในขอบเขตทีมที่ Server อนุญาต']],active=views.find(([key])=>key===state.view)||views[0];
    const drafts=state.history.filter(row=>row.Status==='Draft').length,submitted=state.history.filter(row=>row.Status==='Submitted').length,activeFilters=[f.status,f.departmentId,f.safetyUnitId,f.q].filter(value=>value!==''&&value!==null).length;
    return `<div data-history-workspace class="space-y-5"><section class="overflow-hidden rounded-3xl border border-emerald-200 bg-white shadow-sm"><div class="bg-gradient-to-br from-emerald-950 via-emerald-800 to-teal-700 p-5 text-white md:p-6"><p class="text-[10px] font-black tracking-[.18em] text-emerald-200">OBSERVATION HISTORY</p><div class="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 class="text-2xl font-black">${escHtml(active[1])}</h2><p class="mt-2 text-sm text-emerald-50">${escHtml(active[2])} · ปี ${state.historyYear}</p></div><span class="self-start rounded-full bg-white/15 px-3 py-1.5 text-xs font-black">ทั้งหมด ${n(state.listMeta.history?.total)||state.history.length} รายการ</span></div></div><div class="grid gap-2 p-3 sm:grid-cols-3" role="group" aria-label="มุมมองประวัติ">${views.map(([key,label,description])=>`<button type="button" data-bbs-view="${key}" aria-pressed="${state.view===key?'true':'false'}" class="min-h-16 rounded-2xl border p-3 text-left ${state.view===key?'border-emerald-500 bg-emerald-50 text-emerald-800 shadow-sm':'border-slate-200 text-slate-700 hover:bg-slate-50'}"><span class="block text-sm font-black">${label}</span><span class="mt-1 block text-[11px] opacity-70">${description}</span></button>`).join('')}</div></section><section class="overflow-hidden rounded-2xl border border-slate-200 bg-white"><div class="space-y-4 border-b p-4 md:p-5"><div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h3 class="font-black text-slate-800">ค้นหาและกรองประวัติ</h3><p class="mt-1 text-xs text-slate-500">ในหน้านี้: ส่งแล้ว ${submitted} · Draft ${drafts}</p></div>${activeFilters?`<button type="button" data-history-reset class="min-h-11 rounded-xl border border-slate-300 px-3 text-xs font-black text-slate-600">ล้างตัวกรอง ${activeFilters}</button>`:''}</div><div class="grid sm:grid-cols-2 xl:grid-cols-5 gap-2"><label class="text-[11px] font-bold text-slate-500 xl:col-span-1">ค้นหา<input data-list-search="history" value="${escHtml(f.q)}" type="search" placeholder="เลข Observation หรือชื่อพนักงาน" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"></label><label class="text-[11px] font-bold text-slate-500">ปี<select data-history-filter="year" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm">${[yearNow,yearNow-1,yearNow-2].map(v=>`<option value="${v}" ${state.historyYear===v?'selected':''}>${v}</option>`).join('')}</select></label><label class="text-[11px] font-bold text-slate-500">สถานะ<select data-history-filter="status" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกสถานะ</option><option ${f.status==='Draft'?'selected':''}>Draft</option><option ${f.status==='Submitted'?'selected':''}>Submitted</option></select></label><label class="text-[11px] font-bold text-slate-500">แผนก<select data-history-filter="departmentId" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกแผนก</option>${masterFilterOptions(f.departmentId)}</select></label><label class="text-[11px] font-bold text-slate-500">Safety Unit<select data-history-filter="safetyUnitId" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุก Unit</option>${masterFilterOptions(f.safetyUnitId,true,f.departmentId)}</select></label></div></div>${historyRows()}${listPager('history',state.listMeta.history,'Observation')}</section></div>`;
}
...
function personalCardEligibleLevels() {
    const levels=masterBbsLevels(),start=levels.indexOf('Group Leader');
    return levels.slice(start<0?1:start);
}

function personalCardBbsLevelOptions() {
    return personalCardEligibleLevels().map(value=>`<option value="${escHtml(value)}">${escHtml(value)}</option>`).join('');
}

function personalTemplateLevelEligible(template) {
    return !template?.BBSLevel||personalCardEligibleLevels().includes(String(template.BBSLevel));
}
...
function masterArtworkView(kindFilter='') {
    const data=state.masterArtwork||{},kinds=kindFilter?[kindFilter]:['Personal','Department'];
    if(data.error)return `<section class="rounded-2xl border border-amber-200 bg-amber-50 p-5"><h3 class="font-black text-amber-800">Master Artwork ยังไม่พร้อม</h3><p class="mt-1 text-xs text-amber-700">${escHtml(data.error)} · ต้องใช้ migration ล่าสุดก่อนอัปโหลดภาพมาสเตอร์</p></section>`;
    const summary=row=>row?`<div class="mt-3 rounded-xl bg-slate-50 p-3"><div class="truncate text-xs font-bold text-slate-700">${escHtml(row.originalName)}</div><div class="mt-1 text-[10px] text-slate-500">ACTIVE V${n(row.versionNo)} · ${Math.max(1,Math.round(n(row.fileSize)/1024))} KB${row.pixelWidth&&row.pixelHeight?` · ${n(row.pixelWidth)}×${n(row.pixelHeight)} px`:''}</div><button type="button" data-master-artwork-preview="${n(row.versionId)}" class="mt-2 min-h-11 rounded-lg border border-slate-300 px-3 text-xs font-black text-slate-700">เปิดดูภาพปัจจุบัน</button></div>`:'<p class="mt-3 text-xs text-amber-700">ยังไม่มีภาพ Active ในขอบเขตนี้</p>';
    const global=data.globalBack;
    const backCard=`<form data-master-artwork-upload data-artwork-role="global-back" class="rounded-2xl border border-violet-200 bg-white p-4"><div class="flex items-start justify-between gap-3"><div><span class="text-[10px] font-black tracking-widest text-violet-700">GLOBAL BACK · SHARED</span><h4 class="mt-1 font-black text-slate-800">ภาพมาสเตอร์ด้านหลังกลาง</h4><p class="mt-1 text-[11px] text-slate-500">ใช้ร่วมกันกับ Personal และ Department Card ทุกใบ</p></div><span class="rounded-full px-2 py-1 text-[10px] font-black ${global?'bg-emerald-100 text-emerald-700':'bg-amber-100 text-amber-700'}">${global?'พร้อมใช้':'ยังไม่ตั้งค่า'}</span></div>${summary(global)}<label class="mt-3 block text-[11px] font-bold text-slate-600">${global?'อัปโหลดเวอร์ชันใหม่':'เลือกภาพด้านหลังกลาง'}<input name="artwork" required type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 block w-full text-xs text-slate-500"></label><button class="mt-3 min-h-11 w-full rounded-xl bg-violet-600 px-3 text-sm font-black text-white">${global?'แทนที่สำหรับงานใหม่':'ตั้งเป็น Global Back'}</button></form>`;
    const frontCards=kinds.map(kind=>{const fronts=(data.scopedFronts||[]).filter(row=>row.templateKind===kind),tone=kind==='Personal'?'sky':'emerald';return `<form data-master-artwork-upload data-artwork-role="scoped-front" data-kind="${kind}" class="rounded-2xl border border-${tone}-200 bg-white p-4"><div><span class="text-[10px] font-black tracking-widest text-${tone}-700">${kind.toUpperCase()} · SCOPED FRONT</span><h4 class="mt-1 font-black text-slate-800">ภาพด้านหน้าตามแผนก / Unit</h4><p class="mt-1 text-[11px] text-slate-500">Unit ตรงกันจะถูกเลือกก่อน แล้วจึง fallback ไปค่าแผนก</p></div><input type="hidden" name="templateKind" value="${kind}"><div class="mt-3 grid gap-2 sm:grid-cols-2"><label class="text-[11px] font-bold text-slate-600">แผนก<select name="departmentId" required class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">เลือกแผนก</option>${masterDepartmentOptions()}</select></label><label class="text-[11px] font-bold text-slate-600">Unit (ไม่บังคับ)<select name="safetyUnitId" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">ค่าเริ่มต้นทั้งแผนก</option></select></label></div><label class="mt-3 block text-[11px] font-bold text-slate-600">ภาพด้านหน้า<input name="artwork" required type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 block w-full text-xs text-slate-500"></label><button class="mt-3 min-h-11 w-full rounded-xl bg-${tone}-600 px-3 text-sm font-black text-white">บันทึก Scoped Front เวอร์ชันใหม่</button>${fronts.length?`<details class="mt-3 rounded-xl bg-slate-50 p-3"><summary class="cursor-pointer text-xs font-black text-slate-700">ขอบเขตที่พร้อมแล้ว (${fronts.length})</summary><div class="mt-2 grid gap-2">${fronts.map(row=>`<div class="flex items-center justify-between gap-2 rounded-lg bg-white p-2 text-[11px]"><span><strong>${escHtml(row.departmentName||'-')}</strong>${row.safetyUnitName?` / ${escHtml(row.safetyUnitName)}`:' / ค่าแผนก'} · V${n(row.versionNo)}</span><button type="button" data-master-artwork-preview="${n(row.versionId)}" class="rounded border px-2 py-1 font-bold">ดูภาพ</button></div>`).join('')}</div></details>`:''}</form>`;}).join('');
    const history=(data.records||[]).filter(row=>row.status==='Archived'&&(!kindFilter||!row.templateKind||row.templateKind===kindFilter)).slice(0,8);
    return `<section data-master-artwork-manager class="rounded-2xl border border-violet-200 bg-violet-50/30 p-5"><div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div class="text-[10px] font-black tracking-widest text-violet-700">CARD ARTWORK LIBRARY · SERVER RESOLVED</div><h3 class="mt-1 font-black text-slate-800">ด้านหน้าแยกตามขอบเขต · ด้านหลังใช้มาสเตอร์กลาง</h3><p class="mt-1 max-w-3xl text-xs text-slate-600">Personal และ Department ยังใช้ Layout/Template คนละชุด ระบบเลือกภาพหน้าจาก Unit ก่อนแผนก และใช้ภาพหลังกลางร่วมกัน โดยไม่แก้ประวัติงานเดิมย้อนหลัง</p></div><span class="rounded-full bg-white px-3 py-2 text-xs font-black text-violet-700">Front พร้อม ${(data.scopedFronts||[]).filter(row=>!kindFilter||row.templateKind===kindFilter).length} ขอบเขต · Back ${global?'พร้อม':'ยังขาด'}</span></div><div class="mt-4 grid grid-cols-1 ${kindFilter?'xl:grid-cols-2':'xl:grid-cols-3'} gap-3">${frontCards}${backCard}</div>${history.length?`<details class="mt-4 rounded-xl border border-violet-100 bg-white p-3"><summary class="cursor-pointer text-xs font-black text-slate-700">ประวัติเวอร์ชันที่ถูกแทนที่ (${history.length})</summary><div class="mt-3 grid gap-2 sm:grid-cols-2">${history.map(row=>`<div class="rounded-lg bg-slate-50 p-2 text-[11px] text-slate-600"><strong>${escHtml(row.slotKey)} V${n(row.versionNo)}</strong> · ${escHtml(row.originalName)}</div>`).join('')}</div></details>`:''}</section>`;
}

async function openTemplateFormFromArtwork(row) {
    const kind=String(row?.templateKind||''),workspace=kind==='Personal'?'personal':kind==='Department'?'department':'';
    if(!workspace||!n(row?.departmentId))return showToast('ขอบเขตของ Scoped Front ไม่ถูกต้อง','error');
    try {
        state.cardWorkspace=workspace;
        if(workspace==='department')state.departmentConfigSelectedId=n(row.departmentId);
        await trackSectionLoad('cards',()=>loadCardWorkspace(workspace));
        render({preserveScroll:false});
        const form=document.getElementById(workspace==='personal'?'bbs-template-form':'bbs-dept-template-form');
        if(!form)throw new Error('ไม่พบฟอร์มสร้าง Template');
        const department=form.elements?.departmentId,unit=form.elements?.safetyUnitId;
        if(department){department.value=String(n(row.departmentId));department.dispatchEvent(new Event('change',{bubbles:true}));}
        if(unit){unit.value=row.safetyUnitId==null?'':String(n(row.safetyUnitId));unit.dispatchEvent(new Event('change',{bubbles:true}));}
        requestAnimationFrame(()=>{form.scrollIntoView({behavior:'smooth',block:'start'});form.elements?.templateName?.focus({preventScroll:true});});
        showToast(`เลือก ${kind} Scoped Front ให้แล้ว กรอกชื่อและสร้าง Template Draft ได้เลย`,'success');
    } catch(error) { showToast(error?.message||'เปิดฟอร์มสร้าง Template ไม่สำเร็จ','error'); }
}

function installArtworkTemplateCtas() {
    document.querySelectorAll('[data-master-artwork-manager] [data-master-artwork-preview]').forEach(preview=>{
        const row=(state.masterArtwork?.scopedFronts||[]).find(item=>n(item.versionId)===n(preview.dataset.masterArtworkPreview));
        if(!row||preview.parentElement?.querySelector('[data-create-template-from-artwork]'))return;
        const button=document.createElement('button');
        button.type='button';button.dataset.createTemplateFromArtwork=String(n(row.versionId));
        button.className='ml-2 min-h-11 rounded-lg border border-violet-300 bg-violet-50 px-3 text-[11px] font-black text-violet-700';
        button.textContent='สร้าง Template จาก Artwork นี้';
        preview.insertAdjacentElement('afterend',button);
        bindBusy(button,'onclick',()=>openTemplateFormFromArtwork(row),'กำลังเปิดฟอร์ม Template...');
    });
}
...
function departmentConfigurationRows() {
    ...
    return masterDepartments().map(department => {
        ...
        const designerReadyTemplates = activeTemplates.filter(row => n(row.ActiveLayoutID) > 0);
        const artworkReadyTemplates = designerReadyTemplates.filter(row => { const artwork=resolveTemplateArtwork('Department',n(row.DepartmentID),n(row.SafetyUnitID));return Boolean(artwork?.front&&artwork?.back); });
        ...
        if (!designerReadyTemplates.length) missing.push('Designer Layout');
        if (!artworkReadyTemplates.length) missing.push('Artwork');
        if (!qr) missing.push('QR');
        if (!handler) missing.push('Owner/Verifier');
        const readySteps = 5 - missing.length;
        return { id, name:department.Name, templates, activeTemplates, designerReadyTemplates, artworkReadyTemplates, qr, handler, missing, readySteps, ready:missing.length === 0 };
    });
}

function departmentTemplateWorkflowHtml(row,qr,handler){
    const artwork=resolveTemplateArtwork('Department',n(row.DepartmentID),n(row.SafetyUnitID));
    const checks=[
        ['Artwork',Boolean(artwork?.front&&artwork?.back),artwork?.front&&artwork?.back?'Scoped Front + Global Back พร้อม':'ตั้งค่า Scoped Front และ Global Back'],
        ['Template',String(row.Status)==='Active',String(row.Status)==='Active'?'Template Active':`Template ${String(row.Status||'Draft')}`],
        ['Designer',n(row.ActiveLayoutID)>0,n(row.ActiveLayoutID)>0?`Active Layout V${n(row.ActiveLayoutVersionNo)}`:n(row.DraftLayoutID)>0?'มี Draft แต่ยังไม่ผ่าน/ยังไม่ Activate':'ยังไม่มี Designer Draft'],
        ['Department QR',Boolean(qr),qr?`Active รุ่น ${n(qr.Generation)}`:'ยังไม่มี Active QR'],
        ['Owner / Verifier',Boolean(handler),handler?'กำหนดผู้รับผิดชอบแล้ว':'ยังไม่ได้กำหนด'],
    ];
    const ready=checks.every(([,ok])=>ok);
    return `<div data-department-template-workflow class="mt-3 rounded-xl border ${ready?'border-emerald-200 bg-emerald-50':'border-amber-200 bg-amber-50'} p-3"><div class="flex flex-wrap gap-2">${checks.map(([label,ok,detail])=>`<span title="${escHtml(detail)}" class="rounded-full px-2 py-1 text-[10px] font-black ${ok?'bg-emerald-100 text-emerald-700':'bg-amber-100 text-amber-800'}">${escHtml(label)} · ${ok?'พร้อม':'ยังไม่พร้อม'}</span>`).join('')}</div><div class="mt-2 text-[11px] font-black ${ready?'text-emerald-800':'text-amber-900'}">${ready?'Ready to Print · พร้อมใช้งานจริง':'ยังไม่พร้อมพิมพ์ · เปิด Visual Designer หรือ Preview + Readiness เพื่อแก้รายการที่ยังไม่พร้อม'}</div></div>`;
}

function installDepartmentWorkflowStatus(){
    const data=state.communityAdmin||{},qrByDepartment=new Map((data.qrCards||[]).filter(row=>row.Status==='Active').map(row=>[n(row.DepartmentID),row])),handlerByDepartment=new Map((data.handlers||[]).filter(row=>n(row.IsActive)!==0).map(row=>[n(row.DepartmentID),row]));
    const statusByDepartment=new Map(departmentConfigurationRows().map(row=>[row.id,row]));
    document.querySelectorAll('[data-department-config-row]').forEach(button=>{const row=statusByDepartment.get(n(button.dataset.departmentId)),badge=button.querySelector('span');if(row&&badge)badge.textContent=row.ready?'READY':`${row.readySteps}/5`;});
    document.querySelectorAll('[data-card-designer-department]').forEach(button=>{
        const row=(data.templates||[]).find(item=>n(item.id)===n(button.dataset.cardDesignerDepartment)),host=button.parentElement?.previousElementSibling;
        if(!row||!host||host.querySelector('[data-department-template-workflow]'))return;
        host.insertAdjacentHTML('beforeend',departmentTemplateWorkflowHtml(row,qrByDepartment.get(n(row.DepartmentID))||null,handlerByDepartment.get(n(row.DepartmentID))||null));
    });
}
...
function cardSizeEditor({ includeDisplayOrder=false }={}) {
    return `<section data-card-size-editor class="rounded-xl border border-slate-200 bg-slate-50 p-3">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><div class="text-xs font-black text-slate-700">ขนาดและแนววางบัตร</div><div class="mt-0.5 text-[11px] text-slate-500">ค่าเริ่มต้นมาตรฐาน BBS 60 × 85 mm แนวตั้ง</div></div>
        <div class="flex rounded-xl bg-slate-200/70 p-1" role="group" aria-label="เลือกแนววางบัตร"><button type="button" data-card-orientation="portrait" aria-pressed="true" class="rounded-lg bg-white px-3 py-2 text-xs font-black text-emerald-700 shadow-sm">แนวตั้ง</button><button type="button" data-card-orientation="landscape" aria-pressed="false" class="rounded-lg px-3 py-2 text-xs font-black text-slate-600">แนวนอน</button></div>
      </div>
      <div class="mt-3 grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_72px] items-end gap-2">
        <label class="text-[11px] font-bold text-slate-600">กว้าง (mm)<input name="widthMM" type="number" required min="40" max="500" step="0.01" value="60" inputmode="decimal" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"></label>
        <label class="text-[11px] font-bold text-slate-600">สูง (mm)<input name="heightMM" type="number" required min="40" max="500" step="0.01" value="85" inputmode="decimal" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"></label>
        <div class="flex flex-col items-center"><div data-card-size-preview class="h-16 rounded-md border-2 border-emerald-400 bg-white shadow-sm" style="aspect-ratio:60/85"></div><span data-card-size-summary class="mt-1 whitespace-nowrap text-[9px] font-bold text-slate-500">60 × 85</span></div>
      </div>
      ${includeDisplayOrder?'<label class="mt-3 block text-[11px] font-bold text-slate-600">ลำดับการแสดง<input name="displayOrder" type="number" required min="0" max="9999" step="1" value="0" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"></label>':''}
    </section>`;
}

function bindCardSizeEditors() {
    document.querySelectorAll('[data-card-size-editor]').forEach(editor => {
        const form=editor.closest('form'),width=form?.elements?.widthMM,height=form?.elements?.heightMM,preview=editor.querySelector('[data-card-size-preview]'),summary=editor.querySelector('[data-card-size-summary]');
        if(!width||!height)return;
        const sync=()=>{const w=Number(width.value)||60,h=Number(height.value)||85,orientation=w<=h?'portrait':'landscape';if(preview)preview.style.aspectRatio=`${w}/${h}`;if(summary)summary.textContent=`${w} × ${h}`;editor.querySelectorAll('[data-card-orientation]').forEach(button=>{const active=button.dataset.cardOrientation===orientation;button.setAttribute('aria-pressed',String(active));button.classList.toggle('bg-white',active);button.classList.toggle('text-emerald-700',active);button.classList.toggle('shadow-sm',active);button.classList.toggle('text-slate-600',!active);});};
        editor.querySelectorAll('[data-card-orientation]').forEach(button=>button.onclick=()=>{const portrait=button.dataset.cardOrientation==='portrait';width.value=portrait?'60':'85';height.value=portrait?'85':'60';sync();});
        width.addEventListener('input',sync);height.addEventListener('input',sync);sync();
    });
}

function resolveTemplateArtwork(kind, departmentId, safetyUnitId) {
    const fronts=(state.masterArtwork?.scopedFronts||[]).filter(row=>row.templateKind===kind&&n(row.departmentId)===n(departmentId));
    const exact=safetyUnitId?fronts.find(row=>n(row.safetyUnitId)===n(safetyUnitId)):null;
    const departmentDefault=fronts.find(row=>!n(row.safetyUnitId));
    return { front:exact||departmentDefault||null,back:state.masterArtwork?.globalBack||null,usedFallback:Boolean(safetyUnitId&&!exact&&departmentDefault) };
}

function templateArtworkMatchHtml(kind, departmentId, safetyUnitId) {
    if(!departmentId)return '<div class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-3 text-xs text-slate-500">เลือกแผนกและ Unit แล้วระบบจะแสดงภาพด้านหน้าและด้านหลังที่จะใช้กับ Designer Draft</div>';
    const artwork=resolveTemplateArtwork(kind,departmentId,safetyUnitId),missing=[];
    if(!artwork.front)missing.push(`${kind} Scoped Front`);
    if(!artwork.back)missing.push('Global Back');
    if(missing.length)return `<div role="alert" class="rounded-xl border border-amber-300 bg-amber-50 p-3 text-xs text-amber-800"><div class="font-black">Artwork ยังไม่พร้อมสำหรับขอบเขตนี้</div><div class="mt-1">ขาด ${escHtml(missing.join(' และ '))} — กรุณาเพิ่มใน Card Artwork Library ก่อนสร้าง Designer Draft${!artwork.front?' หรือใช้ไฟล์ Legacy fallback ด้านล่างเพื่อสร้างเฉพาะ Template':''}</div></div>`;
    const scope=artwork.usedFallback?'ใช้ภาพค่าเริ่มต้นระดับแผนก':safetyUnitId?'ตรงกับ Unit ที่เลือก':'ใช้ภาพระดับแผนก';
    return `<div class="rounded-xl border border-emerald-300 bg-emerald-50 p-3 text-xs text-emerald-900"><div class="flex flex-wrap items-center justify-between gap-2"><div><div class="font-black">Artwork พร้อมสำหรับ Designer Draft</div><div class="mt-1">ด้านหน้า: ${escHtml(artwork.front.originalName||'Scoped Front')} · V${n(artwork.front.versionNo)} <span class="font-bold">(${scope})</span></div><div class="mt-1">ด้านหลัง: ${escHtml(artwork.back.originalName||'Global Back')} · V${n(artwork.back.versionNo)}</div></div><div class="flex flex-wrap gap-2"><button type="button" data-template-artwork-preview="${n(artwork.front.versionId)}" class="min-h-11 rounded-lg border border-emerald-300 bg-white px-3 text-[11px] font-black">ดูภาพด้านหน้า</button><button type="button" data-template-artwork-preview="${n(artwork.back.versionId)}" class="min-h-11 rounded-lg border border-violet-300 bg-white px-3 text-[11px] font-black">ดูภาพด้านหลัง</button></div></div><p class="mt-2 border-t border-emerald-200 pt-2 text-[11px] text-emerald-800">กรอกข้อมูล Template แล้วกดสร้างได้เลย ไม่ต้องอัปโหลดภาพซ้ำ ระบบจะจัดทำ Legacy fallback จาก Scoped Front ให้อัตโนมัติ</p></div>`;
}

function updateTemplateArtworkMatch(form) {
    const target=form.querySelector('[data-template-artwork-match]');
    if(!target)return;
    const kind=form.dataset.templateKind,departmentId=n(form.elements?.departmentId?.value),safetyUnitId=n(form.elements?.safetyUnitId?.value);
    target.innerHTML=templateArtworkMatchHtml(kind,departmentId,safetyUnitId);
    const fallbackInput=form.elements?.template,fallbackField=fallbackInput?.closest('label'),artwork=departmentId?resolveTemplateArtwork(kind,departmentId,safetyUnitId):null;
    if(fallbackInput&&fallbackField){const showFallback=Boolean(departmentId&&!artwork?.front);fallbackInput.required=false;fallbackField.dataset.legacyFallbackField='1';fallbackField.hidden=!showFallback;fallbackField.classList.toggle('hidden',!showFallback);fallbackField.style.display=showFallback?'block':'none';const labelText=fallbackField.firstChild;if(labelText?.nodeType===Node.TEXT_NODE)labelText.textContent=`ยังไม่มี ${kind} Scoped Front — อัปโหลดภาพ Legacy fallback ชั่วคราว (ไม่บังคับเมื่อ Artwork พร้อม)`;}
    const submit=form.querySelector('button:not([type="button"])');if(submit)submit.textContent=`สร้าง ${kind} Template Draft`;
    target.querySelectorAll('[data-template-artwork-preview]').forEach(button=>bindBusy(button,'onclick',()=>previewMasterArtwork(n(button.dataset.templateArtworkPreview)),'กำลังเปิดภาพ Card Artwork...'));
}

function templateLifecycleReadinessHtml(row,kind) {
    const artwork=resolveTemplateArtwork(kind,n(row.DepartmentID),n(row.SafetyUnitID)),missing=[];
    if(!artwork.front)missing.push(`${kind} Scoped Front`);
    if(!artwork.back)missing.push('Global Back');
    const editable=String(row.Status)==='Draft',ready=!missing.length;
    const frontText=artwork.front?(artwork.usedFallback?'Scoped Front พร้อม · ใช้ค่าแผนก':'Scoped Front พร้อม'):'ขาด Scoped Front';
    const backText=artwork.back?'Global Back พร้อม':'ขาด Global Back';
    const summary=ready?(editable?'พร้อมสร้างหรือแก้ไข Designer Draft':`${escHtml(row.Status)} · เปิด Preview ได้ แต่แก้ Layout ไม่ได้`):`ยังไม่พร้อมเปิด Designer · ขาด ${escHtml(missing.join(' และ '))}`;
    return `<div data-template-lifecycle-readiness role="status" class="mt-2 rounded-xl border ${ready?'border-emerald-200 bg-emerald-50/70':'border-amber-300 bg-amber-50'} p-2.5"><div class="flex flex-wrap gap-1.5"><span class="rounded-full px-2 py-1 text-[10px] font-black ${artwork.front?'bg-sky-100 text-sky-700':'bg-amber-100 text-amber-800'}">${frontText}</span><span class="rounded-full px-2 py-1 text-[10px] font-black ${artwork.back?'bg-violet-100 text-violet-700':'bg-amber-100 text-amber-800'}">${backText}</span></div><div class="mt-1.5 text-[11px] font-bold ${ready?'text-emerald-800':'text-amber-900'}">${summary}</div>${missing.length?`<div class="mt-2 flex flex-wrap gap-2">${!artwork.front?`<button type="button" data-template-readiness-fix="front" data-template-kind="${kind}" data-department-id="${n(row.DepartmentID)}" data-safety-unit-id="${n(row.SafetyUnitID)}" class="min-h-11 rounded-lg border border-sky-300 bg-white px-3 text-[11px] font-black text-sky-700">ไปตั้งค่า Scoped Front</button>`:''}${!artwork.back?'<button type="button" data-template-readiness-fix="back" class="min-h-11 rounded-lg border border-violet-300 bg-white px-3 text-[11px] font-black text-violet-700">ไปตั้งค่า Global Back</button>':''}</div>`:''}</div>`;
}

function installTemplateLifecycleReadiness() {
    const install=(selector,records,kind)=>document.querySelectorAll(selector).forEach(anchor=>{
        const id=n(kind==='Personal'?anchor.dataset.cardDesignerPersonal:anchor.dataset.cardDesignerDepartment),row=records.find(item=>n(item.id)===id),host=anchor.parentElement?.previousElementSibling;
        if(!row||!host||host.querySelector('[data-template-lifecycle-readiness]'))return;
        host.insertAdjacentHTML('beforeend',templateLifecycleReadinessHtml(row,kind));
    });
    install('[data-card-designer-personal]',state.cardTemplates||[],'Personal');
    install('[data-card-designer-department]',state.communityAdmin?.templates||[],'Department');
    document.querySelectorAll('[data-template-readiness-fix]').forEach(button=>button.onclick=()=>focusArtworkRequirement(button));
}

function focusArtworkRequirement(button) {
    const role=button.dataset.templateReadinessFix,manager=document.querySelector('[data-master-artwork-manager]');
    if(!manager)return showToast('ไม่พบ Card Artwork Library ใน Workspace นี้','error');
    const form=role==='back'?manager.querySelector('[data-master-artwork-upload][data-artwork-role="global-back"]'):manager.querySelector(`[data-master-artwork-upload][data-kind="${button.dataset.templateKind}"]`);
    if(!form)return showToast('ไม่พบช่องตั้งค่า Artwork ที่ต้องการ','error');
    if(role==='front'){
        const department=form.elements?.departmentId,unit=form.elements?.safetyUnitId;
        if(department){department.value=button.dataset.departmentId||'';department.dispatchEvent(new Event('change',{bubbles:true}));}
        if(unit){unit.value=n(button.dataset.safetyUnitId)?button.dataset.safetyUnitId:'';unit.dispatchEvent(new Event('change',{bubbles:true}));}
    }
    form.scrollIntoView({behavior:'smooth',block:'center'});
    form.elements?.artwork?.focus({preventScroll:true});
}

function bindTemplateScopeForms() {
    document.querySelectorAll('[data-template-scope-form]').forEach(form=>{
        const department=form.elements?.departmentId,unit=form.elements?.safetyUnitId;
        if(!department||!unit)return;
        const refresh=()=>{const selected=n(unit.value),departmentId=n(department.value),rows=(state.masterReference?.units||[]).filter(row=>departmentId&&n(row.department_id||row.DepartmentID)===departmentId);unit.innerHTML=`<option value="">ค่าเริ่มต้นทั้งแผนก</option>${rows.map(row=>`<option value="${n(row.id)}" ${selected===n(row.id)?'selected':''}>${escHtml(row.name||row.Name)}</option>`).join('')}`;unit.disabled=!departmentId;updateTemplateArtworkMatch(form);};
        department.addEventListener('change',refresh);unit.addEventListener('change',()=>updateTemplateArtworkMatch(form));refresh();
    });
}
      ...
      <div class="grid grid-cols-1 xl:grid-cols-[360px_minmax(0,1fr)] min-w-0">
        ...
        <div data-department-config-detail="${selected.id}" class="min-w-0">
          ...
          <div class="grid grid-cols-1 2xl:grid-cols-2 gap-5 p-5">
            <form id="bbs-dept-template-form" data-template-scope-form data-template-kind="Department" class="rounded-2xl border border-slate-200 p-4 space-y-3"><div><h4 class="font-black text-slate-800">1. สร้าง Template Draft</h4><p class="mt-1 text-[11px] text-slate-500">ระบบเลือก Scoped Front ตามแผนก/Unit และใช้ Global Back ร่วมกัน ไม่ต้องอัปโหลดภาพซ้ำ</p></div><input type="hidden" name="departmentId" value="${selected.id}"><label class="block text-[11px] font-bold text-slate-600">ชื่อ Template<input name="templateName" required maxlength="160" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm" placeholder="เช่น BBS Department Card"></label><label class="block text-[11px] font-bold text-slate-600">Unit (ไม่บังคับ)<select name="safetyUnitId" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">ค่าเริ่มต้นทั้งแผนก</option>${(state.masterReference?.units||[]).filter(row=>n(row.department_id||row.DepartmentID)===n(selected.id)).map(row=>`<option value="${n(row.id)}">${escHtml(row.name||row.Name)}</option>`).join('')}</select></label><div data-template-artwork-match role="status" aria-live="polite"></div>${cardSizeEditor({includeDisplayOrder:true})}<label data-legacy-fallback-field class="hidden block text-[11px] font-bold text-slate-600">Legacy fallback ชั่วคราว (ใช้เฉพาะเมื่อยังไม่มี Department Scoped Front)<input name="template" type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 w-full text-xs"></label><button class="w-full rounded-xl bg-emerald-600 px-3 py-2.5 text-sm font-black text-white">สร้าง Department Template Draft</button></form>
          ...
          <div class="border-t border-slate-100"><div class="p-5 flex items-center justify-between gap-3"><div><h4 class="font-black text-slate-800">3. Template ของแผนกนี้</h4><p class="mt-1 text-[11px] text-slate-500">Composite Preview จะตรวจรูป ขนาด QR และพื้นที่พิมพ์ก่อน Activate</p></div><span class="text-xs font-bold text-slate-500">${selected.templates.length} รายการ</span></div><div class="divide-y">${selected.templates.map(row=>`<div class="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><div class="font-bold text-slate-800">${escHtml(row.TemplateName)}</div><div class="mt-1 text-xs text-slate-500">${escHtml(row.SafetyUnitName||'ค่าเริ่มต้นทั้งแผนก')} · ${escHtml(row.Status)} · ${n(row.WidthMM)}×${n(row.HeightMM)} mm · ลำดับ ${n(row.DisplayOrder)}</div></div><div class="flex flex-wrap gap-2"><button data-card-designer-department="${row.id}" class="rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-xs font-bold text-violet-700">Visual Designer</button><button data-dept-template-preview="${row.id}" class="rounded-lg border px-3 py-2 text-xs font-bold text-sky-700">Preview + Readiness</button>${row.Status==='Draft'?`<button data-dept-template-action="activate" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Activate</button>`:''}${row.Status==='Active'&&selected.qr?`<button data-dept-template-print="${row.id}" class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-black text-white">พิมพ์ / บันทึก</button>`:''}${row.Status!=='Archived'?`<button data-dept-template-action="archive" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600">Archive</button>`:''}</div></div>`).join('')||'<div class="p-8 text-center text-sm text-slate-400">ยังไม่มี Template สำหรับแผนกนี้</div>'}</div></div>
        </div>
      </div>
    </section>`;
}
...
function departmentCardsAdminView() {
    ...
    return `<div class="space-y-5" data-card-guided-workflow="department">
      ...
      ${masterArtworkView('Department')}
      ${departmentConfigurationView()}
      ${departmentCardOutputHistoryView()}
    </div>`;
}

function departmentCardOutputHistoryView(){
    const f=state.departmentPrintFilters,rows=state.departmentPrints||[];
    return `<section class="rounded-2xl border border-slate-200 bg-white overflow-hidden" data-department-output-history><div class="p-5 border-b space-y-3"><div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3"><div><h3 class="font-black text-slate-800">บัตร Department ที่ออกและพิมพ์แล้ว</h3><p class="mt-1 text-xs text-slate-500">ดูย้อนหลังทีละ 20 รายการ พร้อม Template, รุ่น QR, Print Snapshot และผู้ดำเนินการ · พิมพ์ใหม่ได้เฉพาะ Template และ QR ที่ยัง Active</p></div><span class="text-xs font-black text-emerald-700">${n(state.listMeta.departmentPrints?.total)} งานพิมพ์</span></div><div class="grid sm:grid-cols-[1fr_260px] gap-2"><input data-list-search="department-prints" value="${escHtml(f.q)}" type="search" placeholder="ค้นหา Template แผนก Unit รุ่น QR หรือผู้พิมพ์" class="rounded-xl border px-3 py-2.5 text-sm"><select data-department-print-filter="departmentId" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกแผนก</option>${masterFilterOptions(f.departmentId)}</select></div></div><div class="divide-y">${rows.map(departmentPrintRow).join('')||'<div class="p-8 text-center text-sm text-slate-400">ยังไม่มีประวัติงานพิมพ์ตามตัวกรอง</div>'}</div>${listPager('department-prints',state.listMeta.departmentPrints,'งานพิมพ์')}</section>`;
}

function departmentPrintRow(row){
    const active=row.TemplateStatus==='Active'&&!n(row.TemplateDeleted)&&row.QRStatus==='Active';
    return `<article class="p-4 grid lg:grid-cols-[minmax(0,1fr)_auto] gap-3"><div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><span class="font-black text-slate-800">${escHtml(row.TemplateName)}</span><span class="rounded-full px-2 py-1 text-[10px] font-black ${active?'bg-emerald-100 text-emerald-700':'bg-slate-100 text-slate-600'}">${active?'พร้อมพิมพ์ใหม่':'ประวัติเท่านั้น'}</span>${row.SnapshotID?'<span class="rounded-full bg-violet-50 px-2 py-1 text-[10px] font-black text-violet-700">Designer Snapshot</span>':'<span class="rounded-full bg-slate-50 px-2 py-1 text-[10px] font-black text-slate-500">Legacy output</span>'}</div><div class="mt-1 text-xs text-slate-500">${escHtml(row.DepartmentName)} / ${escHtml(row.SafetyUnitName||'ค่าเริ่มต้นทั้งแผนก')} · QR รุ่น ${n(row.QRGeneration)} (${escHtml(row.QRTokenFingerprint)})</div><div class="mt-2 text-xs text-slate-400">พิมพ์ ${n(row.Copies)} ใบ · ${escHtml(row.PaperSize)} · ${fmtDateTime(row.PrintedAt)} · โดย ${escHtml(row.PrintedByName||row.PrintedBy)}</div>${row.SnapshotID?`<div class="mt-1 text-[11px] text-violet-600">Snapshot #${n(row.SnapshotID)} · Layout #${n(row.LayoutVersionID)} · Contract ${escHtml(String(row.RenderContractHash||'').slice(0,12))}</div>`:''}</div><div class="flex flex-wrap items-center gap-2">${active?`<button type="button" data-department-output-preview="${n(row.TemplateID)}" class="min-h-11 rounded-xl border px-3 text-xs font-black text-sky-700">Preview ปัจจุบัน</button><button type="button" data-department-output-print="${n(row.TemplateID)}" class="min-h-11 rounded-xl bg-emerald-600 px-3 text-xs font-black text-white">พิมพ์ / บันทึกใหม่</button>`:'<span class="text-xs font-bold text-slate-400">Template/QR เดิมไม่ Active</span>'}</div></article>`;
}
...
function personalCardsView() {
    ...
    const issueRows=selectedPersonalIssueRows(),issueCount=issueRows.length,issueTemplate=cardTemplateRecord('personal',n(state.personalIssue.templateId)),incompatible=issueTemplate?issueRows.filter(row=>!personalTemplateMatchesEmployee(issueTemplate,row)):[];
    const selectablePage=state.cardEmployees.filter(row=>!row.ActiveCardID),selectedPage=selectablePage.filter(row=>state.personalIssue.selected.has(String(row.EmployeeID))).length;
    const printablePage=state.cards.filter(row=>row.Status==='Active'),selectedPrintPage=printablePage.filter(row=>state.personalReprint.selected.has(String(row.id))).length,printCount=state.personalReprint.selected.size;
    return `<div class="space-y-5">
      ...
      ${masterArtworkView('Personal')}
      <section class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <form id="bbs-template-form" data-template-scope-form data-template-kind="Personal" class="rounded-2xl border border-slate-200 bg-white p-5 space-y-3"><div><h3 class="font-black text-slate-800">Personal Card Template</h3><p class="text-xs text-slate-500 mt-1">ระบบเลือก Scoped Front ตามแผนก/Unit และใช้ Global Back ร่วมกัน ไม่ต้องอัปโหลดภาพซ้ำ</p></div><label class="block text-[11px] font-bold text-slate-600">ชื่อ Template<input name="templateName" required maxlength="160" placeholder="เช่น BBS Personal Card" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"></label><div class="grid grid-cols-1 sm:grid-cols-3 gap-2"><select name="departmentId" required data-master-source="departments" aria-label="ขอบเขตแผนกของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">เลือกแผนก</option>${masterDepartmentOptions()}</select><select name="safetyUnitId" aria-label="ขอบเขต Unit ของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm" disabled><option value="">ค่าเริ่มต้นทั้งแผนก</option></select><select name="bbsLevel" data-master-source="bbs-levels" aria-label="ระดับ BBS ของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">ทุกระดับที่ออก Personal Card ได้</option>${personalCardBbsLevelOptions()}</select></div><p class="text-[11px] font-semibold text-slate-500">Personal Card รองรับ Group Leader ขึ้นไปเท่านั้น; เลือก “ทุกระดับ” เพื่อใช้ Template เดียวกับผู้มีสิทธิ์ทุกระดับ</p><div data-template-artwork-match role="status" aria-live="polite"></div>${cardSizeEditor()}<label class="flex items-center gap-2 text-xs font-bold text-slate-600"><input name="includeEmployeeId" type="checkbox" checked> แสดง EmployeeID บนบัตร</label><label data-legacy-fallback-field class="hidden block text-[11px] font-bold text-slate-600">Legacy fallback ชั่วคราว (ใช้เฉพาะเมื่อยังไม่มี Personal Scoped Front)<input name="template" aria-label="ไฟล์สำรอง Card Template" type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 block w-full text-xs text-slate-500"></label><button class="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white">สร้าง Personal Template Draft</button></form>
      ...
      <section class="rounded-2xl border border-slate-200 bg-white overflow-hidden"><div class="p-5 border-b space-y-3"><div class="flex flex-col lg:flex-row lg:items-end justify-between gap-3"><div><h3 class="font-black text-slate-800">ออกบัตร Personal ใหม่</h3><p class="text-xs text-slate-500 mt-1">ใช้สำหรับออกบัตรใหม่ให้พนักงานที่ยังไม่มีบัตร Active · ระบบตรวจขอบเขต Template และทำทั้งชุดแบบ Transaction</p></div><div class="flex flex-col sm:flex-row gap-2"><select id="bbs-issue-template" aria-label="เลือก Active Personal Card Template สำหรับออกบัตร" class="rounded-xl border border-slate-200 px-3 py-2 text-sm"><option value="">เลือก Active Template</option>${activeTemplates.map(t => `<option value="${t.id}" ${n(state.personalIssue.templateId)===n(t.id)?'selected':''}>${escHtml(t.TemplateName)} · ${escHtml(t.DepartmentName || 'ทุกแผนก')} · ${escHtml(t.BBSLevel || 'ทุกระดับ')}</option>`).join('')}</select><button data-bbs-issue class="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-black text-white">ตรวจและออกบัตรใหม่ ${issueCount?issueCount:''} ใบ</button></div></div><div class="grid sm:grid-cols-[1fr_240px] gap-2"><input data-list-search="card-employees" value="${escHtml(state.cardEmployeeFilters.q)}" type="search" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm" placeholder="ค้นหารหัส ชื่อ แผนก Unit หรือตำแหน่ง"><select data-card-employee-filter="departmentId" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกแผนก</option>${masterFilterOptions(state.cardEmployeeFilters.departmentId)}</select></div><div class="flex flex-col gap-3 rounded-2xl border ${incompatible.length?'border-amber-200 bg-amber-50':'border-emerald-200 bg-emerald-50/50'} p-3 sm:flex-row sm:items-center sm:justify-between"><div><div class="text-sm font-black text-slate-800">เลือกออกบัตรใหม่ ${issueCount} / 100 คน</div><div class="mt-1 text-xs ${incompatible.length?'text-amber-800':'text-slate-500'}">${!issueCount?'ยังไม่ได้เลือกพนักงาน':!issueTemplate?'เลือก Active Template เพื่อ Preflight ทั้งชุด':incompatible.length?`มี ${incompatible.length} คนไม่ตรงขอบเขต Template — ระบบจะยังไม่ออกบัตร`:`พร้อมตรวจ Preview และออกบัตรใหม่ ${issueCount} ใบในงานเดียว`}</div></div><div class="flex flex-wrap gap-2"><button type="button" data-card-select-page class="min-h-11 rounded-xl border border-emerald-300 bg-white px-3 text-xs font-black text-emerald-700">${selectablePage.length&&selectedPage===selectablePage.length?'ยกเลิกทั้งหน้านี้':'เลือกทั้งหน้านี้'} (${selectablePage.length})</button><button type="button" data-card-selection-clear class="min-h-11 rounded-xl border bg-white px-3 text-xs font-black text-slate-600" ${issueCount?'':'disabled'}>ล้างทั้งหมด</button></div></div></div><div id="bbs-card-employees" class="grid md:grid-cols-2 xl:grid-cols-3 gap-3 p-5 max-h-[34rem] overflow-y-auto">${state.cardEmployees.map(employeeCardChoice).join('') || personalCardEmployeeEmptyState()}</div>${listPager('card-employees',state.listMeta.cardEmployees,'พนักงาน')}</section>
      <section class="rounded-2xl border border-slate-200 bg-white overflow-hidden"><div class="p-5 border-b space-y-3"><div class="flex justify-between gap-3"><div><h3 class="font-black text-slate-800">บัตร Personal ที่ออกแล้ว</h3><p class="text-xs text-slate-500 mt-1">เลือกบัตร Active หลายใบเพื่อ Replace + Print พร้อมกัน · QR เดิมจะถูกยกเลิกและหมุน QR ใหม่ทั้งชุดแบบ Transaction</p></div><span class="text-xs font-bold text-emerald-700">${n(state.listMeta.cards?.total)} รายการ</span></div><div class="grid sm:grid-cols-3 gap-2"><input data-list-search="cards" value="${escHtml(state.cardFilters.q)}" type="search" placeholder="ค้นหาพนักงาน Template หรือ QR fingerprint" class="rounded-xl border px-3 py-2.5 text-sm"><select data-card-filter="status" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกสถานะ</option>${['Active','Revoked','Replaced'].map(v=>`<option ${state.cardFilters.status===v?'selected':''}>${v}</option>`).join('')}</select><select data-card-filter="departmentId" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกแผนก</option>${masterFilterOptions(state.cardFilters.departmentId)}</select></div><div class="flex flex-col gap-3 rounded-2xl border border-violet-200 bg-violet-50/50 p-3 sm:flex-row sm:items-center sm:justify-between"><div><div class="text-sm font-black text-slate-800">เลือกพิมพ์แล้ว ${printCount} / 100 ใบ</div><div class="mt-1 text-xs text-violet-700">เลือกได้เฉพาะบัตร Active · การพิมพ์จะ Replace และหมุน Personal QR ใหม่เพื่อรักษาความปลอดภัย</div></div><div class="flex flex-wrap gap-2"><button type="button" data-card-print-select-page class="min-h-11 rounded-xl border border-violet-300 bg-white px-3 text-xs font-black text-violet-700">${printablePage.length&&selectedPrintPage===printablePage.length?'ยกเลิกทั้งหน้านี้':'เลือก Active ทั้งหน้านี้'} (${printablePage.length})</button><button type="button" data-card-print-selection-clear class="min-h-11 rounded-xl border bg-white px-3 text-xs font-black text-slate-600" ${printCount?'':'disabled'}>ล้างทั้งหมด</button><button type="button" data-bbs-card-batch-replace-print class="min-h-11 rounded-xl bg-violet-700 px-4 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-50" ${printCount?'':'disabled'}>Replace + พิมพ์ ${printCount} ใบ</button></div></div>${personalBatchPrintControls()}</div><div class="divide-y">${state.cards.map(activeCardRow).join('') || `<div class="p-8 text-center text-sm text-slate-400">ไม่พบบัตรตามตัวกรอง</div>`}</div>${listPager('cards',state.listMeta.cards,'บัตร')}</section>
    </div>`;
}
...
function templateRow(row) { const levelReady=personalTemplateLevelEligible(row);return `<div class="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div class="min-w-0"><div class="font-bold text-slate-800 truncate">${escHtml(row.TemplateName)}</div><div class="text-xs text-slate-500 mt-1">${escHtml(row.DepartmentName || 'ยังไม่กำหนดแผนก')} · ${escHtml(row.SafetyUnitName||'ค่าเริ่มต้นทั้งแผนก')} · ${escHtml(row.BBSLevel || 'ทุกระดับ')} · ${n(row.WidthMM).toFixed(2)}×${n(row.HeightMM).toFixed(2)} mm · ${escHtml(row.Status)}</div>${levelReady?'':`<div class="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-2 text-[11px] font-bold text-amber-800">ระดับ ${escHtml(row.BBSLevel)} ใช้ออก Personal Card ไม่ได้ เพราะผู้รับบัตรเริ่มที่ Group Leader</div>`}</div><div class="flex flex-wrap gap-2"><button data-card-designer-personal="${row.id}" class="rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-xs font-bold text-violet-700">Visual Designer</button><button data-bbs-template-preview="${row.id}" class="rounded-lg border px-3 py-2 text-xs font-bold text-sky-700">Preview + Readiness</button>${!levelReady?`<button data-bbs-template-level-repair="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-black text-amber-800">แก้เป็น Group Leader</button>`:''}${row.Status === 'Draft'&&levelReady ? `<button data-bbs-template-action="activate" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Activate</button>` : ''}${row.Status !== 'Archived' ? `<button data-bbs-template-action="archive" data-template-id="${row.id}" data-row-version="${row.RowVersion}" class="rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600">Archive</button>` : ''}</div></div>`; }
function employeeCardChoice(row) { const search=escHtml([row.EmployeeID,row.EmployeeName,row.Department,row.Unit,row.Position].join(' ').toLowerCase()),selected=state.personalIssue.selected.has(String(row.EmployeeID)); return `<label data-card-search="${search}" class="rounded-xl border ${row.ActiveCardID ? 'border-emerald-200 bg-emerald-50/40' : selected?'border-emerald-400 bg-emerald-50':'border-slate-200'} p-4 flex gap-3"><input type="checkbox" data-card-employee="${escHtml(row.EmployeeID)}" ${selected?'checked':''} ${row.ActiveCardID ? 'disabled' : ''} class="mt-1"><div class="min-w-0"><div class="font-bold text-slate-800 truncate">${escHtml(row.EmployeeName)}</div><div class="text-xs text-slate-500 mt-1">${escHtml(row.EmployeeID)} · ${escHtml(row.BBSLevel)}</div><div class="text-xs text-slate-400 mt-1 truncate">${escHtml(row.Department || '-')} / ${escHtml(row.Unit || '-')}</div>${row.ActiveCardID ? `<span class="inline-flex mt-2 rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-black text-emerald-700">มีบัตร Active · เลือกออกซ้ำไม่ได้</span>` : selected?'<span class="inline-flex mt-2 rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-black text-emerald-700">อยู่ในชุดออกบัตรใหม่</span>':''}</div></label>`; }
function activeCardRow(row) { const active=row.Status==='Active',selected=active&&state.personalReprint.selected.has(String(row.id));return `<article class="p-4 grid md:grid-cols-[auto_1fr_auto] gap-3 ${selected?'bg-violet-50/60':''}">${active?`<label class="flex min-h-11 items-start pt-1" title="เลือกเพื่อ Replace + Print"><input type="checkbox" data-card-print-select="${row.id}" ${selected?'checked':''} class="h-4 w-4 accent-violet-700"><span class="sr-only">เลือกบัตรของ ${escHtml(row.EmployeeName)}</span></label>`:'<span></span>'}<div class="min-w-0"><div class="font-bold text-slate-800 truncate">${escHtml(row.EmployeeName)} <span class="font-normal text-slate-400">· ${escHtml(row.EmployeeID)}</span></div><div class="text-xs text-slate-500 mt-1">${escHtml(row.Department || '-')} / ${escHtml(row.Unit || '-')} · ${escHtml(row.TemplateName)} · QR ${escHtml(row.TokenFingerprint)}</div><div class="mt-2 flex flex-wrap items-center gap-2"><span class="inline-flex rounded-full px-2 py-1 text-[10px] font-black ${active?'bg-emerald-100 text-emerald-700':'bg-slate-100 text-slate-600'}">${escHtml(row.Status)}</span><span class="text-[11px] text-slate-400">ออก ${fmtDateTime(row.IssuedAt)} · พิมพ์ ${n(row.PrintCount)} ครั้ง${row.LastPrintedAt?` · ล่าสุด ${fmtDateTime(row.LastPrintedAt)}`:''}</span></div>${row.RevokeReason?`<div class="mt-1 text-[11px] text-rose-600">เหตุผล: ${escHtml(row.RevokeReason)}</div>`:''}</div><div class="flex flex-wrap items-center gap-2"><button type="button" data-bbs-card-detail="${row.id}" class="min-h-11 rounded-xl border px-3 text-xs font-black text-slate-700">รายละเอียด / ประวัติ</button><button type="button" data-bbs-card-preview="${row.id}" class="min-h-11 rounded-xl border px-3 text-xs font-black text-sky-700">Preview ปัจจุบัน</button>${active?`<button data-bbs-card-replace="${row.id}" class="min-h-11 rounded-xl bg-amber-500 px-3 text-xs font-black text-white">Replace + Reprint</button><button data-bbs-card-revoke="${row.id}" class="min-h-11 rounded-xl border border-rose-200 px-3 text-xs font-black text-rose-600">บัตรสูญหาย / Revoke</button>`:''}</div></article>`; }

function historyRows() {
    if (!state.history.length) return `<div class="p-10 text-center"><div class="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-100 font-black text-slate-500">i</div><h4 class="mt-4 font-black text-slate-800">ไม่พบประวัติในมุมมองนี้</h4><p class="mt-1 text-sm text-slate-500">ลองเปลี่ยนมุมมอง ปี หรือคืนค่าตัวกรอง</p><button type="button" data-history-reset class="mt-4 min-h-11 rounded-xl border border-slate-300 px-4 text-sm font-black text-slate-700">ล้างตัวกรอง</button></div>`;
    const currentEmployeeId = String(state.context?.employee?.EmployeeID || '');
    return `<div data-history-timeline class="divide-y">${state.history.map(row => { const canResume=row.Status==='Draft'&&String(row.ObserverEmployeeID)===currentEmployeeId,personLabel=state.view==='observed'?'ผู้ตรวจ':state.view==='team'?'ผู้ถูกสังเกต':'ผู้ถูกสังเกต',personName=state.view==='observed'?row.ObserverNameSnapshot:row.ObservedNameSnapshot;return `<article class="relative grid gap-3 p-4 hover:bg-slate-50 md:p-5 sm:grid-cols-[minmax(0,1fr)_auto]"><span aria-hidden="true" class="absolute bottom-0 left-5 top-0 w-px bg-slate-200"></span><span aria-hidden="true" class="absolute left-[1.05rem] top-6 h-2.5 w-2.5 rounded-full ${row.Status==='Draft'?'bg-amber-500':'bg-emerald-500'} ring-4 ring-white"></span><button type="button" data-bbs-detail="${row.id}" class="min-w-0 pl-5 text-left"><div class="flex flex-wrap items-center gap-2"><span class="font-black text-slate-800">${escHtml(row.ObservationNo)}</span>${statusBadge(row.Status)}<span class="text-xs text-slate-400">${fmtDate(row.ObservationDate)}</span></div><div class="mt-2 text-sm font-bold text-slate-700">${personLabel}: ${escHtml(personName||'-')}</div><div class="mt-1 text-xs text-slate-500">${escHtml(row.ObservedDepartmentSnapshot||'-')} / ${escHtml(row.ObservedUnitSnapshot||'-')} · ผู้ตรวจ ${escHtml(row.ObserverNameSnapshot||'-')}</div><div class="mt-3 flex flex-wrap gap-2"><span class="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-black text-emerald-700">Safe ${n(row.SafeCount)}</span><span class="rounded-full bg-rose-50 px-2.5 py-1 text-xs font-black text-rose-700">Unsafe ${n(row.UnsafeCount)}</span></div></button><div class="flex items-center gap-2 pl-5 sm:pl-0">${canResume?`<button type="button" data-bbs-resume="${row.id}" class="min-h-11 rounded-xl bg-amber-500 px-4 text-xs font-black text-white">ทำ Draft ต่อ</button>`:`<button type="button" data-bbs-detail="${row.id}" class="min-h-11 rounded-xl border border-slate-300 px-4 text-xs font-black text-slate-700">ดูรายละเอียด</button>`}</div></article>`; }).join('')}</div>`;
}
...
function bind() {
    ...
    installTemplateLifecycleReadiness();
    installDepartmentWorkflowStatus();
    document.querySelector('[data-bbs-skip-link]')?.addEventListener('click', event => {
    ...
    document.querySelectorAll('[data-bbs-group]').forEach(button=>button.onclick=()=>switchBbsTab(button.dataset.bbsGroupTarget));
    ...
    document.querySelectorAll('[data-overview-open]').forEach(button => button.onclick = () => switchBbsTab(button.dataset.overviewOpen));
    document.querySelectorAll('[data-open-actions]').forEach(button => bindBusy(button,'onclick',async()=>{state.tab='actions';state.actionScope=button.dataset.actionScope||'overdue';await trackSectionLoad('actions',loadActions);render({preserveScroll:false,focusSelector:`#${activeTabId()}`});scrollBbsContentStart();},'กำลังเปิด Corrective Action...'));
    document.querySelectorAll('[data-action-detail]').forEach(btn => btn.onclick = () => openActionDetail(n(btn.dataset.actionDetail), btn));
    document.querySelectorAll('[data-action-queue]').forEach(button=>bindBusy(button,'onclick',()=>selectActionQueue(button.dataset.actionQueue),'กำลังโหลด...'));
    document.querySelectorAll('[data-action-reset]').forEach(button=>bindBusy(button,'onclick',resetActionWorkspace,'กำลังโหลด...'));
    ...
    document.querySelectorAll('[data-bbs-start]').forEach(btn => bindBusy(btn,'onclick',()=>startObservation(btn.dataset.bbsStart),'กำลังสร้าง Draft...'));
    document.querySelectorAll('[data-bbs-resume]').forEach(btn => bindBusy(btn,'onclick',()=>resumeDraft(n(btn.dataset.bbsResume)),'กำลังเปิด Draft...'));
    ...
    document.querySelectorAll('[data-batch-upload]').forEach(input=>input.onchange=()=>withBusy(input,operation=>uploadBatchEvidence(n(input.dataset.observationId),n(input.dataset.batchUpload),input.files?.[0],operation),'กำลังอัปโหลดหลักฐาน...'));
    ...
    document.querySelectorAll('[data-bbs-detail]').forEach(btn => bindBusy(btn,'onclick',()=>openDetail(n(btn.dataset.bbsDetail),btn),'กำลังเปิดรายละเอียด...'));
    ...
    document.querySelectorAll('[data-history-reset]').forEach(button=>bindBusy(button,'onclick',resetHistoryWorkspace,'กำลังโหลด...'));
    ...
    document.querySelector('[data-single-back]')?.addEventListener('click', singleWizardBack);
    document.querySelectorAll('[data-single-jump]').forEach(button=>button.onclick=()=>{collectDraft();state.singleStep=n(button.dataset.singleJump)||1;render({preserveScroll:false});scrollBbsContentStart();});
    bindBusy(document.querySelector('[data-single-next]'), 'onclick', singleWizardNext, 'กำลังบันทึก...');
    ...
    document.querySelectorAll('[data-bbs-upload]').forEach(input => input.onchange = () => withBusy(input,operation=>uploadEvidence(n(input.dataset.bbsUpload),input.files?.[0],operation),'กำลังอัปโหลดหลักฐาน...'));
    document.querySelectorAll('[data-bbs-file]').forEach(btn => bindBusy(btn,'onclick',()=>openEvidence(n(btn.dataset.bbsFile)),'กำลังเปิดหลักฐาน...'));
    document.querySelectorAll('[data-bbs-file-delete]').forEach(btn => bindBusy(btn,'onclick',()=>deleteEvidence(n(btn.dataset.bbsFileDelete)),'กำลังลบ...'));
    bindCardSizeEditors();
    bindTemplateScopeForms();
    ...
    installPersonalIssueCompatibility();
    document.querySelectorAll('[data-personal-print-option]').forEach(input=>input.onchange=()=>{const key=input.dataset.personalPrintOption,value=['backOffsetXMM','backOffsetYMM'].includes(key)?Math.max(-5,Math.min(5,n(input.value))):input.value;state.personalReprint[key]=value;input.value=String(value);});
    document.querySelector('[data-card-select-page]')?.addEventListener('click',()=>{const rows=state.cardEmployees.filter(row=>!row.ActiveCardID),allSelected=rows.length>0&&rows.every(row=>state.personalIssue.selected.has(String(row.EmployeeID)));if(allSelected)rows.forEach(row=>state.personalIssue.selected.delete(String(row.EmployeeID)));else{let omitted=0;for(const row of rows){const key=String(row.EmployeeID);if(state.personalIssue.selected.has(key))continue;if(state.personalIssue.selected.size>=100){omitted++;continue;}state.personalIssue.selected.set(key,{...row});}if(omitted)showToast(`เลือกได้สูงสุด 100 คน · ไม่ได้เพิ่ม ${omitted} คน`,'error');}render();});
    document.querySelector('[data-card-selection-clear]')?.addEventListener('click',()=>{state.personalIssue.selected.clear();render();});
    bindBusy(document.querySelector('[data-bbs-issue]'), 'onclick', () => issueCards(), 'กำลังออกบัตร...');
    document.querySelector('[data-card-print-select-page]')?.addEventListener('click',()=>{const rows=state.cards.filter(row=>row.Status==='Active'),allSelected=rows.length>0&&rows.every(row=>state.personalReprint.selected.has(String(row.id)));if(allSelected)rows.forEach(row=>state.personalReprint.selected.delete(String(row.id)));else{let omitted=0;for(const row of rows){const key=String(row.id);if(state.personalReprint.selected.has(key))continue;if(state.personalReprint.selected.size>=100){omitted++;continue;}state.personalReprint.selected.set(key,{...row});}if(omitted)showToast(`เลือกได้สูงสุด 100 ใบ · ไม่ได้เพิ่ม ${omitted} ใบ`,'error');}render();});
    document.querySelector('[data-card-print-selection-clear]')?.addEventListener('click',()=>{state.personalReprint.selected.clear();render();});
    document.querySelectorAll('[data-card-print-select]').forEach(input=>input.onchange=()=>{const row=state.cards.find(item=>n(item.id)===n(input.dataset.cardPrintSelect));if(!row||row.Status!=='Active'){input.checked=false;return;}const key=String(row.id);if(input.checked){if(state.personalReprint.selected.size>=100&&!state.personalReprint.selected.has(key)){input.checked=false;return showToast('เลือกบัตรได้สูงสุด 100 ใบต่อ Batch','error');}state.personalReprint.selected.set(key,{...row});}else state.personalReprint.selected.delete(key);render();});
    bindBusy(document.querySelector('[data-bbs-card-batch-replace-print]'),'onclick',replaceSelectedCards,'กำลัง Replace และจัดหน้าพิมพ์...');
    ...
    document.querySelectorAll('[data-bbs-card-detail]').forEach(btn => bindBusy(btn,'onclick',()=>openPersonalCardDetail(n(btn.dataset.bbsCardDetail),btn),'กำลังเปิดประวัติบัตร...'));
    document.querySelectorAll('[data-bbs-card-preview]').forEach(btn => bindBusy(btn,'onclick',()=>previewIssuedPersonalCard(n(btn.dataset.bbsCardPreview)),'กำลังสร้าง Preview...'));
    ...
    document.querySelectorAll('[data-department-print-filter]').forEach(input=>input.onchange=async()=>{state.departmentPrintFilters[input.dataset.departmentPrintFilter]=input.value;state.departmentPrintFilters.page=1;await trackSectionLoad('cards',loadDepartmentPrintRows);render();});
    ...
    document.querySelectorAll('[data-analytics-reset]').forEach(button=>bindBusy(button,'onclick',resetAnalyticsWorkspace,'กำลังโหลด...'));
    document.querySelectorAll('[data-analytics-export]').forEach(btn=>bindBusy(btn,'onclick',()=>exportAnalytics(btn.dataset.analyticsExport),'กำลังจัดเตรียมรายงาน...'));
    document.querySelectorAll('[data-analytics-drilldown]').forEach(btn=>bindBusy(btn,'onclick',()=>openAnalyticsDrilldown(btn.dataset.analyticsDrilldown,btn),'กำลังเปิดรายละเอียด...'));
    ...
    document.querySelectorAll('[data-community-risk-detail]').forEach(btn=>bindBusy(btn,'onclick',()=>openCommunityRiskDetail(n(btn.dataset.communityRiskDetail),btn),'กำลังเปิดรายละเอียด...'));
    bindBusy(document.getElementById('bbs-inspector-enroll-form'), 'onsubmit', appointInspector, 'กำลังแต่งตั้ง...');
    document.querySelectorAll('[data-inspector-open]').forEach(btn=>bindBusy(btn,'onclick',()=>openInspectorTeam(n(btn.dataset.inspectorOpen),btn),'กำลังเปิดข้อมูลทีม...'));
    document.querySelectorAll('[data-inspector-modal-close]').forEach(btn=>btn.onclick=()=>closeInspectorTeamModal());
    document.querySelectorAll('[data-inspector-team-modal]').forEach(modal=>{modal.onclick=event=>{if(event.target===modal)closeInspectorTeamModal();};modal.onkeydown=event=>{if(event.key==='Escape'){event.preventDefault();closeInspectorTeamModal();return;}if(event.key!=='Tab')return;const dialog=modal.querySelector('[role="dialog"]'),items=[...(dialog?.querySelectorAll('button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')||[])];if(!items.length)return;const first=items[0],last=items.at(-1);if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus();}};requestAnimationFrame(()=>modal.querySelector('[role="dialog"]')?.focus());});
...
function render(options = {}) {
    ...
    const scrollTop = options.restoreScroll ? n(state.restoredScrollTop) : (preserveScroll ? n(container?.scrollTop) : 0);
    const focusSelector = options.focusSelector || focusSelectorFor(document.activeElement);
    page.innerHTML = shell();
    bind();
    enhanceAccessibility();
    persistBbsUiState(scrollTop);
    requestAnimationFrame(() => {
        if ((preserveScroll||options.restoreScroll) && container) container.scrollTop = scrollTop;
...
async function switchBbsTab(nextTab) {
    ...
    if (state.tab === 'start') {
        try {
            ...
            showToast('บันทึกฉบับร่างปัจจุบันไม่สำเร็จ กรุณาอยู่ในหน้านี้แล้วลองอีกครั้ง', 'error');
            return;
        }
    }
...
function collectDraft() {
    ...
    const answers = state.draft.answers.map(a => {
        const responseInput=form?.querySelector(`[name="response-${a.id}"]:checked`),remarkInput=form?.querySelector(`[name="remark-${a.id}"]`),actionInput=form?.querySelector(`[name="action-${a.id}"]`);
        return { answerId:n(a.id), response:responseInput?responseInput.value:(a.Response||null), remark:remarkInput?remarkInput.value:(a.Remark||''), immediateAction:actionInput?actionInput.value:(a.ImmediateAction||'') };
    });
    ...
    if(form?.elements?.generalRemark)state.draft.GeneralRemark=form.elements.generalRemark.value;
    return answers;
}
...
async function leaveSingleDraft() {
    try {
        ...
        state.singleStep = 1;
        render();
        showToast('บันทึกฉบับร่างแล้ว คุณกลับมาทำต่อได้จากหน้านี้หรือแท็บประวัติ', 'success');
    } catch (error) {
        showToast(error?.message || 'บันทึกฉบับร่างไม่สำเร็จ', 'error');
    }
}
...
async function resumeDraft(id) {
    try {
        ...
        if (draft?.Status !== 'Draft') throw new Error('Observation นี้ไม่ใช่ฉบับร่างแล้ว');
        if (String(draft.ObserverEmployeeID) !== String(state.context?.employee?.EmployeeID || '')) throw new Error('เฉพาะผู้สังเกตที่สร้างฉบับร่างเท่านั้นที่ทำรายการต่อได้');
        state.draft = draft;
        state.singleStep = 1;
        ...
        showToast(error?.message || 'เปิดฉบับร่างเพื่อทำต่อไม่สำเร็จ', 'error');
    }
}
...
async function startObservation(employeeId) {
    try {
        ...
        if (existing) { await resumeDraft(n(existing.id)); showToast('เปิดฉบับร่างเดิมให้ทำต่อแล้ว เพื่อไม่สร้างรายการซ้ำ', 'success'); return; }
...
async function uploadBatchEvidence(observationId,answerId,file,operation){
    if(!file)return;try{operation?.update({label:'กำลังเตรียมรูปหลักฐาน...',detail:'ตรวจสอบและลดขนาดรูปก่อนอัปโหลด'});await saveBatchDraft(false);const compressed=await compressObservationImage(file),data=new FormData();data.append('answerId',String(answerId));data.append('evidence',compressed,compressed.name||file.name);await API.upload(`/bbs/observations/${observationId}/evidence`,data,{onProgress:uploadProgress(operation,'กำลังอัปโหลดหลักฐาน...')});operation?.update({label:'กำลังประมวลผลหลักฐาน...',detail:'อัปโหลดครบแล้ว กำลังโหลดข้อมูลล่าสุด',progress:100});state.batchDraft=(await API.get(`/bbs/batch-observations/${state.batchDraft.id}`)).data;saveBatchRecovery();showToast(compressed.size<file.size?'อัปโหลดและลดขนาดรูปแล้ว':'อัปโหลดหลักฐานแล้ว','success');render();}catch(error){showToast(error?.message||'อัปโหลดหลักฐานไม่สำเร็จ','error');}
}

function validateClient() {
    return validateClientAnswers();
}

function validateClientAnswers(answerIds = null) {
    const answers = collectDraft();
    const included=answerIds?new Set(answerIds.map(n)):null;
    const groups=singleObservationGroups(state.draft);
    for (const answer of answers) {
        if(included&&!included.has(n(answer.answerId)))continue;
        ...
        const step=Math.max(1,groups.findIndex(group=>group.answers.some(item=>n(item.id)===n(answer.answerId)))+1),base={step};
        if (n(rule.IsRequiredSnapshot) && !answer.response) return { ...base, message:`กรุณาตอบ ${rule.ItemCodeSnapshot}`, selector:`[data-answer-card="${answer.answerId}"]` };
        if (answer.response === 'Unsafe' && n(rule.UnsafeRequiresRemarkSnapshot) && !answer.remark.trim()) return { ...base, message:`กรุณาระบุหมายเหตุ Unsafe: ${rule.ItemCodeSnapshot}`, selector:`#bbs-remark-${answer.answerId}` };
        if (answer.response === 'Unsafe' && n(rule.UnsafeRequiresActionSnapshot) && !answer.immediateAction.trim()) return { ...base, message:`กรุณาระบุการแก้ไขทันที: ${rule.ItemCodeSnapshot}`, selector:`#bbs-action-${answer.answerId}` };
        if (answer.response === 'Unsafe' && n(rule.UnsafeRequiresPhotoSnapshot) && !files.length) return { ...base, message:`กรุณาแนบหลักฐาน: ${rule.ItemCodeSnapshot}`, selector:`[data-bbs-upload-trigger="${answer.answerId}"]` };
    }
    return null;
}

function singleWizardBack(){collectDraft();if(state.singleStep>1){state.singleStep-=1;render({preserveScroll:false});scrollBbsContentStart();return;}leaveSingleDraft();}

async function singleWizardNext(){
    collectDraft();
    const groups=singleObservationGroups(state.draft),current=groups[Math.max(0,state.singleStep-1)],issue=validateClientAnswers((current?.answers||[]).map(answer=>n(answer.id)));
    if(issue){focusValidationTarget(issue.selector);showToast(issue.message,'error');return;}
    try{await saveDraft(false,false);state.singleStep=Math.min(groups.length+1,state.singleStep+1);render({preserveScroll:false});scrollBbsContentStart();}catch(error){showToast(error?.message||'บันทึก Draft ก่อนเปลี่ยนขั้นตอนไม่สำเร็จ','error');}
}
...
async function submitObservation() {
    try { const issue = validateClient(); if (issue) { state.singleStep=issue.step;render({preserveScroll:false});focusValidationTarget(issue.selector);return showToast(issue.message, 'error'); } await saveDraft(false); const result = await API.post(`/bbs/observations/${state.draft.id}/submit`, { rowVersion:n(state.draft.RowVersion) }); showToast(result.reused ? 'รายการนี้ส่งแล้ว' : 'ส่ง Observation สำเร็จ', 'success'); state.draft = null;state.singleStep=1; state.tab = 'workspace'; await loadData(); render({preserveScroll:false});scrollBbsContentStart(); } catch (error) { showToast(error?.message || 'ส่ง Observation ไม่สำเร็จ', 'error'); }
}

async function uploadEvidence(answerId, file, operation) {
    if (!file) return;
    try { operation?.update({label:'กำลังเตรียมหลักฐาน...',detail:'บันทึก Draft ก่อนแนบไฟล์'});await saveDraft(false); const data = new FormData(); data.append('answerId', String(answerId)); data.append('evidence', file); await API.upload(`/bbs/observations/${state.draft.id}/evidence`, data,{onProgress:uploadProgress(operation,'กำลังอัปโหลดหลักฐาน...')});operation?.update({label:'กำลังประมวลผลหลักฐาน...',detail:'อัปโหลดครบแล้ว กำลังโหลดข้อมูลล่าสุด',progress:100}); state.draft = (await API.get(`/bbs/observations/${state.draft.id}`)).data; showToast('อัปโหลดหลักฐานแล้ว', 'success'); render(); } catch (error) { showToast(error?.message || 'อัปโหลดไม่สำเร็จ', 'error'); }
}
...
async function openActionDetail(id, returnFocus = document.activeElement) {
    try {
        ...
        modal.querySelectorAll('[data-action-upload]').forEach(input=>input.onchange=()=>withBusy(input,async operation=>{const file=input.files?.[0];if(!file)return;try{const form=new FormData();form.append('evidenceType',input.dataset.actionUpload);form.append('evidence',file);await API.upload(`/bbs/actions/${id}/evidence`,form,{onProgress:uploadProgress(operation,'กำลังอัปโหลดหลักฐาน Action...')});operation.update({label:'กำลังประมวลผลหลักฐาน...',detail:'อัปโหลดครบแล้ว กำลังเปิดข้อมูลล่าสุด',progress:100});close();await openActionDetail(id);await refreshActions();showToast('อัปโหลดหลักฐานแล้ว','success');}catch(error){showToast(error?.message||'อัปโหลดไม่สำเร็จ','error');}},'กำลังอัปโหลดหลักฐาน...'));
        modal.querySelectorAll('[data-action-file]').forEach(btn=>bindBusy(btn,'onclick',()=>openActionEvidence(id,n(btn.dataset.actionFile)),'กำลังเปิดหลักฐาน...'));
...
async function selectActionQueue(scope){state.actionScope=['all','mine','verify','overdue'].includes(scope)?scope:'all';state.actionStatus='';state.actionPriority='';state.actionFilters={...state.actionFilters,year:'',departmentId:'',safetyUnitId:'',q:'',page:1};await trackSectionLoad('actions',loadActions);render({preserveScroll:false,focusSelector:`[data-action-queue="${state.actionScope}"]`});}
async function resetActionWorkspace(){state.actionScope='all';state.actionStatus='';state.actionPriority='';state.actionFilters={...state.actionFilters,year:'',departmentId:'',safetyUnitId:'',q:'',page:1};await trackSectionLoad('actions',loadActions);render({preserveScroll:false,focusSelector:'[data-action-queue="all"]'});}
...
async function resetAnalyticsWorkspace(){state.analyticsFilters={...state.analyticsFilters,departmentId:'',safetyUnitId:'',risk:''};await trackSectionLoad('analytics',loadAnalytics);render({preserveScroll:false,focusSelector:'[data-analytics-controls]'});}
...
async function loadCommunity(){const [cards,dashboard,employees]=await Promise.all([API.get('/bbs/department-cards/me'),loadCommunityDashboard().then(()=>null),API.get('/bbs/community/employees')]);state.departmentCards=cards.data||null;state.communityEmployees=employees.data||{rows:[],units:[]};if(state.context?.permissions?.configure){const[active,trash]=await Promise.all([API.get('/bbs/admin/department-cards'),API.get('/bbs/admin/department-cards?trash=1')]);state.communityAdmin=active.data||state.communityAdmin;state.trashedDepartmentTemplates=trash.data?.templates||[];}}
async function submitCommunityReport(event,operation){event.preventDefault();const form=new FormData(event.currentTarget);try{const result=await API.upload('/bbs/community/reports',form,{onProgress:uploadProgress(operation,'กำลังส่งรายงานและหลักฐาน...')});operation?.update({label:'กำลังประมวลผลรายงาน...',detail:'ส่งข้อมูลครบแล้ว กำลังโหลดรายการล่าสุด',progress:100});showToast(result.data?.reportType==='Risky'?'ส่งรายงานเสี่ยงและสร้าง Corrective Action แล้ว':'เผยแพร่พฤติกรรมดีแล้ว','success');event.currentTarget.reset();await loadCommunity();render();}catch(error){showToast(error?.message||'ส่ง Community Report ไม่สำเร็จ','error');}}
...
async function loadActiveDesignerPreview(kind,id){
    const pathKind=kind==='department'?'department':'personal';
    const versions=(await API.get(`/bbs/admin/card-designer/${pathKind}/${id}/versions`)).data||[];
    const active=versions.find(row=>row.Status==='Active'&&!n(row.IsDeleted));
    if(!active)return null;
    const record=(await API.get(`/bbs/admin/card-designer/versions/${n(active.id)}`)).data;
    if(!record?.layout?.sides?.length)throw new Error('Active Designer Layout ไม่มีข้อมูลด้านหน้าหรือด้านหลัง');
    const layout=record.layout,resources=new Map(),pendingUrls=new Set(),jobs=[];
    for(const side of layout.sides){
        side.backgroundUrl=`/bbs/admin/card-designer/versions/${n(record.id)}/sides/${String(side.side).toLowerCase()}/background`;
    ...
    for(const element of layout.elements||[]){
        if(!n(element.assetId))continue;
        element.assetUrl=`/bbs/admin/card-designer/assets/${n(element.assetId)}/file`;
        if(resources.has(element.assetUrl)||pendingUrls.has(element.assetUrl))continue;
...
function personalTemplateScopeMismatch(template,employee){
    ...
    const mismatch=[];
    if(template.DepartmentID&&n(template.DepartmentID)!==n(employee.DepartmentID))mismatch.push(`แผนก Template: ${template.DepartmentName||template.DepartmentID} / พนักงาน: ${employee.Department||employee.DepartmentID||'-'}`);
    if(template.SafetyUnitID&&n(template.SafetyUnitID)!==n(employee.SafetyUnitID))mismatch.push(`Unit Template: ${template.SafetyUnitName||template.SafetyUnitID} / พนักงาน: ${employee.Unit||employee.SafetyUnitID||'-'}`);
    if(template.BBSLevel&&String(template.BBSLevel)!==String(employee.BBSLevel))mismatch.push(`ระดับ Template: ${template.BBSLevel} / พนักงาน: ${employee.BBSLevel||'-'}`);
    return mismatch;
}
function personalTemplateMatchesEmployee(template,employee){return personalTemplateLevelEligible(template)&&personalTemplateScopeMismatch(template,employee).length===0;}
function selectedPersonalIssueRows(){return[...state.personalIssue.selected.values()];}
function selectedPersonalReprintRows(){return[...state.personalReprint.selected.values()].filter(row=>row.Status==='Active');}
function personalPrintOptions(){return{paperSize:['A4','A5','A6'].includes(state.personalReprint.paperSize)?state.personalReprint.paperSize:'A4',preset:state.personalReprint.preset==='safe'?'safe':'compact',backOffsetXMM:Math.max(-5,Math.min(5,n(state.personalReprint.backOffsetXMM))),backOffsetYMM:Math.max(-5,Math.min(5,n(state.personalReprint.backOffsetYMM)))};}
function personalBatchPrintControls(){const option=personalPrintOptions();return`<fieldset class="rounded-2xl border border-violet-200 bg-violet-50/50 p-3"><legend class="px-2 text-xs font-black text-violet-800">Batch Sheet Layout / Duplex Alignment</legend><div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><label class="text-[11px] font-bold text-slate-600">กระดาษ<select data-personal-print-option="paperSize" class="mt-1 w-full rounded-xl border bg-white px-3 py-2.5 text-sm">${['A4','A5','A6'].map(value=>`<option ${option.paperSize===value?'selected':''}>${value}</option>`).join('')}</select></label><label class="text-[11px] font-bold text-slate-600">การจัดพื้นที่<select data-personal-print-option="preset" class="mt-1 w-full rounded-xl border bg-white px-3 py-2.5 text-sm"><option value="compact" ${option.preset==='compact'?'selected':''}>Compact · ประหยัดกระดาษ</option><option value="safe" ${option.preset==='safe'?'selected':''}>Safe · ช่องไฟกว้าง</option></select></label><label class="text-[11px] font-bold text-slate-600">ชดเชยด้านหลัง X (mm)<input data-personal-print-option="backOffsetXMM" type="number" min="-5" max="5" step="0.1" value="${option.backOffsetXMM}" class="mt-1 w-full rounded-xl border bg-white px-3 py-2.5 text-sm"></label><label class="text-[11px] font-bold text-slate-600">ชดเชยด้านหลัง Y (mm)<input data-personal-print-option="backOffsetYMM" type="number" min="-5" max="5" step="0.1" value="${option.backOffsetYMM}" class="mt-1 w-full rounded-xl border bg-white px-3 py-2.5 text-sm"></label></div><p class="mt-2 text-[11px] text-violet-700">ค่าเริ่มต้น X/Y = 0 คือหน้าหลังตรงกันทางคณิตศาสตร์ · ปรับเฉพาะเมื่อเครื่องพิมพ์ดึงกระดาษคลาด และต้องพิมพ์ที่ 100% / Actual size ตามด้านกลับกระดาษที่ Designer กำหนด</p></fieldset>`;}
function installPersonalIssueCompatibility(){
    const select=document.getElementById('bbs-issue-template'),button=document.querySelector('[data-bbs-issue]');
    if(!select||!button)return;
    let status=document.getElementById('bbs-issue-compatibility');
    if(!status){status=document.createElement('div');status.id='bbs-issue-compatibility';status.setAttribute('role','status');status.setAttribute('aria-live','polite');status.className='rounded-xl border px-3 py-2 text-xs font-semibold';select.parentElement?.parentElement?.appendChild(status);}
    const sync=()=>{
        const employees=selectedPersonalIssueRows();
        let compatible=0;
        [...select.options].forEach(option=>{if(!option.value)return;const template=cardTemplateRecord('personal',n(option.value)),disabled=!template||!personalTemplateLevelEligible(template)||(employees.length>0&&employees.some(employee=>!personalTemplateMatchesEmployee(template,employee)));option.disabled=disabled;if(!disabled)compatible++;});
        const selectedTemplate=cardTemplateRecord('personal',n(select.value));
        if(selectedTemplate&&select.selectedOptions[0]?.disabled){select.value='';state.personalIssue.templateId='';}
        if(!employees.length){status.className='rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-xs font-semibold text-sky-800';status.textContent='เลือกพนักงานก่อน ระบบจะแสดงเฉพาะ Active Template ที่ตรงกับแผนก, Unit และระดับ';}
        else if(!compatible){status.className='rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800';status.textContent=`พนักงานที่เลือก ${employees.length} คนยังไม่มี Active Template ที่ใช้ร่วมกันได้ โปรดตรวจแผนก, Unit และระดับ BBS`;}
        else{status.className='rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800';status.textContent=`พบ Active Template ที่ตรงกับพนักงานที่เลือก ${compatible} รายการ`;}
        button.disabled=!employees.length||!select.value;
        button.classList.toggle('opacity-50',button.disabled);button.classList.toggle('cursor-not-allowed',button.disabled);
    };
    select.onchange=()=>{state.personalIssue.templateId=select.value;sync();};
    document.querySelectorAll('[data-card-employee]').forEach(input=>input.onchange=()=>{const key=String(input.dataset.cardEmployee),row=state.cardEmployees.find(item=>String(item.EmployeeID)===key);if(input.checked&&row){if(state.personalIssue.selected.size>=100&&!state.personalIssue.selected.has(key)){input.checked=false;showToast('เลือกพนักงานได้สูงสุด 100 คน','error');return;}state.personalIssue.selected.set(key,{...row});}else state.personalIssue.selected.delete(key);render();});
    sync();
}
...
function assessCardTemplate(kind,template,asset,{intent='',employee=null,departmentContext=null}={}){
    ...
    if(kind==='personal'){
        ...
        add('ready','QR ด้านหน้า','Personal QR จะสร้างเฉพาะตอนออกหรือ Replace บัตร และผูกกับเจ้าของบัตรคนนี้');
        if(departmentContext?.loadError)add(['issue','replace'].includes(intent)?'blocked':'warning','QR ด้านหลัง',departmentContext.loadError);
        else if(!departmentContext?.qr?.qrUrl)add(['issue','replace'].includes(intent)?'blocked':'warning','QR ด้านหลัง','แผนกของพนักงานยังไม่มี Active Department QR');
        else add('ready','QR ด้านหลัง',`Department QR · ${departmentContext.department?.Name||employee?.Department||'-'} · รุ่น ${n(departmentContext.qr.Generation)}`);
...
function personalDuplexPreview(template,asset,assessment,person,departmentContext){
    const wide=assessment.width/assessment.height>=1,maxWidth=wide?'680px':'430px',departmentQr=departmentContext?.qr?.qrUrl||'',departmentName=departmentContext?.department?.Name||person.Department||template.DepartmentName||'แผนกตัวอย่าง';
    const shell=(side,content)=>`<div><div class="mb-2 text-center text-[10px] font-black uppercase tracking-widest text-slate-500">${side}</div><div class="bbs-card-cut-line relative mx-auto overflow-hidden bg-white shadow-2xl" style="aspect-ratio:${assessment.width}/${assessment.height};max-width:${maxWidth};background-image:url('${asset.dataUrl}');background-size:cover;background-position:center"><div class="bbs-card-safe-area" aria-hidden="true"></div>${content}</div></div>`;
    const front=shell('ด้านหน้า · Personal QR',`<div class="absolute inset-[6%] flex items-stretch justify-between gap-[4%]"><div class="min-w-0 w-[64%] self-center"><div class="flex h-12 w-12 items-center justify-center rounded-full bg-emerald-700 text-xl font-black text-white">${escHtml((person.EmployeeName||'?').trim().charAt(0).toUpperCase())}</div><div class="mt-3 truncate text-base sm:text-xl font-black text-slate-900">${escHtml(person.EmployeeName)}</div><div class="mt-1 truncate text-[10px] sm:text-sm font-bold text-slate-700">${escHtml(person.Department||'-')} / ${escHtml(person.Unit||'-')}</div><div class="mt-1 truncate text-[10px] sm:text-sm font-bold text-slate-700">${escHtml(person.Position||'-')}</div>${n(template.IncludeEmployeeID)!==0?`<div class="mt-2 font-mono text-xs sm:text-base font-black">${escHtml(person.EmployeeID||'-')}</div>`:''}</div><div class="w-[27%] max-w-[150px] self-end rounded-xl bg-white p-2 text-center shadow-lg">${cardPreviewQr(null,false)}<div class="mt-1 text-[8px] sm:text-[10px] font-black">PERSONAL QR</div><div class="text-[7px] sm:text-[9px] text-slate-500">Scan → Login → Workspace</div></div></div>`);
    const back=shell('ด้านหลัง · Department QR',`<div class="absolute inset-[6%] flex flex-col items-center justify-center"><div class="w-[34%] max-w-[170px] rounded-xl bg-white p-2 text-center shadow-lg">${cardPreviewQr(departmentQr,Boolean(departmentQr))}<div class="mt-1 text-[8px] sm:text-[10px] font-black">DEPARTMENT QR</div></div><div class="mt-3 max-w-[80%] rounded-lg bg-white/90 px-3 py-2 text-center text-[10px] sm:text-sm font-black text-emerald-900">${escHtml(departmentName)}</div><div class="mt-1 text-[8px] sm:text-[10px] font-bold text-slate-600">แจ้งพฤติกรรมดี / เสี่ยงของแผนก</div></div>`);
    return`<div class="bbs-card-preview-stage rounded-2xl border border-slate-200 p-5"><div class="grid gap-6 ${wide?'':'md:grid-cols-2'}">${front}${back}</div><div class="mt-4 flex flex-wrap gap-3 text-[10px] font-bold"><span class="text-orange-700">เส้นสีส้ม = ขอบตัด</span><span class="text-cyan-700">เส้นสีฟ้า = Safe area</span><span class="text-slate-500">ตัวอย่างหน้าหลัง ${assessment.width} × ${assessment.height} mm</span></div></div>`;
}
function compositeCardPreview(kind,template,asset,assessment,{employee=null,departmentContext=null}={}){
    ...
    if(kind==='personal')return personalDuplexPreview(template,asset,assessment,person,departmentContext);
...
function designerPreviewValues(kind,template,employee,departmentContext){
    const person=employee||{},departmentName=departmentContext?.department?.Name||person.Department||template.DepartmentName||'';
    return kind==='personal'?{
        'employee.full_name':person.EmployeeName||'ชื่อพนักงานตัวอย่าง','employee.id':person.EmployeeID||'012345','employee.department':person.Department||departmentName||'-','employee.safety_unit':person.Unit||'-','employee.position':person.Position||'-','employee.bbs_level':person.BBSLevel||template.BBSLevel||'-','employee.photo':'','card.personal_qr':'BBS-PREVIEW-PERSONAL-QR','department.community_qr':departmentContext?.qr?.qrUrl||'BBS-PREVIEW-DEPARTMENT-QR','card.issue_date':new Date().toISOString().slice(0,10),'template.name':template.TemplateName||'','organization.name':'Thai Summit Harness Co., Ltd.','organization.logo':''
    }:{'department.name':departmentName||'แผนกตัวอย่าง','department.community_qr':departmentContext?.qr?.qrUrl||'BBS-PREVIEW-DEPARTMENT-QR','template.name':template.TemplateName||'','organization.name':'Thai Summit Harness Co., Ltd.','organization.logo':''};
}
function assessDesignerPreview(kind,template,designer,{intent='',employee=null,departmentContext=null}={}){
    const layout=designer.layout,checks=[],add=(level,title,detail)=>checks.push(cardPreviewCheck(level,title,detail));
    const width=n(layout.widthMM),height=n(layout.heightMM),front=layout.sides.find(side=>side.side==='Front'),back=layout.sides.find(side=>side.side==='Back');
    add(width>=40&&width<=500&&height>=40&&height<=500?'ready':'blocked','ขนาดจาก Designer',`${width} × ${height} mm`);
    ...
    const elements=layout.elements||[],hasPersonalFront=elements.some(item=>item.visible&&item.side==='Front'&&item.elementType==='QR'&&item.dataSourceKey==='card.personal_qr'),hasDepartmentBack=elements.some(item=>item.visible&&item.side==='Back'&&item.elementType==='QR'&&item.dataSourceKey==='department.community_qr'),hasDepartmentFront=elements.some(item=>item.visible&&item.side==='Front'&&item.elementType==='QR'&&item.dataSourceKey==='department.community_qr'),departmentQrSide=hasDepartmentFront?'ด้านหน้า':hasDepartmentBack?'ด้านหลัง':'';
    ...
    else add(departmentQrSide?'ready':'blocked','ตำแหน่ง QR ใน Designer',departmentQrSide?`Department QR ${departmentQrSide}พร้อม`:'ต้องมี Department QR ที่ด้านหน้าหรือด้านหลัง');
    ...
    if(kind==='personal')add(employee?'ready':'warning','ข้อมูลพนักงานตัวอย่าง',employee?`${employee.EmployeeName} · ${employee.Department||'-'} / ${employee.Unit||'-'}`:'ยังไม่พบพนักงานตรงขอบเขต จึงใช้ข้อมูลตัวอย่าง');
    const status=checks.some(row=>row.level==='blocked')?'blocked':checks.some(row=>row.level==='warning')?'warning':'ready';
    return{status,checks,width,height};
}
function designerCompositeCardPreview(kind,template,designer,assessment,{employee=null,departmentContext=null}={}){
    const layout=designer.layout,values=designerPreviewValues(kind,template,employee,departmentContext),wide=assessment.width/assessment.height>=1,displayWidth=wide?620:320,pixelsPerMM=displayWidth/assessment.width;
    const sideHtml=side=>{
        const face=designerCardFaceHtml({layout,values},side.side,designer.resources,qrDataUrl,{pixelsPerMM,cardStyle:'left:0;top:0;',className:'bbs-card-cut-line shadow-2xl'});
        return`<div><div class="mb-2 text-center text-[10px] font-black uppercase tracking-widest text-slate-500">${side.side==='Front'?(kind==='personal'?'ด้านหน้า · Personal QR':'ด้านหน้า'):'ด้านหลัง · Department QR'}</div><div class="relative mx-auto max-w-full" data-designer-preview-side="${side.side}" data-renderer-contract="bbs-designer-card-face-v2" style="width:${displayWidth}px;height:${assessment.height*pixelsPerMM}px">${face}</div></div>`;
    };
    return`<div class="bbs-card-preview-stage rounded-2xl border border-violet-200 bg-violet-50/30 p-5"><div class="mb-4 flex flex-wrap items-center justify-between gap-2"><span class="rounded-full bg-violet-100 px-3 py-1 text-[10px] font-black text-violet-700">ACTIVE DESIGNER · V${n(designer.record.VersionNo)}</span><span class="text-[10px] font-bold text-slate-500">ใช้ Layout และ Artwork contract เดียวกับงานพิมพ์</span></div><div class="grid gap-6 ${wide?'':'md:grid-cols-2'}">${layout.sides.map(sideHtml).join('')}</div><div class="mt-4 flex flex-wrap gap-3 text-[10px] font-bold"><span class="text-orange-700">เส้นสีส้ม = ขอบตัด</span><span class="text-cyan-700">เส้นสีฟ้า = Safe area</span><span class="text-slate-500">Designer ${assessment.width} × ${assessment.height} mm</span></div></div>`;
}
...
async function openCardTemplatePreview(kind,id,options={}){
    ...
    try{
        ...
        const mode=designer?`Active Designer V${n(designer.record.VersionNo)}`:'Legacy Preview · ยังไม่มี Active Designer';
        ...
        const batchCount=Math.max(0,n(options.batchCount)),printOptions=options.printOptions||{},sheetMetrics=batchCount>0&&designer?designerSheetMetrics(designer.layout,printOptions.paperSize||'A4',printOptions):null,actionLabel=options.intent==='issue'&&batchCount>1?`ออกบัตร ${batchCount} ใบและเตรียมพิมพ์`:{activate:'ยืนยัน Activate',print:'พิมพ์จาก Preview นี้',issue:'ออกบัตรและพิมพ์',replace:'Replace และพิมพ์'}[options.intent]||'';
        modal.innerHTML=`<div class="w-full max-w-6xl overflow-y-auto rounded-3xl bg-slate-50"><div class="sticky top-0 z-10 flex items-start justify-between gap-3 border-b bg-white p-5"><div><div class="text-[10px] font-black tracking-widest text-emerald-700">COMPOSITE CARD PREVIEW</div><h3 class="mt-1 font-black text-slate-800">${escHtml(template.TemplateName)}</h3><p class="mt-1 text-xs text-slate-500">${kind==='personal'?'Personal Card':'Department Card'} · Template ${escHtml(template.Status||'-')} · ${escHtml(mode)}</p>${batchCount>0?`<p class="mt-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800">Batch Preflight: Personal Card ${batchCount} ใบ · ${sheetMetrics?`${sheetMetrics.paperSize} ${sheetMetrics.columns}×${sheetMetrics.rows} = สูงสุด ${sheetMetrics.capacity} ใบ/แผ่น · ${Math.ceil(batchCount/sheetMetrics.capacity)} ชุดหน้า–หลัง · ชดเชย Back X ${sheetMetrics.backOffsetXMM} / Y ${sheetMetrics.backOffsetYMM} mm`:'ระบบจะคำนวณ Sheet จาก Designer'} · Server จะยกเลิกทั้งชุดหากมีรายการใดไม่พร้อม</p>`:''}</div><button data-close class="text-2xl text-slate-400">×</button></div><div class="grid gap-5 p-4 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,.75fr)]">${preview}${previewReadinessPanel(assessment)}</div><div class="sticky bottom-0 flex flex-col-reverse gap-2 border-t bg-white p-4 sm:flex-row sm:justify-end"><button data-close class="rounded-xl border border-slate-300 px-4 py-3 text-sm font-black text-slate-700">ปิด Preview</button>${actionLabel?`<button data-preview-confirm ${assessment.status==='blocked'?'disabled':''} class="rounded-xl bg-emerald-700 px-5 py-3 text-sm font-black text-white disabled:bg-slate-300">${actionLabel}</button>`:''}</div></div>`;
        ...
        if(confirmButton&&options.onConfirm)bindBusy(confirmButton,'onclick',async()=>{close();await options.onConfirm({template,asset,designer,assessment,departmentContext,employee});},'กำลังดำเนินการ...');
...
async function openInspectorTeam(id,returnFocus=null){try{state.inspectorModalReturnFocus=returnFocus||document.activeElement;state.inspectorSelectedId=id;const{year,month}=state.inspectorScheduleFilters;const[team,schedule]=await Promise.all([API.get(`/bbs/inspectors/${id}/team`),API.get(`/bbs/inspectors/${id}/schedule?year=${year}&month=${month}`)]);state.inspectorTeam=team.data;state.inspectorScheduleDetail=schedule.data;render();}catch(error){state.inspectorModalReturnFocus=null;showToast(error?.message||'เปิดทีมและตารางตรวจไม่สำเร็จ','error');}}
function closeInspectorTeamModal(){const returnFocus=state.inspectorModalReturnFocus;state.inspectorModalReturnFocus=null;state.inspectorSelectedId=null;state.inspectorTeam=null;state.inspectorScheduleDetail=null;render();requestAnimationFrame(()=>returnFocus?.focus?.({preventScroll:true}));}
...
async function loadMasterArtwork({force=false}={}) {
    ...
    const masterArtwork=await API.get('/bbs/admin/card-artwork').catch(error=>({data:{globalBack:null,scopedFronts:[],records:[],error:error?.message||'โหลด Card Artwork ไม่สำเร็จ'}}));
...
async function loadPersonalCardRows() {
    ...
    for(const row of state.cardEmployees){const key=String(row.EmployeeID);if(row.ActiveCardID)state.personalIssue.selected.delete(key);else if(state.personalIssue.selected.has(key))state.personalIssue.selected.set(key,{...row});}
    state.cards=listRows(cards.data);state.listMeta.cards=listPagination(cards.data);
    for(const row of state.cards){const key=String(row.id);if(row.Status!=='Active')state.personalReprint.selected.delete(key);else if(state.personalReprint.selected.has(key))state.personalReprint.selected.set(key,{...row});}
}
...
async function loadDepartmentPrintRows(){
    const f=state.departmentPrintFilters,q=new URLSearchParams({paged:'1',page:String(f.page),pageSize:String(f.pageSize)});
    for(const key of ['q','departmentId'])if(f[key]!==''&&f[key]!==null)q.set(key,String(f[key]));
    const payload=(await API.get(`/bbs/admin/department-card-output-history?${q}`)).data;
    state.departmentPrints=listRows(payload);state.listMeta.departmentPrints=listPagination(payload);
}

async function loadDepartmentCardAdmin({force=false}={}) {
    if(state.cardDataLoaded.department&&!force)return;
    await Promise.all([loadDepartmentCardCatalog(),loadDepartmentPrintRows()]);
...
async function uploadCardTemplate(event,operation){event.preventDefault();try{const form=new FormData(event.currentTarget);form.set('includeEmployeeId',event.currentTarget.elements.includeEmployeeId.checked?'1':'0');await API.upload('/bbs/admin/card-templates',form,{onProgress:uploadProgress(operation,'กำลังสร้าง Personal Template...')});operation?.update({label:'กำลังสร้าง Personal Template...',detail:'กำลังจัดเตรียม Artwork และโหลดรายการ Template ล่าสุด',progress:100});showToast('สร้าง Personal Template Draft แล้ว','success');await loadPersonalTemplateCatalog({trash:false});render();}catch(error){showToast(error?.message||'สร้าง Personal Template ไม่สำเร็จ','error');}}
async function uploadMasterArtwork(event,operation){event.preventDefault();const formElement=event.currentTarget,role=formElement.dataset.artworkRole,form=new FormData(formElement),label=role==='global-back'?'Global Back':`${formElement.dataset.kind} Scoped Front`;try{await API.upload(`/bbs/admin/card-artwork/${role}`,form,{onProgress:uploadProgress(operation,`กำลังอัปโหลด ${label}...`)});operation?.update({label:'กำลังเปิดใช้ Card Artwork เวอร์ชันใหม่...',detail:'งาน Draft และ Active เดิมจะไม่ถูกแก้ไขย้อนหลัง',progress:100});showToast(`ตั้งค่า ${label} แล้ว`,'success');await loadMasterArtwork({force:true});render();}catch(error){showToast(error?.message||'อัปโหลด Card Artwork ไม่สำเร็จ','error');}}
async function previewMasterArtwork(id){const popup=window.open('','_blank');if(!popup)return showToast('Browser ปิดกั้นหน้าต่าง Preview','error');try{const response=await apiFetch(`/bbs/admin/card-artwork/versions/${id}/file`),blob=await response.blob(),url=URL.createObjectURL(blob);popup.location.href=url;setTimeout(()=>URL.revokeObjectURL(url),60000);}catch(error){popup.close();showToast(error?.message||'เปิด Card Artwork ไม่สำเร็จ','error');}}
...
async function repairPersonalTemplateLevel(id,rowVersion){const template=cardTemplateRecord('personal',id);if(!template)return;if(!window.confirm(`เปลี่ยนระดับ Template “${template.TemplateName}” จาก ${template.BBSLevel||'ทุกระดับ'} เป็น Group Leader โดยคง Designer Layout, Artwork และสถานะ ${template.Status} เดิมใช่หรือไม่?`))return;try{await API.post(`/bbs/admin/card-templates/${id}/repair-personal-level`,{rowVersion,bbsLevel:'Group Leader'});showToast('แก้ระดับ Template เป็น Group Leader แล้ว Layout และ Artwork เดิมยังอยู่ครบ','success');await loadPersonalTemplateCatalog({trash:false});render();}catch(error){showToast(error?.message||'แก้ระดับ Template ไม่สำเร็จ','error');}}
async function previewTemplate(id){return openCardTemplatePreview('personal',id);}
async function previewIssuedPersonalCard(id){const row=state.cards.find(item=>n(item.id)===n(id));if(!row)return showToast('ไม่พบบัตร Personal ที่เลือก','error');return openCardTemplatePreview('personal',n(row.TemplateID),{employee:row});}
async function openPersonalCardDetail(id,returnFocus=document.activeElement){try{
    const data=(await API.get(`/bbs/admin/cards/${id}`)).data||{},card=data.card||{},prints=data.printLogs||[],lifecycle=data.lifecycle||[],modal=document.createElement('div');
    modal.className='fixed inset-0 z-[140] bg-slate-950/70 p-3 md:p-8 flex items-center justify-center';
    const lifecycleRows=lifecycle.map(row=>`<div class="rounded-xl border p-3"><div class="flex flex-wrap items-center justify-between gap-2"><span class="font-black text-slate-800">บัตร #${n(row.id)} · QR ${escHtml(row.TokenFingerprint)}</span><span class="rounded-full px-2 py-1 text-[10px] font-black ${row.Status==='Active'?'bg-emerald-100 text-emerald-700':'bg-slate-100 text-slate-600'}">${escHtml(row.Status)}</span></div><div class="mt-1 text-[11px] text-slate-500">ออก ${fmtDateTime(row.IssuedAt)}${row.RevokedAt?` · สิ้นสุด ${fmtDateTime(row.RevokedAt)}`:''}${row.ReplacedByCardID?` · แทนด้วยบัตร #${n(row.ReplacedByCardID)}`:''}</div>${row.RevokeReason?`<div class="mt-1 text-[11px] text-rose-600">${escHtml(row.RevokeReason)}</div>`:''}</div>`).join('');
    const printRows=prints.map(row=>`<div class="rounded-xl border p-3"><div class="flex flex-wrap items-center justify-between gap-2"><span class="font-black text-slate-800">Print #${n(row.id)} · ${escHtml(row.PrintMode)}</span><span class="text-[11px] text-slate-500">${fmtDateTime(row.PrintedAt)}</span></div><div class="mt-1 text-[11px] text-slate-500">โดย ${escHtml(row.PrintedByName||row.PrintedBy)}${row.Reason?` · ${escHtml(row.Reason)}`:''}</div>${row.SnapshotID?`<div class="mt-2 rounded-lg bg-violet-50 px-3 py-2 text-[11px] font-bold text-violet-700">Designer Snapshot #${n(row.SnapshotID)} · Layout #${n(row.LayoutVersionID)} · Contract ${escHtml(String(row.RenderContractHash||'').slice(0,16))}</div>`:'<div class="mt-2 text-[11px] font-bold text-slate-400">Legacy print · ไม่มี Designer Snapshot</div>'}</div>`).join('');
    modal.innerHTML=`<section class="w-full max-w-4xl max-h-full overflow-y-auto rounded-3xl bg-slate-50 shadow-2xl"><header class="sticky top-0 z-10 flex items-start justify-between gap-3 border-b bg-white p-5"><div><p class="text-[10px] font-black tracking-widest text-sky-700">PERSONAL CARD RECORD</p><h3 class="mt-1 font-black text-slate-800">${escHtml(card.EmployeeName)} · ${escHtml(card.EmployeeID)}</h3><p class="mt-1 text-xs text-slate-500">${escHtml(card.TemplateName)} · QR fingerprint ${escHtml(card.TokenFingerprint)} · ${escHtml(card.Status)}</p></div><button type="button" data-close class="text-2xl text-slate-400">×</button></header><div class="space-y-5 p-4 md:p-5"><div class="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-bold text-amber-900">เพื่อความปลอดภัย ระบบไม่เก็บหรือเปิดเผย Personal QR ดิบหลังออกบัตร จึงดาวน์โหลดบัตรเดิมซ้ำไม่ได้ หากต้องพิมพ์บัตร Active ใหม่ ให้ใช้ Replace + Reprint เพื่อยกเลิก QR เดิมและออก QR ใหม่</div><section><h4 class="font-black text-slate-800">ข้อมูลบัตร</h4><div class="mt-3 grid gap-2 sm:grid-cols-2 text-xs"><div class="rounded-xl border bg-white p-3"><span class="text-slate-400">แผนก / Unit</span><div class="mt-1 font-bold text-slate-800">${escHtml(card.Department||'-')} / ${escHtml(card.Unit||'-')}</div></div><div class="rounded-xl border bg-white p-3"><span class="text-slate-400">การตรวจสอบ QR</span><div class="mt-1 font-bold text-slate-800">สแกน ${n(card.ResolveCount)} ครั้ง${card.LastResolvedAt?` · ล่าสุด ${fmtDateTime(card.LastResolvedAt)}`:''}</div></div></div></section><section><h4 class="font-black text-slate-800">ประวัติพิมพ์ (${prints.length})</h4><div class="mt-3 grid gap-2">${printRows||'<div class="rounded-xl bg-white p-5 text-center text-sm text-slate-400">ยังไม่มี Print Log</div>'}</div></section><section><h4 class="font-black text-slate-800">วงจรบัตรของพนักงาน (${lifecycle.length})</h4><div class="mt-3 grid gap-2">${lifecycleRows}</div></section></div><footer class="sticky bottom-0 flex flex-wrap justify-end gap-2 border-t bg-white p-4"><button type="button" data-card-detail-preview class="min-h-11 rounded-xl border px-4 text-sm font-black text-sky-700">Preview Template ปัจจุบัน</button>${card.Status==='Active'?`<button type="button" data-card-detail-replace class="min-h-11 rounded-xl bg-amber-500 px-4 text-sm font-black text-white">Replace + Reprint</button>`:''}<button type="button" data-close class="min-h-11 rounded-xl border px-4 text-sm font-black text-slate-700">ปิด</button></footer></section>`;
    document.body.appendChild(modal);const close=mountBbsDialog(modal,`ประวัติบัตร Personal ${card.EmployeeName||''}`,returnFocus);
    modal.querySelector('[data-card-detail-preview]')?.addEventListener('click',()=>{close();previewIssuedPersonalCard(id);});
    const replace=modal.querySelector('[data-card-detail-replace]');if(replace)bindBusy(replace,'onclick',async()=>{close();await replaceCard(id);},'กำลังออกบัตรใหม่...');
}catch(error){showToast(error?.message||'เปิดประวัติบัตรไม่สำเร็จ','error');}}
...
async function issueCards(previewConfirmed=false,printOptionsOverride=null){const selectedEmployees=selectedPersonalIssueRows(),employeeIds=selectedEmployees.map(row=>String(row.EmployeeID)),templateId=n(state.personalIssue.templateId||document.getElementById('bbs-issue-template')?.value),template=cardTemplateRecord('personal',templateId),printOptions=printOptionsOverride||personalPrintOptions();if(!employeeIds.length||!templateId||!template)return showToast('กรุณาเลือกพนักงานและ Active Template ที่ตรงกัน','error');if(employeeIds.length>100)return showToast('เลือกพนักงานได้สูงสุด 100 คนต่อ Batch','error');const unavailable=selectedEmployees.find(row=>row.ActiveCardID);if(unavailable)return showToast(`${unavailable.EmployeeName||unavailable.EmployeeID} มีบัตร Active แล้ว กรุณาโหลดรายการใหม่`,'error');const incompatible=selectedEmployees.filter(row=>!personalTemplateMatchesEmployee(template,row));if(incompatible.length){const detail=personalTemplateScopeMismatch(template,incompatible[0]).join(' · ');return showToast(`Template ไม่ตรงกับ ${incompatible[0].EmployeeName}: ${detail}${incompatible.length>1?` · และอีก ${incompatible.length-1} คน`:''}`,'error');}if(!previewConfirmed){return openCardTemplatePreview('personal',templateId,{intent:'issue',batchCount:employeeIds.length,printOptions,employee:selectedEmployees[0],onConfirm:()=>issueCards(true,printOptions)});}if(typeof window.qrcode!=='function')return showToast('QR library is unavailable. No card was changed.','error');const popup=openCardPrintPopup();if(!popup)return;let issued=false;try{const backgrounds=await preloadCardBackgrounds([templateId]);const result=await API.post('/bbs/admin/cards/issue',{employeeIds,templateId,reason:employeeIds.length>1?'Initial batch issue':'Initial issue'});issued=true;state.personalIssue.selected.clear();await printIssuedCards(result.data||[],employeeIds.length>1?'Initial batch issue':'Initial issue',popup,backgrounds,printOptions);await loadCardAdmin();render();}catch(error){if(!issued&&!popup.closed)popup.close();showToast(error?.message||'ออกบัตรไม่สำเร็จ','error');}}
...
async function replaceSelectedCards(){const selected=selectedPersonalReprintRows();if(!selected.length)return showToast('กรุณาเลือกบัตร Active อย่างน้อย 1 ใบ','error');if(selected.length>100)return showToast('เลือกบัตรได้สูงสุด 100 ใบต่อ Batch','error');const reason=window.prompt(`เหตุผลการ Replace + Print จำนวน ${selected.length} ใบ`,'Batch replace / reprint');if(reason===null||!reason.trim())return;if(!window.confirm(`ยืนยัน Replace บัตร ${selected.length} ใบ?\n\nPersonal QR เดิมของทุกใบจะถูกยกเลิก และระบบจะออก QR ใหม่ทั้งชุดก่อนเปิดหน้าพิมพ์`))return;if(typeof window.qrcode!=='function')return showToast('QR library is unavailable. No card was changed.','error');const popup=openCardPrintPopup();if(!popup)return;let replaced=false;try{const backgrounds=await preloadCardBackgrounds([...new Set(selected.map(row=>n(row.TemplateID)).filter(Boolean))]);const result=await API.post('/bbs/admin/cards/replace-batch',{cardIds:selected.map(row=>n(row.id)),reason:reason.trim()});replaced=true;state.personalReprint.selected.clear();await printIssuedCards(result.data||[],reason.trim(),popup,backgrounds,personalPrintOptions());await loadCardAdmin();render();}catch(error){if(!replaced&&!popup.closed)popup.close();showToast(error?.message||'Replace + Print แบบ Batch ไม่สำเร็จ','error');}}
...
function cardOutputFilename(label='BBS_Cards'){const stamp=new Date().toISOString().replace(/[-:]/g,'').slice(0,13).replace('T','-');return`${String(label||'BBS_Cards').replace(/[^a-z0-9ก-๙_-]+/gi,'_')}_${stamp}.pdf`;}
function mountDesignerOutputActions(popup,{filename='BBS_Cards.pdf'}={}){
    ...
    toolbar.querySelector('[data-card-output-pdf]').onclick=event=>run(event.currentTarget,'PDF',async()=>{await saveDesignerPrintPdf(doc,{filename});return 1;});
    toolbar.querySelectorAll('[data-card-output-image]').forEach(button=>button.onclick=event=>{const format=event.currentTarget.dataset.cardOutputImage;return run(event.currentTarget,format.toUpperCase(),()=>saveDesignerPrintImages(doc,{filename,format}));});
}
async function designerPrintResources(cards){const urls=[...new Set(cards.flatMap(card=>{const render=card.designerRender||{};return[...(render.layout?.sides||[]).map(side=>side.backgroundUrl),...(render.layout?.elements||[]).map(element=>element.assetUrl)].filter(Boolean)}))],resources=new Map();await Promise.all(urls.map(async url=>{const response=await apiFetch(url);resources.set(url,await blobDataUrl(await response.blob()));}));return resources;}
function legacyPersonalPrintDocument(cards,backgrounds){
    const first=cards[0]||{},width=n(first.widthMM)||60,height=n(first.heightMM)||85,gap=5,usableWidth=194,usableHeight=281,columns=Math.max(1,Math.floor((usableWidth+gap)/(width+gap))),rows=Math.max(1,Math.floor((usableHeight+gap)/(height+gap))),capacity=columns*rows;
    const article=(card,side)=>{const background=backgrounds.get(n(card.templateId));if(!background)throw new Error('Card background is unavailable. Prepare the card again.');if(side==='Front')return`<article class="card front" style="width:${width}mm;height:${height}mm;background-image:linear-gradient(90deg,rgba(255,255,255,.96),rgba(255,255,255,.58)),url('${background}')"><div class="safe"></div><div class="identity"><div class="avatar">${escHtml((card.employeeName||'?').trim().charAt(0).toUpperCase())}</div><div class="name">${escHtml(card.employeeName)}</div><div class="meta">${escHtml(card.department||'-')} / ${escHtml(card.unit||'-')}</div><div class="meta">${escHtml(card.position||'-')}</div>${card.includeEmployeeId?`<div class="emp">${escHtml(card.employeeId)}</div>`:''}</div><div class="qr"><img src="${qrDataUrl(card.qrUrl)}"><strong>PERSONAL QR</strong><small>Scan → Login → Workspace</small></div></article>`;const departmentQr=card.departmentQr||{};if(!departmentQr.qrUrl)throw new Error(`Department QR is unavailable for ${card.employeeName||card.employeeId||'this card'}.`);return`<article class="card back" style="width:${width}mm;height:${height}mm;background-image:url('${background}')"><div class="safe"></div><div class="back-content"><div class="qr"><img src="${qrDataUrl(departmentQr.qrUrl)}"><strong>DEPARTMENT QR</strong></div><div class="department">${escHtml(departmentQr.departmentName||card.department||'-')}</div><small>แจ้งพฤติกรรมดี / เสี่ยงของแผนก</small></div></article>`;};
    const sheets=[];for(let offset=0;offset<cards.length;offset+=capacity){const group=cards.slice(offset,offset+capacity);sheets.push(`<section class="sheet front-sheet">${group.map(card=>article(card,'Front')).join('')}</section>`,`<section class="sheet back-sheet">${group.map(card=>article(card,'Back')).join('')}</section>`);}return`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>BBS Personal Card Print</title><style>@page{size:A4;margin:8mm}*{box-sizing:border-box}body{margin:0;font-family:Kanit,Arial,sans-serif;color:#0f172a}.sheet{width:194mm;height:281mm;display:grid;grid-template-columns:repeat(${columns},${width}mm);grid-auto-rows:${height}mm;gap:${gap}mm;align-content:start;break-after:page;page-break-after:always}.sheet:last-child{break-after:auto;page-break-after:auto}.back-sheet{direction:rtl}.back-sheet .card{direction:ltr}.card{position:relative;overflow:hidden;background-size:cover;background-position:center;border:.2mm dashed #f97316;border-radius:3mm;break-inside:avoid}.front{display:flex;justify-content:space-between;padding:5mm}.safe{position:absolute;inset:4%;border:.2mm dashed #0891b2;pointer-events:none}.identity{width:64%;min-width:0;z-index:1;align-self:center}.avatar{width:14mm;height:14mm;border-radius:50%;background:#047857;color:#fff;display:flex;align-items:center;justify-content:center;font-size:7mm;font-weight:900}.name{font-size:4.2mm;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2mm}.meta{font-size:2.7mm;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1mm}.emp{font:900 3.2mm monospace;margin-top:2mm}.qr{width:23mm;text-align:center;z-index:1}.front>.qr{align-self:end}.qr img{width:22mm;height:22mm;display:block;background:#fff}.qr strong{font-size:2.2mm;display:block;margin-top:1mm}.qr small,.back-content>small{font-size:1.7mm;display:block}.back-content{position:absolute;inset:8%;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}.back-content .qr{width:26mm}.back-content .qr img{width:26mm;height:26mm}.department{margin-top:2mm;max-width:90%;padding:1.5mm 3mm;border-radius:2mm;background:#ffffffe8;font-size:3mm;font-weight:900}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.safe{display:none}}</style></head><body>${sheets.join('')}<script>window.onload=()=>setTimeout(()=>window.print(),250)<\/script></body></html>`;
}
async function printIssuedCards(cards,reason,popup,backgrounds=new Map(),outputOptions={}){if(!cards.length){if(popup&&!popup.closed)popup.close();return;}if(!cards.some(card=>card.designerRender?.layout)){try{for(const card of cards){if(!backgrounds.has(n(card.templateId))){const response=await apiFetch(`/bbs/admin/card-templates/${card.templateId}/file`);backgrounds.set(n(card.templateId),await blobDataUrl(await response.blob()));}}popup.document.open();popup.document.write(legacyPersonalPrintDocument(cards,backgrounds));popup.document.close();try{await API.post('/bbs/admin/cards/print-log',{cardIds:cards.map(card=>card.cardId),reason});}catch(logError){console.warn('[bbs] card print log failed:',logError?.message||logError);showToast('Cards are ready to print, but the print log could not be saved.','error');return;}showToast(`เตรียมพิมพ์บัตรหน้าหลัง ${cards.length} ใบแล้ว`,'success');}catch(error){if(popup&&!popup.closed){popup.document.open();popup.document.write('<!doctype html><html><head><meta charset="utf-8"><title>BBS card recovery</title></head><body style="font-family:Arial,sans-serif;padding:32px"><h2>Card was issued, but the duplex print page could not be rendered.</h2><p>Keep this window open and contact Admin before replacing the card.</p></body></html>');popup.document.close();}showToast(error?.message||'สร้างไฟล์พิมพ์หน้าหลังไม่สำเร็จ','error');}return;}try{const designerCards=cards.filter(card=>card.designerRender?.layout);if(designerCards.length){
    const resources=await designerPrintResources(cards);
    const documentHtml=designerPrintDocument(cards,resources,qrDataUrl,outputOptions);
    popup.document.open();popup.document.write(documentHtml);popup.document.close();
    mountDesignerOutputActions(popup,{filename:cardOutputFilename(cards.length===1?`BBS_Personal_${cards[0].employeeId||cards[0].employeeName||'Card'}`:`BBS_Personal_Batch_${cards.length}`)});
    try{await API.post('/bbs/admin/cards/print-log',{cardIds:cards.map(card=>card.cardId),reason,designerReceipts:cards.map(card=>({cardId:card.cardId,receipt:card.designerRender.printSnapshot.receipt}))});}
    catch(logError){showToast('Cards are ready to print, but the print log could not be saved. Keep this window open.','error');return;}
    showToast('เตรียมพิมพ์ '+cards.length+' บัตรแล้ว','success');return;
}for(const card of cards){if(card.designerRender?.layout)continue;if(!backgrounds.has(n(card.templateId))){const response=await apiFetch(`/bbs/admin/card-templates/${card.templateId}/file`);backgrounds.set(n(card.templateId),await blobDataUrl(await response.blob()));}}const pages=cards.map(card=>`<article class="card" style="width:${n(card.widthMM)}mm;height:${n(card.heightMM)}mm;background-image:linear-gradient(90deg,rgba(255,255,255,.96),rgba(255,255,255,.58)),url('${backgrounds.get(n(card.templateId))}')"><div class="safe"></div><div class="identity"><div class="avatar">${escHtml((card.employeeName||'?').trim().charAt(0).toUpperCase())}</div><div class="name">${escHtml(card.employeeName)}</div><div class="meta">${escHtml(card.department||'-')} / ${escHtml(card.unit||'-')}</div><div class="meta">${escHtml(card.position||'-')}</div>${card.includeEmployeeId?`<div class="emp">${escHtml(card.employeeId)}</div>`:''}</div><div class="qr"><img src="${qrDataUrl(card.qrUrl)}"><strong>BBS SMART CARD</strong><small>Scan → Login → Workspace</small></div></article>`).join('');popup.document.open();popup.document.write(`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>BBS Smart Card Print</title><style>@page{size:A4;margin:8mm}*{box-sizing:border-box}body{margin:0;font-family:Kanit,Arial,sans-serif;color:#0f172a}.sheet{display:grid;grid-template-columns:repeat(2,85.6mm);gap:5mm;align-content:start}.card,.designer-card{position:relative;overflow:hidden;background-size:cover;background-position:center;border:.2mm dashed #f97316;border-radius:3mm;break-inside:avoid}.card{display:flex;justify-content:space-between;padding:5mm}.safe,.designer-safe{position:absolute;border:.2mm dashed #0891b2;pointer-events:none}.safe{inset:4%}.designer-bleed{position:absolute;inset:0;border:.2mm dashed #f97316;pointer-events:none}.designer-element{position:absolute;object-fit:contain}.designer-text{overflow:hidden;white-space:pre-wrap}.designer-qr{background:#fff}.identity{width:55mm;min-width:0;z-index:1}.avatar{width:14mm;height:14mm;border-radius:50%;background:#047857;color:#fff;display:flex;align-items:center;justify-content:center;font-size:7mm;font-weight:900}.name{font-size:4.2mm;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2mm}.meta{font-size:2.7mm;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1mm}.emp{font:900 3.2mm monospace;margin-top:2mm}.qr{width:23mm;text-align:center;align-self:end;z-index:1}.qr img{width:22mm;height:22mm;display:block;background:#fff}.qr strong{font-size:2.2mm;display:block;margin-top:1mm}.qr small{font-size:1.7mm;display:block}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.safe,.designer-safe,.designer-bleed{display:none}}</style></head><body><main class="sheet">${pages}</main><script>window.onload=()=>setTimeout(()=>window.print(),250)<\/script></body></html>`);popup.document.close();try{await API.post('/bbs/admin/cards/print-log',{cardIds:cards.map(card=>card.cardId),reason});}catch(logError){console.warn('[bbs] card print log failed:',logError?.message||logError);showToast('Cards are ready to print, but the print log could not be saved.','error');return;}showToast(`เตรียมพิมพ์ ${cards.length} บัตรแล้ว`,'success');}catch(error){if(popup&&!popup.closed){popup.document.open();popup.document.write('<!doctype html><html><head><meta charset="utf-8"><title>BBS card recovery</title></head><body style="font-family:Arial,sans-serif;padding:32px"><h2>Card was issued, but the print page could not be rendered.</h2><p>Keep this window open and contact Admin before replacing the card.</p></body></html>');popup.document.close();}showToast(error?.message||'สร้างไฟล์พิมพ์ไม่สำเร็จ','error');}}
