...
state.personalIssue={templateId:'',selected:new Map(),paperSize:'A4',preset:'compact',backOffsetXMM:0,backOffsetYMM:0};
...
function personalCardsView() {
    ...
    return `<div class="space-y-5">
      ...
      <section class="rounded-2xl border border-slate-200 bg-white overflow-hidden"><div class="p-5 border-b space-y-3"><div class="flex flex-col lg:flex-row lg:items-end justify-between gap-3"><div><h3 class="font-black text-slate-800">ออกบัตรรายบุคคล / Batch</h3><p class="text-xs text-slate-500 mt-1">เลือกพนักงานได้ข้ามหน้า สูงสุด 100 คน · ระบบตรวจ Active Card และขอบเขต Template ก่อนออกบัตรทั้งชุดแบบ Transaction · Personal Card แยกจาก Department Card</p></div><div class="flex flex-col sm:flex-row gap-2"><select id="bbs-issue-template" aria-label="เลือก Active Personal Card Template สำหรับออกบัตร" class="rounded-xl border border-slate-200 px-3 py-2 text-sm"><option value="">เลือก Active Template</option>${activeTemplates.map(t => `<option value="${t.id}" ${n(state.personalIssue.templateId)===n(t.id)?'selected':''}>${escHtml(t.TemplateName)} · ${escHtml(t.DepartmentName || 'ทุกแผนก')} · ${escHtml(t.BBSLevel || 'ทุกระดับ')}</option>`).join('')}</select><button data-bbs-issue class="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-black text-white">ตรวจและออกบัตร ${issueCount?issueCount:''} ใบ</button></div></div><div class="grid sm:grid-cols-[1fr_240px] gap-2"><input data-list-search="card-employees" value="${escHtml(state.cardEmployeeFilters.q)}" type="search" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm" placeholder="ค้นหารหัส ชื่อ แผนก Unit หรือตำแหน่ง"><select data-card-employee-filter="departmentId" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกแผนก</option>${masterFilterOptions(state.cardEmployeeFilters.departmentId)}</select></div><div class="flex flex-col gap-3 rounded-2xl border ${incompatible.length?'border-amber-200 bg-amber-50':'border-emerald-200 bg-emerald-50/50'} p-3 sm:flex-row sm:items-center sm:justify-between"><div><div class="text-sm font-black text-slate-800">เลือกแล้ว ${issueCount} / 100 คน</div><div class="mt-1 text-xs ${incompatible.length?'text-amber-800':'text-slate-500'}">${!issueCount?'ยังไม่ได้เลือกพนักงาน':!issueTemplate?'เลือก Active Template เพื่อ Preflight ทั้งชุด':incompatible.length?`มี ${incompatible.length} คนไม่ตรงขอบเขต Template — ระบบจะยังไม่ออกบัตร`:`พร้อมตรวจ Preview และออกบัตร ${issueCount} ใบในงานเดียว`}</div></div><div class="flex flex-wrap gap-2"><button type="button" data-card-select-page class="min-h-11 rounded-xl border border-emerald-300 bg-white px-3 text-xs font-black text-emerald-700">${selectablePage.length&&selectedPage===selectablePage.length?'ยกเลิกทั้งหน้านี้':'เลือกทั้งหน้านี้'} (${selectablePage.length})</button><button type="button" data-card-selection-clear class="min-h-11 rounded-xl border bg-white px-3 text-xs font-black text-slate-600" ${issueCount?'':'disabled'}>ล้างทั้งหมด</button></div></div>${personalBatchPrintControls()}</div><div id="bbs-card-employees" class="grid md:grid-cols-2 xl:grid-cols-3 gap-3 p-5 max-h-[34rem] overflow-y-auto">${state.cardEmployees.map(employeeCardChoice).join('') || personalCardEmployeeEmptyState()}</div>${listPager('card-employees',state.listMeta.cardEmployees,'พนักงาน')}</section>
      <section class="rounded-2xl border border-slate-200 bg-white overflow-hidden"><div class="p-5 border-b space-y-3"><div class="flex justify-between gap-3"><div><h3 class="font-black text-slate-800">บัตร Personal ที่ออกแล้ว</h3><p class="text-xs text-slate-500 mt-1">เปิดรายละเอียดเพื่อตรวจวงจรบัตรและประวัติพิมพ์ · ระบบไม่เก็บ QR ดิบ จึงพิมพ์บัตร Active ใหม่ด้วย Replace เพื่อหมุน QR ใหม่เท่านั้น</p></div><span class="text-xs font-bold text-emerald-700">${n(state.listMeta.cards?.total)} รายการ</span></div><div class="grid sm:grid-cols-3 gap-2"><input data-list-search="cards" value="${escHtml(state.cardFilters.q)}" type="search" placeholder="ค้นหาพนักงาน Template หรือ QR fingerprint" class="rounded-xl border px-3 py-2.5 text-sm"><select data-card-filter="status" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกสถานะ</option>${['Active','Revoked','Replaced'].map(v=>`<option ${state.cardFilters.status===v?'selected':''}>${v}</option>`).join('')}</select><select data-card-filter="departmentId" class="rounded-xl border px-3 py-2.5 text-sm"><option value="">ทุกแผนก</option>${masterFilterOptions(state.cardFilters.departmentId)}</select></div></div><div class="divide-y">${state.cards.map(activeCardRow).join('') || `<div class="p-8 text-center text-sm text-slate-400">ไม่พบบัตรตามตัวกรอง</div>`}</div>${listPager('cards',state.listMeta.cards,'บัตร')}</section>
    </div>`;
}
...
function employeeCardChoice(row) { const search=escHtml([row.EmployeeID,row.EmployeeName,row.Department,row.Unit,row.Position].join(' ').toLowerCase()),selected=state.personalIssue.selected.has(String(row.EmployeeID)); return `<label data-card-search="${search}" class="rounded-xl border ${row.ActiveCardID ? 'border-emerald-200 bg-emerald-50/40' : selected?'border-emerald-400 bg-emerald-50':'border-slate-200'} p-4 flex gap-3"><input type="checkbox" data-card-employee="${escHtml(row.EmployeeID)}" ${selected?'checked':''} ${row.ActiveCardID ? 'disabled' : ''} class="mt-1"><div class="min-w-0"><div class="font-bold text-slate-800 truncate">${escHtml(row.EmployeeName)}</div><div class="text-xs text-slate-500 mt-1">${escHtml(row.EmployeeID)} · ${escHtml(row.BBSLevel)}</div><div class="text-xs text-slate-400 mt-1 truncate">${escHtml(row.Department || '-')} / ${escHtml(row.Unit || '-')}</div>${row.ActiveCardID ? `<span class="inline-flex mt-2 rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-black text-emerald-700">มีบัตร Active · เลือกออกซ้ำไม่ได้</span>` : selected?'<span class="inline-flex mt-2 rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-black text-emerald-700">อยู่ใน Batch</span>':''}</div></label>`; }
function activeCardRow(row) { return `<article class="p-4 grid md:grid-cols-[1fr_auto] gap-3"><div class="min-w-0"><div class="font-bold text-slate-800 truncate">${escHtml(row.EmployeeName)} <span class="font-normal text-slate-400">· ${escHtml(row.EmployeeID)}</span></div><div class="text-xs text-slate-500 mt-1">${escHtml(row.Department || '-')} / ${escHtml(row.Unit || '-')} · ${escHtml(row.TemplateName)} · QR ${escHtml(row.TokenFingerprint)}</div><div class="mt-2 flex flex-wrap items-center gap-2"><span class="inline-flex rounded-full px-2 py-1 text-[10px] font-black ${row.Status==='Active'?'bg-emerald-100 text-emerald-700':'bg-slate-100 text-slate-600'}">${escHtml(row.Status)}</span><span class="text-[11px] text-slate-400">ออก ${fmtDateTime(row.IssuedAt)} · พิมพ์ ${n(row.PrintCount)} ครั้ง${row.LastPrintedAt?` · ล่าสุด ${fmtDateTime(row.LastPrintedAt)}`:''}</span></div>${row.RevokeReason?`<div class="mt-1 text-[11px] text-rose-600">เหตุผล: ${escHtml(row.RevokeReason)}</div>`:''}</div><div class="flex flex-wrap items-center gap-2"><button type="button" data-bbs-card-detail="${row.id}" class="min-h-11 rounded-xl border px-3 text-xs font-black text-slate-700">รายละเอียด / ประวัติ</button><button type="button" data-bbs-card-preview="${row.id}" class="min-h-11 rounded-xl border px-3 text-xs font-black text-sky-700">Preview ปัจจุบัน</button>${row.Status==='Active'?`<button data-bbs-card-replace="${row.id}" class="min-h-11 rounded-xl bg-amber-500 px-3 text-xs font-black text-white">Replace + Reprint</button><button data-bbs-card-revoke="${row.id}" class="min-h-11 rounded-xl border border-rose-200 px-3 text-xs font-black text-rose-600">บัตรสูญหาย / Revoke</button>`:''}</div></article>`; }
...
function bind() {
    ...
    document.querySelectorAll('[data-personal-print-option]').forEach(input=>input.onchange=()=>{const key=input.dataset.personalPrintOption,value=['backOffsetXMM','backOffsetYMM'].includes(key)?Math.max(-5,Math.min(5,n(input.value))):input.value;state.personalIssue[key]=value;input.value=String(value);});
...
function personalPrintOptions(){return{paperSize:['A4','A5','A6'].includes(state.personalIssue.paperSize)?state.personalIssue.paperSize:'A4',preset:state.personalIssue.preset==='safe'?'safe':'compact',backOffsetXMM:Math.max(-5,Math.min(5,n(state.personalIssue.backOffsetXMM))),backOffsetYMM:Math.max(-5,Math.min(5,n(state.personalIssue.backOffsetYMM)))};}
