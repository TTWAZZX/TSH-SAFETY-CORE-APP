...
async function withBusy(control, task, label = 'กำลังดำเนินการ...') {
    ...
    targets.forEach(target => { target.disabled = true; target.setAttribute('aria-disabled','true'); if (target.tagName === 'BUTTON') target.textContent = label; else target.value = label; });
    try {
        return await task();
    } finally {
...
function personalCardsView() {
    ...
    return `<div class="space-y-5">
      ...
      <section class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <form id="bbs-template-form" class="rounded-2xl border border-slate-200 bg-white p-5 space-y-3"><div><h3 class="font-black text-slate-800">Personal Card Template</h3><p class="text-xs text-slate-500 mt-1">ตั้งค่าขนาดก่อนอัปโหลดพื้นหลัง JPG/PNG/WebP สูงสุด 10 MB · แผนกและระดับมาจาก Master Data</p></div><label class="block text-[11px] font-bold text-slate-600">ชื่อ Template<input name="templateName" required maxlength="160" placeholder="เช่น BBS Personal Card" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"></label><div class="grid grid-cols-1 sm:grid-cols-2 gap-2"><select name="departmentId" data-master-source="departments" aria-label="ขอบเขตแผนกของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">ทุกแผนก</option>${masterDepartmentOptions()}</select><select name="bbsLevel" data-master-source="bbs-levels" aria-label="ระดับ BBS ของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">ทุกระดับ</option>${masterBbsLevelOptions()}</select></div>${cardSizeEditor()}<label class="flex items-center gap-2 text-xs font-bold text-slate-600"><input name="includeEmployeeId" type="checkbox" checked> แสดง EmployeeID บนบัตร</label><label class="block text-[11px] font-bold text-slate-600">ภาพพื้นหลังด้านหน้า<input name="template" aria-label="ไฟล์พื้นหลัง Card Template" required type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 block w-full text-xs text-slate-500"></label><button class="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white">อัปโหลดเป็น Draft</button></form>
...
function bind() {
    ...
    document.querySelectorAll('[data-open-actions]').forEach(button => button.addEventListener('click', async () => { state.tab='actions';state.actionScope=button.dataset.actionScope||'overdue';await trackSectionLoad('actions',loadActions);render({ preserveScroll:false, focusSelector:`#${activeTabId()}` });scrollBbsContentStart(); }));
    ...
    document.querySelectorAll('[data-inspector-open]').forEach(btn=>btn.onclick=()=>openInspectorTeam(n(btn.dataset.inspectorOpen),btn));
...
async function printIssuedCards(cards,reason,popup,backgrounds=new Map()){if(!cards.length){if(popup&&!popup.closed)popup.close();return;}try{const designerCards=cards.filter(card=>card.designerRender?.layout);if(designerCards.length){
