...
import { openBbsCardDesigner } from './bbs-card-designer.js?v=20260914-bbs-scoped-artwork-r1';
...
function accessibilityStyles() {
    return `<style>
      ...
      #bbs-smart-card-page [data-bbs-skip-link]{position:fixed;left:1rem;top:1rem;z-index:1000;transform:translateY(-180%);transition:transform .15s ease}
      #bbs-smart-card-page [data-bbs-skip-link]:focus{transform:translateY(0)}
      ...
      <div class="grid grid-cols-1 xl:grid-cols-[360px_minmax(0,1fr)] min-w-0">
        ...
        <div data-department-config-detail="${selected.id}" class="min-w-0">
          ...
          <div class="grid grid-cols-1 2xl:grid-cols-2 gap-5 p-5">
            <form id="bbs-dept-template-form" data-template-scope-form class="rounded-2xl border border-slate-200 p-4 space-y-3"><div><h4 class="font-black text-slate-800">1. เพิ่ม Template</h4><p class="mt-1 text-[11px] text-slate-500">ด้านหน้าเลือกตามแผนก/Unit ของ Template ส่วนด้านหลังใช้ Global Back ร่วมกัน</p></div><input type="hidden" name="departmentId" value="${selected.id}"><label class="block text-[11px] font-bold text-slate-600">ชื่อ Template<input name="templateName" required maxlength="160" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm" placeholder="เช่น BBS Department Card"></label><label class="block text-[11px] font-bold text-slate-600">Unit (ไม่บังคับ)<select name="safetyUnitId" class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"><option value="">ค่าเริ่มต้นทั้งแผนก</option>${(state.masterReference?.units||[]).filter(row=>n(row.department_id||row.DepartmentID)===n(selected.id)).map(row=>`<option value="${n(row.id)}">${escHtml(row.name||row.Name)}</option>`).join('')}</select></label>${cardSizeEditor({includeDisplayOrder:true})}<label class="block text-[11px] font-bold text-slate-600">ภาพสำรองสำหรับ Legacy Renderer<input name="template" required type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 w-full text-xs"></label><button class="w-full rounded-xl bg-emerald-600 px-3 py-2.5 text-sm font-black text-white">อัปโหลดเป็น Draft</button></form>
...
function personalCardsView() {
    ...
    return `<div class="space-y-5">
      ...
      <section class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <form id="bbs-template-form" data-template-scope-form class="rounded-2xl border border-slate-200 bg-white p-5 space-y-3"><div><h3 class="font-black text-slate-800">Personal Card Template</h3><p class="text-xs text-slate-500 mt-1">ด้านหน้าเลือกตามแผนก/Unit ของ Template ส่วนด้านหลังใช้ Global Back ร่วมกัน</p></div><label class="block text-[11px] font-bold text-slate-600">ชื่อ Template<input name="templateName" required maxlength="160" placeholder="เช่น BBS Personal Card" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"></label><div class="grid grid-cols-1 sm:grid-cols-3 gap-2"><select name="departmentId" required data-master-source="departments" aria-label="ขอบเขตแผนกของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">เลือกแผนก</option>${masterDepartmentOptions()}</select><select name="safetyUnitId" aria-label="ขอบเขต Unit ของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm" disabled><option value="">ค่าเริ่มต้นทั้งแผนก</option></select><select name="bbsLevel" data-master-source="bbs-levels" aria-label="ระดับ BBS ของ Template" class="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">ทุกระดับ</option>${masterBbsLevelOptions()}</select></div>${cardSizeEditor()}<label class="flex items-center gap-2 text-xs font-bold text-slate-600"><input name="includeEmployeeId" type="checkbox" checked> แสดง EmployeeID บนบัตร</label><label class="block text-[11px] font-bold text-slate-600">ภาพสำรองสำหรับ Legacy Renderer<input name="template" aria-label="ไฟล์สำรอง Card Template" required type="file" accept="image/jpeg,image/png,image/webp" class="mt-1 block w-full text-xs text-slate-500"></label><button class="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white">อัปโหลดเป็น Draft</button></form>
