// js/pages/committee.js (เวอร์ชันสมบูรณ์ อัปเดตล่าสุด)
...
export async function loadCommitteePage() {
    ...
    container.innerHTML = `
        ...
        <div id="current-committee-container" class="mb-6"><div class="card p-4">กำลังโหลดข้อมูลปัจจุบัน...</div></div>
        <div>
            <h3 class="text-lg font-semibold mb-3 border-b dark:border-slate-700 pb-2">ประวัติ</h3>
            <div id="past-committee-container" class="space-y-4"><div class="card p-4">กำลังโหลดประวัติ...</div></div>
        </div>
    `;
...
async function handleSetCurrentCommittee(committeeId) {
    ...
    try {
        await apiFetch(`/api/committees/${committeeId}`, { method: 'PUT', body: { IsCurrent: 1 } });
        await loadCommitteePage();
        showToast('อัปเดตสถานะเป็นชุดปัจจุบันสำเร็จ');
    } catch (error) { /* showError handled in apiFetch */ }
    finally { hideLoading(); }
}
...
function createCommitteeCard(committee, isCurrent) {
    const currentUser = JSON.parse(localStorage.getItem('currentUser'));
    const isAdmin = currentUser?.role === 'Admin';
    const formatDate = (dateString) => dateString ? new Date(dateString).toLocaleDateString('th-TH', { year: 'numeric', month: 'long', day: 'numeric' }) : 'N/A';

    const subCommitteesHtml = (committee.SubCommitteeData && committee.SubCommitteeData.length > 0)
        ? `<div class="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">${committee.SubCommitteeData.map(sc => {
            const latestVersion = sc.versions.length > 0 ? sc.versions.sort((a, b) => b.version - a.version)[0] : null;
            ...
            return `<div class="card flex flex-col"><div class="p-4 border-b dark:border-slate-700 flex items-center gap-3"><span class="h-3 w-3 rounded-full ${statusDotColor}"></span><h5 class="font-semibold text-slate-800 dark:text-slate-200 flex-grow">${sc.department}</h5></div><div class="p-4 flex-grow space-y-2 text-sm"><div class="flex justify-between"><span class="text-slate-500 dark:text-slate-400">อัปเดตล่าสุด:</span><span class="font-medium">${lastUpdatedText}</span></div><div class="flex justify-between"><span class="text-slate-500 dark:text-slate-400">เวอร์ชันปัจจุบัน:</span><span class="font-medium">${versionText}</span></div></div><div class="p-4 border-t dark:border-slate-700 flex justify-end items-center gap-2"><button data-committee-id="${committee.id}" data-department="${sc.department}" class="btn btn-secondary btn-sm btn-view-history" ${sc.versions.length === 0 ? 'disabled' : ''}>ดูประวัติ</button>${sc.activeLink ? `<a href="${sc.activeLink}" data-action="view-doc" data-title="ผังปัจจุบัน: ${sc.department}" class="btn btn-primary btn-sm">ดูผังปัจจุบัน</a>` : `<button class="btn btn-secondary btn-sm" disabled>ไม่มีผัง</button>`}</div></div>`;
        }).join('')}</div>`
    ...
    if (isAdmin) {
        ...
        adminButtons = `<div class="mt-4 pt-4 border-t dark:border-slate-700 flex items-center gap-3"><button data-id="${committee.id}" class="btn btn-secondary btn-sm btn-edit-committee">แก้ไข</button><button data-id="${committee.id}" class="btn btn-danger btn-sm btn-delete-committee">ลบ</button>${setCurrentButton}</div>`;
    }
    ...
    <div class="card overflow-hidden" id="committee-card-${committee.id}">
        <div class="p-5">
            ...
            <div class="mt-4">${committee.MainOrgChartLink ? `<a href="${committee.MainOrgChartLink}" data-action="view-doc" data-title="ผังองค์กรหลัก" class="btn btn-secondary">ผังองค์กรหลัก</a>` : ''}</div>
            ${adminButtons}
        </div>
