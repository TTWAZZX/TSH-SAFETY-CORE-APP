...
async function _loadHeroStats() {
    try {
        ...
        const [pRes, aRes, ppeRes, dRes] = await Promise.all(fetches);
...
function buildPrinciplesHtml() {
    const cards = _principles.map(p => {
        ...
        <div class="card overflow-hidden hover:shadow-md transition-shadow flex flex-col">
            ${p.ImageUrl
                ? `<img src="${escHtml(p.ImageUrl)}" alt="${escHtml(p.Title)}" class="w-full h-44 object-cover">`
...
function buildAssessmentHtml() {
    const rows = _assessments.map(a => {
        ...
        <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
            ...
            <td class="px-4 py-3 text-sm text-slate-600">${escHtml(a.Area)}</td>
...
function buildPPEHtml() {
    const inspections = _filterPPEDept
        ? _ppeInspections.filter(r => (r.Department || '').trim() === _filterPPEDept)
        : _ppeInspections;

    const itemStats = {};
    PPE_ITEMS.forEach(item => {
        const all = inspections.filter(r => r[item.key] && r[item.key] !== '');
    ...
    const overallPct = inspections.length
        ? (inspections.reduce((s, r) => s + parseFloat(r.CompliancePct || 0), 0) / inspections.length).toFixed(1)
        : null;
    ...
    const rows = inspections.map(r => {
        ...
        <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
            ...
            <td class="px-4 py-3 text-sm text-slate-600">${escHtml(r.Area || r.Department || '-')}</td>
            <td class="px-4 py-3 text-sm text-slate-600">${escHtml(r.InspectorName || '-')}</td>
            ${PPE_ITEMS.map(item => {
                const v = r[item.key];
    ...
    const deptFilterBar = _departments.length > 0 ? `
        <div class="card p-4 flex flex-wrap items-center gap-3">
            <span class="text-sm font-semibold text-slate-600 flex-shrink-0">กรองตามแผนก:</span>
            <select onchange="window._scSetDeptFilter(this.value)"
                    class="form-input text-sm py-1.5 px-3 flex-1 min-w-40 max-w-xs">
                <option value="">— ทุกแผนก —</option>
                ${_departments.map(d => `<option value="${escHtml(d)}" ${_filterPPEDept === d ? 'selected' : ''}>${escHtml(d)}</option>`).join('')}
            </select>
            ${_filterPPEDept ? `<button onclick="window._scSetDeptFilter('')" class="text-xs text-slate-500 hover:text-red-500 underline">ล้างตัวกรอง</button>` : ''}
        </div>` : '';
    ...
    <div class="space-y-5">
        ...
        <div class="card overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                <h3 class="font-semibold text-slate-700">บันทึกการตรวจ PPE ปี ${_filterYear}${_filterPPEDept ? ` — ${escHtml(_filterPPEDept)}` : ''}</h3>
                <span class="text-xs text-slate-400">${inspections.length} รายการ</span>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-slate-50">
...
// ── PPE Dept Breakdown (client-side) ───────────────────────────────────────
function _buildPPEDeptBreakdown() {
    if (!_ppeInspections.length) return '';
    const deptMap = {};
    _ppeInspections.forEach(r => {
        const dept = r.Department || '(ไม่ระบุ)';
        if (!deptMap[dept]) deptMap[dept] = { totalItems: 0, compliantItems: 0, count: 0 };
        deptMap[dept].totalItems     += parseInt(r.TotalItems)     || 0;
        deptMap[dept].compliantItems += parseInt(r.CompliantItems) || 0;
    ...
    const breakdown = Object.entries(deptMap)
        .map(([dept, d]) => ({
            dept,
            count: d.count,
            pct: d.totalItems > 0 ? Math.round((d.compliantItems / d.totalItems) * 100) : null,
        }))
    ...
    const rows = breakdown.map(r => {
        ...
        return `<tr class="border-b border-slate-100 last:border-0">
            <td class="px-4 py-2.5 text-sm text-slate-700 font-medium whitespace-nowrap">${escHtml(r.dept)}</td>
            <td class="px-4 py-2.5 text-xs text-slate-500 text-right whitespace-nowrap">${r.count} ครั้ง</td>
            <td class="px-4 py-2.5 w-full">
                <div class="flex items-center gap-2">
                    <div class="flex-1 bg-slate-100 rounded-full h-2">
                        <div class="h-2 rounded-full" style="width:${p ?? 0}%;background:${barClr}"></div>
                    </div>
    ...
    return `
    <div class="card overflow-hidden" style="box-shadow:0 4px 16px rgba(5,150,105,0.1),0 1px 4px rgba(0,0,0,0.06)">
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <h3 class="font-semibold text-slate-700">PPE Compliance รายแผนก — ปี ${_filterYear}</h3>
            <span class="text-xs text-slate-400">${breakdown.length} แผนก</span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">จำนวนตรวจ</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Compliance</th>
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
            </table>
...
function openAssessmentForm(id) {
    ...
    <div><label class="block text-sm font-semibold text-slate-700 mb-1">${label}</label>
        <input name="${name}" type="number" min="1" max="5" step="0.1" value="${val != null ? val : ''}" placeholder="1–5" class="form-input w-full"></div>`;
...
function openPPEForm() {
        ...
        <div><label class="block text-sm font-semibold text-slate-700 mb-1.5">แผนก</label>
            ${_departments.length > 0
                ? `<select name="Department" class="form-input w-full">
                       ...
                       ${_departments.map(d => `<option value="${escHtml(d)}">${escHtml(d)}</option>`).join('')}
                   </select>`
                : `<input name="Department" type="text" placeholder="ชื่อแผนก" class="form-input w-full">`
            }</div>
    ...
    setTimeout(() => {
        document.getElementById('sc-ppef')?.addEventListener('submit', async e => {
            ...
            try {
                await API.post('/safety-culture/ppe-inspections', data);
                closeModal();
...
function viewPPERecord(id) {
    ...
    <div class="space-y-4">
        ...
        ${r.ImageUrl ? `<a href="${escHtml(r.ImageUrl)}" target="_blank" rel="noopener" class="inline-block text-sm text-emerald-600 hover:underline">ดูรูปภาพหลักฐาน →</a>` : ''}
    </div>`, 'max-w-md');
}
...
async function exportPDF() {
    ...
    try {
        ...
        const scoreRows  = [
            ['T1', 'เดินบน Walk Way',          avg?.avg_t1],
            ['T2', 'ไม่ใช้โทรศัพท์ขณะเดิน',   avg?.avg_t2],
            ['T3', 'ข้ามถนนทางม้าลาย',          avg?.avg_t3],
            ['T4', 'หยุดยืนชี้นิ้ว',             avg?.avg_t4],
            ['T5', 'ไม่ล้วงกระเป๋า',            avg?.avg_t5],
            ['T6', `PPE Control (${ppePct2 || 'N/A'}% compliance)`, null],
