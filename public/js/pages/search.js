...
let _activeProfileTab = 'overview';
let _searchRequestSeq = 0;
let _profileRequestSeq = 0;
let _activeSearchSignature = '';
let _activeProfileSignature = '';
...
export async function loadSearchPage() {
    ...
    _selectedEmployeeId = _isAdmin ? (_selectedEmployeeId || _currentUser.id || '') : (_currentUser.id || '');
...
function shell() {
    ...
    <div class="space-y-5 animate-fade-in pb-10">
        <div class="ds-section px-5 py-4 flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div>
                <p class="text-[11px] font-extrabold uppercase tracking-wider text-emerald-700">Employee Safety 360</p>
                <h1 class="text-xl font-bold text-slate-800">${_isAdmin ? BI.title : 'Safety 360 ของฉัน / My Safety 360'}</h1>
                <p class="text-xs text-slate-500 mt-1">${_isAdmin ? 'สรุปสถานะก่อน แล้วเปิดดูหลักฐานเมื่อต้องการ / Summary first, audit detail on demand.' : 'ดูสถานะ เป้าหมาย และกิจกรรมความปลอดภัยของคุณ / View your safety status, targets, and activity.'}</p>
            </div>
            <div class="self-start md:self-auto flex items-center gap-2">
                <span id="people-view-year" class="rounded-lg bg-slate-50 border border-slate-100 px-3 py-2 text-xs font-bold text-slate-600">${_isAdmin ? 'Admin view' : 'My profile'} · ${_year}</span>
                ${_isAdmin ? '' : `<select id="people-year" class="form-input text-sm w-24">${yearOptions()}</select>`}
            </div>
        </div>

        <div class="${_isAdmin ? 'grid grid-cols-1 xl:grid-cols-[360px_1fr] gap-5' : ''}">
            ${_isAdmin ? `<aside class="space-y-4">
...
function bindEvents() {
    ...
    document.addEventListener('change', (e) => {
        ...
        if (e.target.id === 'people-year') {
            ...
            const yearBadge = document.getElementById('people-view-year');
            if (yearBadge) yearBadge.textContent = `${_isAdmin ? 'Admin view' : 'My profile'} · ${_year}`;
    ...
    document.addEventListener('click', guardActionHandler(async (e) => {
        ...
        if (profileTab && _selectedProfile) {
            ...
            if (wrap) wrap.innerHTML = renderProfile(_selectedProfile);
            return;
        }
...
async function runSearch(q = '') {
    ...
    if (Array.from(query).length < 2) {
        _searchRequestSeq += 1;
        _activeSearchSignature = '';
        wrap.innerHTML = emptyState({ title: 'กรอกอย่างน้อย 2 ตัวอักษร / Enter at least 2 characters', text: 'ค้นหาด้วยชื่อ รหัส แผนก หรือ ตำแหน่ง / Search by name, ID, department, or position.' });
        return;
    }
    if (Array.from(query).length > 100) {
        _searchRequestSeq += 1;
        _activeSearchSignature = '';
        wrap.innerHTML = emptyState({ title: 'คำค้นหายาวเกินไป / Search is too long', text: 'ใช้คำค้นหาไม่เกิน 100 ตัวอักษร / Enter no more than 100 characters.' });
        return;
    }
    const dept = document.getElementById('people-dept-filter')?.value || 'all';
    const signature = `${query}\u0000${dept}\u00001\u000030`;
    if (_activeSearchSignature === signature) return;
    ...
    try {
        const res = await API.get(`/person-search/employees?q=${encodeURIComponent(query)}&department=${encodeURIComponent(dept)}&page=1&limit=30`);
        ...
        if (requestSeq !== _searchRequestSeq) return;
        wrap.innerHTML = emptyState({ title: 'ค้นหาไม่สำเร็จ / Search failed', text: 'ระบบไม่สามารถโหลดรายชื่อพนักงานได้ในตอนนี้ / Employee list is not available right now.' });
        console.error(err);
    } finally {
        if (requestSeq === _searchRequestSeq) _activeSearchSignature = '';
    }
}
...
async function loadProfile(employeeId) {
    const wrap = document.getElementById('person-profile');
    if (!wrap) return;
    const signature = `${String(employeeId)}\u0000${_year}`;
    if (_activeProfileSignature === signature) return;
    const requestSeq = ++_profileRequestSeq;
    ...
    try {
        ...
        if (requestSeq !== _profileRequestSeq) return;
        _selectedProfile = res?.data || null;
        _activeProfileTab = 'overview';
        _timelineFilter = 'all';
        wrap.innerHTML = renderProfile(_selectedProfile);
    } catch (err) {
        if (requestSeq !== _profileRequestSeq) return;
        wrap.innerHTML = `<div class="ds-section border-red-100">${emptyState({ title: BI.profileLoadError, text: BI.profileLoadErrorText })}</div>`;
        console.error(err);
    } finally {
        if (requestSeq === _profileRequestSeq) _activeProfileSignature = '';
    }
}
...
function statusTone(status) {
    ...
    if (s.includes('no target') || s.includes('no data')) return { color: '#64748b', bg: '#f8fafc', border: '#e2e8f0' };
...
function renderProfile(data) {
    ...
    const targetRows = data.activityTargets || [];
    const targetSummary = data.activityTargetSummary || {};
    const targetConfigured = targetSummary.configured ?? targetRows.length;
    const targetPassed = targetSummary.passed ?? targetRows.filter(row => !row.noData && row.passed === true).length;
    const riskEvents = Number(m.accidents || 0) + Number(m.ppeViolations || 0) + Number(m.patrolIssues || 0);
    ...
    <div class="space-y-5">
        <section class="ds-section overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center text-lg text-white font-extrabold flex-shrink-0"
                         style="background:linear-gradient(135deg,#064e3b,#0d9488)">${escHtml((emp.EmployeeName || '?').charAt(0))}</div>
                    <div>
                        <div class="flex flex-wrap items-center gap-2">
                            <h2 class="text-lg font-bold text-slate-800">${escHtml(emp.EmployeeName || '-')}</h2>
                            <span class="text-[10px] font-extrabold px-2 py-1 rounded-md border" style="color:${overallTone.color};background:${overallTone.bg};border-color:${overallTone.border}">${escHtml(overall)}</span>
                        </div>
                        <p class="text-xs text-slate-500">${escHtml(emp.EmployeeID || '-')} · ${escHtml(emp.Department || '-')} · ${escHtml(emp.Position || '-')} · ${_year}</p>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2">
                    ${canExport ? '<button id="btn-export-person-excel" class="btn btn-secondary px-4 py-2 text-sm">Excel</button><button id="btn-print-person-audit" class="btn btn-secondary px-4 py-2 text-sm">Print</button>' : ''}
...
function latestTimelineDate(items) {
    const latest = [...items].filter(item => item.date).sort((a, b) => new Date(b.date) - new Date(a.date))[0];
    return latest ? formatDate(latest.date) : 'No activity';
}
...
function renderProfileTabs() {
    const tabs = [
        ...
        ['targets', 'Targets'],
        ['activity', 'Activity'],
        ['training', 'Training & PPE'],
        ['patrol', 'Patrol'],
        ['timeline', 'Timeline'],
    ];
    return `<nav class="ds-section p-1.5 flex gap-1 overflow-x-auto" aria-label="Person profile details">
        ${tabs.map(([key, label]) => `<button type="button" data-profile-tab="${key}" class="whitespace-nowrap rounded-lg px-3.5 py-2 text-xs font-bold transition-colors ${_activeProfileTab === key ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}" aria-selected="${_activeProfileTab === key}">${label}</button>`).join('')}
    </nav>`;
}
...
function renderProfileTabContent(data) {
    const emp = data.employee || {};
    const m = data.metrics || {};
    if (_activeProfileTab === 'targets') {
        return renderActivityTargets(data.activityTargets || [], data.activityTargetSummary || {}) || `<section class="ds-section">${emptyState({ title: 'ยังไม่มี Activity Targets / No activity targets', text: 'ไม่มีเป้าหมายที่ประเมินได้สำหรับบุคคลและปีที่เลือก / No evaluable target for this person and year.' })}</section>`;
    }
    if (_activeProfileTab === 'activity') {
        return `<div class="space-y-5">
            <section class="grid grid-cols-2 md:grid-cols-5 gap-3">
                ${moduleCard('patrol', m.patrol, `${m.patrolIssues} ประเด็น / issues`)}
                ${moduleCard('cccf', (m.cccfWorker || 0) + (m.cccfPermanent || 0), 'Worker / Permanent')}
                ${moduleCard('hiyari', m.hiyari, 'Near-miss')}
                ${moduleCard('ky', m.ky, 'KY Activity')}
                ${moduleCard('yokoten', m.yokoten, 'Responses')}
                ${moduleCard('fourm', (m.fourmOwner || 0) + (m.fourmCreated || 0), 'Owner / Created')}
                ${moduleCard('fourmMatrix', m.fourmScopes, 'Training Matrix')}
                ${moduleCard('ppe', m.ppeViolations, 'PPE violations')}
                ${moduleCard('training', m.training, `${m.trainingPassed || 0} passed`)}
                ${moduleCard('accident', m.accidents, 'Reports')}
            </section>
            <section class="ds-section overflow-hidden"><div class="px-5 py-4 border-b border-slate-100"><h3 class="font-bold text-slate-800">CCCF Activity</h3></div><div class="divide-y divide-slate-100">${renderCccfRows(data.cccfRecords || [])}</div></section>
        </div>`;
    }
    if (_activeProfileTab === 'training') {
        return `<div class="grid grid-cols-1 xl:grid-cols-2 gap-5">
            ${renderTrainingPpePanel(data)}
            <section class="ds-section p-5">${renderFourmScopes(data.fourmScopes || [])}</section>
        </div>`;
    }
    if (_activeProfileTab === 'patrol') {
        return `<section class="ds-section overflow-hidden"><div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between"><h3 class="font-bold text-slate-800">ประวัติ Patrol / Patrol History</h3><span class="text-xs text-slate-400">${_year}</span></div><div class="divide-y divide-slate-100">${renderPatrolRows(data.patrolRecords || [])}</div></section>`;
    }
    if (_activeProfileTab === 'timeline') {
        return `<section class="ds-section overflow-hidden"><div id="person-timeline-panel">${renderTimelinePanel(data)}</div></section>`;
    }
    return `<div class="space-y-5">
        ${renderFollowUpSignals(data)}
        <section class="grid grid-cols-1 xl:grid-cols-[0.8fr_1.2fr] gap-5 items-start">
            <div class="ds-section p-5"><div class="flex items-center justify-between mb-4"><h3 class="font-bold text-slate-800">ข้อมูลพนักงาน / Person Snapshot</h3><span class="text-xs text-slate-400">${_year}</span></div>${renderProfileFacts(emp, m, data.access?.canViewSensitive === true)}</div>
            <div class="ds-section overflow-hidden">${renderRiskProfile(data.riskProfile || {}, m, data.access?.canViewRiskDetail === true)}</div>
        </section>
        ${renderLatestActivityPreview(data.timeline || [])}
    </div>`;
}

function renderFollowUpSignals(data) {
    const signals = (data.complianceSignals || []).filter(item => !String(item.status || '').toLowerCase().includes('good'));
    const hasTarget = Number(data.activityTargetSummary?.configured || 0) > 0;
    return `<section class="ds-section overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100"><h3 class="font-bold text-slate-800">สัญญาณที่ต้องติดตาม / Follow-up Signals</h3><p class="text-xs text-slate-400">แสดงเฉพาะรายการที่ควรตรวจสอบต่อ / Only items that may need attention.</p></div>
        <div class="p-5 ${signals.length ? 'grid grid-cols-1 md:grid-cols-3 gap-3' : ''}">
            ${signals.length ? signals.map(item => { const tone = statusTone(item.status); return `<div class="rounded-lg border px-4 py-3" style="background:${tone.bg};border-color:${tone.border}"><div class="flex items-start justify-between gap-2"><p class="text-sm font-bold" style="color:${tone.color}">${escHtml(item.label || item.key || 'Safety signal')}</p><span class="text-[10px] font-extrabold uppercase" style="color:${tone.color}">${escHtml(item.status || 'Watch')}</span></div><p class="text-xs text-slate-500 mt-1">${escHtml(item.detail || item.value || '')}</p></div>`; }).join('') : hasTarget ? '<div class="flex items-center gap-3 text-sm font-bold text-emerald-700"><span class="w-7 h-7 rounded-full bg-emerald-50 flex items-center justify-center">&#10003;</span><span>เป้าหมายที่ตั้งไว้เป็นไปตามเกณฑ์ / Configured targets are on track</span></div>' : '<div class="text-sm font-semibold text-slate-500">ไม่ได้ตั้งเป้าหมายสำหรับบุคคลนี้ / No effective target configured for this person.</div>'}
        </div>
    </section>`;
}

function renderLatestActivityPreview(items) {
    const latest = items.slice(0, 5);
    return `<section class="ds-section overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between gap-3"><div><h3 class="font-bold text-slate-800">กิจกรรมล่าสุด / Latest Activity</h3><p class="text-xs text-slate-400">${latest.length ? `${latest.length} รายการล่าสุด / Latest records` : `ปี ${_year} / Year ${_year}`}</p></div><button type="button" data-profile-tab="timeline" class="text-xs font-bold text-emerald-700 hover:text-emerald-800">ดูทั้งหมด / View all</button></div>
        ${latest.length ? `<div class="divide-y divide-slate-100">${renderTimeline(latest)}</div>` : '<div class="px-5 py-4 text-sm text-slate-500">ยังไม่มีกิจกรรมในปีที่เลือก / No activity in the selected year.</div>'}
    </section>`;
}

function renderTrainingPpePanel(data) {
    const training = data.trainingRecords || [];
    const inspections = data.ppeInspections || [];
    const violations = data.ppeViolations || [];
    const hasPpe = inspections.length || violations.length;
    const emptyLabels = [!training.length ? 'No Training' : '', !hasPpe ? 'No PPE' : ''].filter(Boolean);
    return `<section class="ds-section overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100"><h3 class="font-bold text-slate-800">Training & PPE</h3><p class="text-xs text-slate-400">Recent records for ${_year}</p></div>
        ${emptyLabels.length === 2 ? `<div class="px-5 py-4 text-sm font-semibold text-slate-500">${emptyLabels.join(' · ')}</div>` : `<div class="divide-y divide-slate-100 max-h-[620px] overflow-y-auto">${training.length ? renderTrainingRows(training) : '<div class="px-5 py-3 text-sm text-slate-400">No Training</div>'}${hasPpe ? renderPpeRows(inspections, violations) : '<div class="px-5 py-3 text-sm text-slate-400">No PPE</div>'}</div>`}
    </section>`;
}

function renderActivityTargets(rows, summary) {
    if (!rows.length) return '';
    const configured = Math.max(0, Number(summary.configured ?? rows.length) || 0);
    const evaluable = Math.max(0, Number(summary.evaluable ?? rows.filter(row => !row.noData && row.passed !== null).length) || 0);
    const passed = Math.max(0, Number(summary.passed ?? rows.filter(row => !row.noData && row.passed === true).length) || 0);
    const noData = Math.max(0, Number(summary.noData ?? Math.max(0, rows.length - evaluable)) || 0);
    ...
    return `<section class="ds-section overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between gap-3">
            ...
            <span class="text-xs font-bold text-slate-500">${passed}/${configured} passed${noData ? ` · ${noData} no data` : ''}</span>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-px bg-slate-100">
            ${rows.map(row => {
                const pct = Math.max(0, Math.min(100, Number(row.completionPct) || 0));
                const actual = Number.isFinite(Number(row.actualCount)) ? Number(row.actualCount) : 0;
                const target = Number.isFinite(Number(row.yearlyTarget)) ? Number(row.yearlyTarget) : 0;
                const passPct = Math.max(0, Math.min(100, Number(row.passPct) || 0));
...
function kpiBox(value, label, color, extraClass = '') {
    return `<div class="bg-white px-4 py-4 ${extraClass}">
        <p class="text-xl font-extrabold truncate" style="color:${color}">${escHtml(String(value ?? '-'))}</p>
...
function renderRiskProfile(profile, metrics, canViewDetail = false) {
    ...
    return `
        <div class="p-5 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">คำนวณจาก effective targets ที่ Admin ตั้งไว้ และ risk events ที่เกิดขึ้นจริง / Based on Admin-configured effective targets and actual risk events.</p>
            </div>
        ...
        ${canViewDetail ? `<details class="border-t border-slate-100 group">
            <summary class="px-5 py-3 cursor-pointer list-none flex items-center justify-between gap-3 text-sm font-bold text-slate-600 hover:bg-slate-50">
                <span>ดูรายละเอียดคะแนนและเหตุผล / View score details</span>
                <span class="text-slate-400 group-open:rotate-180 transition-transform">&#9662;</span>
            </summary>
            <div class="grid grid-cols-1 xl:grid-cols-[1.25fr_1fr] gap-0 border-t border-slate-100">
                <div class="p-5 border-b xl:border-b-0 xl:border-r border-slate-100">
                    ...
                    <div class="space-y-3">${factors.map(renderRiskFactor).join('') || '<p class="text-xs text-slate-400">No factor score available</p>'}</div>
                </div>
                <div class="p-5 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-1 gap-4">${renderRiskList('เหตุผล / Reasons', reasons)}${renderRiskList('สิ่งที่ควรทำต่อ / Next actions', actions)}</div>
            </div>
        </details>` : ''}`;
}
...
function renderPatrolRows(rows) {
    if (!rows.length) return emptyState({
        ...
        text: 'ข้อมูลจะแสดงเมื่อมีการเดิน Patrol / Records will appear after Patrol activity.'
    });
    return rows.map(r => `
